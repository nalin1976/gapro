...
?>			</td>
	...
	$SqlDetail2="SELECT 
				...
				Inner Join issuesdetails AS ID ON ID.intIssueNo = RSD.intIssueNo AND RSD.intIssueYear = ID.intIssueYear AND ID.intStyleId = RSD.intStyleId AND RSD.strBuyerPoNo = ID.strBuyerPONO AND ID.intMatDetailID = RSD.intMatdetailID AND ID.strColor = RSD.strColor AND RSD.strSize = ID.strSize AND RSD.intGrnYear = ID.intGrnYear AND RSD.intGrnNo = ID.intGrnNo AND RSD.intMatrequisitionNo = ID.intMatRequisitionNo 
				WHERE 

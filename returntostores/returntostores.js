		...
		if (tblPopUp.rows[loop].cells[0].childNodes[0].childNodes[1].checked)	
		{
			...
			var booCheck =false;
				for (var mainLoop =1 ;mainLoop < tblMain.rows.length; mainLoop++ )
				{
					...
					var mainMRNNo		= tblMain.rows[mainLoop].cells[11].id;
					
					if ((mainMatDetaiId==itemDetailID) && (mainBuyerPoNo==buyerPoNo) && (mainColor==color) && (mainSize==size) && (mainIssueNo==issueNo) && (mainStyleId==StyleID) && (grnNo == mainGRNno) && (grnType==mainGRNtype) && (mrnNo==mainMRNNo))
					{
								booCheck =true;
					}	
				}
...
function LoadPopUpNo()
{
	...
	// ===================================================================================
	// Comment On - 01/21/2014
	// Description - To avoid reload years to the drop down
	// ===================================================================================
...
function loadStyleName()
{
	...
	var url = 'returntostoresxml.php?RequestType=loadStyleName&scNO=' +scNO;
	var htmlobj=$.ajax({url:url,async:false});

...
$reportname = "poValidateReport.php";
...
<title>GaPro - Pending PO Cancelation</title>
...
function submitForm()
{
	document.frmPOCancel.submit();
}
function ClearChangePoList()
{
	document.frmPOCancel.reset();
...
function deletePO(poNo,poYear)
{
	if(confirm("Are you sure you want to delete PO \""+poNo+"/"+poYear +"\" ?"))
	{
		var url="POMiddle.php";
					url=url+"?RequestType=changePOstatus";
					url += '&poNo='+poNo;
					url += '&poYear='+poYear;
					
					var htmlobj=$.ajax({url:url,async:false});
					var XMLresponse = htmlobj.responseXML.getElementsByTagName("status")[0].childNodes[0].nodeValue;
...
function getStyleSCNo()
{
	...
	//alert(Math.ceil(0.0001));
}
</script>
</head>

<body>
<form id="frmPOCancel" name="frmPOCancel" method="post" action="pendingPOcacel.php">
  <table width="100%" border="0" align="center" bgcolor="#FFFFFF">
    ...
    <tr>
      <td><table width="950" border="0" align="center" class="bcgl1">
        ...
        <tr>
          ...
          <td width="96%" class="head1">Purchase Orders (Pending) - Cancelation </td>
        </tr>
        <tr>
	...
	?>
                  ...
                  <td width="155" ><select name="cboOrderNo" id="cboOrderNo" style="width:152px;" onchange="resetCompanyBuyer();getStyleSCNo();">
                  <option value="" selected="selected">Select One</option>
                    <?php
          ...
          <td colspan="2"><div id="divData" style="width:930px; height:400px; overflow: scroll; border-width:3px; border-style:solid;border-color:#FFD5AA;">
            <table width="910" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCFF" class="bcgl1" id="tblPreOders" >
              <tr>
                ...
                <td width="11%" bgcolor="#804000" class="normaltxtmidb2">Delete</td>
              </tr>
              <?php
...
purchaseorderheader.strSupplierID = suppliers.strSupplierID and purchaseorderheader.intStatus in ($status)  " ;
			...
			?>
			  ...
			  if ($pos % 2 == 0)
			   ...
			   ?>">
                ...
                <td class="normalfntMid"><img src="images/deletered.png" width="16" height="16" onclick="deletePO(<?php echo  $row["intPONo"].','.$row["intYear"]?>);" /></td>
              </tr>
              <?php
			  $pos ++;
			}
		...
		<img src="images/new.png" alt="new" border="0" onclick="ClearChangePoList();" />

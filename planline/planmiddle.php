...
if($queryRequest == "GetMaxNo"){
    ...
    /*if(is_null($arrRes[0])){
        $ResponseXML .= "<LINE_NO><![CDATA[" . 10000  . "]]></LINE_NO>\n";
    }else{
        $ResponseXML .= "<LINE_NO><![CDATA[" . $arrRes[0]  . "]]></LINE_NO>\n";
    }*/
        
    while($row=mssql_fetch_array($resMaxNo)){

        if(is_null($row["TRLineNo"])){
            $ResponseXML .= "<LINE_NO><![CDATA[" . 10000  . "]]></LINE_NO>\n";
        }else{
...
if($queryRequest == "UpdatePlanLines"){
    ...
    $sql = 'UPDATE [dbHela].[dbo].[HELA CLOTHING (PVT) LTD$JobTaskPlanning] SET LineAmountLKR = $ItemValue, UnitCost = $unitPrice, TotCost = $ItemValue, UnitPrice = $unitPrice, TotalPrice = $ItemValue, LineAmountLKR2 = $ItemValue, QTY = $ItemQty, AmountUSD = $ItemValue, B_Transfered = NULL WHERE (JOBNo = '."'".$JobNo."'".') AND (TaskNo = '."'".$taskCode."'".') AND (ItemNo = '."'".$itemCode."'".')';

    $resUpdate = $connectNavision->ExecuteQuery($sql);  

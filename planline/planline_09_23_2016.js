...
function GetOrderDetails(prmStyleID){
	...
	for(iRows = 0; iRows<XMLSCNO.length; iRows++){
		...
		if(IsExist == 0){
			...
			UpdatePlanningLines(JobNo, TaskCode, sItemCode, UnitPrice, ItmeTotVal, TotReqQty);
		}
	...
	for(iRows = 0; iRows<XMLSCNO.length; iRows++){
		...
		if(InHouseQty > 0){
			...
			if(IsExist == 0){
				...
				UpdatePlanningLines(JobNo, "14", "E001", UnitPrice, ItmeTotVal, TotReqQty);
			}
		}
	}
}
...
function FinanceProcessing(prmStyleId){
	...
	for(iRows = 0; iRows<XMLFIN.length; iRows++){
		...
		if(IsExist == 0){
			...
			UpdatePlanningLines(JobNo, "506", "F001", UnitPrice, ItmeTotVal, TotReqQty);
		}
...
function UpdatePlanningLines(prmJobNo, prmTaskCode, prmItemCode, prmUnitPrice, prmTotalPrice, prmTotQty){

	var urlSave				= "planmiddle.php?reqid=UpdatePlanLines&jobno="+prmJobNo+"&taskcode="+prmTaskCode+"&itemcode="+prmItemCode+"&itemprice="+prmUnitPrice+"&itemvalue="+prmTotalPrice+"&itemqty="+prmTotQty;
	var htmlObjSaveBuget 	= $.ajax({url:urlSave, async:false});
	...
	if(iResult==0){
		...
		alert("Update Error in query " + sQuery);
	}
}

...
history_orders.dblFacProfit ;";
		...
		{			
			...
			$oldrunningRefNo = $row["intStyleId"];
			...
			$oldFOBRate = $reaFOB;
			$olddblFacProfit = $row["dblFacProfit"];
		}
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td class="normalfnt2Black" id="tdStyleName1">&nbsp;</td>
            <td rowspan="2">&nbsp;</td>
			...
			while($row_Category = mysql_fetch_array($result_Category))
			{
				...
				{	
					...
					while($row_order = mysql_fetch_array($result_order))
					{
						...
						//$TotFabFinance Value
						if($row_Category["intID"]=="1" & $row_order["intType"]=="0")
						{
						...
						if($row_Category["intID"]=="2" & $row_order["intType"]=="0")
						{
						...
						else if($row_Category["intID"]=="3" & $row_order["intType"]=="0")
						{
							$TrimFinance=$row_order["dbltotalcostpc"]*$reaFinPercntage/100;
							$TotTrimFinance+=$TrimFinance;
						}
...
<table width="800" align="center">
  <tr>
    <td><table width="100%" cellpadding="0" cellspacing="0"><tr><td class="tophead"><table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <!--<td width="24%"><img src="images/logo.jpg" alt="" width="191" height="47" class="normalfnt" /></td>
        <td width="5%" class="normalfnt">&nbsp;</td>-->
        <td width="84%" >
 		<?php		
...
WHERE (((orders.intStyleId)='".$strStyleID."'));";
		...
		{			
			...
			$strDescription=$row["strDescription"];
			//$strCustomerRefNo=$row["strCustomerRefNo"];
			$strCustomerRefNo=$row["intStyleId"];
			$intSeasonId=$row["intSeasonId"];
			$strRPTMark=$row["strRPTMark"];
			$reaEfficiencyLevel=$row["reaEfficiencyLevel"];
			$reaCostPerMinute=$row["reaCostPerMinute"];
			...
			while($row_Category = mysql_fetch_array($result_Category))
			{
				...
				{	
					...
					while($row_new = mysql_fetch_array($result_new))
					{
						...
						$sqlOldItem = "SELECT strOrderNo,intApprovalNo,intStyleId,intMatDetailID,strUnit,dblUnitPrice,reaConPc,reaWastage,strCurrencyID,
						ho.intOriginNo,dblReqQty,dblTotalQty,dblTotalValue,dblCostPC,dblFreight,dbltotalcostpc, strOriginType 
						FROM history_orderdetails  ho inner join itempurchasetype i on
ho.intOriginNo = i.intOriginNo
						 WHERE intApprovalNo = '$maxApprovalNo' AND intStyleId = '$strStyleID' AND intMatDetailID = '" . $row_order["intItemSerial"] . "'";
...
history_orderdetails.dbltotalcostpc  , matitemlist.strItemDescription ,itempurchasetype.strOriginType
...
ho.intOriginNo = i.intOriginNo
						...
						for ($inloop = $maxApprovalNo ; $inloop < $maxApprovalNo ; $inloop ++)
						{
							$sqlOldItem = "SELECT strOrderNo,intApprovalNo,intStyleId,intMatDetailID,strUnit,dblUnitPrice,reaConPc,reaWastage,strCurrencyID,intOriginNo,dblReqQty,dblTotalQty,dblTotalValue,dblCostPC,dblFreight,dbltotalcostpc 
							FROM history_orderdetails WHERE intApprovalNo = '$inloop' AND intStyleId = '$strStyleID' AND intMatDetailID = '" . $row_order["intItemSerial"] . "'";
...
(SELECT matitemlist.intItemSerial FROM ((orderdetails INNER JOIN matitemlist ON orderdetails.intMatDetailID = matitemlist.intItemSerial) INNER JOIN matmaincategory ON matitemlist.intMainCatID = matmaincategory.intID) INNER JOIN itempurchasetype ON orderdetails.intOriginNo = itempurchasetype.intOriginNo WHERE (((orderdetails.intStyleId)='$strStyleID') AND ((matitemlist.intMainCatID)='" .$row_Category["intID"] ."')))
          ...
          <tr>
			...
			//echo number_format(($CountFinanace + $reaESC ),4);//new
			?></td>
            <td class="normalfntRiteTAB"><?php 
			//$CountFinanace=$TotFabFinance+$TotTrimFinance + $reaESC ;
          ...
          <tr>
			...
			// echo round($reaSMV*$reaSMVRate,4);
			//$CMVAL = round($reaFOB   - ( $TotaleDrectCost + $reaESC  + ($TotFabFinance+$TotTrimFinance)),4);
			$CMVAL =round($reaSMV*$reaSMVRate,4); 
			echo number_format(($CMVAL),4);//current CMV
			?></td>
            <td class="normalfntRiteTAB"><?php
			echo  number_format((($CMVAL) * $intQty ),2);
			?></td>
          </tr>
          ...
          <tr>
           ...
           <tr>
            <td colspan="10" class="normalfntTAB"><strong>REQD FOB </strong></td>
            <td class="normalfntRiteTAB"><?php
			$reqFOB = $reaFOB-$dblFacProfit;
			 echo number_format($reqFOB,4);
			?></td>
            <td class="normalfntRiteTAB">&nbsp;</td>
          </tr>
		  
            <td colspan="10" class="normalfntTAB"><strong>Gross Profit </strong></td>
            <td class="normalfntRiteTAB"><?php
			echo number_format($dblFacProfit,4);
			?></td>
            <td class="normalfntRiteTAB"><?php echo number_format(($dblFacProfit*$intQty),4);?></td>
          </tr>
		  <tr>
		    <td colspan="12">&nbsp;</td>
		    </tr>
		...
		<!--  <tr>
		 ...
		 <!-- <tr>
		  ...
		  <tr bgcolor="#CCCCFF">
			...
			echo "FAB Fin: ";
			echo "<span class=\"normalfnth2B\">";
			$totoldfabricfinance = round($oldfabricfinance*$oldtotQty,2);
			echo number_format($totoldfabricfinance,2);
			echo "</span>";
			echo " TRIM Fin: ";
			echo "<span class=\"normalfnth2B\">";
			$totoldtrimfinance = round($oldtrimfinance*$oldtotQty,2);
			echo number_format($totoldtrimfinance,4);
			echo "</span>";
			?>            </td>
            <td class="normalfntRiteTAB"><?php 
			//$AvgFinanace=$CountFinanace/$intQty;
			//echo number_format($AvgFinanace,4); echo number_format($oldfinanceRate,4);
$oldCountFinanace=round($totoldfabricfinance+$totoldtrimfinance,4);
echo number_format(($oldCountFinanace/$oldtotQty),4);
//$totoldFinance = number_format(($oldCountFinanace + $reaESC ),4);
			//echo number_format(($oldCountFinanace/$oldtotQty),4);
			?></td>
            <td class="normalfntRiteTAB"><?php 
			echo number_format($oldCountFinanace,2);
			?></td>
          </tr>
          <tr bgcolor="#CCCCFF">
            <td colspan="10" class="normalfntTAB">TOTAL DIRECT COST </td>
         ...
         <!-- <tr bgcolor="#CCCCFF">
          ...
          <tr bgcolor="#CCCCFF">
			...
			// echo round($reaSMV*$reaSMVRate,4);
			//echo $TotaleDrectCost . "<br>";
		$oldCMRate = $oldsmv*$olddollarSMV;
			echo number_format(($oldCMRate),4);//previous cmv details
			?></td>
            <td class="normalfntRiteTAB"><?php
			$oldCMValue = $oldsmv*$olddollarSMV*$oldodrQty;
          ...
          <tr bgcolor="#CCCCFF">
			...
			?></td>
            <td class="normalfntRiteTAB"><?php
			$oldFOBValue = $oldFOBRate*$oldodrQty;
		 ...
		 <tr bgcolor="#CCCCFF">
            <td colspan="10" class="normalfntTAB"><strong>REQD FOB </strong></td>
            <td class="normalfntRiteTAB"><?php
			$oldReqFob = $oldFOBRate-$olddblFacProfit;
			 echo number_format($oldReqFob,4);
			//echo number_format($oldFOBRate,4);
			?></td>
            <td class="normalfntRiteTAB">&nbsp;</td>
          </tr>
           <tr bgcolor="#CCCCFF">
            <td colspan="10" class="normalfntTAB"><strong>Gross Profit </strong></td>
            <td class="normalfntRiteTAB"><?php
				 echo number_format($olddblFacProfit,4);
			//echo number_format($oldFOBRate,4);
			?></td>
            <td class="normalfntRiteTAB"><?php echo number_format(($olddblFacProfit*$oldodrQty),4);?></td>
          </tr>
...
document.getElementById("tdStyleName1").innerHTML = ":&nbsp;<?php echo $styleName;?>";

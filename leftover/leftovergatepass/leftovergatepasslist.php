...
<title>GaPro | LeftOver-GatePass Details List</title>
<link href="file:///C|/Program Files/Apache Software Foundation/Apache2.2/htdocs/css/erpstyle.css" rel="stylesheet" type="text/css" />
<style type="text/css">
<!--
...
<form name="frmLeftOverGPListing" id="frmLeftOverGPListing" action="leftovergatepasslist.php" method="post">
<table width="950" border="0" align="center" bgcolor="#FFFFFF" class="tableBorder">
  <tr>
    <td class="mainHeading"><table width="100%" border="0"  >
      <tr>
        <td width="20%">LeftOver - Gate Pass List</td>
        </tr>
    </table></td>
  </tr>
  <tr>
      ...
      <tr>
        <td><div id="divGatePassDetails" style="overflow:scroll; height:450px; width:950px;">
          <table id="tblGatePassDetails" width="100%" cellpadding="0" border="0" cellspacing="1" bgcolor="#CCCCFF" >
			...
			<?php
$SQL =  "SELECT DISTINCT LGPH.intGatePassNo,LGPH.intGPYear,date(LGPH.dtmDate) as dtmDate ,LGPH.intStatus, 
				(select Name from useraccounts UA where UA.intUserID=LGPH.intUserId)as userName,
				(select strName from mainstores MS  where MS.strMainID=LGPH.strDestination) as Destination
				 FROM leftover_gatepass AS LGPH 
				 Inner Join leftover_gatepassdetails AS LGPD ON LGPH.intGatePassNo = LGPD.intGatePassNo AND LGPH.intGPYear = LGPD.intGPYear ";
				 
	if($storesCategory=="E")
	{
	$SQL .= " Inner Join subcontractors ON LGPH.strDestination = subcontractors.strSubContractorID ";
	}
	elseif($storesCategory=="I")
	{
	$SQL .= "Inner Join mainstores ON LGPH.strDestination = mainstores.strMainID ";
	}
	$SQL .= " WHERE LGPH.intGatePassNo !=0 and LGPH.strCategory='$storesCategory' and LGPH.intCompany='$companyId'";
	
	if ($gatePassNoFrom!="")
			{
				$SQL .=" AND LGPH.intGatePassNo >= $gatePassNoFrom ";	
			}		
	 if ($gatePassNoTo!="")
			{
				$SQL .=" AND LGPH.intGatePassNo <= $gatePassNoTo ";	
			}		
	...
	if ($category!="2")
	{
		$SQL .=" AND LGPH.intStatus =$category ";		
	}
	if($destination !="")
		$SQL .=" AND LGPH.strDestination =$destination ";	
		
		$SQL .=" order by LGPH.dtmDate desc ";
...
?>
         <tr class="bcgcolor-tblrowWhite">
              ...
              <td width="12%" class="normalfntMid" ><a href="leftovergatepass.php?id=1&amp;GatePassNo=<?php echo $row["intGatePassNo"];?>&amp;GatePassYear=<?php echo $row["intGPYear"];?>&amp;Status=<?php echo $row["intStatus"];?>" class="non-html pdf" target="_blank"><?php echo $row["intGPYear"].'/'.$row["intGatePassNo"]?></a></td>
			  ...
			  <td width="15%" class="normalfntMid"><a href="leftovergatepassrpt.php?id=1&amp;GatePassNo=<?php echo $row["intGatePassNo"];?>&amp;GatePassYear=<?php echo $row["intGPYear"];?>" class="non-html pdf" target="_blank"><img border="0" src="../../images/view.png" alt="view" /></a></td>
			  </tr>
<?php
$No++;
}

...
<title>LeftOver Gate Pass TransferIn :: Report</title>
...
$SQLHeader="SELECT 
			...
			TIH.intCompanyId 				
			FROM 
			leftover_gatepasstransferin_header AS TIH 
...
?>
  ...
  <tr>
    <td colspan="3"><table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td height="36" colspan="6" class="head2">TRANSFER IN (From LeftOver Gate Pass)</td>
      </tr>
...
<?php
	$SqlDetail1="select distinct O.strOrderNo,GTD.intStyleId,GTD.strBuyerPONO 
				from leftover_gatepasstransferin_details GTD 
...
?>			</td>
	...
	$SqlDetail2="SELECT 
				...
				GPD.intMatDetailId 
				FROM 
				leftover_gatepasstransferin_details AS GPD 
	...
	while($row2=mysql_fetch_array($resultDetail2))
	{
			...
			$Qty				=$row2["dblQty"];	
			 ...
			 $sqlBin = "SELECT strBinName,SL.strLocName,ST.dblQty as BinQty,ST.strUnit FROM stocktransactions_leftover ST
...
AND intDocumentNo ='$pubTransInNo' AND strColor = '".$row2["strColor"]."' AND strSize ='".$row2["strSize"]."' AND strType = 'LTI' 

...
function ClearForm()
{	
	document.frmLOTIListing.reload();
}
...
function LoadSubStores()
{
	...
	var url  = "leftovertransferinXML.php";
	...
	var url = 'leftovertransferinXML.php?RequestType=LoadSubStores&MainStoreId=' +strMainStores;
	var htmlobj=$.ajax({url:url,async:false});
...
{	
	...
	var url = 'leftovertransferinXML.php?RequestType=LoadGatePassDetails&gatePassNo=' +gatePassNo;
...
function CreateMainGrid(tbl,styleId,buyerPoNo,styleName,buyerPoName,matDetailID,itemDesc,color,size,units,qty,subCatId,grnNo,grnYear,mainArrayIndex,pub_commonBin,grnTypeId,grnType)
{
	...
	cell.className 	= "normalfntRite";
	cell.innerHTML	= "<input type=\"text\" id=\"txtTransInQty\" name=\"txtTransInQty\" class=\"txtbox\" size=\"8\" style=\"text-align:right\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\" value=\""+qty+"\" onkeydown=\"RemoveBinColor(this);\" onkeyup=\"ValidateQty(this);SetQty(this," + mainArrayIndex  + ");\">";
...
function SetQty(obj,index)
{
		...
		Materials[index][4] = parseFloat(rw.cells[8].childNodes[0].value);	
}
...
function OpenBinPopUp(obj,index)
{
	...
	var ReqQty =parseFloat(rw.cells[8].childNodes[0].value);
		...
		if (ReqQty=="" || ReqQty==0 || ReqQty==isNaN)
		{
			...
			rw.cells[8].childNodes[0].value=stockQty
			 return false;
		}	
...
{
	var url = 'leftovertransferinXML.php?RequestType=ShowSubCatName&subCatID=' +subCatID;
	var htmlobj=$.ajax({url:url,async:false});
...
{		
	...
	xmlHttp.open("GET",'leftovertransferinXML.php?RequestType=LoadBinDetails&mainStoreID=' +mainStoreID+ '&subStores=' +subStores+ '&subCatID=' +subCatID ,true);
	xmlHttp.send(null);
}
		 ...
		 var strText = "<tr class=\"mainHeading4\">"+
					...
					for (loop=0;loop<XMLSubStoresID.length;loop++)
					{
						strText +="<tr class=\"bcgcolor-tblrowWhite\">"+
									  ...
									  "<td nowrap=\"nowrap\" class=\" normalfntRite\"><input type=\"text\" name=\"txtIssueQty\" id=\"txtIssueQty\" class=\"txtbox\" size=\"8\" style=\"text-align:right\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\" value=\"0\" /></td>"+
 ...
 function ValidateQty(obj){
	var rw 				= obj.parentNode.parentNode;
	var GatePass 		= parseFloat(rw.cells[7].childNodes[0].nodeValue);
	var TransInQty		= parseFloat(rw.cells[8].childNodes[0].value);
	
	if(GatePass<TransInQty){		
		rw.cells[8].childNodes[0].value=GatePass;
	}
}
...
{		
	...
	if (TansferIn=="")
	{
		var url = 'leftovertransferinXML.php?RequestType=LoadGPTransferInNo';
		htmlobj=$.ajax({url:url,async:false});
...
{	
	...
	var url = 'leftovertransferinXML.php?RequestType=SaveHeaderDetails&transferIn=' +transferIn+ '&transferInYear=' +transferInYear+  '&GatePassNo=' +GatePassNo+ '&remarks=' +URLEncode(remarks);
	htmlobj=$.ajax({url:url,async:false});
	SaveDetails();
}
		...
		if 	((details!=null) && (Add!=0))
		{
			...
			{			
				for (i = 0; i < binArray.length; i++)
				{
					...
					var url = 'leftovertransferinXML.php?RequestType=SaveBinDetails&transferIn=' +transferIn+ '&transferInYear=' +transferInYear+ '&StyleID=' +URLEncode(StyleID)+ '&BuyerPoNO=' +URLEncode(BuyerPoNO)+ '&MatDetailID=' +MatDetailID+ '&Color=' +URLEncode(Color)+ '&Size=' +URLEncode(Size)+ '&Unit=' +Unit+ '&MainStores=' +MainStores+ '&SubStores=' +SubStores+ '&Location=' +Location+ '&BinID=' +BinID+ '&BinQty=' +BinQty+ '&SubCatID=' +SubCatID +'&grnNo='+grnNo+'&grnYear='+grnYear+ '&GRNType='+grnType;
					var htmlobj=$.ajax({url:url,async:false});	
				}
			}
			else
...
{
	var url = 'leftovertransferinXML.php?RequestType=ResponseValidate&transferIn=' +transferIn+ '&transferInYear=' +transferInYear+ '&validateCount=' +validateCount+ '&validateBinCount=' +validateBinCount;
	var htmlobj=$.ajax({url:url,async:false});	
...
function showReport()
{
	...
	if(TransInNo!="0")
	{		
		newwindow=window.open('leftovertransferinrpt.php?TransferInNo='+TransInNo+'&TransferInYear='+year,'name');
...
{	
    ...
    var url = 'leftovertransferinXML.php?RequestType=LoadPopUpTransIn&state='+state+'&year='+year;
    var htmlobj=$.ajax({url:url,async:false});
...
function loadPopUpReturnToStores(){
	...
	var url = 'leftovertransferinXML.php?RequestType=LoadPopUpHeaderDetails&No='+No+ '&Year='+Year;
	var htmlobj=$.ajax({url:url,async:false});
	LoadHeaderDetailsRequest(htmlobj);
	
	var url = 'leftovertransferinXML.php?RequestType=LoadPopUpDetails&No=' +No+ '&Year=' +Year;
	var htmlobj=$.ajax({url:url,async:false});
...
function LoadHeaderDetailsRequest(htmlobj)
{
	...
	document.getElementById('txtDtmDate').value =XMLdtmDate;	
...
function loadGPnolist()
{
	...
	var url = 'leftovertransferinXML.php?RequestType=getGPNolist';	
		url += '&styleId='+styleId;
		
		htmlobj=$.ajax({url:url,async:false});
		var XMLItem = htmlobj.responseText;
		document.getElementById('cboGatePassNo').innerHTML = XMLItem;		 
	...
	if(SaveValidate(tbl,strMainStores,strSubStores,type))
	{
		for(var i = 1; i < tbl.rows.length ; i++)
		{
			...
			for(var a = 1; a < tbl.rows.length ; a++)
			{
				...
				var gpQty = parseFloat(tbl.rows[a].cells[8].childNodes[0].value);
				
					binAllocation = true;
					var mainSubCatId = tbl.rows[a].cells[4].id;
					var index = tbl.rows[a].cells[10].id;
					
					if(mainSubCatId != subCatID)
					{
						var url = 'leftovertransferinXML.php?RequestType=checkBinavailabiltyforSubcat';
						url += '&strMainStores='+strMainStores;
					...
					if(binAllocation)
					{
						...
						if(pub_binQty>gpQty)
						{
							...
							Bindetails[4] =   tbl.rows[a].cells[8].childNodes[0].value; // IssueQty								
...
{
	var styleID = styleId;
	var url = 'leftovertransferinXML.php?RequestType=loadItem&styleID='+URLEncode(styleID);
	htmlobj=$.ajax({url:url,async:false});
	var XMLItem = htmlobj.responseText;
...
{
	var url = 'leftovertransferinXML.php?RequestType=loadSCNo&styleID='+URLEncode(styleID);
	htmlobj=$.ajax({url:url,async:false});
...
{
	document.frmLOTIListing.submit();
}
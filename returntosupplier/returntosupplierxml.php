...
else if ($RequestType=="SaveDetails")
{
	...
	$GrnBalanceQty = GetTotGrnBalanceQty($grnNoArray[1],$grnNoArray[0]);
	
	if($GrnBalanceQty==0){
		
		$sqlUpdateGrnHeader = " update grnheader set intStatus = 9 where intGrnNo = '$grnNoArray[1]' and intGRNYear = '$grnNoArray[0]' ";
		$db->ExecuteQuery($sqlUpdateGrnHeader);
	}
...
function GetTotGrnBalanceQty($Grn_No, $Grn_Year){
	
global $db;

$dblBalanceQty = 0;

$sql = "select sum(dblBalance) as Bal_Qty from grndetails where intGrnNo = '$Grn_No' and intGRNYear = '$Grn_Year'";

	$result = $db->RunQuery($sql);
	
	while($row=mysql_fetch_array($result)){
		$dblBalanceQty = $row["Bal_Qty"];
	}

...
session_start();
	...
	{	
		 ...
		 $result=findAdvacePayment($supID,$dateFrom,$dateTo,$strPaymentType,$poNo,$poYear,$cboUser);
		 while($row = mysql_fetch_array($result))
		 {
			...
			$ResponseXML .= "<userID><![CDATA[" . $row["userID"] . "]]></userID>\n";
	...
	function findAdvacePayment($supid,$dateFrom,$dateTo,$strPaymentType,$poNo,$poYear,$cboUser)
	{
		global $db;
	
			$strSQL=" SELECT  distinct advancepayment.PaymentNo, advancepayment.paydate,advancepayment.poamount,advancepayment.taxamount,
			advancepayment.totalamount,advancepayment.userID
			FROM advancepayment 

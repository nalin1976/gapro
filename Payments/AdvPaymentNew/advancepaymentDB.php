<?php
	...
	{	
		...
		$otherCharges=$_GET["otherCharges"];
		...
		if(SaveAdvancePayment($paymentno,$supid,$description,$batchno,$draft,$discount,$frightcharge,$couriercharge,$bankcharge,$otherCharges,$poamount,$taxamount,$totalamount,$currency,$strPaymentType,$exRate,$entryNo,$lineNo))
		{ 
...
else if(strcmp($DBOprType,"loadBatchesAccordingToSupCurrency")==0){
$cboSuppliers = $_GET['cboSuppliers'];
header('Content-Type: text/xml'); 
echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";

 $ResponseXML .= "<loadBatches>";
 $SQLC="SELECT
			batch.intBatch,
			...
			WHERE
			suppliers.strSupplierID =  '$cboSuppliers' and posted=0 and intBatchType=2";
//echo $SQLC;
 $resultC = $db->RunQuery($SQLC);
		$ResponseXML= "<option value=\"". '0' ."\"></option>" ;	
...
//--------------------------------------------------------------------------------------------------------------------
else if(strcmp($DBOprType,"LoadSupplierGL")==0){
	//$SupId = $_GET["SupplierId"];
	$SupId = 77;
	$FacCd = $_SESSION["FactoryID"];
	$ResponseXML .= "<Result>";
...
function SaveAdvancePayment($paymentno, $supid, $description, $batchno, $draft, $discount, $frightcharge, $couriercharge,$bankcharge,$otherCharges,$poamount,$taxamount,$totalamount,$currency,$strPaymentType,$exRate,$entryNo,$lineNo) //OK
{
	...
	$sqlEntryNo = "select MAX(intEntryNo+1)as EntryNo from batch where intBatch=$batchno and intBatchType='2'";
	$result = $db->ExecuteQuery($sqlEntryNo);
	while($row = mysql_fetch_array($result)){
	$EntryNo = $row["EntryNo"];
	}

		$strSQL="INSERT INTO advancepayment (PaymentNo, paydate, supid, description, batchno, draftno, discount, frightcharge, couriercharge,bankcharge,otherCharges,poamount,taxamount,totalamount,currency,userID,strType,userFactory,dblexRate,intEntryNo,intLineNo) VALUES('$paymentno',now(), '$supid', '$description', '$batchno','$draft','$discount','$frightcharge','$couriercharge','$bankcharge','$otherCharges','$poamount','$taxamount','$totalamount','$currency','$userID','$strPaymentType','$userFactory','$exRate','$EntryNo','$lineNo')";
	
	//echo $strSQL;
	$result = $db->ExecuteQuery($strSQL);
	$sqlUpBatch="update batch set intUsed='1' where intBatch='$batchno' and intBatchType='2'";
	$db->RunQuery($sqlUpBatch);
	
	$strSQLE="UPDATE batch SET intEntryNo='$EntryNo' WHERE intBatch='$batchno' and intBatchType='2'";
	$db->RunQuery($strSQLE);
...
$sql="select ex.rate,b.strCurrency CId,ct.strCurrency from exchangerate ex 
	...
	where ex.intStatus='1' and b.intBatch='$batch' and b.intBatchType='2'";

  ...
  <tr>
	...
	{ 
      ...
      <tr>
        ...
        <td class="normalfntRiteTAB"><?php echo(number_format($row["poamount"],2))  ; ?></td>
        <td class="normalfntRiteTAB"><?php echo(number_format($row["taxamount"],2)) ; ?></td>
        <td class="normalfntRiteTAB"><?php echo(number_format($row["charges"],2))  ; ?></td>
        <td class="normalfntMidTAB"><?php echo(number_format($row["discount"],2))  ; ?></td>
        <td class="normalfntRiteTAB"><?php 
		
		$total += $row["totalamount"];
		
		echo(number_format($row["totalamount"],2)); ?></td>
        </tr>
		<?php
		}
		?>
  ...
  <tr>
    <td><table width="100%" height="60" border="0" cellpadding="0" cellspacing="0">
      <tr>
		...
		$strSQL="SELECT advancepaymentsglallowcation.Amount,advancepaymentsglallowcation.glAccNo,companies.strName, glaccounts.strDescription,advancepayment.userFactory, companies.strComCode FROM advancepaymentsglallowcation INNER JOIN glaccounts ON (advancepaymentsglallowcation.glAccNo = glaccounts.strAccID) INNER JOIN advancepayment ON (advancepaymentsglallowcation.paymentNo = advancepayment.PaymentNo) INNER JOIN companies ON advancepayment.userFactory = companies.intCompanyID WHERE advancepaymentsglallowcation.paymentNo='$intPaymentNo'  AND advancepaymentsglallowcation.strType='$strPaymentType' AND advancepaymentsglallowcation.Amount>0 GROUP BY advancepaymentsglallowcation.Amount,advancepaymentsglallowcation.glAccNo,companies.strName,glaccounts.strDescription,advancepayment.userFactory,companies.strComCode";

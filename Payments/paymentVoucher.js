...
function getBank()
{
	var batchId=document.getElementById("cboBatchNo").value.trim();
	if(batchId=="0"){
		document.getElementById("txtBank").value="0";
		document.getElementById("cboCurrency").value="0";
		document.getElementById("txtExtRange").value="";
		document.getElementById("txtEntryNo").value="";
		return false;
	}
	
	var url='paymentVoucherDB.php?DBOprType=getBank&batchID='+ batchId ;
	htmlobj=$.ajax({url:url,async:false});
	
	var XMLbankID = htmlobj.responseXML.getElementsByTagName("bankID");
	var XMLbank = htmlobj.responseXML.getElementsByTagName("bankName");
	var XMLCurrency = htmlobj.responseXML.getElementsByTagName("Currency");
	var XMLExtRate = htmlobj.responseXML.getElementsByTagName("ExtRate");
	var XMLEntryNo = htmlobj.responseXML.getElementsByTagName("EntryNo");
	
	
	if(XMLbankID.length > 0){
		var bank = XMLbank[0].childNodes[0].nodeValue;
		document.getElementById("txtBank").value=XMLbankID[0].childNodes[0].nodeValue;
		document.getElementById("cboCurrency").value=XMLCurrency[0].childNodes[0].nodeValue;
		document.getElementById("txtExtRange").value=XMLExtRate[0].childNodes[0].nodeValue;
		document.getElementById("txtEntryNo").value=XMLEntryNo[0].childNodes[0].nodeValue;
		
	}
	//if(doument.getElementById('cboSupliers').value.trim()!="")
		getDets();
}
...
function setBatchCurrency(obj){
	var url='paymentVoucherDB.php?DBOprType=getExRate&invoiceNo=&batchNo='+batchNo;
	htmlobj=$.ajax({url:url,async:false});
	var XMLCurrency=htmlobj.responseXML.getElementsByTagName('Currency');
...
function clearPayementVoucherIF()
{
		...
		document.getElementById("hdnVId").value="";
		clearSelectControl("cboSchedual");
		
		var strtable="";
		document.getElementById("tblInvoice").tBodies[0].innerHTML=strtable;
		//getAdvPaymentNo(1)
...
{
	var batchId=document.getElementById('cboBatchNo').value.trim();
	var currency=document.getElementById('cboCurrency').value.trim(); 
	var bt='&batch=';
	if(batchId!='0'){
		bt=bt+batchId+'&currency='+currency;
	}
	var supID=document.getElementById("cboSupliers").value;
	if(supID==0){return;}

		var type = document.getElementById("cboPaymentType").value.trim();
		var url= 'paymentVoucherDB.php?DBOprType=getSupData&strPaymentType=' + type + '&supID=' + supID;
		htmlobj=$.ajax({url:url,async:false});
	...
	document.getElementById("txtTaxCode").value="";
			...
			var XMLTaxType = htmlobj.responseXML.getElementsByTagName("strTaxType");
			...
			if(XMLTaxType.length > 0){
				document.getElementById("txtTaxCode").value=XMLTaxType[0].childNodes[0].nodeValue;
			}
	
	var path='paymentVoucherDB.php?DBOprType=getScheduals&strPaymentType=' + type + '&supID=' + supID+bt;
	htmlobj1=$.ajax({url:path,async:false});
	var XMLSchNo = htmlobj1.responseXML.getElementsByTagName("schNo");
			...
			if(XMLSchNo.length==0)
			{
				...
				document.getElementById("txtTaxCode").value="";
				var strIvnsTable="";
				document.getElementById("tblInvoice").tBodies[0].innerHTML=	strIvnsTable	
...
function getDets(){
	...
	var path='paymentVoucherDB.php?DBOprType=getInvoiceSchedual&strPaymentType=' + type + '&schdNo=' + schdNo + '&supid=' + supID+bt;
	...
	var XMLScheduelNo	= htmlobj.responseXML.getElementsByTagName("ScheduelNo");
	var XMLCurrency	= htmlobj.responseXML.getElementsByTagName("Currency");
	...
	if(XMLInvNo.length==0)
	{
		...
		var sss="";
		if(supID.trim()!=""){xname.options[document.getElementById("cboSchedual").selectedIndex].text;}
		if(supID.trim()==""){		
			alert("There is no any Invoice with " + sss);
		}

		document.getElementById("cboSchedual").innerHTML="";
		document.getElementById("txtAccPacID").value="";
		document.getElementById("txtTotalAmont").value="";
		document.getElementById("tblInvoice").tBodies[0].innerHTML="";
		return;
	...
	for ( var loop = 0; loop < XMLInvNo.length; loop ++)
	 {
		...
		var SNo =	XMLScheduelNo[loop].childNodes[0].nodeValue;
		...
		strIvnsTable+="<tr class=\""+cls+"\">"+
						...
						"<td class=\""+cls+"\" style=\"text-align:right;\">" + SNo + "</td>"+
						"<td class=\""+cls+"\"><div  onmouseover=\"highlight(this.parentNode)\" style=\"text-align:left;\">" + invNo + "</div></td>"+
						...
						"<td class=\""+cls+"\" class=\"normalfnt\">" + Currency + "</td>"+
...
function chkUnchk(obj){
	var tbl=document.getElementById('tblInvoice').tBodies[0];
...
function loadInvoiceSchedual()
{
	...
	var type = document.getElementById("cboPaymentType").value.trim();
	var url='paymentVoucherDB.php?DBOprType=getInvoiceSchedual&strPaymentType=' + type + '&schdNo=' + schdNo + '&supid=' + supID;
	htmlobj=$.ajax({url:url,async:false});
	var XMLInvNo = htmlobj.responseXML.getElementsByTagName("invNo");
	var XMLAmount = htmlobj.responseXML.getElementsByTagName("amount");
	var XMLPaidAmt = htmlobj.responseXML.getElementsByTagName("paidAmt");
	...
	var strIvnsTable="";
			...
			if(XMLInvNo.length==0)
			{
				...
				alert("There is no any Invoice with " + sss);
				//strIvnsTable+="</table>"
				//document.getElementById("divInvsList").tBodies[0].innerHTML=strIvnsTable;
				//return;
			...
			for ( var loop = 0; loop < XMLInvNo.length; loop ++)
			 {
				...
				strIvnsTable+="<tr class=\""+cls+"\">"+
								"<td class=\""+cls+"\"><input type=\"checkbox\" onmouseover=\"highlight(this.parentNode)\" name=\"checkbox\" id=\"checkbox\"  checked=\"checked\"   onclick=\"calculetTheTotalAmount()\"/></td>"+
								"<td class=\""+cls+"\" onmouseover=\"highlight(this.parentNode)\" style=\"text-align:right;\">" + SNo + "</td>"+
								"<td class=\""+cls+"\"><div onmouseover=\"highlight(this.parentNode)\" style=\"text-align:left;\">" + invNo + "</div></td>"+
								"<td class=\""+cls+"\" onmouseover=\"highlight(this.parentNode)\" style=\"text-align:right;\">" + amount + "</td>"+
								"<td class=\""+cls+"\" onmouseover=\"highlight(this.parentNode)\" style=\"text-align:right;\">" + paidAmount + "</td>"+
...
var theRow=document.getElementById("tblInvoice").getElementsByTagName("TR")
	...
	for(var i=1;i<theRow.length;i++)
	{
		...
		if(cells[0].firstChild.checked==true)
		{
			...
			payAmt=cells[6].firstChild.value
			//	alert(payAmt)
	...
	document.getElementById("txtTotalAmont").value=totAmount.toFixed(4);
...
function paymentVoucherSave()
{
	...
	var currency=document.getElementById("cboCurrency").value.trim();//change to currency id
	...
	var url='paymentVoucherDB.php?DBOprType=saveNewPaymentVoucher&strPaymentType=' + strPaymentType + '&voucherNo=' + voucherNo + '&schedualeNo=' + schedualeNo + '&description=' + description + '&batchNo=' + batchNo + '&bankCode=' + bankCode + '&supID=' + supID + '&chequeNo=' + chequeNo + '&datex=' + payDate + '&amount=' + amount + '&currency=' + currency + '&rate=' + rate + '&accpakID=' + accpakID +'&userid=' + userid+'&entryNo='+entryNo;
	htmlobj=$.ajax({url:url,async:false});

			var XMLResult = htmlobj.responseXML.getElementsByTagName("Result");
			var XMLvoucher = htmlobj.responseXML.getElementsByTagName("voucherNo");
			if (XMLResult[0].childNodes[0].nodeValue == "True")
			{
				...
				for(var i=1;i<theRow.length;i++)
				{
					...
					if(cells[0].firstChild.checked==true)
					{
						...
						var scheduleNo=cells[1].innerHTML;
						//alert(cells[1].innerHTML);
						var invoiceNo=cells[2].firstChild.innerHTML;
						var amount=cells[3].innerHTML;
						var balance=cells[5].innerHTML;
						var paidAmount=cells[6].firstChild.value;
						
						var type = document.getElementById("cboPaymentType").value.trim();
						strPaymentType = type;
						var urlg='paymentVoucherDB.php?DBOprType=saveNewPaymentVoucherDetails&strPaymentType=' + strPaymentType + '&voucherNo=' + voucherNo + '&invoiceNo=' + URLEncode(invoiceNo) + '&amount=' + amount + '&balance=' + balance + '&paidAmount=' + paidAmount+'&scheduleNo='+scheduleNo;
	
						htmlobj=$.ajax({url:urlg,async:false});
						var XMLResult = htmlobj.responseXML.getElementsByTagName("Result");
						if (XMLResult[0].childNodes[0].nodeValue == "True")
						{
						}
				...
				document.getElementById("hdnVId").value=voucherNo;
			...
			for ( var loop = 0; loop < XMLpayVoucherNo.length; loop ++)
			 {
				...
				var tax=XMLTax[loop].childNodes[0].nodeValue;
					tax = new Number(tax).toFixed(4);
					amount=amount-tax
					
				var totAmt=parseFloat(amount)+parseFloat(tax)
				    totAmt = new Number(totAmt).toFixed(4);
...
function fillAvailablePaymentData()
{
	...
	strPaymentType = type;
	var path='paymentVoucherDB.php?DBOprType=getPaymentVouchers&strPaymentType=' + strPaymentType + '&supID=' + supID + '&dateFrom=' + dateFrom + '&dateTo=' + dateTo + '&voucherNo=' +voucherNo+ '&invoiceNo=' +URLEncode(invoiceNo);
	htmlobj=$.ajax({url:path,async:false});
	var XMLpaySupId = htmlobj.responseXML.getElementsByTagName("strSupCode");
			var XMLpayVoucherNo = htmlobj.responseXML.getElementsByTagName("payVoucherNo");
			var XMLdatex = htmlobj.responseXML.getElementsByTagName("datex");
			var XMLamount = htmlobj.responseXML.getElementsByTagName("amount");
			var XMLchequeNo= htmlobj.responseXML.getElementsByTagName("chequeNo");
			...
			for ( var loop = 0; loop < XMLpayVoucherNo.length; loop ++)
			 {
				...
				//<input type=\"checkbox\" name=\"checkbox\" value=\"checkbox\" />
					...
					strPVoucher+="<tr class=\"bcgcolor-tblrowWhite\" onmouseover=\"this.style.background ='#D6E7F5';\" onmouseout=\"this.style.background=''\">"+
								"<td  class=\""+ cls +"\" id=\""+supid+"\" style='text-align:left;'>" + XMLTitle[loop].childNodes[0].nodeValue+ "</td>"+
								"<td  class=\""+ cls +"\" style='text-align:right;'>" + VoucherNo + "</td>"+
								"<td  class=\""+ cls +"\">" + datex + "</td>"+
								"<td  class=\""+ cls +"\" style=\"text-align:right\">" + amount + "</td>"+
								"<td  class=\""+ cls +"\" style=\"text-align:right\">" + chequeNo + "</td>"+
								/*"<td  class=\""+ cls +"\" style=\"text-align:center\">" + type + "</td>"+*/
								"<td  class=\""+ cls +"\"><div align=\"center\"><img src=\"../images/view.png\" onclick=\"previewReportPV(this,strPaymentType)\"/></div></td>"+
								"<td class=\""+ cls +"\"><div align=\"center\"><img src=\"../images/view.png\" onclick=\"previewReportPS(this,strPaymentType)\"/></div></td>"+
								"</tr>"
			...
			document.getElementById("tblPVData").tBodies[0].innerHTML=strPVoucher
			
	
	/*CreateXMLHttpForPVoucher() 
	xmlHttpPV.onreadystatechange = HandlePaymentVoucher;
    xmlHttpPV.open("GET",path , true);
	xmlHttpPV.send(null);*/ 
}
...
function voucherReport(){
	var selectedPaymentNo 	=  	document.getElementById('hdnVId').value.trim();
	var strPaymentType		= 	document.getElementById('cboPaymentType').value.trim();
	if(selectedPaymentNo==""){
		alert("Please save details.");
		return false;
	}

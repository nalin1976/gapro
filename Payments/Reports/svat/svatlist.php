...
function checkDate()
{
	...
	function showReport()
{
		var suplierId  = document.getElementById("cboFactory").value;
		...
		if(ReportType==1)
		{
			...
			newwindow=window.open('svat4rpt.php?dateFrom='+dateFrom+'&dateTo='+dateTo+'&supplierId='+suplierId+'&vatRate='+Vat+'&currency='+currency,'svat4rpt');
			if (window.focus) {newwindow.focus()}
		}
		if(ReportType==2)
		{
			newwindow=window.open('svatrpt.php?dateFrom='+dateFrom+'&dateTo='+dateTo+'&supplierId='+suplierId+'&vatRate='+Vat+'&currency='+currency,'svatrpt');
			if (window.focus) {newwindow.focus()}
		}
		if(ReportType==3)
		{
		...
		window.open('svat4rpt.php?dateFrom='+dateFrom+'&dateTo='+dateTo+'&supplierId='+suplierId+'&vatRate='+Vat+'&currency='+currency,'svat4rpt');
		window.open('svatrpt.php?dateFrom='+dateFrom+'&dateTo='+dateTo+'&supplierId='+suplierId+'&vatRate='+Vat+'&currency='+currency,'svatrpt');
...
<form id="frmSVATReport" name="frmSVATReport" >
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="1" align="center" class="bcgl1">
      <tr>
      ...
      <tr>
        <td class="normalfnt">&nbsp;</td>
        <td colspan="2" class="normalfnt">Supplier Name</td>
        <td colspan="3"><select name="cboFactory" style="width:265px" id="cboFactory" tabindex="1">
          <?php
		$SQL ="select distinct S.strSupplierID,strTitle
				from invoiceheader IH
				inner join suppliers S on IH.strSupplierId=S.strSupplierID
				where IH.intSuspendedVat=1
				order by strTitle";	
	...
	while($row = mysql_fetch_array($result))
	{
      ...
      <tr>
        <td class="normalfnt" style="text-align:center"><input name="chkDate" id="chkDate" type="checkbox" checked="checked" value="1" onclick="checkDate()" /></td>
        <td colspan="2" class="normalfnt">Invoice Date From</td>
        <td><span class="normalfnt">
          <input name="txtFromDate" type="text" tabindex="7" class="txtbox" id="txtFromDate" style="width:90px"  onmousedown="DisableRightClickEvent();" onmouseout="EnableRightClickEvent();" onfocus="return showCalendar(this.id, '%d/%m/%Y');" value="<?php echo date('d/m/Y');?>" onkeypress="return ControlableKeyAccess(event);"  onclick="return showCalendar(this.id, '%d/%m/%Y');"/><input name="reset" type="reset"  class="txtbox" style="visibility:hidden;"   onclick="return showCalendar(this.id, '%d/%m/%Y');" value="" />
        </span></td>
        <td><span class="normalfnt">To</span></td>
        <td width="123" class="normalfnt"><input name="txtToDate" type="text" tabindex="7" class="txtbox" id="txtToDate" style="width:90px"  onmousedown="DisableRightClickEvent();" onmouseout="EnableRightClickEvent();" onfocus="return showCalendar(this.id, '%d/%m/%Y');" value="<?php echo date('d/m/Y');?>" onkeypress="return ControlableKeyAccess(event);"  onclick="return showCalendar(this.id, '%d/%m/%Y');"/><input name="resetto" type="reset"  class="txtbox" style="visibility:hidden;"   onclick="return showCalendar(this.id, '%d/%m/%Y');" value="" /></td>
        </tr>
      <tr>
        <td class="normalfnt">&nbsp;</td>
        <td colspan="2" class="normalfnt">VAT Percentage<span class="compulsoryRed"> *</span></td>
        <td width="129" class="normalfnt"><input name="txtVat" type="text" class="txtbox" id="txtVat" onkeypress="return CheckforValidDecimal(this.value, 2,event);"  style="width:90px" maxlength="3" tabindex="3"/>&nbsp;&nbsp;%</td>
      ...
      <tr>
        ...
        <td colspan="3" class="normalfnt"><select name="cboCurrency" id="cboCurrency" style="width:165px" disabled="disabled">

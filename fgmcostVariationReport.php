...
history_orders.strRevisedReason,
...
history_orders.intApprovalNo ;";
		...
		{			
			...
			$revisedDate = $row["strRevisedDate"];
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC" class="tablez">
          ...
          <tr>
			...
			$oldFOBValue = number_format(($reaFOB * $totalOrderQty),2);
			?></td>
          </tr>
		  <tr>
		    <td colspan="12">&nbsp;</td>
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC" class="tablez">
          <tr>
            ...
            <td width="10%" bgcolor="#CCCCCC" class="bcgl1txt1B">COST PC</td>
            <td width="8%" bgcolor="#CCCCCC" class="bcgl1txt1B">VALUE (USD)</td>
            <td width="8%" bgcolor="#CCCCCC" class="bcgl1txt1B">SHORTAGE</td>
          </tr>
          <?php
		  $index=1;	//for print IndexNo
		  $loop=0;  //for MainCategories
		  ...
		  $SQL_Cetegory="SELECT DISTINCT matmaincategory.strDescription, matmaincategory.intID
			...
			while($row_Category = mysql_fetch_array($result_Category))
			{
				...
				{	
					...
					echo "<tr>"."<td>&nbsp;</td><td class=\"normalfnt2BITAB\">";
					echo $row_Category["strDescription"];
					echo"</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>"; 					
					...
					while($row_new = mysql_fetch_array($result_new))
					{
						...
						if ($oldrow_order["dblTotalValue"] != $row_order["dblTotalValue"])
						{
							$shortage =  $row_order["dblTotalValue"] - $oldrow_order["dblTotalValue"];
							echo "<td class=\"normalfntRiteTAB\">".number_format($shortage,2)."</td>";
						}
...
FROM ((history_orderdetails INNER JOIN matitemlist ON history_orderdetails.intMatDetailID = matitemlist.intItemSerial) INNER JOIN matmaincategory ON matitemlist.intMainCatID = matmaincategory.intID) INNER JOIN itempurchasetype ON history_orderdetails.intOriginNo = itempurchasetype.intOriginNo WHERE (((history_orderdetails.intStyleId)='JB1509-06-21-08') AND ((matitemlist.intMainCatID)='" .$row_Category["intID"] ."'))
...
(SELECT matitemlist.intItemSerial FROM ((orderdetails INNER JOIN matitemlist ON orderdetails.intMatDetailID = matitemlist.intItemSerial) INNER JOIN matmaincategory ON matitemlist.intMainCatID = matmaincategory.intID) INNER JOIN itempurchasetype ON orderdetails.intOriginNo = itempurchasetype.intOriginNo WHERE (((orderdetails.intStyleId)='$strStyleID') AND ((matitemlist.intMainCatID)='" .$row_Category["intID"] ."')))
          ...
          <tr>
			...
			?></td>
            <td  class="normalfntRiteTAB"><?php echo number_format(($TotaleDrectCost * $totalOrderQty),2);?></td>
          </tr>
          <tr>
            <td colspan="10" class="normalfntTAB">C&amp;M EARNED + (UPcharge (<?php echo number_format($upcharge,4);  ?>) Reason for Upcharge : <?php echo $upchargereason; ?> </td>
            <td class="normalfntRiteTAB"><?php
          ...
          <tr>
			...
			?></td>
            <td class="normalfntRiteTAB"><?php echo number_format(($costOfSale * $totalOrderQty),2); ?></td>
          </tr>
          <tr>
            <td colspan="10" class="normalfntTAB">TOTAL COST  ( Cost of Sales + Coporate Cost) </td>
            <td class="normalfntRiteTAB"><?php echo number_format(($costOfSale + $coporatecost) ,4); ?></td>
            <td class="normalfntRiteTAB"><?php echo number_format((($costOfSale + $coporatecost) * $totalOrderQty) ,2); ?></td>
          </tr>
          <tr>
            <td colspan="10" class="normalfntSMB">Revised Date/Reason : </td>
          </tr>
          <tr>
            <td colspan="12" class="normalfntTAB">
			<?php
			
			echo $revisedDate.'/'.$strrevicedReason ;
          ...
          <tr bgcolor="#CCCCFF">
			...
			?></td>
            <td class="normalfntRiteTAB"><?php echo number_format($olddirectCostValue,2);?></td>
          </tr>
          <tr bgcolor="#CCCCFF">
          ...
          <tr bgcolor="#CCCCFF">
			...
			echo number_format($oldOHCostRate,4); 
			//echo "<br>$intQty   " . " $subcontractQty";
			//echo '   $reaSMV / $reaEfficiencyLevel / 100 * $companycostperminute <br>';
			//echo "   $reaSMV " . '/' . $reaEfficiencyLevel . '/' .  '100 * ' . $companycostperminute . '<br>';
			...
			<?php
			 	...
			 	echo number_format($oldOHCostValue,2); 
          ...
          <tr bgcolor="#CCCCFF">
            <td colspan="10" class="normalfntTAB">CORPORATE COST </td>
            <td class="normalfntRiteTAB"><?php
			echo number_format($oldcorporateRate,4);
			?></td>
            <td class="normalfntRiteTAB"><?php
			echo number_format($oldcorporateValue,2);
			?></td>
          </tr>
          <tr bgcolor="#CCCCFF">
            <td colspan="10" class="normalfntTAB">NET MARGIN </td>
			...
			//echo '(' . $reaSMVRate . ' * ' . $reaSMV . ' ) - (' . $reaFOB .' 0.05 ) - ' . $labsuboverheadcost;
			echo number_format($oldnetMarginRate,4);
			//echo "<br>$reaSMVRate";
			
			?></td>
            <td class="normalfntRiteTAB"><?php echo number_format($oldnetMargeinValue,2); ?></td>
          </tr>
          <tr bgcolor="#CCCCFF">
            <td colspan="10" class="normalfntTAB"><strong>SELLING PRICE  (FOB / CIF / LDP ) </strong></td>
            <td class="normalfntRiteTAB"><?php
          ...
          <tr bgcolor="#CCCCFF">
            <td colspan="10" class="normalfntTAB">COST OF SALE  (RM + Serv. + Othr + Fin. + Esc + Lab.Oh + Sub.) </td>
            <td class="normalfntRiteTAB"><?php
			echo number_format($oldcostOfSaleRate,4);
			
			?></td>
            <td class="normalfntRiteTAB"><?php echo number_format($oldcostOfSaleValue,2); ?></td>
          </tr>
          <tr bgcolor="#CCCCFF">
            <td colspan="10" class="normalfntTAB">TOTAL COST  ( Cost of Sales + Coporate Cost) </td>
            <td class="normalfntRiteTAB"><?php echo number_format($oldtotCostRate,4); ?></td>
            <td class="normalfntRiteTAB"><?php echo number_format($oldtotCostValue,2); ?></td>
          </tr>
     		<?php
			}
			?>

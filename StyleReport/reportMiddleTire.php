...
if (strcmp($RequestType,"getBuyerPO") == 0)
{
	ob_get_clean();
	
	$styleID = $_GET["styleID"];
	
	$sql = "SELECT DISTINCT strBuyerPONO FROM styleratio";	
	//echo $sql;
	if ($styleID!="")
	{
		$sql .= " WHERE styleratio.intStyleId = '$styleID' ";
	}
		$sql .= "  ORDER BY strBuyerPONO ";
...
else if (strcmp($RequestType,"getMaterial") == 0)
{
	ob_get_clean();
...
else if (strcmp($RequestType,"getCategory") == 0)
{
	...
	$sql = "
SELECT DISTINCT matsubcategory.StrCatName,  matsubcategory.intSubCatNo FROM purchaseorderdetails INNER JOIN matitemlist ON purchaseorderdetails.intMatDetailID  = matitemlist.intItemSerial INNER JOIN matsubcategory ON  matitemlist.intSubCatID= matsubcategory.intSubCatNo WHERE matitemlist.intItemSerial<>0 ";
...
else if (strcmp($RequestType,"getItemDetails") == 0)
{
	...
	$sql = "SELECT DISTINCT matitemlist.intItemSerial,  matitemlist.strItemDescription FROM orderdetails INNER JOIN matitemlist ON orderdetails.intMatDetailID  = matitemlist.intItemSerial  WHERE matitemlist.intItemSerial<>0 ";
	
	if ($styleID)
	{
		$sql = $sql." and orderdetails.intStyleId='$styleID'";
	}
	...
	if ($categoryNo!="")
	{
		$sql = $sql." and matitemlist.intSubCatID=$categoryNo ";
	}
	
	if($matItem !="")
		$sql .= " and  matitemlist.strItemDescription like '%$matItem%'";
		
	$sql = $sql." ORDER BY matitemlist.strItemDescription ";
	//echo $sql;
	$result = $db->RunQuery($sql);
	echo "<option value=\"\"></option>";
	while ($row = mysql_fetch_array($result))

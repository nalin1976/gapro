	...
	<tr>
    	<td>
            <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tablez">
                <thead>          
                    <tr height="25" >
                        <td width="19" bgcolor="#CCCCCC" class="normalfntBtab">&nbsp;</td>
                        <td width="110" bgcolor="#CCCCCC" class="normalfntBtab">Style No</td>
                        <td width="139" bgcolor="#CCCCCC" class="normalfntBtab">Order No</td>
                        <td width="78" bgcolor="#CCCCCC" class="normalfntBtab">SCNo</td>
                        <td width="122" bgcolor="#CCCCCC" class="normalfntBtab">Buyer PONo</td>            
                        <td width="51" bgcolor="#CCCCCC" class="normalfntBtab">Mat</td>
                        <td width="295" bgcolor="#CCCCCC" class="normalfntBtab">Description</td>
                        <td width="95" bgcolor="#CCCCCC" class="normalfntBtab">Color</td>
                        <td width="95" bgcolor="#CCCCCC" class="normalfntBtab">Size</td>
                        <td width="28" bgcolor="#CCCCCC" class="normalfntBtab">Unit</td>			
                        <td width="33" bgcolor="#CCCCCC" class="normalfntBtab">PO Qty</td>
                        <td width="57" bgcolor="#CCCCCC" class="normalfntBtab">Confirm GRN Qty </td>
                        <td width="31" bgcolor="#CCCCCC" class="normalfntBtab">Rate</td>
                        <td width="62"  bgcolor="#CCCCCC" class="normalfntBtab">Amount</td>
                        <td width="79"  bgcolor="#CCCCCC" class="normalfntBtab">Last GRN Date</td>
                        <td width="79"  bgcolor="#CCCCCC" class="normalfntBtab">Last GRN Confirmed</td>
                        <td width="60"  bgcolor="#CCCCCC" class="normalfntBtab">Issued Qty</td>
                    </tr>
                </thead>
<?php 
...
?>
 <tr bgcolor="#E4E4E4" height="20">	
	<td colspan="4" class="normalfnt" style="text-align:center">PONo :&nbsp;<?php echo $details["intYear"].'/'.$details["intPoNo"]; ?>&nbsp;&nbsp;</td>
	...
	<td colspan="10" class="normalfnt">Supplier : &nbsp;<?php echo($details["strTitle"]); ?></td>
	</tr>
<?php
}
?>
          ...
          <tr  onmouseover="this.style.background ='#ECECFF';" onmouseout="this.style.background=''" height="20">	
            ...
            <td class="normalfntRiteTAB"><?php echo GetLastGRNDate($details["intPoNo"],$details["intYear"],$details["intStyleId"],$details["intMatDetailID"],$details["strColor"],$details["strSize"],$details["strBuyerPONO"])   ; ?></td>
            <td class="normalfntRiteTAB"><?php echo GetLastConfirmedGRNDate($details["intPoNo"],$details["intYear"],$details["intStyleId"],$details["intMatDetailID"],$details["strColor"],$details["strSize"],$details["strBuyerPONO"])   ; ?></td>
            <td class="normalfntRiteTAB"><?php echo GetIssueQty($details["intPoNo"],$details["intYear"],$details["intStyleId"],$details["intMatDetailID"],$details["strColor"],$details["strSize"],$details["strBuyerPONO"])   ; ?></td>
...
function GetLastGRNDate($prmPONo, $prmPOYear, $prmStyleID, $prmItemCode, $prmColor, $prmSize, $prmBuyerPONo){
	
global $db;
$LastGRNDate = '';

$sql = " SELECT Max(grnheader.dtmRecievedDate) AS LastGRNDate ".
       ...
       " WHERE grnheader.intPoNo = '$prmPONo' AND grnheader.intYear = '$prmPOYear' AND grndetails.intStyleId = '$prmStyleID' AND ".
       "       grndetails.intMatDetailID = '$prmItemCode' AND grndetails.strColor = '$prmColor' AND grndetails.strSize = '$prmSize' AND ".
       "       grndetails.strBuyerPONO = '$prmBuyerPONo'";
       //echo $sql;
	$result = $db->RunQuery($sql);
	
	while($row=mysql_fetch_array($result)){
		$LastGRNDate = $row["LastGRNDate"];		
	}
	
	$LastGRNDate = date("Y-m-d", strtotime($LastGRNDate));
	
	if($LastGRNDate == '1970-01-01')
		return '';
	else		
		return $LastGRNDate ; 	
}

function GetLastConfirmedGRNDate($prmPONo, $prmPOYear, $prmStyleID, $prmItemCode, $prmColor, $prmSize, $prmBuyerPONo){
	
global $db;
$LastGRNConfirmedDate = '';

$sql = " SELECT Max(grnheader.dtmConfirmedDate) AS LastGRNConfirmedDate ".
	   " FROM grnheader Inner Join grndetails ON grnheader.intGrnNo = grndetails.intGrnNo AND grnheader.intGRNYear = grndetails.intGRNYear ".
       " WHERE grnheader.intPoNo = '$prmPONo' AND grnheader.intYear = '$prmPOYear' AND grndetails.intStyleId = '$prmStyleID' AND ".
       "       grndetails.intMatDetailID = '$prmItemCode' AND grndetails.strColor = '$prmColor' AND grndetails.strSize = '$prmSize' AND ".
       "       grndetails.strBuyerPONO = '$prmBuyerPONo'";
	...
	while($row=mysql_fetch_array($result)){
		$LastGRNConfirmedDate = $row["LastGRNConfirmedDate"];		
	}
	
	$LastGRNConfirmedDate = date("Y-m-d", strtotime($LastGRNConfirmedDate));
	
	if($LastGRNConfirmedDate == '1970-01-01')
		return '';
	else		
		return $LastGRNConfirmedDate ; 	
}

function GetIssueQty($prmPONo, $prmPOYear, $prmStyleID, $prmItemCode, $prmColor, $prmSize, $prmBuyerPONo){
    
    global $db;
    $_dblIssuedQty = 0;
    
    
    $sql = " SELECT Sum(issuesdetails.dblQty) AS Issue_Qty
              FROM issuesdetails INNER JOIN grndetails ON issuesdetails.intGrnNo = grndetails.intGrnNo AND issuesdetails.intGrnYear = grndetails.intGRNYear AND issuesdetails.intStyleId = grndetails.intStyleId AND issuesdetails.strBuyerPONO = grndetails.strBuyerPONO AND issuesdetails.intMatDetailID = grndetails.intMatDetailID AND issuesdetails.strColor = grndetails.strColor AND issuesdetails.strSize = grndetails.strSize
                   INNER JOIN grnheader ON grndetails.intGrnNo = grnheader.intGrnNo AND grndetails.intGRNYear = grnheader.intGRNYear
              WHERE grnheader.intPoNo = '$prmPONo' AND grnheader.intYear = '$prmPOYear' AND grndetails.intStyleId = '$prmStyleID' AND grndetails.intMatDetailID = '$prmItemCode' AND grndetails.strColor = '$prmColor' AND grndetails.strSize = '$prmSize' AND grndetails.strBuyerPONO = '$prmBuyerPONo'";
   // echo $sql."<br />";
    $result = $db->RunQuery($sql);
    while($row = mysql_fetch_array($result)){
       $_dblIssuedQty = $row["Issue_Qty"];
    }

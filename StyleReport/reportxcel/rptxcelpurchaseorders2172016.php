				...
				//echo  $detailSql;
				 ...
				 $i=5;
				   xlsWriteLabel(2,5,"PURCHASE ORDER REGISTER"); 
					...
					xlsWriteLabel(4,14,"Unit Price");
					xlsWriteLabel(4,15,"Amount");
					xlsWriteLabel(4,16,"GRN Location");
				 ...
				 {			 	
					...
					xlsWriteNumber($i,14,number_format($row["dblUnitPrice"],4));
					xlsWriteNumber($i,15,number_format($row["amount"],4));
					xlsWriteLabel($i,16, GetGRNLocation($row["intPoNo"],$row["intYear"]));
					$totalQty += $row["dblQty"];
					...
					xlsWriteNumber($i,15,round($totalAmount,2));
				 	$i += 2;				 	 
					xlsEOF();

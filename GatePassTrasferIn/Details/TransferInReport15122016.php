...
?>			</td>
	...
	while($row2=mysql_fetch_array($resultDetail2))
	{
			...
			//$Qty1				=$row2["dblQty"];	
			 ...
			 $sqlBin ="SELECT distinct strBinName,SL.strLocName,ST.strUnit,ST.dblQty FROM stocktransactions ST
Inner Join storeslocations SL ON SL.strMainID = ST.strMainStoresID AND ST.strSubStores = SL.strSubID AND ST.strLocation = SL.strLocID 
INNER JOIN storesbins SB ON ST.strBin = SB.strBinID AND ST.strMainStoresID=SB.strMainID AND ST.strSubStores=SB.strSubID and ST.strLocation = SB.strLocID WHERE intDocumentYear = '$pubTransInYear' AND intStyleId = '".$row1["intStyleId"]."' AND strBuyerPoNo = '".$row1["strBuyerPONO"]."' 
AND intDocumentNo ='$pubTransInNo' AND strColor = '".$row2["strColor"]."' AND strSize ='".$row2["strSize"]."' AND strType = 'TI' 
AND intMatDetailId = '".$row2["intMatDetailId"]."'" /*"SELECT strBinName,SL.strLocName,ST.dblQty as BinQty,ST.strUnit FROM stocktransactions ST
...
AND intMatDetailId = '".$row2["intMatDetailId"]."'"*/;
		...
		<?php echo $Qty1;
				$sum1 +=$Qty1;
		?></td>
        <td class="normalfntTAB"><?php echo $row["strLocName"].':'.$row["strBinName"];?></td>
		<?php
		$printed =true;
			}
		?>
...
?>
		...
		<td class="normalfntRiteTABb-ANS"><?php echo $sum1?></td> 
        </tr>  
    <!--  <tr>       

...
$remarks 			= $_GET["remarks"];
	...
	$SQL= "INSERT INTO gategasstransferinheader       (intTransferInNo,intTINYear,intGatePassNo,intGPYear,dtmDate,intUserid,intStatus,strRemarks,intCompanyId) ".
		 "VALUES ($transferIn,$transferInYear,$GatePassNoArray[1],$GatePassNoArray[0],now(),'$UserID',1,'$remarks','$companyId') ";		
	$db->executeQuery($SQL);
}
...
$grnType		= $_GET["GRNType"];
	...
	$sqlHeader ="INSERT INTO gategasstransferindetails (intTransferInNo,intTINYear,intStyleId,strBuyerPONO,intMatDetailId, ".
				"strColor,strSize,dblQty,intGrnNo,intGRNYear,strGRNType) ".
				"VALUES ".
				"($transferIn,$transferInYear,'$StyleID','$BuyerPoNO',$MatDetailID,'$Color','$Size',$Qty,$grnNo,$grnYear,'$grnType')";				
	$db->executeQuery($sqlHeader);
	
	$sqlUpDate ="UPDATE gatepassdetails SET dblBalQty=dblBalQty-$Qty WHERE intGatePassNo=$GPArray[1] AND intGPYear=$GPArray[0] AND intStyleId='$StyleID' AND strBuyerPONO='$BuyerPoNO' AND intMatDetailId = $MatDetailID AND strColor ='$Color' AND strSize='$Size' and intGrnNo='$grnNo' and  intGRNYear='$grnYear' and strGRNType='$grnType'";
...
$db->executeQuery($sqlbinallocation);
	...
	$binSql="INSERT INTO stocktransactions(intYear,strMainStoresID,strSubStores,strLocation,strBin,intStyleId,strBuyerPoNo,intDocumentNo,intDocumentYear, ".
         " intMatDetailId,strColor,strSize,strType,strUnit,dblQty,dtmDate,intUser,intGrnNo, intGrnYear,strGRNType) VALUES ".
		 " ($InYear,'$MainStores','$SubStores','$Location','$BinID','$StyleID','$BuyerPoNO',$transferIn,$InYear,$MatDetailID, ".
         " '$Color','$Size','TI','$Unit',$BinQty,now(),$UserID,$grnNo,$grnYear,'$grnType') ";
...
else if ($RequestType=="ResponseValidate")
{
	...
	$SQLHeder="SELECT COUNT(intTransferInNo) AS headerRecCount FROM gategasstransferinheader where intTransferInNo=$TransferIn AND intTINYear=$Year";
	...
	$SQLDetail="SELECT COUNT(intTransferInNo) AS DetailsRecCount FROM gategasstransferindetails where intTransferInNo=$TransferIn AND intTINYear=$Year";
	...
	$SQL="SELECT COUNT(intDocumentNo) AS binDetails FROM stocktransactions where intDocumentNo=$TransferIn AND intDocumentYear=$Year AND strType='TI'";	
...
$ResponseXML.="<LoadPopUpTransIn>";
	...
	$SQL="SELECT DISTINCT GP.intTransferInNo ".
		 "FROM gategasstransferinheader AS GP ".
		 "INNER JOIN gategasstransferindetails AS GPD ".
...
else if ($RequestType=="LoadPopUpHeaderDetails")
{
	...
	$SQL="SELECT CONCAT(GTIH.intTINYear,'/',GTIH.intTransferInNo) AS GpTranferNo, ".
		 ...
		 "GTIH.dtmDate, ".
		 "GTIH.strRemarks ".		 
		 "FROM gategasstransferinheader AS GTIH ".
...
$ResponseXML .="<LoadPopUpDetails>\n";
	...
	$SQL="SELECT  ".
		 ...
		 "GTID.strGRNType ".
		 "FROM ".
		 "gategasstransferindetails AS GTID ".
		 "Inner Join matitemlist AS MIL ON GTID.intMatdetailID = MIL.intItemSerial ".
		 "Inner Join gategasstransferinheader AS GTIH ON ".
		 "GTIH.intTransferInNo=GTID.intTransferInNo 
		 inner join orders O on  O.intStyleId = GTID.intStyleId AND ".
		 "GTIH.intTINYear=GTID.intTINYear ".
		 "WHERE ".
...
else if ($RequestType=="getGPNolist")
{
	...
	$ResponseXML .="<loadGPlist>\n";
		...
		$SQL= "SELECT DISTINCT CONCAT(GP.intGPYear,'/',GP.intGatePassNo) AS GatePassNo FROM gatepass AS GP 
			...
			WHERE GP.intStatus =1 AND 	GPD.dblBalQty >0 AND MS.intCompanyId ='$companyId' ";
		...
		while($row=mysql_fetch_array($result))
		{
			$str .= "<option value=\"". "" ."\">".$row["GatePassNo"]."</option>";	
		}
	
	$ResponseXML .= "<GPNo><![CDATA[" . $str . "]]></GPNo>\n";
...
{
	global $db;
	$sqlunit="select strUnit from stocktransactions ".
...
function getGetPassTrnsferInNo()
{
	...
	$strSQL="update syscontrol set  dblInTransferNo= dblInTransferNo+1 WHERE syscontrol.intCompanyID='$compCode'";
	$result=$db->RunQuery($strSQL);
	$strSQL="SELECT dblInTransferNo FROM syscontrol WHERE syscontrol.intCompanyID='$compCode'";
	$result=$db->RunQuery($strSQL);
	$GPNo = 'NA';
	while($row = mysql_fetch_array($result))
	 {
		$GPNo  =  $row["dblInTransferNo"] ;
		break;
	 }
	return $GPNo;
}

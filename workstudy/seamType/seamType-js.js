...
{ 
	 if(trim(document.getElementById('txtDescription').value)=="")
		...
		var url = "seamType-db-set.php?type=save";
...
function DeleteData()
{
	var url = "seamType-db-set.php?type=delete&cboSearch="+document.getElementById("cboSearch").value;
	var httpobj = $.ajax({url:url,async:false});
	alert(httpobj.responseText);
	ClearForm();			
} 
...
{	
	...
	loadCombo('SELECT intId ,strName FROM ws_seamtype ORDER BY strName ASC ','cboSearch');
	document.getElementById('txtDescription').focus();
...
{	
	if(obj.value.trim()=="")
	   ...
	   var url="seamType-db-get.php?type=loadtex&intID="+obj.value;
...
function ValidateSave()
 {	
	...
	var x_find=checkInField('ws_seamtype','strName',description,'intId',cboSearch);

...
//-----------------------------------------------------------------------------------------------------------------------------------------------------
//Nirodha
if($request=="loadMachines")
{
 header('Content-Type: text/xml'); 
 echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
 $machineId=$_GET['machineId'];
 
 $ResponseXML .= "<loadGrid>";
 $sql1 = "SELECT
			ws_machinetypes.intMachineTypeId,
			ws_machinetypes.strMachineCode,
			ws_machinetypes.strMachineName
			FROM
			ws_machinetypes where intMachineTypeId = '$machineId' and intStatus='1'";
 $result1 = $db->RunQuery($sql1);
	while($row = mysql_fetch_array($result1))
	{
	 $ResponseXML .= "<MachineTypeId><![CDATA[".trim($row["intMachineTypeId"])  . "]]></MachineTypeId>\n";
	 $ResponseXML .= "<MachineCode><![CDATA[".trim($row["strMachineCode"])  . "]]></MachineCode>\n";
	 $ResponseXML .= "<MachineName><![CDATA[".trim($row["strMachineName"])  . "]]></MachineName>\n";
	}
	 $ResponseXML .= "</loadGrid>";
  echo $ResponseXML;
}

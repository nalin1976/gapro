...
{   	
	    ...
	    if(trim(document.getElementById('txtMachineCode').value)=="") 
		{
...
{	
    ...
    var url="machines-db-get.php?type=loadMachines&machineId="+machineId;
	...
	if(httpobj)  //check if exist the response
	{
		...
		var XMLMachineCode = httpobj.responseXML.getElementsByTagName("MachineCode");
	    document.getElementById("txtMachineCode").value= XMLMachineCode[0].childNodes[0].nodeValue;
		
		var XMLMachineName = httpobj.responseXML.getElementsByTagName("MachineName");
	    document.getElementById("txtMachine").value= XMLMachineName[0].childNodes[0].nodeValue;
...
function editRowMachine(typeId,code,name,status) 
{
	//alert(status);
    ...
    document.getElementById("cboMachines").value	=typeId;

...
$ResponseXML = "<XMLLoadDetails>\n";
				...
				WHERE
				materialratio.intStatus = 1 AND matmaincategory.intID NOT IN (4,5) AND
specificationdetails.intStatus = 1   ";
...
//	else 
	...
	if($row["strBuyerPONO"]=='#Main Ratio#' && $poQty==""){
		$chkBlkRequestPoQty=chkBlkRequest($styleId,$row["strMatDetailID"]);
	...
	if($bulkItemReqStatus!=1){
		...
		if($row["strBuyerPONO"]!='#Main Ratio#'){
				...
				//echo "XXXXXXXXXXXXX".$delPCDdate."xx";
			}else{
				//echo "XXXXXXXXXXXXX".$row["dtPCD"]."xx";
				$delPCDdate	= $row["dtPCD"];
				}
...
function chkBlkRequest($styleId,$matId){
global $db;	

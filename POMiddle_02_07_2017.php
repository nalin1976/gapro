	 ...
	 while($row = mysql_fetch_array($result))
  	 {
		...
		if($row["strBuyerPONO"]!='#Main Ratio#')
         ...
         while(list($strBuyerPO) = mysql_fetch_array($resBuyerPO)){
             $ResponseXML .= "<PO><![CDATA[" . $strBuyerPO  . "]]></PO>\n";
...
function GetBuyerPOFromStyleBuyerPO($prmStyleId){
    ...
    $sql = " SELECT DISTINCT strBuyerPoName FROM style_buyerponos WHERE intStyleId = $prmStyleId ";
    $result=$db->RunQuery($sql);

...
else if (strcmp($RequestType,"getMatInfo") == 0)
{
	...
	$sql = " SELECT intMainCatID,matitemlist.strItemDescription,materialratio.strColor,
			...
			specificationdetails.sngConPc,specificationdetails.sngWastage 
		   ...
		   WHERE materialratio.intStyleId='$styleID' AND materialratio.strBuyerPONO='$buyerPo' ";
	 ...
	 while($row = mysql_fetch_array($result))
  	 {
		...
		$sqlStock = " select intGrnNo,intGrnYear,round(sum(dblQty),2) as stockQty,strGRNType from stocktransactions
					where intStyleId='$styleID' and strBuyerPoNo='$buyerPo' and strColor='$color'
		...
		if($recCount >0)
		{
			 ...
			 {	
				...
				//get issue qty
				$issueQty=getIssueQty($styleID,$year,$buyerPo,$MatID,$color,$size,"Qty",$storeID,$intGrnNo,$intGrnYear,$GrnType) * -1;	
				$mrnReq=getMRNQuantity($styleID,$buyerPo,$MatID,$color,$size,$storeID,$intGrnNo,$intGrnYear,$GrnType);
				
				$approvedQtyArr=getNotApprovedQty($styleID,$buyerPo,$MatID,$color,$size,$intGrnNo,$intGrnYear);

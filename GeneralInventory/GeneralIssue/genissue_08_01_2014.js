...
function popUpGrnList(prmMatDetailId, prmLindId, prmIssueQty, prmItemCode, prmStockQty){ 
 ...
 var _dblIssueQty = document.getElementById('tblIssueList').rows[prmLindId+1].cells[3].lastChild.value;
...
// Set the selected item's item code
 var intMatDetaildId = prmMatDetailId;
 ...
 for( var loop = 0; loop < GRNList.length; loop++ ){
	...
	if(dblQty>0){
		...
		cellUnit.innerHTML = "<input type=\"text \" size=\"8 \" class=\"txtbox \" style=\" text-align:right;\" value=\"0\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event); \" id=txt_"+_iLineId+" onkeyup=\"ValidateReqQty("+_dblIssueQty+","+prmStockQty+")\"  />";
		...
		cellSelect.innerHTML = "<div align=\"center\"><input type=\"checkbox\" name=\"checkbox\" id="+strId+" value=\"checkbox\" onclick=\"addToList(this,"+_dblIssueQty+","+prmStockQty+") \" /></div>";
...
function addToList(prmObj, prmMRNQty, prmStockQty){
	...
	var _iPosId = parseInt(arrSplitId[3]);
	
	if(document.getElementById(prmObj.id).checked){
		
		var _dblReqQty = ValidateGRNListQty();
		var _dblBalMRNQty = prmMRNQty - _dblReqQty;
		
		//document.getElementById('IssueItem').rows[intLineId].cells[5].innerHTML = strGRNNo ;	
		
		var _dblGrnBalQty = parseFloat(document.getElementById('td_'+_iPosId).innerHTML);	
		
		if(prmStockQty>=prmMRNQty){
		
			if(_dblGrnBalQty > _dblBalMRNQty){
				document.getElementById('txt_'+_iPosId).value = _dblBalMRNQty ;	
			}else{
				document.getElementById('txt_'+_iPosId).value = _dblGrnBalQty ;	
			}
			
			if(_dblBalMRNQty == 0){
				document.getElementById(prmObj.id).checked = false;	
			}
		}else{
			
			var _dblBalMRNQty = prmStockQty - _dblReqQty;
		...
		document.getElementById('txt_'+_iPosId).value = 0;
	}
...
function GRNAddToList(){
	...
	//alert(mainRw);
	ArrayGRN[mainRw] = [,,];
	//*var _iArrLen = ArrayGRN.length;
	//*_iArrLen++;
	//alert(_iArrLen);
	//*ArrayGRN[_iArrLen-1] = [,,];	
	
	//ArrayGRN[ArrayGRN.length] = [,,];
	//alert(ArrayGRN.length);
	var iarrIndex = 0;
	var _dblReqQty = 0;
	var tb = document.getElementById('tbGRNList');
	//alert(tb.rows.length);x
	for(i=1;i<tb.rows.length;i++){
		...
		if(typeof(tb.rows[i].cells[0]) != 'undefined'){
			...
			if(tb.rows[i].cells[4].childNodes[0].childNodes[0].checked == true){
				...
				/*_dblReqQty += parseFloat(tb.rows[i].cells[2].childNodes[0].value);
				alert(_dblReqQty);
				if(_dblReqQty>dblMRNQty){
					alert("Issue quantity cannot be more than MRN quantity");
					tb.rows[i].cells[2].childNodes[0].value = 0;
					tb.rows[i].cells[4].childNodes[0].childNodes[0].checked = false;
					return;	
				}else{*/
					//alert(tb.rows[i].cells[2].childNodes[0].value);
					ArrayGRN[mainRw][iarrIndex] = new Array([tb.rows[i].cells[0].innerHTML, tb.rows[i].cells[2].childNodes[0].value, strGenItemCode]);
					//ArrayGRN[_iArrLen-1][iarrIndex] = new Array([tb.rows[i].cells[0].innerHTML, tb.rows[i].cells[2].childNodes[0].value, strGenItemCode]);	
					document.getElementById('tblIssueList').rows[intSelectedLineNo+1].style.backgroundColor = "#A6CFEE";
					//document.getElementById('IssueItem').rows[intSelectedLineNo+1].cells[8].innerHTML  = _dblReqQty;
					//document.getElementById('IssueItem').rows[intSelectedLineNo+1].cells[0].childNodes[0].childNodes[0].checked = true;
					iarrIndex++; 
				//}
	...
	ArrayMRN[intSelectedLineNo+1]=ArrayGRN;
	//alert(ArrayMRN);
	//dblTotReqQty = _dblReqQty;

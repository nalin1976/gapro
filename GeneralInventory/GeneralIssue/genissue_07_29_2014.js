...
function popUpGrnList(prmMatDetailId, prmLindId, prmMRNQty, prmItemCode, prmStockQty){ 
 ...
 for( var loop = 0; loop < GRNList.length; loop++ ){
	...
	if(dblQty>0){
		...
		cellUnit.innerHTML = "<input type=\"text \" size=\"8 \" class=\"txtbox \" style=\" text-align:right;\" value=\"0\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event); \" id=txt_"+_iLineId+" onkeyup=\"ValidateReqQty("+prmMRNQty+","+prmStockQty+")\"  />";
		...
		var strId = intGRNYear+"/"+intGRNNo+"/"+prmLindId+"/"+_iLineId;
		
		var cellSelect = row.insertCell(4);
		cellSelect.className = "normalfntSM";
		cellSelect.innerHTML = "<div align=\"center\"><input type=\"checkbox\" name=\"checkbox\" id="+strId+" value=\"checkbox\" onclick=\"addToList(this,"+prmMRNQty+","+prmStockQty+") \" /></div>";
...
function ValidateReqQty(prmMRNQty, prmStockQty){
		...
		if(typeof(tb.rows[i].cells[0]) != 'undefined'){
			...
			if(tb.rows[i].cells[4].childNodes[0].childNodes[0].checked == true){
				...
				if(_dblReqQty>dblMRNQty){
					...
					//ArrayGRN[intSelectedLineNo][iarrIndex] = new Array([tb.rows[i].cells[0].innerHTML, tb.rows[i].cells[2].childNodes[0].value, strGenItemCode]);
					ArrayGRN[_iArrLen-1][iarrIndex] = new Array([tb.rows[i].cells[0].innerHTML, tb.rows[i].cells[2].childNodes[0].value, strGenItemCode]);	
					document.getElementById('IssueItem').rows[intSelectedLineNo+1].style.backgroundColor = "#A6CFEE";

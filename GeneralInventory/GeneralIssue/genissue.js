	...
	for ( var loop = 1 ;loop < tbl.rows.length ; loop ++ )
  	{
		...
		{		
			...
			var Req_Qty	 	 	= parseFloat(tbl.rows[loop].cells[3].childNodes[0].nodeValue);
			//var Req_Qty	 	 	= tbl.rows[loop].cells[8].childNodes[0].nodeValue;
			var stockBalance 	= parseFloat(tbl.rows[loop].cells[4].childNodes[0].nodeValue);
...
//					
					...
					var mainMrnNo = tblIssueList.rows[mainLoop].cells[7].lastChild.nodeValue;
					var grnno = tblIssueList.rows[mainLoop].cells[8].lastChild.nodeValue;
					
									
					// Check if item already added to the main table 
					//==============================================================
					...
					if ((mainItemDetailId==itemdDetailID) && (mainMrnNo==strMrnNo))
					{
						alert("Sorry !\nItem : "+ itemdDetail + "\nAlready added.");
...
//End -issue.php
			...
			if (booCheck)
			{
				...
				if(parseFloat(Req_Qty)>parseFloat(stockBalance)){
					...
					Req_Qty = stockBalance.toFixed(2);
				}
				...
				cellIssuelist.innerHTML = stockBalance.toFixed(2);
...
{	
	...
	for (loop = 1; loop < tbl.rows.length ; loop ++)
	{
		...
		//alert(ArrayIssueGRNList.length);
		for(x=0;x<ArrayIssueGRNList.length;x++){
			...
			for(j=0;j<ArrayIssueGRNList[x].length;j++){
				if(typeof(ArrayIssueGRNList[x][j])!='undefined'){
					...
					var _strMRNNo = _arrResult[3];
					
					
					
					if((iGenItemCode == itemdDetailID) && (mrnNo == _strMRNNo)) {
...
{	
	...
	for (loop = 1 ;loop < tbl.rows.length; loop++)
	{
		var issueQty 	= parseFloat(tbl.rows[loop].cells[3].childNodes[0].value);
		...
		/* 12/18/2014  */
		//Increase upto 3 decimal points 
		
		mrnQty = mrnQty.toFixed(4);
		stockQty = stockQty.toFixed(4);
...
function popUpGrnList(prmItemCode, prmLindId, prmIssueQty, prmStockQty, prmMRNNo){ 
 ...
 for( var loop = 0; loop < GRNList.length; loop++ ){
	...
	var dblQty = parseFloat(htmlobj.responseXML.getElementsByTagName("QTY")[loop].childNodes[0].nodeValue);	
	...
	if(dblQty>0){
		...
		cellQty.innerHTML = dblQty.toFixed(3);
		...
		var cellMRN = row.insertCell(5);
		cellMRN.className = "normalfntSM";
		cellMRN.innerHTML = prmMRNNo;
...
function segregateGRN(prmObj, prmIssuingQty, prmStockQty, prmRowIndex, prmGrnBalQty){
	...
	prmGrnBalQty = parseFloat(prmGrnBalQty);
	...
	if(prmIssuingQty>_dblGrnWiseIssueQty){
		...
		if(parseFloat(prmIssuingQty)>parseFloat(prmGrnBalQty)){
			...
			if(_dblIssueBalQty>prmGrnBalQty){
				_tb.rows[prmRowIndex].cells[2].childNodes[0].value=	prmGrnBalQty.toFixed(4);
			}else{
				_tb.rows[prmRowIndex].cells[2].childNodes[0].value=	_dblIssueBalQty.toFixed(4);
			}
	...
	for(i=1;i<tb.rows.length;i++){
		...
		if(typeof(tb.rows[i].cells[0]) != 'undefined'){
			...
			if(tb.rows[i].cells[4].childNodes[0].checked == true){
					...
					ArrayGRN[intSelectedIssueLineNo-1][iarrIndex] = new Array([tb.rows[i].cells[0].innerHTML, tb.rows[i].cells[2].childNodes[0].value, intGenItemCode,tb.rows[i].cells[5].innerHTML]);
...
$(document).ready(function(e) {
    ...
    $(document).on("click", "img", function(){	
		...
		if(_iRowIndex>0){
			...
			_mrnNo = $(this).closest('tr').find("td").eq(7).html();
			
			dblIssueQty = parseFloat(_dblIssuingQty);
			
			popUpGrnList(_GenItemCode, _iRowIndex, _dblIssuingQty, _dblStockBal,_mrnNo);
		}

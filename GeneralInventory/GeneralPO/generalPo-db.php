...
$shipmentMode			= $_GET["shipmentMode"];
$shipmentTerms			= $_GET["shipmentTerms"];
$instructions			= $_GET["strInstructions"];
$FactoryID				= $_SESSION["FactoryID"];
$userId					= $_SESSION["UserID"];
$sundrySupplierName	    = $_GET["strSundrySypplier"];
...
if($booCheck)
	$result_check = InsertPOHeader($genPoNo,$poYear,$supplierId,$dtmDate,$dtmDeliveryDate,$currencyId,$userId,$invoiceCompId,$FactoryID,$deliverToId,$payTerm,$payMode,$instructions,$totalValue,$shipmentMode,$shipmentTerms, $sundrySupplierName);
else
	$result_check = UpdatePOHeader($genPoNo,$poYear,$supplierId,$dtmDate,$dtmDeliveryDate,$currencyId,$userId,$invoiceCompId,$FactoryID,$deliverToId,$payTerm,$payMode,$instructions,$totalValue,$shipmentMode,$shipmentTerms, $sundrySupplierName);
...
$dtmDate		= date("Y-m-d H:i:s");
	...
	$SQL =    " UPDATE generalpurchaseorderheader set intStatus = 2,intCheckedBy = $intUserId, dtmCheckedDate = now() WHERE 			       intGenPONo = '$intGenPONo' AND intYear = '$intYear' ";			
	$result = $db->RunQuery($SQL);
	echo $result;
	if ($result!=1)
	{
...
$dtmDate		= date("Y-m-d H:i:s");
	...
	$SQL =    "UPDATE generalpurchaseorderheader set intStatus = 1,intConfirmedBy = $intUserId,dtmConfirmedDate = now() WHERE 			       intGenPONo = '$intGenPONo' AND intYear = '$intYear' ";			
	$result = $db->RunQuery($SQL);
	echo $result;
	if ($result!=1)
	{
...
{	
		...
		$ResponseXML .= "<suppliers><![CDATA[" . $str  . "]]></suppliers>\n";
		$ResponseXML .= "</currSup>";
		echo $ResponseXML;
}
...
function InsertPOHeader($genPoNo,$poYear,$supplierId,$dtmDate,$dtmDeliveryDate,$currencyId,$userId,$invoiceCompId,$FactoryID,$deliverToId,$payTerm,$payMode,$instructions,$totalValue,$shipmentMode,$shipmentTerms, $sundrySupplierName)
{
global $db;
		$sql="insert into generalpurchaseorderheader (intGenPONo, intYear, intSupplierID, dtmDate, dtmDeliveryDate, strCurrency, intStatus,  intUserID, intInvoiceComp, intPrintStatus, intCompId, intDeliverTo, intPreviousUserId, strPayTerm, intPayMode, strInstructions, dblPOValue, dblPoBalance,intCreatedBy,dtmCreatedDate,intShipmentMode,intShipmentTerms, strSundrySupplier) values ('$genPoNo', '$poYear', '$supplierId', now(), '$dtmDeliveryDate', '$currencyId', '0','$userId', '$invoiceCompId', '0', '$FactoryID', '$deliverToId', '0', '$payTerm', '$payMode', '$instructions','$totalValue', '$totalValue','$userId',now(),'$shipmentMode','$shipmentTerms', '$sundrySupplierName');";
		return $db->RunQuery($sql);
}

function UpdatePOHeader($genPoNo,$poYear,$supplierId,$dtmDate,$dtmDeliveryDate,$currencyId,$userId,$invoiceCompId,$FactoryID,$deliverToId,$payTerm,$payMode,$instructions,$totalValue,$shipmentMode,$shipmentTerms, $sundrySupplierName)
{
global $db;
		$sql="update generalpurchaseorderheader set intSupplierID='$supplierId', dtmDate=now(), dtmDeliveryDate='$dtmDeliveryDate', strCurrency='$currencyId', intStatus=0, intUserID='$userId', intInvoiceComp='$invoiceCompId', intPrintStatus=0, intCompId='$FactoryID', intDeliverTo='$deliverToId', strPayTerm='$payTerm', intPayMode='$payMode', strInstructions='$instructions', dblPOValue='$totalValue', dblPoBalance='$totalValue', intShipmentMode='$shipmentMode', intShipmentTerms='$shipmentTerms', strSundrySupplier='$sundrySupplierName'  where intGenPONo='$genPoNo' and intYear='$poYear';";
		return $db->RunQuery($sql);
}
?>
...
if($id=="loadBulkPoHeader")
{
	...
	$CopyToStat		=$_GET["CopyToStat"];
		...
		$result = $db->RunQuery($SQL);
			...
			$sqlUserRole = "select role.RoleName from useraccounts, role, userpermission where useraccounts.intUserID =" . $_SESSION["UserID"] . " and role.RoleID = userpermission.RoleID and userpermission.intUserID = useraccounts.intUserID and intStatus=1 and role.RoleName = 'Allow To Increase Unit Price In PO' ";
			
			$result_IncreasePOPrice = $db->RunQuery($sqlUserRole);
			
			if(mysql_num_rows($result_IncreasePOPrice)>0){
				$ResponseXML .= "<IsAllowToIncreasePOPrice><![CDATA[1]]></IsAllowToIncreasePOPrice>\n";		
			}else{
				$ResponseXML .= "<IsAllowToIncreasePOPrice><![CDATA[0]]></IsAllowToIncreasePOPrice>\n";			
			}
...
if($id=="ListGeneralPO"){
	...
	$iStatus 		= $_GET["postatus"];
	$iSupplierId	= $_GET["supid"];
	$poFrom			= $_GET["pofrom"];
	$poTo 			= $_GET["poto"];
	$poNoLike		= $_GET["polike"];
        $deliveryId     = (int)$_GET["delid"];
	
	# ===============================================================
	// Comment On - 01/07/2016
	// Comment By - Nalin Jayakody
	// Comment For - To get total PO value and currency for the list
    # ==============================================================
	/*$sql = "SELECT
GH.intGenPONo,
GH.intYear,
...
GH.dtmDate,
GH.intStatus,
round(SUM(generalpurchaseorderdetails.dblUnitPrice * generalpurchaseorderdetails.dblQty),2) AS POValue,
(select strCurrency from currencytypes where currencytypes.intCurID = GH.strCurrency ) AS CurrencyType
FROM generalpurchaseorderheader AS GH
Inner Join suppliers AS S ON S.strSupplierID = GH.intSupplierID
Inner Join generalpurchaseorderdetails ON GH.intGenPONo = generalpurchaseorderdetails.intGenPoNo AND GH.intYear = generalpurchaseorderdetails.intYear
WHERE GH.intStatus =  '$iStatus'  ";

	if(($poFrom!="") && ($poTo != ""))
		$sql .= " and date(GH.dtmDate) between '$poFrom' and '$poTo' ";
	
	if($poNoLike!="")
		$sql .= " and GH.intGenPONo LIKE '%$poNoLike%' ";
	
	if($iSupplierId!="")
		$sql .= " and GH.intSupplierID= '$iSupplierId' ";
        
        if($deliveryId != 0)
             $sql .= " and GH.intDeliverTo = '$deliveryId'";
	
	$sql .= " GROUP BY GH.intGenPONo, GH.intYear, GH.dtmDate";
	$sql .= " ORDER BY GH.intGenPONo DESC ";
	...
	while($row=mysql_fetch_array($result)){
		...
		$ResponseXML.="<poyear><![CDATA[" . $row["intYear"]. "]]></poyear>\n";
		$ResponseXML.="<supplier><![CDATA[" . $row["strTitle"]. "]]></supplier>\n";
		$ResponseXML.="<user><![CDATA[" . $row["userName"]. "]]></user>\n";	
		$ResponseXML.="<podate><![CDATA[" . date("Y-m-d", strtotime($row["dtmDate"])). "]]></podate>\n";
		$ResponseXML.="<povalue><![CDATA[" . number_format($row["POValue"],2). "]]></povalue>\n";
		$ResponseXML.="<currency><![CDATA[" . $row["CurrencyType"]. "]]></currency>\n";	
		$ResponseXML.="</POS>";
	}
...
if($id=="ViewItemHis"){
    ...
    $rs_ItemHistory = GetItemHisDetails($itemcode);
    
    $ResponseXML="";
    header('Content-Type: text/xml'); 
    echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
    $ResponseXML.="<GEN_ITEM_HIS>";
    
    while($row=mysql_fetch_array($rs_ItemHistory)){
        $ResponseXML.="<pono><![CDATA[" . $row["poNO"]. "]]></pono>\n";        
        $ResponseXML.="<supplier><![CDATA[" . $row["strTitle"]. "]]></supplier>\n";
        $ResponseXML.="<UOM><![CDATA[" . $row["strUnit"]. "]]></UOM>\n";	
        $ResponseXML.="<unitprice><![CDATA[" . $row["dblUnitPrice"]. "]]></unitprice>\n";
        $ResponseXML.="<poqty><![CDATA[" . $row["dblQty"]. "]]></poqty>\n";
        $ResponseXML.="<dellocation><![CDATA[" . $row["strName"]. "]]></dellocation>\n";	
    }
...
function GetItemHisDetails($prmItemCode){
    ...
    $sql = " SELECT concat(generalpurchaseorderheader.intYear,'/',generalpurchaseorderheader.intGenPONo) AS poNO, suppliers.strTitle, generalpurchaseorderdetails.strUnit, generalpurchaseorderdetails.dblUnitPrice,
             ...
             WHERE generalpurchaseorderdetails.intMatDetailID = '$prmItemCode'";

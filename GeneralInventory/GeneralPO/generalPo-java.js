...
clearItemTable('tblMaterial');
	...
	for ( var loop = 0; loop < XMLid.length; loop++)
	{
		...
		var str_ItemCode = XMLItemCode[loop].childNodes[0].nodeValue;
		...
		cell.className ="normalfnt";
		dbl_unitPrice = XMLUnitPrice[loop].childNodes[0].nodeValue;
		cell.innerHTML ="<input id=\""+str_ItemCode+" \" type=\"text\" class=\"txtbox\" style=\"width:70px; text-align:right\" value=\""+dbl_unitPrice+"\"  onkeypress=\"return CheckforValidDecimal(this.value, 3,event);\" onblur=\" ValidateItemPrice(this,'"+str_ItemCode+"'); \"  onkeyup=\"\"/>";
		
		var cell = row.insertCell(5);
		cell.className ="normalfnt";
		cell.innerHTML = str_ItemCode;//XMLItemCode[loop].childNodes[0].nodeValue;
...
function createMainItemGrid(matDetailID,matItem,strremark,strunit,qty,Unitprice,mainCategory,value,disPercentage,intfixed,intPRno,intPRyear,strPRno,costCenter,/*costCenterID,PRNo,GLId,GLCode,*/EXRate)
{
	...
	row.className 		= 'bcgcolor-tblrowWhite';
	//alert(document.getElementById('hnd').value);
	var boolIsCanIncreasePrice = parseInt(document.getElementById('hnd').value);	
	...
	rowCell.id = qty;
	rowCell.innerHTML 	= "<input type=\"text\" class=\"txtbox\" style=\"width:70px; text-align:right\" value=\""+qty+"\" onkeyup=\"calculateRowValue(this);\" />";
	
	//onkeypress=\"return CheckforValidDecimal(this.value, 3,event);\"   onClick=\"selectTextBoxContent(this);\"
	...
	rowCell.innerHTML 	= "<input type=\"text\" disabled='disabled' class=\"txtbox\" style=\"width:70px; text-align:right\" value=\""+Unitprice+"\"  onkeypress=\"return CheckforValidDecimal(this.value, 3,event);\"  onkeyup=\"calculateRowValue(this);\" onClick=\"selectTextBoxContent(this);\"/>";
...
function calculateRowValue(objText)
{
	...
	{
		//alert(objText.parentNode.parentNode.cells[$main5].lastChild.value);
		if(objText.parentNode.parentNode.cells[$main5].lastChild.value>objText.parentNode.parentNode.cells[$main5].id)
		{
			...
			objText.parentNode.parentNode.cells[$main5].lastChild.value=objText.parentNode.parentNode.cells[$main5].lastChild.value;
		}
	}
...
{	
	...
	document.getElementById('hnd').value = xml_http_obj.responseXML.getElementsByTagName("IsAllowToIncreasePOPrice")[0].childNodes[0].nodeValue;
...
{
	 drawPopupArea(515,317,'frmPORevise');
	 var HTMLText ="<table width=\"100%\" border=\"0\" bgcolor=\"#ffffff\" align=\"left\">"+
          "<tr>"+
		"<td width=\"93%\" height=\"25\" bgcolor=\"#498CC2\" class=\"TitleN2white\">General Purchase Order Revision</td>"+		
          "</tr>"+
          "<tr>"+
            "<td height=\"165\">"+
...
"onclick=\"return  showCalendar(this.id, '%d/%m/%Y');\"></td>"+
   ...
   "onclick=\"serchForRevice('revise');\" /></td>"+
                      "</tr>"+                   
                  "</table></td>"+
                "</tr>"+
                "<tr class=\"bcgl1\">"+
     ...
     frame.id = "RevisePOwindow";
	 document.getElementById('frmPORevise').innerHTML=HTMLText;
	 loadSuppliers();
}
...
function loadSuppliers()
{
	...
	var url = "generalPo-xml.php?id=getSuupliers";
	htmlobj=$.ajax({url:url,async:false});
	
	Handlesupliers(htmlobj);
 }

function Handlesupliers(obj)
{
	
	
	var XMLSupplierID = obj.responseXML.getElementsByTagName("SupID");
	var supplier=obj.responseXML.getElementsByTagName("Supplier");
...
function serchForRevice(type)
{
	...
	var url = "generalPo-xml.php?id=RevisePO&pono="+pono+'&supplierID='+supID+'&from='+fromDate+'&to='+toDate+ '&type=' +type;
	htmlobj = $.ajax({url:url,async:false});
		
	var _booIsGrnExist = htmlobj.responseXML.getElementsByTagName("Validate")[0].childNodes[0].nodeValue;
	
	handleSearch(_booIsGrnExist, pono);
	
}
function handleSearch(prmGrnExist, prmpono)
{

	var url = "generalPo-xml.php?id=GetGPO&pono="+prmpono;
	htmlobj = $.ajax({url:url, async:false});	
	...
	// var XMLPonos= htmlobj.responseXML.getElementsByTagName("pono");
	//alert(htmlobj.responseText);
	var poNo=htmlobj.responseXML.getElementsByTagName("pono")[0].childNodes[0].nodeValue;
	var poYear=htmlobj.responseXML.getElementsByTagName("poyear")[0].childNodes[0].nodeValue;
	var poDate=htmlobj.responseXML.getElementsByTagName("podate")[0].childNodes[0].nodeValue;
	var poValue=htmlobj.responseXML.getElementsByTagName("povalue")[0].childNodes[0].nodeValue;				
	createRevisionPOgrid(poNo,poYear, poDate,poValue,prmGrnExist);	
	
}

function createRevisionPOgrid(prmPoNo,prmPoYear, prmPoDate,prmPoValue,prmGrnExist)
{
	...
	if(prmGrnExist=='TRUE')
	{	
		cellDelete.innerHTML = "<div align=\"center\">&nbsp;</div>"; 	
		tbl.rows[lastRow].bgColor="#FF9966";
	}
	else
	{
		cellDelete.innerHTML = "<div align=\"center\"><img src=\"../../images/posted.png\" onClick=\"revisePO("+prmPoNo+","+ prmPoYear + ");\" alt=\"del\" width=\"15\" height=\"15\" /></div>";	
	}
	
	
	var cellStyle=row.insertCell(1);
	cellStyle.className="normalfntMid";
	cellStyle.innerHTML=prmPoNo;
	
	var cellBuyerPO=row.insertCell(2);
	cellBuyerPO.className="normalfntMid";
	cellBuyerPO.innerHTML=prmPoYear;
	
	var cellBuyerPO=row.insertCell(3);
	cellBuyerPO.className="normalfntMid";
	cellBuyerPO.innerHTML=prmPoDate;
	
	var mainCat=row.insertCell(4);
	mainCat.className="normalfntRite";
	mainCat.innerHTML=prmPoValue;
...
function revisePO(prmPONo, prmPoYear){
	
	var url = "generalPo-xml.php?id=PORevision&pono="+prmPONo+"&poyear="+prmPoYear;
...
// Check the item pric from the 
function ValidateItemPrice(prmObj,prmValue){
	
	var objId = prmObj.id;
	var strItemCode = prmValue;
	
	var url =  "generalPo-xml.php?id=GetItemPrice&gitemcode="+strItemCode;
	htmlobj = $.ajax({url:url, async:false});
	
	var _dblUnitPrice = parseFloat(htmlobj.responseXML.getElementsByTagName("unitprice")[0].childNodes[0].nodeValue);
	
	var _dblEntredPrice = parseFloat(document.getElementById(objId).value);
	
	// If item price zero in the system then accept entred price in the PO & update the system item price as a entred price
	if((_dblUnitPrice == 0) ||(_dblEntredPrice<_dblUnitPrice)){
		
		var url =  "generalPo-xml.php?id=UpdateItemPrice&gitemcode="+strItemCode+"&itemPrice="+_dblEntredPrice;
		htmlobj = $.ajax({url:url, async:false});
	}else{
		
		document.getElementById(objId).value = _dblUnitPrice.toFixed(4);

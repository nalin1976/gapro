...
$cboReportSatus = $_POST['cboReportSatus'];
...
<script type="text/javascript" src="genreturn.js"></script>
...
<form name="frmReturnList" id="frmReturnList" method="post" action="genreturnlist.php">
<table width="100%" align="center">
<tr><td><?php include '../../Header.php'; ?></td></tr>
<tr><td>
<table width="950" border="0" align="center" bgcolor="#FFFFFF" class="tableBorder">
<tr>
	<td height="26" bgcolor="#316895"  class="mainHeading"> Return To Stores List&nbsp;&nbsp;&nbsp;&nbsp;
      <select name="cboReportSatus" id="cboReportSatus" style="width:160px">
	  <option value="0" <?php echo ($cboReportSatus=="0"?'selected="selected"':"")?> >Pending Gate Pass</option>
	  <option value="1" <?php echo ($cboReportSatus=="1"?'selected="selected"':"")?> >Confirmed Gate Pass</option>
      </select></td>
	</tr>
      ...
      <tr>
        <td width="100%"><table width="100%" border="0" cellpadding="0" cellspacing="0" >
          <tr>
            ...
            <td width="12%" class="normalfnt">
              <input type="text" name="txtReturnNoFrom" id="txtReturnNoFrom" class="txtbox" style="width:100px" value="<?php echo ($txtReturnNoFrom!="" ? $txtReturnNoFrom:'');?>" />
              </td>
            ...
            <td width="3%" class="normalfnt">
             <input type="checkbox" name="chkDate" id="chkDate" onclick="disableDate(this);" <?php if($chkDate=='on') {?>checked="checked" <?php }?>/>
            </td>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr class="mainHeading2">
        <td height="24" ><div align="center">Saved General Return List</div></td>
        </tr>
      <tr>
        <td class="normalfnt"><div id="divtblIssueDetails" style="overflow:scroll; height:400px; width:950px;">
          <table id="tblIssueDetails" width="100%" cellpadding="0" cellspacing="1" bgcolor="#CCCCFF">
            <tr class="mainHeading4">
              <td width="21%" height="28" >Return No</td>
              <td width="25%" >Date</td>
              <td width="33%"  >Returned By</td>
              <td width="21%"  >View</td>
            </tr>
            <?php
			$SQL ="SELECT concat(intRetYear,'/',intReturnId) as intReturnNo , genreturnheader.dtmRetDate, 
					genreturnheader.intStatus, department.strDepartment 
					FROM  genreturnheader join department on genreturnheader.intReturnedBy=department.intDepID 
					WHERE genreturnheader.intStatus ='$cboReportSatus' AND genreturnheader.intCompanyID='$companyId' ";	
					
	if ($txtReturnNoFrom!="")
	{
			$SQL .="AND intReturnId >='$txtReturnNoFrom' "; 
	}
	if ($txtReturnNoTo!="")
	{
			$SQL .="AND intReturnId <='$txtReturnNoTo' ";
	}	
	if ($fromDate!="")
	{
			$SQL .="AND date(dtmRetDate) >='$fromDate' ";
	}
	if ($toDate!="")
	{
			$SQL .="AND date(dtmRetDate) <='$toDate' ";
	}
	$SQL .=" order by intReturnId,intRetYear";
	$result = $db->RunQuery($SQL);
	while ($row = mysql_fetch_array($result))	
	{
	?>
    	 <tr class="bcgcolor-tblrowWhite">
              <td width="21%" height="20" class="normalfnt" ><a href="genreturn.php?id=1&GenReturnNo=<?php echo $row['intReturnNo'];?>&intStatus=<?php echo $row['intStatus'];?>" class="noborderforlink" target="_blank">&nbsp;<?php echo $row['intReturnNo']; ?>&nbsp;</a></td>
              <td width="25%" class="normalfnt">&nbsp;<?php echo $row['dtmRetDate']; ?>&nbsp;</td>
              <td width="33%" class="normalfnt">&nbsp;<?php echo $row['strDepartment']; ?>&nbsp;</td>
              <td width="21%" class="normalfntMid"><a href="genreturnnote.php?issueNo=<?php echo $row['intReturnNo'];?>" class="" target="_blank"><img src="../../images/view.png" border="0"/></a></td>
            </tr>
    <?php
	}
	?>

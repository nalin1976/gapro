...
RSD.dblQty,
MMC.intID,
(select strDepartment from department D where D.intDepID=RSH.intReturnedBy)as departmentName, 
MMC.strDescription
from genreturnheader RSH
inner join genreturndetail RSD on RSH.intReturnId=RSD.intReturnId and RSH.intRetYear=RSD.intRetYear
inner join genmatitemlist MIL on  MIL.intItemSerial=RSD.intMatdetailID
inner join genmatmaincategory MMC on MIL.intMainCatID=MMC.intID
where RSH.intStatus=$intStatus ";
...
*/			
			...
			$i = 5;
			while ($row_details=mysql_fetch_array($detailResult))
			{
				xlsWriteLabel($i,0,$row_details["intRetYear"].'/'.$row_details["intReturnId"]);
				...
				//materialRatioID
				xlsWriteLabel($i,5,$row_details["strItemCode"]);
				xlsWriteLabel($i,6,$row_details["strItemDescription"]);
				xlsWriteNumber($i,7,$row_details["dblQty"]);
				$i++;
			}

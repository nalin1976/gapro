...
{	
	...
	$ResponseXML .="<LoadNo>";
		 ...
		 //--------------------hem----------------------------------- 
			 $NoYear = date('Y');
		    $SQL="SELECT dblGRetSupNo FROM syscontrol WHERE intCompanyID='$factoryId'";
			$result =  $db->RunQuery($SQL);
			while($row = mysql_fetch_array($result))
			{
				$retSupNo =  $row["dblGRetSupNo"];
				$SQL = "UPDATE syscontrol set dblGRetSupNo=dblGRetSupNo+1 where intCompanyID='$factoryId'";
				$result = $db->RunQuery($SQL);
				break;
			}

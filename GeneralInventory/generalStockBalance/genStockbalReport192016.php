  ...
  <tr>
    <td><table width="900" border="0" cellspacing="1" cellpadding="2" bgcolor="#000000">
      <tr class="normalfntMid" bgcolor="#CCCCCC">
        <th width="115">Main Category</th>
        <th width="126">Sub Category</th>
        <th width="117">Item Code</th>
        <th width="242">Item Description</th>
        <th width="83">Stock Balance</th>
        <!--<th width="85">Stock Balance Value</th>-->
        <th width="96">Stores Location</th>   
      </tr>
      <?php 
	  if($costCenter!="")
	  {
...
where gst.strMainStoresID>0 ";
	...
	if($ItemCode!="")
		...
		$sql_stock .= " and gmil.intItemSerial = '$ItemCode' ";
	...
	if($bal==0)
	  ...
	  {
		  #============================================================================================================================
		  # Comment on 12/20/2013 
		  # Description - To chnage shows stock balance of the each item in location wise,
		  #============================================================================================================================
...
where gst.strMainStoresID>0 ";*/
		  ...
		  $sql_stock = " SELECT Sum(gst.dblQty) AS stockQty, gmil.strItemDescription, gmc.strDescription AS mainCategory, gmsub.StrCatName AS subCategory, gst.intMatDetailId,".
					   "	    gmil.strItemCode, mainstores.strName, gst.strMainStoresID ".
					   " FROM   genstocktransactions AS gst Inner Join genmatitemlist AS gmil ON gmil.intItemSerial = gst.intMatDetailId Inner Join genmatmaincategory AS gmc ".
					   "        ON gmc.intID = gmil.intMainCatID Inner Join genmatsubcategory AS gmsub ON gmsub.intSubCatNo = gmil.intSubCatID Left Join glallowcation AS GLA ".
					   "        ON GLA.GLAccAllowNo = gst.intGLAllowId Inner Join mainstores ON gst.strMainStoresID = mainstores.strMainID ".
					   " WHERE  gst.strMainStoresID >  0 ";
	...
	//$sql_stock .= " group by mainCategory,subCategory,gmil.strItemCode, gmil.strItemDescription ";	
	$sql_stock .= " group by mainCategory,subCategory,gmil.strItemCode, gmil.strItemDescription, mainstores.strName ";	
	if($bal==0)
		$sql_stock .= " having stockQty>=0 ";
	else
		$sql_stock .= " having stockQty>0 ";
	...
	while($rowS = mysql_fetch_array($result_stock))
	{
		...
		$value = getStockValue($rowS["intMatDetailId"],$rowS["strMainStoresID"],$bal);
      ...
      <tr bgcolor="#FFFFFF">
        ...
        <td class="normalfntRite"><?php echo $rowS["strName"]; ?></td>  
      </tr>
      <?php 
	  }
	  ?>

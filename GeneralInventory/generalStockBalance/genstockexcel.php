...
// Select company, main category and date filter
if(($companyId != '') && ($mainCode != '') && ($dt_date != '') && ($subCode != '') && ($itemCode != '') && ($itemDesc == ''))
	$sql .= " WHERE mainstores.intCompanyId = '$companyId' and genmatmaincategory.intID = '$mainCode' and genstocktransactions.dtmDate <= '$dt_date' and genmatsubcategory.intSubCatNo = '$subCode' and genmatitemlist.intItemSerial = '$itemCode' ";

// Get only factory wise detail
if(($companyId != '') && ($mainCode == '') && ($dt_date == '') && ($subCode == '') && ($itemCode == '') && ($itemDesc == ''))
	$sql .= " WHERE mainstores.intCompanyId = '$companyId'";
	
// Get only main category
if(($companyId == '') && ($mainCode != '') && ($dt_date == '') && ($subCode == '') && ($itemCode == '') && ($itemDesc == ''))
	$sql .= " WHERE genmatmaincategory.intID = '$mainCode'";
	
// Get only below given date	

if(($companyId == '') && ($mainCode == '') && ($dt_date != '') && ($subCode == '') && ($itemCode == '') && ($itemDesc == ''))
	$sql .= " WHERE genstocktransactions.dtmDate <= '$dt_date'";
	
if(($companyId == '') && ($mainCode == '') && ($dt_date == '') && ($subCode != '') && ($itemCode == '') && ($itemDesc == ''))
	$sql .= " WHERE genmatsubcategory.intSubCatNo = '$subCode'";	

if(($companyId == '') && ($mainCode == '') && ($dt_date == '') && ($subCode == '') && ($itemCode != '') && ($itemDesc == ''))
	$sql .= " WHERE genmatitemlist.intItemSerial = '$itemCode'";	
	
if(($companyId == '') && ($mainCode == '') && ($dt_date == '') && ($subCode != '') && ($itemCode == '') && ($itemDesc != ''))
	$sql .= " WHERE genmatitemlist.strItemDescription like '%$itemCode%'";	
	
// Get only main category
if(($companyId != '') && ($mainCode != '') && ($dt_date == '') && ($subCode == '') && ($itemCode == '') && ($itemDesc == '')){
	$sql .= " WHERE mainstores.intCompanyId = '$companyId' and genmatmaincategory.intID = '$mainCode'";	
}

...
<table width="900" border="0" align="center" cellpadding="0">
 ...
 $bal        = $_GET['bal']; 
 $itemCode   = $_GET['cboItemCode'];
 
  //$cboCompany = GetMainStoresID($cboCompany);
...
Left Join genmatitemlist ON genmatitemlist.intMainCatID = genmatmaincategory.intID AND genmatitemlist.intSubCatID = genmatsubcategory.intSubCatNo
Left Join genstocktransactions ON genmatitemlist.intItemSerial = genstocktransactions.intMatDetailId
Inner Join mainstores ON genstocktransactions.strMainStoresID = mainstores.strMainID
Inner Join companies ON mainstores.intCompanyId = companies.intCompanyID 
where genstocktransactions.dblQty<>0 ";
	 ...
	 //$sql2 .= " and genstocktransactions.strMainStoresID = '$cboCompany'";		
	 	$sql2 .= " and companies.intCompanyID = '$cboCompany'";		
	 }	
	 if($cboMatItem != ""){		
	 $sql2 .= " and genmatitemlist.intItemSerial = '$cboMatItem'";		
	 }
	 if($itemCode != ""){
		 $sql2 .= " and genmatitemlist.intItemSerial = '$itemCode'";	
	 }
...
genmatsubcategory.StrCatName ASC,
genmatitemlist.strItemDescription ASC,
genstocktransactions.strType DESC,	
genstocktransactions.dtmDate ASC
";	
  ...
  if(mysql_num_rows($result2))
  {
		...
		$rows=0;
			...
			{
			if($row["strType"]=='ADJST'){
			$transaction="Open Stock";
			$reportPath = "";
			}	
			if($row["strType"]=='GRN'){
			$transaction="GRN";
			...
			else if($row["strType"]=='TI'){
			$transaction="Gate Passe Transfer In ".$row["strName"];	
			}
	...
	//-----------confirmed pos which r relevent to the material-----------	
        ...
        <tr onMouseover="this.bgColor='#CECEFF'" onMouseout="this.bgColor='#FFFFFF'"> 
          ...
          <td align="left" class="normalfnt" ><?php echo  "Purchase Order from ".$rowP["strTitle"]." -- (Not effecting to the stock)"; ?></td>
        ...
        <tr onMouseover="this.bgColor='#CECEFF'" onMouseout="this.bgColor='#FFFFFF'"> 
          <td class="normalfntRite"></td>
          <td class="normalfntRite"></td>
          <td class="normalfntMidTAB"><strong>Total Qty</strong></td>
          <td class="normalfntRite" bgcolor="#EFEFEF"><strong><?php echo  $sum-$row["dblQty"]; ?></strong></td>
        </tr>
  </table></td></tr>
<?php  }
?>

...
function LoadSourceBinDetails()
{
	...
	var url = "itemTransferdb.php?request=loadSourceDetails&category="+category+"&subcategory="+subcategory+"&itemLike="+itemLike;
	var htmlobj = $.ajax({url:url,async:false});
...
function Save()
{
	...
	for(var i=1;i<tbl.rows.length;i++)
	{
		...
		var url = "itemTransferdb.php?request=SaveDetails&MatDetId="+MatDetId+"&Iunit="+Iunit+"&TransQty="+TransQty+"&GrnNo="+GrnNo+"&pub_No="+pub_No;
		htmlobj = $.ajax({url:url,async:false});

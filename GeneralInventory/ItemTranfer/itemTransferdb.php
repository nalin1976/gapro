...
if($request=='SaveDetails')
{
	...
	$boolSourceSave = SaveSourceStock($year,$companyId,$pub_No,$MatDetId,$Iunit,$TransQty,$userId,$GrnNoArry[0],$GrnNoArry[1]);
	$boolDestSave = SaveDestStock($year,$companyId,$pub_No,$MatDetId,$Iunit,$TransQty,$userId,$GrnNoArry[0],$GrnNoArry[1]);
	if($boolSourceSave==true && $boolDestSave==true)
		echo 'Saved';
	else
		echo 'Error';
}
function SaveSourceStock($year,$companyId,$pub_No,$MatDetId,$Iunit,$TransQty,$userId,$GrnNo,$GrnYear)
{
	global $db;
	$outQty	= $TransQty*-1;
	...
	'$Iunit', '$outQty', now(), '$userId', '$GrnNo', '$GrnYear', '0','0');";
	$result = $db->RunQuery($sql);
	if($result)
		return true;
	else
...
function SaveDestStock($year,$companyId,$pub_No,$MatDetId,$Iunit,$TransQty,$userId,$GrnNo,$GrnYear)
{
	global $db;
	$sql = "insert into genstocktransactions 
	(intYear, strMainStoresID, intDocumentNo, intDocumentYear, intMatDetailId, strType, strUnit, dblQty, 
	...
	'$Iunit', '$TransQty', now(), '$userId', '$GrnNo', '$GrnYear', '0','0');";
	$result = $db->RunQuery($sql);
	if($result)
		return true;
	else

...
function loadPopupItems(prmLocationId)
{	
	...
	var strItemDescLike = document.getElementById('txtItemDescLike').value;
	...
	url += '&LocationId='+prmLocationId;
	url += '&DescLike='+strItemDescLike;
...
function addDataRowToMainGrid(MainCat,Detail,balQty,matID,Qty,GRNNO,GRNYear/*,GLCode,GLAllowId*/)
{
	...
	cellQty.innerHTML="<input type=\"text\" size=\"12\" class=\"txtbox\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\" style=\"text-align:right\" value="+Qty+"></input>";
...
function saveGenMrnHeader()
{
	...
	var requestLocationId = document.getElementById('cboStores').value;
	...
	var url = 'genMRNMiddle.php?RequestType=SaveHeader&genMRNno='+pub_MRNno+'&genMRNyear='+pub_MRNyear;
		...
		url += '&locationId='+requestLocationId;
	htmlobj=$.ajax({url:url,async:false});
...
function createRequestItemPopUp()
{
	...
	if(document.getElementById("cboStores").value=="")
	{
		showPleaseWait();
		alert("Please select a stores location.");
		document.getElementById("cboStores").focus();
		hidePleaseWait();
		return;
	}
	var storesId = document.getElementById("cboStores").value;
	
	showBackGround('divBG',0);
	var url = "genmrndetailpopup.php?mainStoresId="+storesId;

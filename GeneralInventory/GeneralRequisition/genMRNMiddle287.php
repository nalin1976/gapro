...
if (strcmp($RequestType,"getMatInfo") == 0)
{
	...
	$strNAVItemCode = $_GET['itemCode'];
	//$costCenterId = $_GET["costCenterId"];
	$requestLocationId = $_GET['LocationId'];
	$strItemDesc = $_GET['DescLike'];
	
	
	//$intMainStoresId = GetMainStoresID($companyId);
	$intMainStoresId = GetMainStoresID($requestLocationId);
	
	$ResponseXML .= "<MatInfo>\n";
	
	 $sql= " SELECT genmatitemlist.strItemDescription, genmatitemlist.intItemSerial, genmatitemlist.strItemCode FROM genmatitemlist ";
	 
	 if($strNAVItemCode!=""){
		 $sql .= " WHERE genmatitemlist.strItemCode = '$strNAVItemCode'";
	 }
	 
	 if($strItemDesc !=""){
		 $sql .= " WHERE genmatitemlist.strItemDescription LIKE '%".$strItemDesc."%'";
	 }
	 
	 if($mainCatID!=""){
	 	$sql .= " WHERE genmatitemlist.intMainCatID = '$mainCatID'";
	 }
	 if($subCatID!="")
	...
	//------------------------------lahiru  04/05/2011 --------------------------------------
		/*$sql_stock="SELECT COALESCE(Sum(genstocktransactions.dblQty),0) AS dblStock,intGRNNo,intGRNYear
				    ...
				    WHERE genstocktransactions.strMainStoresID = '". $intMainStoresId ."' 
				    AND genstocktransactions.intMatDetailId =  '". $MatID ."'
					  group by strMainStoresID,intMatDetailId,intGRNYear,intGRNNo;"; 			*/
		
		# =======================================================================
		# 11/20/2013
		# Remove grouping of GRN NO from report
		# =======================================================================
		$sql_stock="SELECT COALESCE(Sum(genstocktransactions.dblQty),0) AS dblStock
				    FROM genstocktransactions 
				    WHERE genstocktransactions.strMainStoresID = '". $intMainStoresId ."' 
				    AND genstocktransactions.intMatDetailId =  '". $MatID ."'
					  group by strMainStoresID,intMatDetailId ;";			  
					  
		$result_stock=$db->RunQuery($sql_stock);
		
		#--- Check whether stock balance is available		
		# If available then show item with available qty
		if(mysql_num_rows($result_stock)>0){
			...
			while($row_stock = mysql_fetch_array($result_stock))
			{
				...
				//continue;
				 ...
				 $ResponseXML .= "<intGRNNo><![CDATA[" ."0". "]]></intGRNNo>\n";
				 $ResponseXML .= "<intGRNYear><![CDATA[" ."0". "]]></intGRNYear>\n";
				 /*$ResponseXML .= "<GLCode><![CDATA[" .$row_stock["GLCode"]. "]]></GLCode>\n";
				 $ResponseXML .= "<GLAllowId><![CDATA[" .$row_stock["intGLAllowId"]. "]]></GLAllowId>\n";*/
				 $ResponseXML .= "<GLCode><![CDATA[" ."-1". "]]></GLCode>\n";
				 $ResponseXML .= "<GLAllowId><![CDATA[" ."-1". "]]></GLAllowId>\n";
				 $ResponseXML .= "<NAVItemCode><![CDATA[" .$strItemCode. "]]></NAVItemCode>\n";
			 
			 }
		}else{
			
			#- If stock balance not available then display only the item to raise a MRN
			...
			$ResponseXML .= "<stockBalance><![CDATA[" ."0". "]]></stockBalance>\n";
			$ResponseXML .= "<intGRNNo><![CDATA[" ."0". "]]></intGRNNo>\n";
			$ResponseXML .= "<intGRNYear><![CDATA[" ."0". "]]></intGRNYear>\n";
			$ResponseXML .= "<GLCode><![CDATA[" ."-1". "]]></GLCode>\n";
			$ResponseXML .= "<GLAllowId><![CDATA[" ."-1". "]]></GLAllowId>\n";
...
else if(strcmp($RequestType,"LoadNo") == 0)
{
	...
	$NoYear = date('Y');	
		 //--------------------hem----------------------------------- 
		    $SQL1="SELECT dblGMRNNo FROM syscontrol WHERE intCompanyID='$companyId'";
			//echo $SQL1."-";
...
else if(strcmp($RequestType,"SaveHeader") == 0)
{
	...
	//$result = saveMrnHeader($genMRNno,$genMRNyear,$departementCode,$userID,0,$userID,$companyId);
	
	$result = saveMrnHeader($genMRNno,$genMRNyear,$departementCode,$userID,0,$userID,$companyId,$requestLocationId);
}
...
//$GLAllowId		= $_GET["GLAllowId"];
	...
	$res = saveMrnDetails($mrnNo,$mrnYear,$MatID,$qty,$balQty,$notes,$assetNo,$GRNNo,$GRNYear);
...
function saveMrnHeader($mrnNo,$mrnYear,$departmentCode,$requestBy,$status,$userID,$companyID,$requestLocationId)
{
	global $db;
	$sql="INSERT INTO 
	genmatrequisition(intMatRequisitionNo,intMRNYear,dtmDate,strDepartmentCode,intRequestedBy,intStatus,intUserId,intCompanyID,intCostCenterId,intRequestLocationId)
	VALUES('$mrnNo','$mrnYear',NOW(),'$departmentCode','$requestBy','$status','$userID','$companyID','0','$requestLocationId');";	
	return $db->RunQuery($sql);
}

function saveMrnDetails($mrnNo,$year,$matDetailID,$qty,$balQty,$notes,$assetNo,$GRNNo,$GRNYear)
{
	global $db;
	$sql="INSERT INTO genmatrequisitiondetails(intMatRequisitionNo,intYear,strMatDetailID,dblQty,dblBalQty,strNotes,intGRNNo,intGRNYear,strAssetNo,intGLAllowId) VALUES('$mrnNo','$year','$matDetailID','$qty','$balQty','$notes','$GRNNo','$GRNYear','$assetNo','0');";	
	return $db->RunQuery($sql);
}
...
if($RequestType=="listGenItems"){

	$strNAVItemCode = $_GET['strItemChar'];
	
	global $db;
	
	$sql = "select strItemCode from genmatitemlist where strItemCode like '%".$strNAVItemCode."%'";
	$result = $db->RunQuery($sql);
	
	$ResponseXML .="<GSTKCODE>";
	
	while($row=mysql_fetch_array($result)){
		$ResponseXML .= "<ITEMCODE><![CDATA[" . trim($row["strItemCode"])  . "]]></ITEMCODE>\n";
	}
	$ResponseXML .="</GSTKCODE>";
	echo $ResponseXML;
}

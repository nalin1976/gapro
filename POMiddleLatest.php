...
else if(strcmp($RequestType,"confirmSendToApprove") == 0)
{
	...
	$poYear = $_GET["year"];//getPOYear($pono);
	global $db;
	$sql="UPDATE purchaseorderheader SET intStatus='5'   WHERE intPONo='".$pono."' AND intYear=$poYear;";
	//echo $sql;
	$result = $db->RunQuery($sql);
	if($result == '1')
	{
...
else if(strcmp($RequestType,"revisePo") == 0)
{
	...
	global $userID;
	$poYear = $_GET["POYear"];
	$sql="UPDATE purchaseorderheader SET intStatus='1' , intRevisionNo = COALESCE(intRevisionNo,0) + 1, dtmRevisionDate = now(), intRevisedBy = " . $_SESSION["UserID"] . " ,
...
global $db;
		...
		$sql="SELECT DISTINCT p.intPONo FROM purchaseorderheader p INNER JOIN purchaseorderdetails d ON d.intPoNo=p.intPONo and d.intYear=p.intYear where p.intStatus='1' and p.intCompanyID = '$intCompanyId' ";
...
Inner Join orders ON materialratio.intStyleId = orders.intStyleId
Inner Join matitemlist ON materialratio.strMatDetailID = matitemlist.intItemSerial
where ( dblBalQty>0 or dblFreightBalQty>0 ) AND orders.intStatus = '11'";
...
if ($RequestType =="GetItemDis11")
{
	...
	$itemList        = $_GET["itemList"];
	 ...
	 $result=getTblGrid1($styleNo,$orderNo,$mainCat,$subCat,$itemList);	
	 // $ComboBuyer .= "<option value=\"". '' ."\">" . '' ."</option>" ;
	 while($row = mysql_fetch_array($result))
  	 {
		    $ResponseXML .= "<ItemID><![CDATA[" . $row["intItemSerial"]  . "]]></ItemID>\n";
	  ...
	  $orderQty = $row["dblQty"];
	 $pendingQty = $row["dblBalQty"];
	 $Qty1 = GetMaterialRatioBalQty($styleNo,$row["intItemSerial"],$row["strColor"],$row["strSize"],0);
			$leftOverQty = getLeftoverStockWithoutPendingQty($row["intItemSerial"],$styleNo,$row["strColor"],$row["strSize"]);
			//echo $leftOverQty;
			$BulkQty = getBulkStockWithoutPendingQty($row["intItemSerial"],$styleNo,$row["strColor"],$row["strSize"]);
...
function getTblGrid1($styleNo,$orderNo,$mainCat,$subCat,$itemList)
{
global $db;
$sql="SELECT matitemlist.strItemDescription,materialratio.strColor,materialratio.strSize,materialratio.dblQty,
				materialratio.dblBalQty,matitemlist.intSubCatID,materialratio.dblFreightBalQty,matitemlist.intItemSerial,
				matitemlist.dblUnitPrice,materialratio.intStyleId,orders.strOrderNo,orders.strStyle,specification.intSRNO,matitemlist.intMainCatID
      FROM materialratio
      Inner Join matitemlist ON matitemlist.intItemSerial = materialratio.strMatDetailID
      Inner Join orders ON materialratio.intStyleId = orders.intStyleId
      Inner Join specification ON materialratio.intStyleId = specification.intStyleId

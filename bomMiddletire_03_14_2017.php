...
elseif ($RequestType=="IsPORaisedBPO") {
	
	$styleId = $_GET['styleNo'];
	$sBPO    = $_GET['buyerPONo'];
	
	$sql = " SELECT * FROM purchaseorderdetails WHERE intStyleId = '$styleId' AND purchaseorderdetails.strBuyerPONO = '$sBPO' ";
	$result = $db->RunQuery($sql);
	//echo $sql;
	$ResponseXML = "";
	$ResponseXML = "<POExistItem>\n";
...
elseif ($RequestType=="GetPrvBPO") {
	
	$styleId = $_GET['styleNo'];
	$sBPO    = $_GET['buyerPONo'];
	
	$sql = " SELECT strBuyerPoName FROM style_buyerponos WHERE intStyleId = '$styleId' AND strBuyerPONO = '$sBPO' ";
	$result = $db->RunQuery($sql);
        $arr_result = mysql_fetch_row($result);
	//echo $sql;
	$ResponseXML = "";
	$ResponseXML = "<PrvBPONo>\n";
        $ResponseXML.="<BPOName><![CDATA[".$arr_result[0]."]]></BPOName>\n";
	$ResponseXML .= "</PrvBPONo>";
...
function getStyleRatioDetails($styleNo,$buyerPONo)
{
	global $db;
        ...
        // ==============================================
        // Comment On - 03/01_2017
        // Comment By - Nalin Jayakody
        // Comment For - To get style ratio link to style buyer po 
        // ==============================================
	# $sql= "select intStyleId,strBuyerPONO,strColor,strSize,dblQty,dblExQty,strUserId,intserial,dblFOB,dblPackQty from styleratio where intStyleId = '$styleNo' AND strBuyerPONO = '$buyerPONo' AND (intStatus != '0' or intStatus IS NULL) order by strColor,strSize" ;
        ...
        $sql = "select styleratio.intStyleId, styleratio.strBuyerPONO,strColor,strSize, styleratio.dblQty,dblExQty,strUserId,intserial,dblFOB,dblPackQty
                FROM styleratio INNER JOIN style_buyerponos ON styleratio.intStyleId = style_buyerponos.intStyleId AND styleratio.strBuyerPONO = style_buyerponos.strBuyerPONO
                where styleratio.intStyleId = '$styleNo' AND style_buyerponos.strBuyerPoName = '$buyerPONo' AND (intStatus != '0' or intStatus IS NULL)
                order by strColor,strSize" ;
        }
	return $db->RunQuery($sql);
}
				...
				if(mysql_num_rows($result2))
				{
					...
					//$sql="UPDATE `materialratio` SET `strColor`='$color',`strSize` = '$size', `dblQty` = '$qty' ,`dblBalQty` = '$balQty' , `dblFreightBalQty` = '$dblFreightBal', `serialNo`  = '$sizeserial' ,  `dblRecutQty`  = '$recutQty' ,`intStatus` = '1' WHERE (`intStyleId`='$styleNo') AND (`strMatDetailID`='$item') AND (`strBuyerPONO`='$buyerPONo')AND (`strColor`='$color') AND (`strSize`='$size');";
                                        ...
                                        if(($isFabric == 1) && ($buyerPONo != "#Main Ratio#") && ($yyQty == 0)){
                                            $sql="UPDATE `materialratio` SET `strColor`='$color',`strSize` = '$size', `dblQty` = '0' ,`dblBalQty` = '0' , `dblFreightBalQty` = '0', `serialNo`  = '$sizeserial' ,  `dblRecutQty`  = '$recutQty' ,intStatus = '0',dblYYConsumption= '$yyQty', dblWastage='$wastage', intMoq='$moq'  WHERE (`intStyleId`='$styleNo') AND (`strMatDetailID`='$item') AND (`strBuyerPONO`='$buyerPONo')AND (`strColor`='$color') AND (`strSize`='$size');";
                                        }else{

...
{ 
	...
	for(var loop = 0; loop < itemDisI.length; loop ++)
	{
		...
		craeteGrid(itemDis,color,size,qty,matDetaiID,roundQty,balanceQty,mrnRaised,issueQty,issueBalance,stockBalance,wastage,Approved,NotApproved,conPCItem,trimInspected,grnNo,grnYear,grnType,grnTypeId,invoiceNo,mainCategoryId, strUOM); 
	}
 getReqQty(document.getElementById("txtReqQty"));		 
}
 
 function craeteGrid(itemDis,color,size,qty,matDetaiID,reqQTY,balanceQty,mrnRaised,issueQty,issueBalance,stockBalance,wastage,Approved,NotApproved,conPCItem,trimInspected,grnNo,grnYear,grnType,grnTypeId,invoiceNo,mainCategoryId, prmUOM)
 {
	var inhose=0	
    var tbl = document.getElementById('mrnMatGrid');
	...
	var cellGRNno=row.insertCell(19);
	...
	var cellShrinkager=row.insertCell(23);
...
function checkQty(obj)
{
	...
	if(isNaN(enterQty)==true)
	{
		...
		cell[7].firstChild.value=maxMrn;
		return;
	}

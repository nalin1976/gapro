...
function GetBuyingOffices()
{
	...
	var url = 'preordermiddletire.php?RequestType=GetBuyingOffice&CustID=' + custID;
	var xmlHttp     	=$.ajax({url:url,async:false});
	//alert(xmlHttp.responseText);
	 		 var XMLID = xmlHttp.responseXML.getElementsByTagName("BuyingOfficeID");
			 var XMLName = xmlHttp.responseXML.getElementsByTagName("BOffice");
...
function loadUnitDetails()
{
	...
	document.getElementById("cboSourceUnit").innerHTML = XMLName[0].childNodes[0].nodeValue;
}
...
{
	if(document.getElementById('cboManuFactory').value.trim() == '')
	{
...
function ModifyItems(itemcode,conpc,unitType,unitPrice,wastage,originName,freight,originID,cat,selection,mill,mainFabric,originType,mainCat)
{
	var tbl = document.getElementById('tblConsumption');
	var position = -1;
    for ( var loop = 1 ;loop < tbl.rows.length ; loop ++ )
...
function ValidateSchedule()
{
	...
	/*else if (isLessQuantity(parseInt(document.getElementById('quantity').value)))
	{
		alert("You are Less the main quantity. Please check it again.");
		document.getElementById('quantity').focus();
		return false;
	}*/
...
function isLessQuantity(newQuantity)
{
	var styleNo = document.getElementById("txtStyleNo").value;
	var url = "preordermiddletire.php?RequestType=getDelQty&styleNo="+styleNo;	
	var htmlobj=$.ajax({url:url,async:false});
...
function LoadSavedItemDetails()
{
		...
		//altxmlHttp.send(null); 
		var url = 'preordermiddletire.php?RequestType=getOrderDetailData&StyleID=' + URLEncode(EditStyleNo);
		var altxmlHttp = $.ajax({url:url,async:false});
...
function validateApprovalPossibility()
{
	...
	{
		//Start - 24-05-2011 - Cannot send for approval, If there is no any main fabric in the order
		var tbl = document.getElementById('tblConsumption');
		var booMFAvailable = false;
		for(var loop=1;loop<tbl.rows.length;loop++)
		{
			if(tbl.rows[loop].cells[16].childNodes[0].nodeValue=='1')
				booMFAvailable = true;
		}
		/*if(!booMFAvailable)
		{
			alert("Sorry!\nCould not send for approvals.Please select one fabric item as 'Main Fabric'.");
			return;
		}*/
		//End - 24-05-2011 - Cannot send for approval, If there is no any main fabric in the order
		
		//Start - Cannot send to approve if profit less than to factory profit margin
		...
		//End - Cannot send to approve if profit less than to factory profit margin
...
function SaveItemInfo()
{
	...
	//var value = RoundNumbers(parseInt(totalQty) * parseFloat(parseFloat(unitPrice) + parseFloat(freight)),4);
	//var value = calculateCostValue(totalQty,unitPrice,freight);
	var value = RoundNumbers(Math.round(parseInt(totalQty)) * parseFloat(parseFloat(unitPrice) + parseFloat(freight)),4);
...
function SaveNewDeliverySchedule()
{
	if (ValidateSchedule() && ValidateBPOAllocation() )
	{
		...
		altxmlHttp.open("GET", 'preordermiddletire.php?RequestType=SaveSchedule&StyleNo=' + URLEncode(StyleNo) + '&ScheduleDate=' + date + '&qty=' + qty + '&exqty=' + exqty  + '&modeID=' + modeID + '&remarks=' + URLEncode(remarks) + '&isbase=' + isbase + '&LeadID=' + LeadID + '&estimateddate=' + estimateddate+ '&bpo=' + bpo + '&handoverDate=' + handoverDate + '&refNo=' + refNo+ '&contry=' + contry , true);
		altxmlHttp.send(null); 	
	}
}
...
function saveDeliveryChanges(editedDate)
{
	...
	altxmlHttp.open("GET", 'preordermiddletire.php?RequestType=ChangeSchedule&StyleNo=' + URLEncode(StyleNo) + '&ScheduleDate=' + date + '&qty=' + qty + '&exqty=' + exqty  + '&modeID=' + modeID + '&remarks=' + URLEncode(remarks) + '&isbase=' + isbase + '&LeadID=' + LeadID + '&oldDate=' + editedDate + '&estimateddate=' + estimateddate+ '&bpo=' + bpo+ '&handOverdate=' + handOverdate+ '&refNo=' + refNo + '&updateEventSchedule=' + updateEventSchedule+ '&cboCountries=' + cboCountries, true);
	altxmlHttp.send(null); 	
}
...
function IsValidSchedules()
{
	...
	if ((odrQty < allocatedQty) || (odrQty > allocatedQty) )
	{
...
function openUnitConvertionPopup()
{
	...
	document.getElementById('cboSourceUnit').value = 'MTS';
	document.getElementById('txtUCConpc').value = obj.parentNode.cells[3].childNodes[0].nodeValue;
	
	if(parseFloat(obj.parentNode.cells[13].childNodes[0].nodeValue)>0)
		document.getElementById('cboUnitsC').disabled = true;
	else
		document.getElementById('cboUnitsC').disabled = false;
}
...
function checkUnitConvertion()
{
	if(!unitConvertionIntbl())
		return;
...
function unitConvertionIntbl()
{
	var tbl 		= document.getElementById('tblConsumption');
	var rwID 		= parseInt(document.getElementById('currRowId').value);
	var matItemId 	= tbl.rows[rwID].cells[0].id;
	var conPC 		= document.getElementById('txtUCConpc').value;
	var prevUnitId 	= document.getElementById('cboSourceUnit').value;
	var currUnit 	= document.getElementById('cboUnitsC').value;
	
	var url = 'preordermiddletire.php?RequestType=GetConPc&ItemID=' + matItemId + '&SelectedUnit=' + URLEncode(currUnit) + '&ConPc=' + conPC + '&previousUnit=' + URLEncode(prevUnitId);
	var htmlobj = $.ajax({url:url,async:false});
	...
	if(XMLUnitValue != 0)
	{
		...
		if(!ValidateBeforeUnitConvert(rwID,totalQty))
			return false;
	 ...
	 {
		 alert('Conversion not available from '+prevUnitId + ' to '+ currUnit);
...
function ValidateBeforeUnitConvert(rwID,totalQty)
{
	var tbl = document.getElementById('tblConsumption');			 
	var currentTotal = totalQty;
	var purchasedQty = parseFloat(tbl.rows[rwID].cells[13].lastChild.nodeValue);
	if (currentTotal < purchasedQty)
	{

...
function uploadBuyerPO(StyleNo,Qty)
{
	...
	var	popwindow= window.open ("BuyerPOStyle/uploader_buyerPO.php?No=" + StyleNo + "&Qty=" + Qty , "Buyer PO Uploader","location=1,status=1,scrollbars=1,width=450,height=200");
...
function LoadBuyerBONosForMaterialRatio()
{
    var StyleNo = document.getElementById('lblStyleNo').innerHTML;
    createXMLHttpRequest();
    xmlHttp.onreadystatechange = HandleBuyerPOs;
...
//		}
		...
		$("#txtbuyerpo").css("background-color", "#FFF");*/
                ...
                // ##########################################
		/* Add By - Nalin Jayakody
                 * Add For - After add form data to grid clear some data from form and enable input controls.
                 * Add On - 03/07/2017
                 */
                
                $("#txtbuyerpo").prop('readonly',false);
                $("#txtbuyerpo").prop('disabled',false);
                $("#txtbuyerpo").css('background-color','#FFFFFF'); 
                $("#txtbuyerpo").val('');
                $("#cboPreviousBPO").val('-1');
                $("#cboPreviousBPO").prop('disabled',false);
...
{
	var StyleNo = document.getElementById('lblStyleNo').innerHTML;
        var SelectedPO = $("#txtbuyerpo").val();
    
        /*var td = obj.parentNode;
	var tr = td.parentNode;*/
        
        
        var booIsPORaisedForSelectedPO = 0;
        
       
        
        if(SelectedPO != ''){
            
            var prvBPOId = validateBPO(StyleNo,SelectedPO);
        
            var url = 'bomMiddletire.php?RequestType=IsPORaisedBPO&styleNo=' + URLEncode(StyleNo) + '&buyerPONo=' + URLEncode(prvBPOId);
            var htmlObjPO = $.ajax({url:url, async:false});

            booIsPORaisedForSelectedPO = htmlObjPO.responseXML.getElementsByTagName("IsPOExist")[0].childNodes[0].nodeValue;
            
            RemoveFromGrid(obj, StyleNo, booIsPORaisedForSelectedPO)
	...
	document.getElementById('dtCutOff').value 		= tr.cells[11].textContent;*/
        ...
        /*var styeBPO = '';
        if(tr.cells[14].textContent != ''){
            styeBPO = tr.cells[14].textContent;
        }else{
            styeBPO = tr.cells[1].textContent;
        }*/
        
        // Varify wether PO been raised for the selected style and for the buyer PO
        // ========================================================================
       /* var url = 'bomMiddletire.php?RequestType=IsPORaisedBPO&styleNo=' + URLEncode(StyleNo) + '&buyerPONo=' + URLEncode(styeBPO)
        var htmlObj = $.ajax({url:url, async:false});
        
        var booIsPORaised = htmlObj.responseXML.getElementsByTagName("IsPOExist")[0].childNodes[0].nodeValue;*/
        
        /*if(booIsPORaised == 1){
            $("#txtqty").prop('readonly',true);
            $("#txtqty").prop('disabled',true);
            $("#txtqty").css('background-color','#F4F0ED');            
        }else{
            $("#txtqty").prop('readonly',false);
            $("#txtqty").prop('disabled',false);
            $("#txtqty").css('background-color','#FFFFFF'); 
        }*/
    // =========================
       /* if(booIsPORaised == 1){
            $("#txtbuyerpo").prop('readonly',true);
            $("#txtbuyerpo").prop('disabled',true);
            $("#txtbuyerpo").css('background-color','#F4F0ED');            
        }else{
            $("#txtbuyerpo").prop('readonly',false);
            $("#txtbuyerpo").prop('disabled',false);
            $("#txtbuyerpo").css('background-color','#FFFFFF'); 
	...
	/*
	$("#cboPreviousBPO").val(tr.cells[14].textContent);
	...
	if(tr.cells[15].textContent == '1')
        ...
        if(booIsPORaisedForSelectedPO == 0){
            tr.parentNode.removeChild(tr);
        }else{
            alert("Sorry! Supplier PO was raised for buyer  ");
            return false;
        }
...
function RemoveFromGrid(obj, StyleNo, booIsPORaisedForSelectedPO){
    //alert(booIsPORaisedForSelectedPO);
    var td = obj.parentNode;
    var tr = td.parentNode;
    
    if(booIsPORaisedForSelectedPO == 0){
        
        $('#txtbuyerpo').val(tr.cells[1].textContent);
        $("#txtqty").val(tr.cells[2].textContent);    
        document.getElementById('cbocountry').value 		= tr.cells[3].id;
        document.getElementById('cboLeadTime').value 		= tr.cells[4].id;
        document.getElementById('deliverydateDD').value 	= tr.cells[5].textContent;
        document.getElementById('estimatedDD').value 		= tr.cells[6].textContent;
        ...
        if(booIsPORaised == 1){
            ...
            $("#txtbuyerpo").prop('disabled',true);
            $("#txtbuyerpo").css('background-color','#F4F0ED');  
            $("#cboPreviousBPO").prop('disabled',true);
        }else{
            $("#txtbuyerpo").prop('readonly',false);
            $("#txtbuyerpo").prop('disabled',false);
            $("#txtbuyerpo").css('background-color','#FFFFFF'); 
	...
	document.getElementById('txttotalqty').value = parseFloat(document.getElementById('txttotalqty').value - qty);	
        ...
        alert("Supplier PO been raised for the above buyer PO"); return false;
    }
...
function validateBPO(prmStyleId, prmBPO){
    
    //Get previous BPO from the buyer po
    var url = 'bomMiddletire.php?RequestType=GetPrvBPO&styleNo=' + URLEncode(prmStyleId) + '&buyerPONo=' + URLEncode(prmBPO);
    var htmlObj = $.ajax({url:url, async:false});
    
    var prvBPOId = htmlObj.responseXML.getElementsByTagName("BPOName")[0].childNodes[0].nodeValue;

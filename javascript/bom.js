...
function AddItem(material,category,purchaseType,placement,itemCode,itemDescription,conPc,unitType,unitPrice,wastage,originName,freight,originid,selection)
{
	...
	cellLeftOver.innerHTML = "<img src=\"images/butt_1.png\"/ onclick=\"LoadAllocation(this,'BOM',1);\">";
...
function AddSelection()
{
    if (isValidColorSizeSelection())
    {
        ...
        //var tablewidth = 655 + (7 * sizes.length);
        var bomDetail         =   document.getElementById('matDetail').value;
        var strBomDetail      =   bomDetail.split("|");
        var matDetails        =   strBomDetail[0];
        var bomWastage        =   strBomDetail[2];

        itemCode          = strBomDetail[3]; 
        styleCode         = $("#cboSR").val();
...
//End 31-03-2010 bookmark  (read the array and add data to the table grid)
		...
		HTMLTextYY += "<td width=\"50%\" bgcolor=\"#498CC2\" class=\"normalfntRite\"><div align=\"center\" class=\"normaltxtmidb2\">Wastage %" +
                	"</div></td>" + 
                        "<td class=\"normalfntRite\"><div align=\"center\" class=\"normalfnth2Bm\">&nbsp</td>" +
                        "<td bgcolor=\"#498CC2\" class=\"normalfntRite\"><div align=\"center\" class=\"normaltxtmidb2\">MOQ/MCQ</td>" +                        
                        "</tr>" ;
...
//End 31-03-2010 bookmark  (read the array and add data to the table grid)
				//alert("poValue1="+poValue+" ratioQty="+ratioQty);
                        ...
                        if(editWaste==1){
                            HTMLTextYY += "<td><input type=\"text\" value=\""+arrayWastege[x]+"\" class=\"txtbox\" onkeyup=\"calcWestage(this); ChangeCellValueCon(this); ValidateWithPoQtyCon(this);\"  id=\"wast" + x + "\" size=\"7\" readonly=\"readonly\" />"+ 
                                      "</td>";
...
function ShowMaterialRatiowindow(obj,purchasedQty)
{
	...
	curObj = obj;
	drawPopupArea(801,770,'frmMatRatio');
	var orderQty = document.getElementById('txtorderqty').value; 
	var ExQty = document.getElementById('txtexcessqty').value;
						...
						"<td><table width=\"100%\" height=\"25\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">" +
							"<tr>" +
							  ...
							  "<td width=\"22%\" class=\"normalfnt\"><select name=\"cbopono\" class=\"txtbox\" id=\"cbopono\" onChange=\"changePOonMaterial(" + addingPercentage + "," + itemID + "); ChangeBPONo(" + totqty + "," + orderQty + "); \" style=\"width:135px\">" +
								...
								"<td class=\"normalfnt\">Total Qty ("+uom+")</td>" +
							  "<td class=\"normalfnt\"><input name=\"txttotalorder\" type=\"text\" class=\"txtbox\" disabled=\"disabled\" id=\"txttotalorder\" value=\"" + totqty + "\" style=\"text-align:right\"/></td>" +
							  "<td width=\"5%\" class=\"normalfnt\"><input name=\"matDetail\" type=\"text\" class=\"txtbox\" id=\"matDetail\" value=\"" + material+"|"+conPC +"|"+wastage+ "|"+itemID+ "\" style=\"text-align:right;display:none;\"/></td>" +
                                                          "<td ><input name=\"bpo_Qty\" type=\"text\" class=\"txtbox\"  id=\"bpo_Qty\" value=\"" + purchasedQty + "\" style=\"text-align:right; display:none\"/></td>" +
...
//End 31-03-2010				
			...
			HTMLTextYY += "<td width=\"50%\" bgcolor=\"#498CC2\" class=\"normaltxtmidb2\"><div align=\"center\">Wastage %</div></td>" +
                                        "<td class=\"normalfntRite\"><div align=\"center\" class=\"normalfnth2Bm\">&nbsp</td>" +
				...
				//HTMLTextYY += "<td class=\"normalfntRite\"><div align=\"center\" class=\"normalfnth2Bm\">xcxcx %" +
                	//"</td>" +
                       ...
                       //alert(bomWastage);
                        ...
                        if(editWaste==1)
                            {
                            HTMLTextYY += "<td><input type=\"text\" class=\"txtbox\" value=\""+westage_array[x]+"\"  onkeyup=\"calcWestage(this); ChangeCellValueCon(this); ValidateWithPoQtyCon(this);\"  id=\"wast" + x + "\" size=\"7\" readonly=\"readonly\" style=\"background-color:#EBEBE4;\" />"+ 
                                          "</td>"+
...
//				}
    ...
    // Create The table header
			...
			var loopindex = 0;
                        ...
                        HTMLTextRR += "<tr> <td><div>"
                        HTMLTextRR += "<table width=100%>"  
                       var review=1;
			for (x in colors)
			{
				...
				HTMLTextRR += "<tr>" +
							"<td id="+x+" class=\"normalfnt\">" + colors[x]+ "1</td>" ;
				for (i in sizes) 
				{
...
function calcWestage(val)
{
    ...
    var url = "bomMiddletire.php?RequestType=GetItemWastage&styleId="+styleCode+"&itemcode="+itemCode;
    var htmlObjWaste = $.ajax({url:url,async:false});

    var _dblPSP = htmlObjWaste.responseXML.getElementsByTagName("PSP_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblDP = htmlObjWaste.responseXML.getElementsByTagName("DP_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblEMB = htmlObjWaste.responseXML.getElementsByTagName("EMB_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblPEOSP = htmlObjWaste.responseXML.getElementsByTagName("PEOSP_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblDPE = htmlObjWaste.responseXML.getElementsByTagName("DPE_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblGEW = htmlObjWaste.responseXML.getElementsByTagName("GEW_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblGPD = htmlObjWaste.responseXML.getElementsByTagName("GPD_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblSPE = htmlObjWaste.responseXML.getElementsByTagName("SPE_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblPEODP = htmlObjWaste.responseXML.getElementsByTagName("PEODP_WASTAGE")[0].childNodes[0].nodeValue;
    var _dblGRW = htmlObjWaste.responseXML.getElementsByTagName("GRW_WASTAGE")[0].childNodes[0].nodeValue;
    var _intLgPanels = htmlObjWaste.responseXML.getElementsByTagName("NO_LG_PANELS")[0].childNodes[0].nodeValue;
    var _intPrnPanels = htmlObjWaste.responseXML.getElementsByTagName("PRINT_PANELS")[0].childNodes[0].nodeValue;
    var _intEmbPanels = htmlObjWaste.responseXML.getElementsByTagName("EMB_PANELS")[0].childNodes[0].nodeValue;
    ...
    //balQtyX
        for(var i=1; i<cellLength-3; i++)
        {
           ...
           /* =====================================  */ 
            var qtyWithOperatioWastage = 0;
            qtyWithOperatioWastage += (((qtyWithWest * _dblPEOSP)/100) * (consuption*(_intEmbPanels/_intLgPanels)));
            qtyWithOperatioWastage += (((qtyWithWest * _dblPEODP)/100) * (consuption*(_intEmbPanels/_intLgPanels))) + (((qtyWithWest * _dblPEODP)/100) * (consuption*(_intPrnPanels/_intLgPanels)));
            qtyWithOperatioWastage += (((qtyWithWest * _dblPSP)/100) * consuption) + (((qtyWithWest * _dblDP)/100) * consuption) + (((qtyWithWest * _dblEMB)/100) * consuption) + (((qtyWithWest * _dblDPE)/100) * consuption) + (((qtyWithWest * _dblGEW)/100) * consuption) + (((qtyWithWest * _dblGPD)/100) * consuption) + (((qtyWithWest * _dblSPE)/100) * consuption) + (((qtyWithWest * _dblGRW)/100) * consuption);
            //alert(qtyWithOperatioWastage);
            qtyWithWest += qtyWithOperatioWastage;
            qtyWithWest = Math.ceil(qtyWithWest);
           /* =====================================  */ 
...
//   var bomCon            =   parseFloat(strBomDetail[1]);
  ...
  // alert("ratioId = "+ratioId); 
   ...
   /*/
    * 
    * Calculate FABRIC wastage by fabric operation 
    * ================================================
    */
    var qtyWithOperatioWastage = 0;
    qtyWithOperatioWastage += (((qtyWithWestage * _dblPEOSP)/100) * (consuption*(_intEmbPanels/_intLgPanels)));
    qtyWithOperatioWastage += (((qtyWithWestage * _dblPEODP)/100) * (consuption*(_intEmbPanels/_intLgPanels))) + (((qtyWithWestage * _dblPEODP)/100) * (consuption*(_intPrnPanels/_intLgPanels)));
    qtyWithOperatioWastage += (((qtyWithWestage * _dblPSP)/100) * consuption) + (((qtyWithWestage * _dblDP)/100) * consuption) + (((qtyWithWestage * _dblEMB)/100) * consuption) + (((qtyWithWestage * _dblDPE)/100) * consuption) + (((qtyWithWestage * _dblGEW)/100) * consuption) + (((qtyWithWestage * _dblGPD)/100) * consuption) + (((qtyWithWestage * _dblSPE)/100) * consuption) + (((qtyWithWestage * _dblGRW)/100) * consuption);
    //alert(qtyWithOperatioWastage);
    qtyWithWestage += qtyWithOperatioWastage;
    qtyWithWestage = Math.ceil(qtyWithWestage);
   /* ============================================= */
   
   document.getElementById(""+ratioId+"").value =  qtyWithWestage;
...
function StartMatSaving(itemCode)
{
   ...
   // alert("moq = "+moq+" moqId = "+moqId+" wastId = "+wastId+" wastage = "+wastage);
                        if(bomMaterial=='FAB'){
                           ...
                           /* =====================================  */ 
                            var qtyWithOperatioWastage = 0;
                            qtyWithOperatioWastage += (((dblTotReq * _dblPEOSP)/100) * (yyQty*(_intEmbPanels/_intLgPanels)));
                            qtyWithOperatioWastage += (((dblTotReq * _dblPEODP)/100) * (yyQty*(_intEmbPanels/_intLgPanels))) + (((dblTotReq * _dblPEODP)/100) * (yyQty*(_intPrnPanels/_intLgPanels)));
                            qtyWithOperatioWastage += (((dblTotReq * _dblPSP)/100) * yyQty) + (((dblTotReq * _dblDP)/100) * yyQty) + (((dblTotReq * _dblEMB)/100) * yyQty) + (((dblTotReq * _dblDPE)/100) * yyQty) + (((dblTotReq * _dblGEW)/100) * yyQty) + (((dblTotReq * _dblGPD)/100) * yyQty) + (((dblTotReq * _dblSPE)/100) * yyQty) + (((dblTotReq * _dblGRW)/100) * yyQty);
                            //alert(qtyWithOperatioWastage);
                            dblTotReq += qtyWithOperatioWastage;
                            dblTotReq = Math.ceil(dblTotReq);
                           /* =====================================  */ 
...
//		}
		...
		row.innerHTML = "<td style=\"width:20px; border: 1px solid #CCC;\"><img src=\"images/del.png\" alt=\"del\" width=\"15\" height=\"15\" border=\"0\" class=\"mouseover\" onclick=\"RemovePONumberFromGrid(this);\"  /></td>" +
					   ...
					   "<td class=\"normalfnt\" style=\"width:75px; border: 1px solid #CCC;\">" + handoverDD + "</td>"+
                                	   "<td class=\"normalfnt\" style=\"width:78px; border: 1px solid #CCC;\">" + pcdDate + "</td>"+																											 
	 ...
	 //--------->Planinng Confirmation Strt.	
		var styleNo = document.getElementById('lblStyleNo').innerHTML;
		var bpoNo   = document.getElementById('txtbuyerpo').value
		var prvBpoNo   = document.getElementById('cboPreviousBPO').value
		
		
	  var bomURL 		= "bomMiddletire.php?RequestType=plConBpo&styleNo=" + styleNo + "&bpoNo=" + bpoNo+ "&prvBpoNo=" + prvBpoNo;
	  var htmlRatioObj 	= $.ajax({url:bomURL, async:false});
	  
	  //--------->Planinng Confirmation End.	
...
function saveDelBPO()
{
	...
	if(ValidateBPOSaving())
	{
		...
		for (var loop = 0 ; loop < tbl.rows.length ; loop ++ )
		{
			...
			var pcdDate = tbl.rows[loop].cells[8].textContent;
			var bpoShipMode = tbl.rows[loop].cells[9].id;
			var bpoRemarks 	= tbl.rows[loop].cells[10].textContent;
			//var bpoRefNo = tbl.rows[loop].cells[9].textContent;
			//var handOverDate = tbl.rows[loop].cells[10].textContent;
			
			// ================================================================
			var bpoDeliveryStatus 	= tbl.rows[loop].cells[11].id;
			var bpoCutOffDate		= tbl.rows[loop].cells[12].textContent;
			var bpoProdStatus		= tbl.rows[loop].cells[13].textContent;
			var bpoPrevoius			= tbl.rows[loop].cells[15].textContent;
			var bpoManuLocation     = tbl.rows[loop].cells[14].id;
			var bpoShortShipped 	= tbl.rows[loop].cells[16].textContent;
                        var ibpoShrtShipReason  = tbl.rows[loop].cells[17].textContent;
		...
		/*	var urlmr 		= 'preordermiddletire.php?RequestType=UpdateMR&styleID=' + URLEncode(StyleNo) + '&buyerPO=' + URLEncode(buyerPO) + '&prvbuyerPO=' + URLEncode(bpoPrevoius) ;
			...
			var url2 = 'preordermiddletire.php?RequestType=AddDeliveryBPO&styleID=' + URLEncode(StyleNo) + '&buyerPO=' + URLEncode(buyerPO) + '&bpoQty=' + bpoQty + '&bpoLeadTime=' + bpoLeadTime + '&bpoDelivery=' + bpoDelivery + '&bpoEstimate=' + bpoEstimate + '&bpoHandover=' + bpoHandover + '&pcdDate=' + pcdDate + '&bpoShipMode=' + bpoShipMode + '&bpoRemarks=' + URLEncode(bpoRemarks) + '&bpoCountry=' + URLEncode(bpoCountry) + '&bpoDeliStatus='+ URLEncode(bpoDeliveryStatus) + '&bpoCutOffDate=' + bpoCutOffDate + '&manuLocationId=' + URLEncode(bpoManuLocation) + "&shipStatus=" + bpoShortShipped + "&prvBPO=" + bpoPrevoius + "&ishtreason=" + ibpoShrtShipReason /*+ '&bpoRefNo=' +bpoRefNo+'&handOverDate='+handOverDate*/;
...
function LoadAllocation(obj,type,opt)
{
	alloType =type; 
	if(type == 'BOM')
	{
	...
	//xmlHttp.open("GET",'allocation_inbom/allocation.php?matDetailId='+matDetailId+ '&styleId=' +styleId+'&bomQty='+bomQty+ '&FreightPrice='+freightPrice,true);
        xmlHttp.open("GET",'allocation_inbom/allocation.php?matDetailId='+matDetailId+ '&styleId=' +styleId+'&bomQty='+bomQty+ '&FreightPrice='+freightPrice+"&strColor='"+strColor+"'&strSize='"+strSize+"'&buyerPO="+buyerPONo+"&opt="+opt,true);
	xmlHttp.send(null);
}

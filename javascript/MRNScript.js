...
function getStyleName()
{
    ...
    xmlHttp.open("GET", 'MRNMiddle.php?RequestType=getStyleID', true);
    xmlHttp.send(null);  */
	var styleSC = document.getElementById('cboscno').value;
	var url = 'MRNMiddle.php?RequestType=getStyleNOS&styleSC=' + URLEncode(styleSC);
	 
	var htmlobj=$.ajax({url:url,async:false});
	var XMLStyleNSC = htmlobj.responseXML.getElementsByTagName("StyleNSC");						
	document.getElementById("cboStyleID").innerHTML = XMLStyleNSC[0].childNodes[0].nodeValue;
}
...
{ 
	...
	for(var loop = 0; loop < itemDisI.length; loop ++)
	{
		...
		var strUOM			= fourthxmlHttp.responseXML.getElementsByTagName("strUOM")[loop].childNodes[0].nodeValue;
		// ===================================
		// 2015/10/30
		// Nalin Jayakody
		// Get buyer po number 
		// ===================================
		var strBuyerPO		= fourthxmlHttp.responseXML.getElementsByTagName("strBuyerPO")[loop].childNodes[0].nodeValue;
		// ===================================
		...
		// Comment For - To add buyer po number to the grid
		// ====================================================
		craeteGrid(itemDis,color,size,qty,matDetaiID,roundQty,balanceQty,mrnRaised,issueQty,issueBalance,stockBalance,wastage,Approved,NotApproved,conPCItem,trimInspected,grnNo,grnYear,grnType,grnTypeId,invoiceNo,mainCategoryId, strUOM, strBuyerPO); 
	}
 getReqQty(document.getElementById("txtReqQty"));		 
}
 
 function craeteGrid(itemDis,color,size,qty,matDetaiID,reqQTY,balanceQty,mrnRaised,issueQty,issueBalance,stockBalance,wastage,Approved,NotApproved,conPCItem,trimInspected,grnNo,grnYear,grnType,grnTypeId,invoiceNo,mainCategoryId, prmUOM, prmBuyerPO)
 {
	var inhose=0	
    var tbl = document.getElementById('mrnMatGrid');
	...
	cellqty.innerHTML=prmBuyerPO
	
	var cellqty=row.insertCell(6);
	cellqty.className="normalfntRite";
	cellqty.innerHTML=prmUOM
	
	var cellqty=row.insertCell(7);
	cellqty.className="normalfntRite";
	cellqty.innerHTML=qty;
		
	var reqQty=row.insertCell(8);
	...
	{	
		...
		else if(parseFloat(stockBalance)>=parseFloat(balanceQty))
		{
			...
			{
				var entqty=parseFloat(stockBalance)+parseFloat(issueQty)-parseFloat(mrnRaised);
			}
		}
		
		entqty = entqty.toFixed(2);
		...
		/* Date - 11/17/2014  
		   By   - Nalin Jayakody 
		   Descr - round 'entqty' varible to 2 decimal points to avoid unnessary fraction values
		   */
		// ###########################################################################
	}
	...
	//alert("Stock Balance " + stockBalance + " and Issueed " + issueQty + " and MRN Raised " + mrnRaised);
	
	
	var Balqty=row.insertCell(9);
	Balqty.className="normalfntRite";
	// ====================================================================
	// Change balance to MRN qty calculation
	//var balQty1 = parseFloat(stockBalance)+parseFloat(issueQty)-parseFloat(mrnRaised);
	// ====================================================================
	// Chnage as below
	var balQty1 = parseFloat(stockBalance)-parseFloat(mrnRaised);
...
function checkQty(obj)
{
	...
	/*var enterQty=cell[7].firstChild.value
	alert(enterQty);
	var ratioQty=cell[6].innerHTML;
	alert(ratioQty);
	var maxMrn = cell[8].innerHTML;
	alert(maxMrn);*/
	
	var enterQty=cell[8].firstChild.value
	var ratioQty=cell[7].innerHTML;
	var maxMrn = cell[9].innerHTML;
	...
	if(isNaN(enterQty)==true)
	{
		...
		cell[8].firstChild.value=maxMrn;
		return;
	}
...
for (var ii = 1; ii < rw.length; ii++)
	{
		...
		if(ss!="ss")
		{
			if (cell[1].firstChild.firstChild.checked==true)
			{
				...
				var qty			= cell[7].lastChild.value; 					
				var grnNo 		= cell[19].childNodes[0].childNodes[0].nodeValue;
				var grnYear 	= cell[20].childNodes[0].nodeValue; 
				var grnTypeId 	= cell[21].id;
				var grnType		= cell[21].childNodes[0].nodeValue; 
				var invoiceNo 	= cell[22].innerHTML; */
				//========================================================
				var buyerpo		= cell[5].childNodes[0].nodeValue;
				var balQty		= cell[9].childNodes[0].nodeValue; 
				var qty			= cell[8].lastChild.value; 					
				var grnNo 		= cell[20].childNodes[0].childNodes[0].nodeValue;
				var grnYear 	= cell[21].childNodes[0].nodeValue; 
				var grnTypeId 	= cell[22].id;
				var grnType		= cell[22].childNodes[0].nodeValue; 
				var invoiceNo 	= cell[23].innerHTML;
...
var cellDelete = row.insertCell(0);   
	...
	// Comment On - 2015-10-30
	// Comment By - Nalin Jayakody
	// Comment for - Remove order no column from MRN grid, becasue style no exist
	//================================================
	...
	var cellDetails=row.insertCell(4);
	...
	var cellBalqty=row.insertCell(7);
	cellBalqty.className="normalfntRiteSML";
	cellBalqty.innerHTML=BuyerPO;
...
function saveMRNDetail(mrnNo)
{
	...
	for ( var loop = 1 ;loop < row.length ; loop ++ )
	{
		...
		var buyerPo	= cell[7].childNodes[0].nodeValue;
		var scNo	= cell[2].childNodes[0].nodeValue;

		var MatID	= cell[3].childNodes[0].nodeValue;
		var color	= cell[5].childNodes[0].nodeValue;
		var size 	= cell[6].childNodes[0].nodeValue;
...
{	//alert(mrnYear);
	...
	var mrn_year = (document.getElementById("mrnYear").value==""? new Date().getFullYear():document.getElementById("mrnYear").value);
	 ...
	 // alert(document.location.hostname);
        ...
        // Add for - When accessing from through internet check port number and add to the path
        // =============================================================
        var path = "";
        if(document.location.port == ""){
           path = document.location.protocol+'//'+document.location.hostname+'/'+document.location.pathname.split("/")[1]+'/';
	 //+document.location.pathname.split("/")[2];
        }else{
           path = document.location.protocol+'//'+document.location.hostname+':'+document.location.port+'/'+document.location.pathname.split("/")[1]+'/';
        }
	// =============================================================

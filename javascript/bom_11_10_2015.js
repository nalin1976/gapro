					  ...
					  "<tr>" +
						"<td><table width=\"100%\" height=\"31\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">" +
							"<tr>" +
							  ...
							  "<td width=\"18%\" bgcolor=\"#D6E7F5\"><img src=\"images/save.png\" alt=\"save\" width=\"84\" height=\"24\" class=\"mouseover\" onClick=\"SaveStyleRatio1(); SaveStyleRatio();\" /></td>" +
	...
	if(xmlHttpreq[1].readyState == 4) 
    {
        ...
        {  
			...
			for (x in colors)
			{
				...
				{
					HTMLText += "<td><div align=\"center\">" +
								"<input name=\"txtratio\" type=\"text\" class=\"txtbox\" value=\"" + FOBs[loopindex] + "\" onkeyup=\"ChangeCellValueFOB(this);\" onmousedown=\"DisableRightClickEvent();\" onmouseout=\"EnableRightClickEvent();\"  onkeypress=\"return isNumberKey(event);\" id=\"txtratio" + x + "" + i + "\" size=\"7\" />" +
								"</div></td>";
					loopindex ++;
				}
			...
			for (x in colors)
			{
				...
				{
					HTMLText += "<td><div align=\"center\">" +
								"<input name=\"txtratio\" type=\"text\" class=\"txtbox\" value=\"" + PackQtys[loopindex] + "\" onkeyup=\"ChangeCellValuePackQty(this);\" onmousedown=\"DisableRightClickEvent();\" onmouseout=\"EnableRightClickEvent();\"  onkeypress=\"return isNumberKey(event);\" id=\"txtratio" + x + "" + i + "\" size=\"7\" />" +
								"</div></td>";
					loopindex ++;
				}
...
function ShowMaterialRatiowindow(obj,purchasedQty)
{
	...
	curObj = obj;
	drawPopupArea(800,600,'frmMatRatio');
	var orderQty = document.getElementById('txtorderqty').value; 
	var ExQty = document.getElementById('txtexcessqty').value;
	...
	var HTMLText = "<table width=\"800\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">" +
				  ...
				  "<tr>" +
					"<td class=\"bcgl1\"><table width=\"800\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">" +
					  ...
					  "</tr>" +
					  "<tr>" +
						"<td><div id=\"divQtyRatio\" style=\"overflow:scroll; height:500px; width:800px;\">" +
						"</div></td>" +
					 "</tr>" +
					 "<tr>" +
						//"<td>&nbsp;</td>" +
...
function StartMatSaving(itemCode)
{
	...
	for ( var loop = 1 ;loop < tblQty.rows.length -1 ; loop ++ )
	{
		for ( var i = 1 ;i < tblQty.rows[loop].cells.length -1 ; i ++ )
		{
			...
			//xmlHttpreq[i].onreadystatechange = HandleStyleRatioRequest;
			//xmlHttpreq[i].index = i;
			xmlHttpreq[incr].open("GET", 'bomMiddletire.php?RequestType=SaveMatRatio&styleNo=' + URLEncode(styleNo) + '&itemCode=' + itemCode  + '&buyerPONo=' + URLEncode(buyerPONo) + '&color=' + URLEncode(color) + '&size=' + URLEncode(size) + '&qty=' + qty + '&posID=' + (loop -1 ) + '&freight=' + qty + '&sizeserial=' + sizeserial, true);
			xmlHttpreq[incr].send(null);
			incr ++ ;	
			pub_ResponceCount++;
...
//		}
		...
		row.innerHTML = "<td style=\"width:20px;\"><img src=\"images/del.png\" alt=\"del\" width=\"15\" height=\"15\" border=\"0\" class=\"mouseover\" onclick=\"RemovePONumberFromGrid(this);\"  /></td>" +
                       "<td class=\"normalfnt\" ondblclick=\"changeToTextBoxBPO(this);\" style=\"width:90px;\" >"  + txtBuyerPO + "</td>"+
                       "<td class=\"normalfntRite\" ondblclick=\"changeToTextBoxBPO(this);\" style=\"width:60px;\" >" + txtQty +"</td>"+
                       "<td class=\"normalfntMid\" id=\"" + countryID + "\" style=\"width:70px;\">" + CountryName + "</td>"+
                       "<td class=\"normalfnt\" id=\"" + leadTimeID + "\" style=\"width:32px;\" >" + leadName + "</td>"+
                       "<td class=\"normalfnt\" style=\"width:78px;\">" + deliveryDate + "</td>"+
                       "<td class=\"normalfnt\" style=\"width:78px;\">" + estimatedDD + "</td>"+
					   "<td class=\"normalfnt\" style=\"width:78px;\">" + handoverDD + "</td>"+
                       "<td class=\"normalfnt\" id=\"" + shippingModeID + "\" style=\"width:56px;\">" + shippingMode + "</td>"+
                       "<td class=\"normalfnt\" NOWRAP style=\"width:75px;\">" + remarks + "</td>" +
					   "<td class=\"normalfnt\" id=\"" + orderstatus + "\" style=\"width:72px;\">" + orderstatusText + "</td>" + 
					   "<td class=\"normalfnt\" style=\"width:70px;\">" + cutoffdate + "</td>";
...
function ValidateBPOSaving()
{
	...
	/* Comment On - 11/09/2015
	   Comment By - Nalin Jayakody
	   Comment For - To prevent consider excesss quantity for calculate order qty in BPO 
	 */
...
{	
	...
	drawPopupBox(850,570,'frmstylewise',1);
	...
	if(xmlHttpreq[2].readyState == 4 && xmlHttpreq[2].status == 200) 
	{
		...
		for ( var loop = 0; loop < XMLStyleID.length; loop ++)
			...
			for (x in colors)
			{
				...
				{
					HTMLText += "<td><div align=\"center\">" +
								"<input name=\"txtratio\" type=\"text\" ondblClick=\"showMaterialRatioHelper(this);\" class=\"txtbox\" value=\"" + POQty_Colsize[x][i] + "\" onkeyup=\"ChangeCellValue(this);\"  onmousedown=\"DisableRightClickEvent();\" onmouseout=\"EnableRightClickEvent();\"  onkeypress=\"return isNumberKey(event);\" id=\"txtPoratio" + x + "" + i + "\" size=\"7\" />" +
								"</div></td>";
					loopindex ++;
				}

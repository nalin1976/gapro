...
var currentStatus = 1;
 ...
 var dblTotalReuiredQty = 0;
 var dblTotalPORaisedQty = 0;
				 ...
				 for ( var loop = 0; loop < Item.length; loop ++)
				  {
					...
					var color  =  XMLcolor[loop].childNodes[0].nodeValue;
					var size   =  XMLsize[loop].childNodes[0].nodeValue;
					var qty    =  Math.ceil(XMLpendingQty[loop].childNodes[0].nodeValue);
					var unitprice = XMLunitprice[loop].childNodes[0].nodeValue;
					var freight = XMLfreight[loop].childNodes[0].nodeValue;
...
{ 
	...
	cellSize.innerHTML =  Math.ceil(totQty);
	
	var cellQty = row.insertCell(5);
	cellQty.className = "normalfntRB";
	//cellQty.innerHTML =  qty;
	cellQty.innerHTML =  "<a href=\"#\" onclick=\"openItemMvPopup(this);\">"+Math.ceil(qty)+"</a>";
...
function RemoveRowsFromMainTable()
{
	...
	{//alert(tblRows);
			...
			if(chk == true)
			{
				...
				var ItemQty				= htmlobj.responseXML.getElementsByTagName("ItemQty")[0].childNodes[0].nodeValue;				
                                ...
                                lineQty += parseFloat(qty);
			var RemarksNew="";
			var intFreight = document.getElementById("cboPoItems").value;
			//alert("lineQty ="+lineQty+" dblTotalPORaisedQty ="+ dblTotalPORaisedQty +" dblTotalReuiredQty ="+dblTotalReuiredQty);
			
			
			//if (qty > 0 && intFreight!=1) 
                        
                        if(lineQty + dblTotalPORaisedQty > dblTotalReuiredQty){
                           
                           alert('There were alredy PO raised for some quantities, exceeding total requirement ');
                           RemoveRowsFromMainTable();
                           return;
   ...
   if(document.getElementById("cboOrderNo").value == '' && document.getElementById("butOk").disabled == true)
	...
	else
	{
	   if(document.getElementById('txtDate').value == "" && document.getElementById('cboMaterial').value != 4)
	   {
        ...
        // Add on - 08/22/2016
        // Add By - Nalin Jayakody
        // Adding - To validate with purhcase order line total quantities with total required quantities
        //=======================================================================
         var URLpath = "POMiddle.php?RequestType=GetItemReqTot";
	     URLpath += "&styleNo="+styleNo+'&mainCat='+mainCat+'&subCat='+subCat+'&itemList='+itemList;
             
        var HTMLObj = $.ajax({url:URLpath, async:false});
       
        var XMLRequiredQty = HTMLObj.responseXML.getElementsByTagName("TotRequiredQty");
        
        var dblTotReqQty = parseFloat(XMLRequiredQty[0].childNodes[0].nodeValue);
        dblTotalReuiredQty = dblTotReqQty;
        dblTotReqQty += 4;
        //=======================================================================
        ...
        // Add on - 11/01/2016
        // Add By - Nalin Jayakody
        // Adding - To check raised for PO total Qty
        //=======================================================================
        var URLpath = "POMiddle.php?RequestType=GetTotPOQty";
	     URLpath += "&styleNo="+styleNo+'&mainCat='+mainCat+'&subCat='+subCat+'&itemList='+itemList;
             
        var HTMLObj = $.ajax({url:URLpath, async:false});
       
        var XMLPORaisedQty = HTMLObj.responseXML.getElementsByTagName("TotPOQty");
        
        dblPORaisedQty = parseFloat(XMLPORaisedQty[0].childNodes[0].nodeValue);
        dblTotalPORaisedQty = dblPORaisedQty;
        //alert(dblPORaisedQty)
        
        if(dblPORaisedQty >= dblTotReqQty-4){
            
            alert("Sorry, already purchase orders has been raised for the total requirement ");
            return;
        }
				...
				//end----------------------------------------------------
				 for ( var loop = 0; loop < Item.length; loop ++)
				 {
					...
					var buyerPONo		= XMLBuyerPONo[loop].childNodes[0].nodeValue;
                                        ...
                                        totLineQty +=  Math.ceil(parseFloat(qty));
                                        
                                        if(totLineQty > dblTotReqQty){
                                            alert("Sorry, sum of line quantities not equal with total required quantity");
                                            RemoveRowFromTable();
                                            return;
                                        }else{

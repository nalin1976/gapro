...
function validateSpecialCharacters(id) 
{ 
	var idValue = document.getElementById(id);
	//alert(idValue.value);
	var spclChars = "!@#$^&*'"; 
	
	for (var i = 0; i < idValue.value.length ; i++) 
	//alert(idValue.value.length);	
	{ 
		if (spclChars.indexOf(idValue.value.charAt(i)) != -1) 
		{ 
...
function CheckforValidDecimal2(value,decimalPoints,evt)
{
	
	decimalPoints = parseInt(decimalPoints)+1;
	
	//assign a variable to pressing key
	var charCode = (evt.which) ? evt.which : evt.keyCode;
	
	var positionCount = parseInt(value.indexOf('.'));
	
	//if position count = 1 and press delete key, do not delete
	if((positionCount>0) && (charCode=='46'))
		return false;
	
	//pressing shift+left arrow and do not allow selecting amount to left selection
	if (evt.shiftKey && charCode==37 ){
		return false;
	}
	
	//pressing shift+right arrow and do not allow selecting amount to right selection
	if (evt.shiftKey && charCode==39 ){
		return false;
	}
...
function incHeader(filename){
	
	var elScript = "<script type='text/javascript' src='"+filename+"'></script>";

...
function SavePartDetails()
{
	...
	for(loop=1;loop<tblMain.rows.length;loop++)
	{
		//var EFF      = tblMain.rows[loop].cells[4].childNodes[0].value;
                var EFF      = parseFloat(tblMain.rows[loop].cells[5].childNodes[0].value);
		...
		var SMV                 = parseFloat(tblMain.rows[loop].cells[3].childNodes[0].value);
                var packSMV             = parseFloat(tblMain.rows[loop].cells[4].childNodes[0].value);
                var isGarmentDye        = ((JSON.parse(tblMain.rows[loop].cells[6].childNodes[0].checked)? 1: 0));
                var isPermanantCreasing = ((JSON.parse(tblMain.rows[loop].cells[7].childNodes[0].checked)? 1: 0));
		...
		var UM=(TotSMV*100)/parseInt(EFF);
		...
		//xmlHttp2[loop].open("GET",'partDetailsXml.php?RequestType=SavePartDetails&PartNo=' +URLEncode(PartNo)+ '&PartName=' +URLEncode(PartName)+ '&SMV=' +SMV+ '&EFF=' +EFF+ '&StyleId=' +URLEncode(StyleId)+ '&SMVRate=' +SMVRate + '&pSMV=' + packSMV  ,true);
		xmlHttp2[loop].open("GET",'partDetailsXml.php?RequestType=SavePartDetails&PartNo=' +URLEncode(PartNo)+ '&PartName=' +URLEncode(PartName)+ '&SMV=' +SMV+ '&EFF=' +EFF+ '&StyleId=' +URLEncode(StyleId)+ '&SMVRate=' +SMVRate + '&pSMV=' + packSMV + '&iDY=' + isGarmentDye + '&iPC=' + isPermanantCreasing ,true);
	...
	var EffSum=(FullSMV/TotUM)*100;
        //alert(EffSum);
	//document.getElementById("txtSMV").value = TotSMV;
	document.getElementById("txtSMV").value = TotSewingSMV;
	document.getElementById("txtPackSMV").value = TotPackSMV;
...
function ShowPartDetailsWindow()
{
	...
	drawPopupArea(570,244,'frmpartDetails');				
...
function CalculateEff(obj)
{
	...
	var SMV  = parseFloat(rw.cells[3].childNodes[0].value);
        var pSMV = parseFloat(rw.cells[4].childNodes[0].value);
	var Qty 	= document.getElementById("txtQTY").value;
	//rw.cells[4].childNodes[0].value ="";
        rw.cells[5].childNodes[0].value ="";
        
        var totalSMV = SMV+ pSMV;
	//alert(totalSMV);
	createXMLHttpRequest();
	xmlHttp.index= rw;
	...
	//xmlHttp.open("GET",'partDetailsXml.php?RequestType=CalculateEff&SMV=' +SMV+ '&Qty=' +Qty ,true);
        xmlHttp.open("GET",'partDetailsXml.php?RequestType=CalculateEff&SMV=' +totalSMV+ '&Qty=' +Qty ,true);
	xmlHttp.send(null);	
}
		...
		if(xmlHttp.readyState==4)
		{
			...
			{				
				...
				//rw.cells[4].childNodes[0].value=XMLEff;
                                rw.cells[5].childNodes[0].value=XMLEff;
			}
		}
	}
...
{	
	...
	cellPartDetails.className = "normalfntMid";	
	cellPartDetails.innerHTML = "<input type=\"text\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\" name=\"txtPackSmv\" id=\"txtPackSmv\" class=\"txtbox\" size=\"8\" style=\"text-align:right\" onkeypress=\"return isNumberKey(event);\" value=\"0\"  onblur=\"CalculateEff(this);\" />";
	
	var cellPartDetails = row.insertCell(5);
	cellPartDetails.className = "normalfntMid";	
	cellPartDetails.innerHTML ="<input type=\"text\" name=\"txtEFF\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\" id=\"txtEFF\" class=\"txtbox\" size=\"8\" style=\"text-align:right\" value=\""+""+"\" readonly=\"readonly\"/>";
        ...
        var cellPartDetails = row.insertCell(6);
	cellPartDetails.className = "normalfntMid";	
	cellPartDetails.innerHTML ="<input type=\"checkbox\" name=\"chkGD\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\" id=\"chkGD\" class=\"txtbox\" size=\"8\" style=\"text-align:right\" value=\""+""+"\" />";
        
        var cellPartDetails = row.insertCell(7);
	cellPartDetails.className = "normalfntMid";	
	cellPartDetails.innerHTML ="<input type=\"checkbox\" name=\"chkPC\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\" id=\"chkPC\" class=\"txtbox\" size=\"8\" style=\"text-align:right\" value=\""+""+"\" />";
}

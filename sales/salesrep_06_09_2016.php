...
$resDeliList = $class_sales->getDeliveries($buyer, $dtFormatFrom, $dtFormatTo);
...
while($row = mysql_fetch_array($resDeliList)){
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6,$rowId,$row["reaFOB"]);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7,$rowId,$row["dtmHandOverDate"]);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8,$rowId,$strAuthoUser);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9,$rowId,$dtAuthoTime);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10,$rowId,$strConfirmUser);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11,$rowId,$dtConfirmTime);

	if(($strAuthoUser != "Not Authorised") && ($strConfirmUser != "Not Confirmed")){

		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12,$rowId,number_format(($row["dblQty"] * $row["reaFOB"]),2));
		$dblTotValue += ($row["dblQty"] * $row["reaFOB"]);
	}else{
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12,$rowId,0);
	}
...
$rowId ++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12,$rowId,number_format($dblTotValue,2));

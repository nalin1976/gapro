...
$ImpExpConnectClass = new ClassConnectImpExp();
...
while($row = mysql_fetch_array($resDeliList)){
	...
	//$FGStock = $class_sales->GetShippedStatusFromD2D($row["intSRNO"],  $row["intBPO"], $d2dConnectClass);
        ...
        $dblShippedQty = $class_sales->GetShippedQty($row["intSRNO"],  $row["intBPO"], $ImpExpConnectClass);
	...
	$resConfirmUser = $class_sales->GetOrderConfirmUser($row["intStyleId"], $row["intBPO"]); 
        $resFacUser     = $class_sales->GetFactoryConfirmUser($row["intStyleId"], $row["intBPO"]); 
        $strRemarks     = $class_sales->GetRemarks($row["intStyleId"], $row["intBPO"]); 
	...
	if(stripos($resConfirmUser,'/')>0){
        ...
        if(stripos($resFacUser,'/')>0){

		$arrFactoryUser = explode('/',$resFacUser);
		$strFactoryUser = $arrFactoryUser[0];
		$dtFactoryTime  = $arrAuthoUser[1];
	}else{
		$strFactoryUser = $resFacUser;
	}
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5,$rowId,number_format($FGStock));
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6,$rowId,number_format($dblShippedQty));
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7,$rowId,$row["dblFOB"]);
        
        if(($strAuthoUser != "Not Authorised") && ($strConfirmUser != "Not Confirmed")){
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8,$rowId,number_format(($row["dblQty"] * $row["dblFOB"]),2));
            $dblTotValue += ($row["dblQty"] * $row["dblFOB"]);
	}else{
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8,$rowId,0);
	}
        
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9,$rowId,$row["dtmHandOverDate"]);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10,$rowId,$row["strName"]);
	
	//$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9,$rowId,$dtAuthoTime);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11,$rowId,$strFactoryUser);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12,$rowId,$strAuthoUser);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(13,$rowId,$strConfirmUser);

	

	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(14,$rowId,$row["ProdLocation"]);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(15,$rowId,$strRemarks);
...
$rowId ++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8,$rowId,number_format($dblTotValue,2));

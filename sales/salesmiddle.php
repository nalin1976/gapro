...
if(strcmp($RequestType,"AddRemarks") == 0){

    $styleId 	= $_GET["styleId"];
    $buyerPONo 	= $_GET["bpono"];
    $Remarks        = $_GET["remark"];

    UpdateRemarks($styleId, $buyerPONo, $Remarks);

}

if(strcmp($RequestType,"IsRemarkPermission") == 0){
    
    $class_permission->SetConnection($db);
    $resPermission = $class_permission->IsPermissionAllowed($userId,'Add remark to sales');
    
    $ResponseXML = "";
    $ResponseXML = "<IsPermission>\n";
...
function UpdateRemarks($prmStyleId, $prmBuyerPO, $prmRemark){
    ...
    if(mysql_num_rows($resSalesExist)>0){
        ...
        $sql = " UPDATE salesconfirmation SET strremarks = '$prmRemark' WHERE styleId = '$prmStyleId' AND bpoNo = '$prmBuyerPO'"; 
    
    }else{
        $sql = " INSERT INTO salesconfirmation(styleId, bpoNo, strremarks) 
	         VALUES('$prmStyleId', '$prmBuyerPO', '$prmRemark')";
    }

...
function AddDetailsToMainTbl(obj)
{
	...
	if(obj.checked)
	{
		...
		else if(XMLGlID.length>1)
		{
			...
			ViewMultipleGLPopUp(pub_mainCat,document.getElementById('cboCostCenter').value,itemId);
			booAvail	=  false;
		}
		else
		{
		...
		{			
			...
			for(loop=0;loop<XMLItem.length;loop++)
			{
				...
				var XMLOrderQty		= 0;
				var XMLDisPercent	= 0;
				pub_mainCat			= XMLMainCat;
				CreateMainItemTbl(XMLItemId,XMLItemDesc,XMLUnit,XMLUPrice,XMLFUPriceDate,XMLUPriceDate,XMLMainCat,XMLReorderLevel,XMLStockBal,XMLOrderQty,XMLDisPercent);
			}
		}
	}
	else
...
function CreateMainItemTbl(XMLItemId,XMLItemDesc,XMLUnit,XMLUPrice,XMLFUPriceDate,XMLUPriceDate,XMLMainCat,XMLReorderLevel,XMLStockBal,XMLOrderQty,XMLDisPercent)
{
	...
	cell.innerHTML 	= "<input type=\"text\" style=\"width:70px;text-align:right\" class=\"txtbox\" onkeypress=\"return CheckforValidDecimal(this.value,1,event);\" maxlength=\"10\" value=\""+XMLUPrice+"\" onkeyup=\"CalculateRowValue(this.parentNode.parentNode.rowIndex)\"/>";
...
function CreateGLItemTbl(glId,glDesc,glCode,itemId,modAmount,addAmount,transInAmount,balAmount,acAmount,varience,requested,pending)
{
	...
	if(IsGlAvailable(tblGl,pub_mainCat))
		return;
...
function RemoveItemFromTbl1(itemId)
{
	...
	for(i=1;i<tblMain.rows.length;i++)
	{
		if(tblMain.rows[i].cells[m_del].id==pub_mainCat)
		{
			booAvail = true;
		}
	}
	if(!booAvail)
	{
		...
		for(loop=1;loop<tblGl.rows.length;loop++)
		{
			if(tblGl.rows[loop].cells[g_gc].id==pub_mainCat)
				tblGl.rows[loop].parentNode.removeChild(tblGl.rows[loop]);
		}
	}
}
...
function CalculateRowValue(rwIndex)
{
	...
	var mainCatId	= tbl.rows[rwIndex].cells[m_del].id;
	var fincalValue = roundNumber(rowValue-rowDisValue,4);
	UpdateGLRowValue(mainCatId,fincalValue);
}
...
function UpdateGLRowValue(mainCatId,fincalValue)
{
	...
	for(i=1;i<tblGlMain.rows.length;i++)
	{
		if(mainCatId==tblGlMain.rows[i].cells[g_gc].id)
		{
			...
			tblGlMain.rows[i].cells[g_currB].id	 					 = fincalValue;
		}
	}
}
...
function AddDetailsToMainTbl_Multiple(obj)
{
	...
	if(obj.checked)
	{
		...
		{
			//alert("Multiple 'GL Account' found.");
			ViewMultipleGLPopUp(pub_mainCat,document.getElementById('cboCostCenter').value);
			booAvail	=  false;
		}*/
		else
		{

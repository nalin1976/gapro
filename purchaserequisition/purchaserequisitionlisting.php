...
$session_CompanyId		= $_SESSION["FactoryID"];
...
{
	var url  = "purchaserequisitionxml.php?RequestType=URLLoadCostCenter";
		url += "&CompanyId="+obj.value;
	var htmlobj_gl = $.ajax({url:url,async:false});
	document.getElementById("cboCostCenter").innerHTML = htmlobj_gl.responseText;
}
</script>
</head>
          ...
          <td colspan="2"><table width="100%" border="0" cellpadding="1" cellspacing="0" class="tableBorder">
            ...
            <tr>
              <td width="100%" height="21"><table width="100%" border="0">
                <tr>
                  <td width="72" class="normalfnt">Company</td>
                  <td width="72" ><select name="cboCompany" class="txtbox" id="cboCompany" style="width:180px" onchange="LoadCostCenter(this);">
                    <option value="" selected="selected">Select One</option>
                    <?php
	 
	$SQL = "SELECT intCompanyID,strName,strComCode FROM companies c where intStatus='1' ";
	if(!$PP_ViewAllFactoryDetailsInPRListing)
		$SQL .= " and intCompanyID='$session_CompanyId' ";
...
if(!$PP_ViewAllFactoryDetailsInPRListing)
{
	$SQL = "select distinct strPRNo from purchaserequisition_header where intCompanyId = $session_CompanyId order by strPRNo";
}
else
{
	...
	?>
                ...
                <tr>
                  <td ><span class="normalfnt">Cost Center</span></td>
                  <td><select name="cboCostCenter" class="txtbox" id="cboCostCenter" style="width:180px">
                    <option value="" selected="selected">Select One</option>
                    <?php
if($PP_ViewAllFactoryDetailsInPRListing)
{
	...
	if($companyId!="" )
		$SQL .= "and intFactoryId='$companyId' ";
		$SQL .= "order by strDescription;";	
}
else
{
		$SQL = "select intCostCenterId,strDescription from costcenters where intStatus=1 and intFactoryId='$session_CompanyId' ";
		$SQL .= "order by strDescription;";	
}
	$result = $db->RunQuery($SQL);		
	while($row = mysql_fetch_array($result))
	...
	?>
        ...
        <tr>
          <td colspan="2" class="tableBorder"><div id="divData" style="width:100%; height:550px; overflow: scroll;">
            <table width="100%" border="0" cellpadding="2" cellspacing="1"  id="tblPreOders" bgcolor="#ECD9FF">
...
#=================================================================================================
# Commente    - 11/04/2013
# Description - Remove supplier join link in the query
#=================================================================================================			  
...
$sql = " SELECT concat(PH.intPRYear,'/',PH.intPRNo)as Id,strPRNo, strJobNo,PH.intStatus, (select Name from useraccounts UA where UA.intUserID=PH.intRequestBy)as requestBy , DATE_FORMAT(PH.dtmRequestDate,'%d-%M-%Y ')as requestDate, (select Name from useraccounts UA where UA.intUserID=PH.intFirstApprovedBy)as firstApprovedBy , DATE_FORMAT(PH.intFirstApprovedDate,'%d-%M-%Y ')as firstApprovedDate, (select Name from useraccounts UA where UA.intUserID=PH.intSecondApprovedBy)as secondApprovedBy , DATE_FORMAT(PH.intSecondApprovedDate,'%d-%M-%Y ')as secondApprovedDate, PH.strCommonSupplierName 
FROM purchaserequisition_header PH  where PH.intStatus=$status and PH.intCompanyId = 1 ";
			
			
			if ($costCenter != "" and isset($costCenter) )
			{
				$sql .= " and PH.intCostCenterId = $costCenter";
			}
			if ($companyId != "")
			{
				$sql .= " and PH.intCompanyId = $companyId";
			}
			...
			if(!$PP_ViewAllFactoryDetailsInPRListing)
			{
				$sql .= " and PH.intCompanyId = $session_CompanyId";
			}
			...
			$serialNoArray	= explode('/',$row["Id"]);
				...
				$strSupTitle	= ($row["strCommonSupplierName"] == ''?$row["strTitle"]:$row["strCommonSupplierName"]);
			...
			?>
			  ...
			  if ($pos % 2 == 0)
			   ...
			   ?>">
                ...
                ?>
                <td class="normalfnt"><?php echo  $strSupTitle; ?></td>
...
{			
	...
	$folder = "../upload files/purchase requestion/". $serialNoArray[0].'-'.$serialNoArray[1]."/jobCard";
...
{			
	...
	/*if($boo)
		...
		echo "<td class=\"normalfntMid\">-</td>";*/
	$folder = "../upload files/purchase requestion/". $serialNoArray[0].'-'.$serialNoArray[1]."/Quatation";
		if(count(glob("$folder/*")) === 0)
			echo  "<td class=\"normalfntMid\">-</td>";
		else
			echo "<td class=\"normalfntMid\"><a href=\"$url$file1\" target=\"_blank\" ><img src=\"../images/pdf.png\" alt=\"pdf\" border=\"0\"/></a></td>";
...
{			
	...
	$folder = "../upload files/purchase requestion/". $serialNoArray[0].'-'.$serialNoArray[1]."/BOQ";
			if(count(glob("$folder/*")) === 0)
				echo  "<td class=\"normalfntMid\">-</td>";
			else
				echo "<td class=\"normalfntMid\"><a href=\"$url$file1\" target=\"_blank\" ><img src=\"../images/pdf.png\" alt=\"pdf\" border=\"0\"/></a></td>";

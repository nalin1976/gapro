...
$prNo			= $_GET["PRNo"];
$supplierId		= -1;//$_GET["SupplierId"];
$currencyId		= 0;// $_GET["CurrencyId"];
$currRate		= 0;//$_GET["CurrRate"];
$deptId			= $_GET["DeptId"];
...
$commonSupName  = $_GET["commonSupName"];

$booCheck = IsHeaderAvailable($serialNo,$serialYear);

if((bool)$booCheck){
	
	$v=InsertPRHeader($serialNo,$serialYear,$prNo,$supplierId,$currencyId,$currRate,$deptId,$companyId,$userId,$attension,$intro,$jobNo,$discount,$totalValue,$totalPRValue,$costCenterId,$discountValue,$jobType,$commonSupName);
	
}
	
else
	UpdatePRHeader($serialNo,$serialYear,$prNo,$supplierId,$currencyId,$currRate,$deptId,$companyId,$userId,$attension,$intro,$jobNo,$discount,$totalValue,$totalPRValue,$costCenterId,$discountValue,$jobType,$commonSupName);
...
$remarks		= $_GET["Remarks"];
	...
	if($booCheck)
		InsertPRDetail($serialNo,$serialYear,$matDetailId,$unit,$unitPrice,$qty,$value,$discount,$disValue,$finalValue,$assest,$glAllowId,$remarks);
	else
		UpdatePRDetail($serialNo,$serialYear,$matDetailId,$unit,$unitPrice,$qty,$value,$discount,$disValue,$finalValue,$assest,$glAllowId,$remarks);
...
$costCenterId	= $_GET["CostCenterId"];
		InsertGLDetail($serialNo,$serialYear,$glAlloId,$year,$month,$currBuAmount,$companyId,$userId,$mainCatId,$currentBudget,$costCenterId);
}
...
else if($requestType=="URLSaveAndConfirmSecondApproval_header")
{
$serialNo		= $_GET["SerialNo"];
$serialYear		= $_GET["SerialYear"];
...
$changeReason	= $_GET["ChangeReason"];
	...
	SaveAndConfirmSecondApproval_header($serialNo,$serialYear,$supplierId,$currencyId,$currRate,$deptId,$companyId,$userId,$attension,$intro,$jobNo,$discount,$totalValue,$totalPRValue,$discountValue,$jobType,$changeReason);
	MoveItemToActualTable($serialYear,$serialNo);
	$sql="update purchaserequisition_header set intStatus=3 ,intSecondApprovedBy='$userId' ,intSecondApprovedDate=now(),intApprovalNo=intApprovalNo+1 where intPRNo=$serialNo and intPRYear=$serialYear";
	$result=$db->RunQuery($sql);
}

else if($requestType=="URLgetCommonSupplierId")
{
	$supplierId = $common_Query->Function_CommonQuery("intCommonGeneralSupplierId");
	echo $supplierId;
}
...
function InsertPRHeader($serialNo,$serialYear,$prNo,$supplierId,$currencyId,$currRate,$deptId,$companyId,$userId,$attension,$intro,$jobNo,$discount,$totalValue,$totalPRValue,$costCenterId,$discountValue,$jobType,$commonSupName)
{
global $db;
	$sql="insert into purchaserequisition_header (intPRNo,intPRYear,strPRNo,intSupplierId,intCurrencyId,dblCurrencyRate,intDepartmentId,intCompanyId,intRequestBy,dtmRequestDate,strAttension,strRemarks,intStatus,strJobNo,dblDiscount,dblTotalValue,dblTotalPRValue,intCreatedBy,dtmCreatedDate,intCostCenterId,dblDiscountValue,intJobType,strCommonSupplierName)values('$serialNo','$serialYear','$prNo','$supplierId','$currencyId','$currRate',$deptId,'$companyId','$userId',now(),'$attension','$intro','0','$jobNo',$discount,$totalValue,$totalPRValue,$userId,now(),$costCenterId,'$discountValue','$jobType','$commonSupName');";
	//return $sql;
	$db->executeQuery($sql);
}

function UpdatePRHeader($serialNo,$serialYear,$prNo,$supplierId,$currencyId,$currRate,$deptId,$companyId,$userId,$attension,$intro,$jobNo,$discount,$totalValue,$totalPRValue,$costCenterId,$discountValue,$jobType,$commonSupName)
{
global $db;
	$sql="update purchaserequisition_header set strPRNo = '$prNo',intSupplierId = '$supplierId',intCurrencyId = '$currencyId',dblCurrencyRate = '$currRate',intDepartmentId = '$deptId',intCompanyId = '$companyId',intRequestBy = '$userId',dtmRequestDate = now(),strAttension = '$attension',strRemarks = '$intro',strJobNo = '$jobNo',dblDiscount =$discount ,dblTotalValue=$totalValue , dblTotalPRValue=$totalPRValue ,intCostCenterId='$costCenterId', dblDiscountValue='$discountValue', intJobType='$jobType', strCommonSupplierName='$commonSupName' where intPRNo = '$serialNo' and intPRYear = '$serialYear';";
	$db->executeQuery($sql);
}

function InsertPRDetail($serialNo,$serialYear,$matDetailId,$unit,$unitPrice,$qty,$value,$discount,$disValue,$finalValue,$assest,$glAllowId,$remarks)
{
global $db;
	$sql="insert into purchaserequisition_details (intPRNo,intPRYear,intMatDetailId,strUnit,dblUnitPrice,dblQty,dblBalanceQty,dblValue,dblDiscount,dblDiscountValue,dblFinalValue,intAssest,intGLAllowId,strRemarks) values ('$serialNo','$serialYear','$matDetailId','$unit','$unitPrice','$qty','$qty','$value','$discount','$disValue',$finalValue,'$assest','$glAllowId','$remarks');";
	$db->executeQuery($sql);
}

function UpdatePRDetail($serialNo,$serialYear,$matDetailId,$unit,$unitPrice,$qty,$value,$discount,$disValue,$finalValue,$assest,$glAllowId,$remarks)
{
global $db;
	$sql="update purchaserequisition_details set strUnit='$unit', dblUnitPrice='$unitPrice', dblQty='$qty', dblBalanceQty='$qty', dblValue='$value' ,dblDiscount='$discount' ,dblDiscountValue='$disValue' ,dblFinalValue='$finalValue' ,intAssest=$assest ,intGLAllowId='$glAllowId', strRemarks='$remarks' where intPRNo=$serialNo and intPRYear=$serialYear and intMatDetailId=$matDetailId;";
	$db->executeQuery($sql);
}

function InsertGLDetail($serialNo,$serialYear,$glAlloId,$year,$month,$currBuAmount,$companyId,$userId,$mainCatId,$currentBudget,$costCenterId)
{
global $db;
$sql="insert into budget_transaction_temp (intSerialYear,intDocumentNo,intDocumentYear,intCompanyId,intBudgetYear,intBudgetMonth,intGLNO,strType,dblQty,intUserId,dtmDate)values('$year','$serialNo','$serialYear','$costCenterId','$year','$month','$glAlloId','bgtPR','-$currBuAmount','$userId',now());";
$db->executeQuery($sql);
...
global $db;
$booAvailable = true;
	$sql="select GD.intGenPoNo 
	from generalpurchaseorderdetails GD
	inner join generalpurchaseorderheader GH on GH.intGenPONo=GD.intGenPONo and GH.intYear=GD.intYear
	where GD.intPRNo='$serialNo' and GD.intPRYear='$serialYear' and intStatus!=10;";
	$result=$db->RunQuery($sql);
	while($row=mysql_fetch_array($result))
	{
		$booAvailable = false;
...
function SaveAndConfirmSecondApproval_header($serialNo,$serialYear,$supplierId,$currencyId,$currRate,$deptId,$companyId,$userId,$attension,$intro,$jobNo,$discount,$totalValue,$totalPRValue,$discountValue,$jobType,$changeReason)
{
global $db;
	$sql="update purchaserequisition_header set intSupplierId = '$supplierId',intCurrencyId = '$currencyId',dblCurrencyRate = '$currRate', intChangeBy = '$userId',dtmChangeDate = now(),strAttension = '$attension',strRemarks = '$intro',strJobNo = '$jobNo',dblDiscount =$discount ,dblTotalValue=$totalValue , dblTotalPRValue=$totalPRValue, dblDiscountValue='$discountValue', intJobType='$jobType',strChangeReason='$changeReason' where intPRNo = '$serialNo' and intPRYear = '$serialYear';";
	$db->executeQuery($sql);
}

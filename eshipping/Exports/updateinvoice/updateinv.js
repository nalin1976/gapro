		...
		for(var r_loop=0; r_loop<xml_ino.length;r_loop++)
			{
				...
				rowCell.innerHTML 	="<input type=\"checkbox\" id=\"checkboxgrid\" name=\"checkboxgrid\" class=\"chkbxupdate\"/> ";	
...
function checkurl()
{
	...
	var req='updateinvdb.php?request=load_pl_grid&des='+des+'&datefrom='+datefrom+'&dateto='+dateto ;
	load_grid(req);	
			...
			if(row_source[loopz].cells[0].childNodes[0].checked==true)
				{
					...
					var url				='updateinvdb.php?request=update_final_inv&INO='+INO+'&Document='+Document+'&Payments='+Payments;
					var xml_http_obj	=$.ajax({url:url,async:false});						
				}
		}
		
	alert("Updated successfully.")					
	$(".chkbxupdate").removeAttr("checked");						
}
...
function saveData()
{
	...
	{			
		...
		var url				='updateinvdb.php?request=save_header&title='+title+'&date='+date;
		var xml_http_obj	=$.ajax({url:url,async:false});	
		view_detail()
	...
	else
	{
		...
		var url				='updateinvdb.php?request=del_details&titleid='+titleid;
		var xml_http_obj	=$.ajax({url:url,async:false});	
	...
	for(var loopz=1;loopz<=row_source.length-1;loopz++)
	{
		...
		var INO	=row_source[loopz].cells[1].childNodes[0].nodeValue;
		var url='updateinvdb.php?request=save_detail&xml_id='+titleid+'&INO='+INO;
		var xml_http_obj	=$.ajax({url:url,async:false});
...
function getData()
{
	...
	del_Maintbl_pop();
	var url				='updateinvdb.php?request=getData&searchid='+searchid ; 
	var xml_http_obj	=$.ajax({url:url,async:false});
	var xml_ino				=xml_http_obj.responseXML.getElementsByTagName('INVNo');
	var xml_isd				=xml_http_obj.responseXML.getElementsByTagName('ISD');
	var xml_date			=xml_http_obj.responseXML.getElementsByTagName('DATE');
...
function showDate()
{
	if(document.getElementById('chDateSelect').checked==false)
	{
		$("#txtInvoiceDate2").val('');
		$("#txtInvoiceDate3").val('');		
	}	

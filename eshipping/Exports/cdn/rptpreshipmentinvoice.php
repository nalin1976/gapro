...
$sqlinvoiceheader="SELECT IH.strInvoiceNo, 
					...
					LEFT JOIN cdn_header ON cdn_header.strInvoiceNo = IH.strInvoiceNo
					WHERE 
					intCDNNo='$cdnNo'  GROUP BY cdn_header.strInvoiceNo";
	...
	$dataholder=mysql_fetch_array($idresult);
		if($idresult)
  		...
  		$str_packtype="SELECT
						...
						FROM
						invoicedetail
						INNER JOIN shipmentforecast_detail ON shipmentforecast_detail.strPoNo = invoicedetail.strBuyerPONo AND invoicedetail.strSC_No = invoicedetail.strStyleID
						where strInvoiceNo='$invoiceNo'
						GROUP BY strInvoiceNo
						order by intItemNo ";
  		$result_packtype=$db->RunQuery($str_packtype);
		$row_pack=mysql_fetch_array($result_packtype);	
		
				  $pack =$row_pack['strPackType'];
	...
	?>	
      ...
      <tr>
        ...
        <td><?php echo $bookinumber['strBookingNo'];?></td>
         ...
         <tr>
        <td style="font-weight:bold">STROKE NO</td>
        ...
        <td colspan="5" class="border-left-fntsize12"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="30%"><?php 
								...
								sum(invoicedetail.intNoOfCTns) as NoOfCTns 
								FROM
								invoicedetail
								...
								invoicedetail.strInvoiceNo";
									...
									$ctn_result=$db->RunQuery($noOfctn);
										$ctn=mysql_fetch_array($ctn_result);
										
										echo $ctn['NoOfCTns'];

 ?> <?php if($dataholder['intPackingMthd']=='1'){echo "Cartoons";} else if($dataholder['intPackingMthd']=='2') {echo "GOH";} ?></td>
      ...
      <tr>
        ...
        <td width="15%" class="border-bottom-fntsize12" style="text-align:center"><span class="normalfnth2B">QTY/<?php echo $row_summary["strUnitID"];?></span></td>
      ...
      <tr>
	   ...
	   <tr>
        ...
        <td nowrap="nowrap" colspan="2"><?php echo $totpk; 
										 if($pack==0)
										  {
											  echo " ";
										  } else
	...
	$goh=mysql_fetch_array($result_goh);	
	   ?>
        ...
        <td style="text-align:center; font-weight:bold">CTN</td>
        <td style="text-align:center; font-weight:bold">PCS</td>
        </tr>
         <?php 
			  		$invNum=$dataholder['strInvoiceNo'];
			 $qty="SELECT invoicedetail.strInvoiceNo,
				 sum(invoicedetail.dblQuantity) as quantity,
				 sum(invoicedetail.intNoOfCTns) as qtyOFcarton, 
				invoicedetail.dblAmount 
				FROM invoicedetail 
				WHERE invoicedetail.strInvoiceNo  = '$invNum'
			";
...
?>	
        ...
        <td colspan="2">: <?php echo $dataholder['dblGrossMass']." KGS";?></td>
        <td style="font-family:'Times New Roman', Times, serif; font-size:11px;" width="15%">APPROVED QTY :</td>
        <td style="text-align:center"><div align="center"><?php echo $qty11['qtyOFcarton']; ?></div></td>      
        <td style="text-align:center"><?php echo $qty11['quantity'];?></td>
        </tr>
        
               <?php 
			  		$invNum=$dataholder['strInvoiceNo'];
			 $final_qty="        SELECT cdn_detail.strInvoiceNo,
						 sum(cdn_detail.dblQuantity) as quantity,
						 sum(cdn_detail.intNoOfCTns) as qtyOFcarton, 
						cdn_detail.dblAmount 
						FROM cdn_detail 
						WHERE cdn_detail.strInvoiceNo = '$invNum'";
			

	$result_final_qty=$db->RunQuery($final_qty);
	 //echo $qty;
	$result_final=mysql_fetch_array($result_final_qty);
?>	  
        ...
        <td colspan="2">: <?php echo $dataholder['dblNetMass']." KGS";?></td>
        <td style="font-family:'Times New Roman', Times, serif; font-size:11px;">SHIPPED QTY :</td>
        <td style="text-align:center"><div align="center"><?php echo $result_final['qtyOFcarton']; ?></div></td>
        <td style="text-align:center"><?php echo $result_final['quantity'];?></td>
        </tr>
        <tr>
        <td style="font-family:'Times New Roman', Times, serif; font-size:11px">TOTAL CBM </td>
        <td colspan="2">: <?php echo $dataholder['intCBM'];?></td>
        <td style="font-family:'Times New Roman', Times, serif; font-size:11px;">S/SHPD QTY :</td>
        <td style="text-align:center"><div align="center"><?php echo ($qty11['qtyOFcarton']- $result_final['qtyOFcarton']);?></div></td>
        <td style="text-align:center"><?php echo  ($qty11['quantity']- $result_final['quantity']);?></td>
        </tr>

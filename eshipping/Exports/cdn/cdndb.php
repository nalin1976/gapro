...
{	
	...
	$invDetail = "select invoiceheader.strInvoiceNo
				  ...
				  where cdn_header.strInvoiceNo is null
...
if ($request=='saveData')
{
	...
	if ($umoqty3=="")
	 ...
	 $sql_deleteHeader = "delete from cdn_header where intCDNNo ='$cdn '";
	 $db->executeQuery($sql_deleteHeader);
...
if($request=='changestatus')
{
	 $invNo= $_GET['invNo'];
		...
		$sql="UPDATE cdn_header
				SET intCDNConform=0
				WHERE intCDNNo='$invNo'";
	$result=$db->RunQuery($sql);
...
if($request=="chkqty")
{
 ...
 {                          
	...
	while($row=mysql_fetch_array($result))
	{
		$html.="<option value=".$row['strInvoiceNo'].">".$row['strInvoiceNo']." </option>";
	}

...
$masInvId=$_GET['masInvNo'];

$select_inv_no = "SELECT strMasterInvNo FROM masterinvoice_header WHERE intMasterInvId=$masInvId";
$select_res_no = $db->RunQuery($select_inv_no);

$row_inv = mysql_fetch_array($select_res_no);

$masInvNo = $row_inv['strMasterInvNo'];
...
";
  ...
  <tr>
    ...
    <td colspan="8" class="" style="text-align:left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  $select_inv_no_mas = "SELECT strInvoiceNo FROM masterinvoice_detail WHERE intMasterInvId = $masInvId";
	  $res_inv_no_mas = $db->RunQuery($select_inv_no_mas);
	  
	  while($row_get = mysql_fetch_array($res_inv_no_mas))
	  {
		  $get_inv_no = $row_get['strInvoiceNo'];
	  $sql_inv_det="SELECT
...
WHERE commercial_invoice_detail.strInvoiceNo='$get_inv_no'

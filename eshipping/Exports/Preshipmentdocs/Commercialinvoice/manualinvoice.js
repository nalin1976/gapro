...
function addToGrid()
{
	//var NewHs		=empty_handle_str(document.getElementById('txtHsCode').value.trim());
	//var inv_no	=document.getElementById("txtInvoiceDetail").value;
	//alert(NewHs);
	
	//alert(invoice_no);			

	
		 //var url		    = "commercialinvoiceDB.php?REQUEST=addHs&NewHs="+NewHs+ '&inv_no='+inv_no;
...
{
	var InvoiceNo		=document.getElementById("txtInvoiceNo").value;
	if(document.getElementById('txtHsCode').value!='')
		{
			var NewHs			=document.getElementById('txtHsCode').value;
		}
	else
		{
			var NewHs			="0"	;
	...
	for (var loop=1;loop<tbl.rows.length;loop++)
		{
			...
			{	item_no++;
				...
				+URLEncode(qty_unit)+ '&value='+value+'&gross='+gross+ '&NewHs='+NewHs+'&net='+net+'&ctns='+ctns+ '&netnet='+netnet+'&pl='+pl+'&fabric='+URLEncode(fabric)+'&item_no='+item_no+'&specdetail='+URLEncode(specdetail)+'&consttype='+URLEncode(consttype)+'&retprice='+URLEncode(retprice)+'&cbm='+URLEncode(cbm)+'&entryno='+URLEncode(entryno)+'&color='+URLEncode(color);
				htmlobj			=$.ajax({url:url,async:false});	

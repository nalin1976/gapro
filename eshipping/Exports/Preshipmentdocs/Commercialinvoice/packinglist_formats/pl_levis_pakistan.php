...
$str_header		="select 	shipmentplheader.strPLNo, 
							...
							strItemNo, 
							commercial_invoice_detail.strSpecDesc as strItem, 
							strManufactOrderNo, 
							strManufactStyle, 
							strDO, 
							strSortingType, 
	...
	$col_atrray[$row_cell_data['intColumnNo']]= $row_cell_data['dblPcs'];?>
      ...
      <tr  class="tbl-h-fnt" style="text-align:left">
		...
		$result_dyn=$db->RunQuery($str_dyn);
		  while($row_dyn=mysql_fetch_array($result_dyn)){
			    ?>
        <td  class="border-top-right-fntsize9 tbl-h-fnt" nowrap="nowrap"><?php  $tot_pcs=size_wise_total($row_dyn['intColumnNo'],$row_len['strLength']);echo ($tot_pcs==0?"":$tot_pcs); $length_tot+=$tot_pcs;?></td>
        <?php }?>

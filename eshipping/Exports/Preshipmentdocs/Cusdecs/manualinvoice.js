...
{			//alert(pos);
	...
	+ '&umoUnit1='+umoUnit1+ '&umoUnit2='+umoUnit2+ '&umoUnit3='+umoUnit3+ '&ISDNo='+ISDNo,true);
	xmlHttp[5].send(null);
...
if(document.getElementById('txtInvoiceNo').value!="")
		{
		window.open("rptpreshipmentinvoice.php?InvoiceNo=" + document.getElementById("txtInvoiceNo").value);
/*
...
function calUOM()
{
	...
	{
		document.getElementById('txtUmoQty3').value=parseFloat(document.getElementById('txtQty').value)/12;
...
function save_to_db_ci()
{
	...
	{	
		...
		xmlHttp[71].open("GET",'invoiceDbDetail.php?REQUEST=save_po_wise_ci&InvoiceNo=' + URLEncode(InvoiceNo) + '&hs=' +hs+ '&desc=' +desc+ '&catno=' +catno+ '&isd=' +isd+ '&po=' +po+ '&qty=' +qty+ '&unt=' +unt+ '&price=' +price + '&currency=' +currency+ '&amount=' +amount  ,true);
...
{	showBackGroundBalck();
	...
	xmlHttp[69].open("GET",'invoiceDbDetail.php?REQUEST=delete_po_wise_ci&InvoiceNo=' + URLEncode(InvoiceNo)  ,true);
...
{	
	...
	xmlHttp[70].onreadystatechange=function()
		...
		xmlHttp[70].open("GET",'invoiceDbDetail.php?REQUEST=retrv_po_wise_ci&InvoiceNo=' + URLEncode(InvoiceNo)  ,true);

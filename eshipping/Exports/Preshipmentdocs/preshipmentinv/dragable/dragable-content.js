	...
	************************************************************************************************************/	
	...
	if(!window.rememberPosition_cookieName)var rememberPosition_cookieName = 'certificateOforigin';
	...
	*/	
	...
	function createCookieString()
	{
		...
		Set_Cookie(rememberPosition_cookieName,stringToSave,600000000000000000);
	}
	...
	{
		  //document.cookie = "v0=1;expires=" + 'certificateOforigin' + ";" + ";";
		  //alert('done');
		  var stringToSave = '';
		for(var prop in dragableContent_cookieItems){
			if(stringToSave)stringToSave = stringToSave + '###';
		 ...
		 del_Cookie(rememberPosition_cookieName,stringToSave,600000000000000000);
		
	}
	
	function del_Cookie(name,value,expires,path,domain,secure) { 
		expires = expires * 60*60*24*1000;
		var today = new Date();
		var expires_date = new Date( today.getTime() + (expires) );
	    var cookieString = name + "=" +escape(value) + 

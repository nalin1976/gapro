...
$(document).ready(function() 
{
		...
		var url12				='exportregistrydb.php?request=loadlotNo';
		var pub_xml_http_obj12	=$.ajax({url:url12,async:false});
		var pub_in_arr			=pub_xml_http_obj12.responseText.split("|");
		
		$( "#txtlotNo" ).autocomplete({
			source: pub_in_arr
		});
//		
...
var filter=0;
	...
	var url				  ='exportregistrydb.php?request=load_er_grid&dateFrom='+dateFrom+
							'&dateTo='+dateTo+'&Reg_type='+Reg_type+'&filter='+filter+'&invoice_No='+invoice_No+'&Location='+Location;
	var xml_http_obj	  =$.ajax({url:url,async:false});
	
	var xml_InvoiceNo	  =xml_http_obj.responseXML.getElementsByTagName('InvoiceNo');
	var xml_inv_status    =xml_http_obj.responseXML.getElementsByTagName('status');
	...
	var xml_LotNo		  		  =xml_http_obj.responseXML.getElementsByTagName('LotNo');
	...
	{
		//alert (xml_http_obj.responseXML.getElementsByTagName('status'));
		//alert(xml_inv_status[r_loop].childNodes[0].nodeValue);
...
//		}
		...
		if(xml_inv_status[r_loop].childNodes[0].nodeValue=='Confirm')
		{
			var cls_status="inv-state-ok"
			//var inv_status="&times;"
			var inv_status="Confirm"
		}
		else if(xml_inv_status[r_loop].childNodes[0].nodeValue=='Cancel')
		{
			var cls_status="inv-state-cancel"
			//var inv_status="&radic;"
			var inv_status="Cancel"
		}
	else
		{
			var cls_status="inv-state-not-ok"
		...
		//rowCell.innerHTML 	= xml_inv_status[r_loop].childNodes[0].nodeValue;
		...
		//rowCell.innerHTML 	="<input name=\"txtFileNo\" type=\"text\" maxlength=\"100\" id=\"txtFileNo\" size=\"25\" />";



		var rowCell 	  	= row.insertCell(12);
		rowCell.className 	= "normalfntMid";
		rowCell.innerHTML 	="<input name=\"txtLotNo\" type=\"text\" maxlength=\"100\" id=\"txtFileNo\" size=\"25\" value=\""+xml_LotNo[r_loop].childNodes[0].nodeValue+"\"/>";
		//rowCell.innerHTML 	="<input name=\"txtLotNo\" type=\"text\" maxlength=\"100\" id=\"txtLotNo\" size=\"25\" />";
...
function saveDetail()
{
var File_No=document.getElementById('txtFileNo').value;
var tbl=document.getElementById('tblER');

for(var x=2; x<tbl.rows.length; x++)
{
	var invNo=tbl.rows[x].cells[0].childNodes[0].nodeValue;
	var File_No=tbl.rows[x].cells[11].childNodes[0].value;
	var lotNo=tbl.rows[x].cells[12].childNodes[0].value;
	var PoNo=tbl.rows[x].cells[13].childNodes[0].nodeValue;
//		var InvoiceNo=xml_InvoiceNo[x].childNodes[0].nodeValue;
	//alert(invNo);
//	alert(File_No);
//	alert(lotNo);
//	alert(PoNo);

	var url		='exportregistrydb.php?request=savefileno&File_No='+File_No+'&invNo='+invNo+'&lotNo='+lotNo+'&PoNo='+PoNo;
	var htmlobj=$.ajax({url:url,async:false});
}
...
{
	var cusId=document.getElementById('cboLocation').value;
	if(cusId!='0')
		{
			//window.open("ExCusSum.php?",'abc');
			window.open("ExCusSum.php?cusId=" + '&cusId='+cusId,'abc');
		}
	else
		{
...
function loadLocation()
{
	...
	var cusId=document.getElementById('cboLocation').value;
		load_pl_grid(cusId,5);
		//reportDetail(cusId);
}



function loadLotNo(obj,evt)
{
		document.getElementById('txtbuyer').value ='';
		document.getElementById('txtCDNBuyerPO').value ='';
		document.getElementById('txtCDNInvNo').value ='';
	...
	if (evt.keyCode == 13)
	{
		...
		var url     = "exportregistrydb.php?request=loadlotNoToGrid&lotNo="+obj.value;
		var htmlobj = $.ajax({url:url,async:false});
		var LotNo=htmlobj.responseText;
			//alert(LotNo);
		load_pl_grid(LotNo,6);

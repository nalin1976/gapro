...
function getItemData(cbo)
{
	...
	xmlHttp[10].onreadystatechange=function()
	{
		...
		{
		var Item	= xmlHttp[10].responseXML.getElementsByTagName('ItemName');	
		var Unit	= xmlHttp[10].responseXML.getElementsByTagName('Unit');	
		//alert(xmlHttp[10].responseText);
		document.getElementById('txtRemarks').value= Item[0].childNodes[0].nodeValue;
		document.getElementById('cboUnit').value= Unit[0].childNodes[0].nodeValue;
		
		}
		
	}
	xmlHttp[10].open("GET",'commodityCodesDB.php?REQUEST=getItemData&commoditycode=' + cbo,true);
...
function setTax()
{
	...
	if (document.getElementById('SRL').checked==true && document.getElementById('SRL').disabled==false )
	{
			tax="SRL";
			setTaxToGrid(tax);
	}
	...
	if (document.getElementById('XID').checked==true && document.getElementById('XID').disabled==false )
	{
			tax="XID";
			setTaxToGrid(tax);
	}
...
document.getElementById('SRL').disabled=false;
...
createNewXMLHttpRequest(1);
	...
	xmlHttp[1].open("GET",'commodityCodesDB.php?REQUEST=checkdb&commoditycode=' + commoditycode+ '&tax=' + taxcode,true);
	xmlHttp[1].send(null);
...
createNewXMLHttpRequest(3);
	...
	xmlHttp[3].open("GET",'commodityCodesDB.php?REQUEST=update&commoditycode=' + commoditycode+ '&tax=' + taxcode+ '&MP=' +MP+ '&rates=' +rates+ '&taxbase=' +taxbase+ '&position=' +changedposition+ '&txtRateOpt=' +txtRateOpt,true);
	xmlHttp[3].send(null);
...
createNewXMLHttpRequest(4);
	...
	xmlHttp[4].open("GET",'commodityCodesDB.php?REQUEST=position&commoditycode=' + code+ '&tax=' + tax+ '&position=' +pos,true);
	xmlHttp[4].send(null);
...
var Item=document.getElementById('txtRemarks').value;
var commoditycode=document.getElementById('txtCommodity').value;
var unit=document.getElementById('cboUnit').value;
	...
	xmlHttp[6].open("GET",'commodityCodesDB.php?REQUEST=editItem&commoditycode=' + commoditycode+ '&Item=' +Item+ '&unit=' +unit+ '&wut=' +wut,true);
	xmlHttp[6].send(null);

      ...
      <?php
		 ...
		 $sql = "SELECT
			...
			WHERE cdn_header.strInvoiceNo NOT IN 
					...
					GROUP BY commercial_invoice_header.strInvoiceNo ) AND intCancel=0
...
/*
	...
	?> 
      <tr bgcolor="#FFFFFF" class="normalfnt">
        ...
        <td class="normalfntRite"><?php echo $cdnyQuantity; ?></td>
        <td><?php echo $row_sub['strCurrency']; ?></td>
        <td style="text-align:right"><?php echo number_format($cdnAamount,2); ?></td>
        </tr>
        <?php
       				 $qty+=$cdnyQuantity; 
					 $grossamt+=$cdnAamount;

...
<?php
		...
		$sql_receipt = "SELECT
						...
						LEFT JOIN receipt_detail ON receipt_header.intSerialNo = receipt_detail.intSerialNo
						LEFT JOIN commercial_invoice_detail ON receipt_detail.strInvoiceNo = commercial_invoice_detail.strInvoiceNo
						LEFT JOIN commercial_invoice_header ON receipt_detail.strInvoiceNo = commercial_invoice_header.strInvoiceNo
						...
						WHERE receipt_header.intSerialNo != '' ";
	...
	while($row_rec=mysql_fetch_array($result_rec))
	{
		 $receiptNo=$row_rec["receiptNo"];
		  $status=$row_rec["intCancelStatus"];

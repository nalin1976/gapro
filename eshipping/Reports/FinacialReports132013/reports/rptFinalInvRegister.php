		...
		$sql.="  GROUP BY CIH.strInvoiceNo ORDER BY CIH.strInvoiceNo, strBuyerPONo ";
			...
			$row_final = mysql_fetch_array($result_final);
				 ...
				 $sql_dis="SELECT
						...
						WHERE
						discount_detail.strInvoiceNo='$inv'
						AND discount_header.intCancelStatus = 0 ";
			...
			$row_dis = mysql_fetch_array($result_dis);
				 ...
				 $sql_rec="SELECT
						...
						WHERE
						receipt_header.intCancelStatus = 0
						AND receipt_detail.strInvoiceNo='$inv'";

			$result_rec = $db->RunQuery($sql_rec);
	...
	?> 
      <tr bgcolor="#FFFFFF" class="normalfnt">
        ...
        <td class="normalfntRite"><?php 
		 ...
		 ?></td>
        <td class="normalfntRite"><?php echo $row_rec['dtmInvoiceDate']; ?>
										
											
        </td>
        <td class="normalfntRite"><?php
		if($row_dis["strRefNo"]!=''){
				echo "DIS".$row_dis["strRefNo"];}

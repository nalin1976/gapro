...
<?php
	  	$sql = "SELECT
					...
					LEFT JOIN invoicedetail AS CID ON CIH.strInvoiceNo = CID.strInvoiceNo
...
while($row = mysql_fetch_array($result))
{
	...
	$invoiceNo=$row["strInvoiceNo"];
	 ...
	 $sql_cdn="SELECT
				SUM(cdn_detail.dblQuantity) AS cdnQuantity,
	...
	$row_cdn = mysql_fetch_array($result_cdn);
	  ...
	  $sql_Finv="SELECT
					...
					Sum(commercial_invoice_detail.dblQuantity) AS finvQuantity,
					Sum(commercial_invoice_detail.dblAmount) AS finvAmount,
...
?>		 
    <tr bgcolor="#FFFFFF" class="normalfnt">
        ...
        <td class="normalfntRite"><?php echo number_format($row_cdn["cdnQuantity"]);?></td>
        <td class="normalfntRite"><?php echo number_format($row_cdn["cdnAmount"],2);?></td>
         <td class="normalfntRite"><?php echo number_format($row_Finv["finvQuantity"]);?></td>
...
?>
	    <tr bgcolor="#FFFFFF" class="normalfnt">
    	<td height="20" colspan="7"><strong>Total</strong></td>
        ...
        <td nowrap class="normalfntRite"><b><?php echo number_format($totGrossAmount,2);?></b></td>
         <td nowrap class="normalfntRite"><b><?php echo number_format($totCDNquantity);?></b></td>
          <td nowrap class="normalfntRite"><b><?php echo number_format($totCDNAmt,2);?></b></td>
         <td nowrap class="normalfntRite"><b><?php echo number_format($totFinvQty);?></b></td>
          <td nowrap class="normalfntRite"><b><?php echo number_format($totFinvAmt,2);?></b></td>
          <td>&nbsp;</td>
        </tr>
<?php
$totQuantity 	= 0;

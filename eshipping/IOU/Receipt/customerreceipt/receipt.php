...
<?php 
	...
	$str_header="select 	(select strName from customers cst where cst.strCustomerID=crh.strCustomerId)as customer, 
	crh.strRcptSerialNo,crh.strCustomerId, crh.strChequerefNo, 	crh.dtmReceiptDate, crh.intBankId,
	crh.strType, crh.dblAmount,	crh.dblBalance, 
	crh.strCreditNote from  customerreceiptheader crh where crh.strRcptSerialNo='$serial_no'";
	...
	$row_header=mysql_fetch_array($result_header);
			...
			$date=$row_header["dtmReceiptDate"];
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="normalfnBLD1">
      ...
      <tr>
		...
		<?php $amount=explode(".",round($Amount,2));
		 $amount1=$amount[0];
		  $amount2=($amount[1]!=""? " And ".convert_number($amount[1])." Cents" : "");
      ...
      <tr>
		...
		$str_detail=" select crd.intInvoiceNo from customerreceiptdetail crd where crd.strRcptSerialNo='$serial_no'";
		$result_detail=$db->RunQuery($str_detail);
		while($row_detail=mysql_fetch_array($result_detail)){echo $row_detail["intInvoiceNo"].", ";}

...
<table width="700" border="0" align="center" bgcolor="#FFFFFF" cellspacing="0">
	<tr>
  	...
  	<tr>
    	<td>
			<table align="center" width="494" style="margin-top:15px;">
				<tr>
					<td width="119" class="normalfnt">Serial No</td>
					<td colspan="3">
						<select name="cboSerialNo" style="width:330px" id="cboSerialNo" class="txtbox" onchange="loadHeaderDetails();">
							...
							<?php
								$sql="SELECT
										...
										FROM
										bankletter_header
										INNER JOIN buyers ON bankletter_header.strBuyerCode = buyers.strBuyerID
										;";
								$result=$db->RunQuery($sql);
								while($row=mysql_fetch_array($result))
								{ 
							?>
								<option value="<?php echo $row['intSerialNo']; ?>"><?php echo $row['intSerialNo']; ?>--><?php echo $row['strName']; ?></option>
								<?php
								}
								?>
						</select>					 </td>
				...
				<tr>
					<td width="119" class="normalfnt">Buyer</td>
					<td colspan="3">
						<select name="cboBuyer" style="width:330px" id="cboBuyer" class="txtbox" onchange="clearGrid();">
							<option value="0"></option>
							<?php
							$sql="SELECT DISTINCT
				...
				<tr>
				  <td class="normalfnt">Search Date Range </td>
				...
				<tr>
			  	  ...
			  	  <td width="125"><input name="txtDateFrom" tabindex="2" type="text" class="txtbox" id="txtDateFrom" size="15" onmousedown="DisableRightClickEvent();" onmouseout="EnableRightClickEvent();" onkeypress="return ControlableKeyAccess(event);" value="<?php echo date('d/m/Y');?>" onclick="return showCalendar(this.id, '%d/%m/%Y');" disabled="disabled" /><input name="reset" type="reset"  class="txtbox" style="visibility:hidden;"   onclick="return showCalendar(this.id, '%d/%m/%Y');" value="" /></td>
				  
				  
					<td width="60" class="normalfnt">Date To</td>
	  	  	  	  <td width="170"><input name="txtDateTo" tabindex="2" type="text" class="txtbox" id="txtDateTo" size="15" onmousedown="DisableRightClickEvent();" onmouseout="EnableRightClickEvent();" onkeypress="return ControlableKeyAccess(event);" value="<?php echo date('d/m/Y');?>" onclick="return showCalendar(this.id, '%d/%m/%Y');" disabled="disabled"/><input name="reset" type="reset"  class="txtbox" style="visibility:hidden;"   onclick="return showCalendar(this.id, '%d/%m/%Y');" value="" /></td>
				</tr>
				<tr>
		  	...
		  	<div align="center">
				<div align="center" style="width:500px; height:200px; overflow:scroll; margin-top:25px; margin-bottom:25px;">
					<table align="center" width="480" id="tblBankLetter">
						<thead>
							<tr>
								<td>Select</td>
								<td>Invoice No</td>						

...
var menu_rowindex	=0;
var start_sizes		=9;
var color_combo		="<input type=\"text\" class=\"txtbox keymove\" style=\"width:110px;text-align:center;\"maxlength=\"30\"  />";
var prepack_combo	="<select class=\"txtbox keymove\" style=\"width:110px\">";
...
var ctn_combo	="<select class=\"txtbox keymove\" style=\"width:200px\">";
ctn_combo		+="</select>";
gen_ctns_combo();
...
$(document).ready(function() 
{
		...
		$('#btnConfirm').click(function()
		{
			...
			if(plno=="")
				return;
			var url				='shipmentpackinglistdb.php?request=confirmpl&plno='+plno;
			var xml_http_obj		=$.ajax({url:url,async:false});
			alert("Packing list # : "+ plno+" has confirmend successfully.")
			$('.td_confirm').css('display','none')	;
		});
		...
		{
			insert_row(menu_rowindex,'above');			
		});
		
		$('#insert_bellow').click(function()
		{
			insert_row(menu_rowindex+1,'below');			
		});
...
function save_pl_header()
{
	...
	var dc					=$('#txtDC').val();
	var marksNos			=$('#txtMarksNos').val();
	var shipTo				=$('#cboShipTo').val();
	var invNo				=$('#txtInvNo').val();
	var exWt				=document.getElementById('txtPolyWt').value;
	
	if(exWt=='')
		exWt=0.02;
	
	
	if(document.getElementById('mul_yes').checked)
		var mul_yes = 1;
	else
		var mul_yes = 0;
	
	
	var url					='shipmentpackinglistdb.php?request=save_pl_header&PLNo='+URLEncode(PLNo.trim())+'&SailingDate='+SailingDate+'&Style='+URLEncode(Style)+'&ProductCode='+URLEncode(ProductCode)+'&material='+URLEncode(material)+'&Fabric='+URLEncode(Fabric)+'&Lable='+URLEncode(Lable)+'&isdno='+URLEncode(isdno)+'&PrePackCode='+URLEncode(PrePackCode)+'&season='+URLEncode(season)+'&division='+URLEncode(division)+'&WashCode='+URLEncode(WashCode)+'&Article='+URLEncode(Article)+'&ItemNo='+URLEncode(ItemNo)+'&Item='+URLEncode(Item)+'&ManufactOrderNo='+URLEncode(ManufactOrderNo)+'&ManufactStyle='+URLEncode(ManufactStyle)+'&DO='+URLEncode(DO)+'&Factory='+Factory+'&SortingType='+URLEncode(SortingType)+'&Unit='+Unit+'&container='+container+'&trnsmode='+URLEncode(trnsmode)+'&destination='+destination+'&dc='+URLEncode(dc)+'&marksNos='+URLEncode(marksNos)+'&shipTo='+URLEncode(shipTo)+'&invNo='+URLEncode(invNo)+'&mulYes='+URLEncode(mul_yes)+'&exWt='+parseFloat(exWt);
	var xml_http_obj		=$.ajax({url:url,async:false});
...
//alert("test1");
	...
	for(var loop_row=0;loop_row<row.length;loop_row++)
	{
		...
		var shade				 ="";
		var prepack				 =URLEncode(row[loop_row].cells[5].childNodes[0].value);
		var pack				 =URLEncode(row[loop_row].cells[6].childNodes[0].value);
		var color				 =URLEncode(row[loop_row].cells[7].childNodes[0].value);
		var lengths				 =URLEncode(row[loop_row].cells[8].childNodes[0].value);
...
{	
	...
	var rowCell 		=document.createElement("th");
	rowCell.width	  	='10%';
	rowCell.innerHTML 	="PrePack#";
	row.appendChild(rowCell);
	
	var rowCell 		=document.createElement("th");
	rowCell.width	  	='10%';
	rowCell.innerHTML 	="Pack#";
	row.appendChild(rowCell);
...
function fill_pl_grid_data(obj)
{
	...
	{	
		...
		{	
			...
			case 4:		
			var rowCell = row.insertCell(loop);
			rowCell.innerHTML ="<input type=\"text\" class=\"txtbox keymove\" style=\"width:110px;text-align:center;\"maxlength=\"30\" onkeypress='setTagNo(this,event);'  />";	
			break;
			
			case 5:			
			var rowCell = row.insertCell(loop);
			rowCell.innerHTML ="<input type=\"text\" class=\"txtbox keymove\" style=\"width:100px;text-align:center;\"maxlength=\"30\"  />";	
			break;
	...
	({
		url: url,success: function(obj)
		{
			...
			{			
				...
				$('#txtMarksNos').val($(this).find("MarksNos").text());
				$('#cboShipTo').val($(this).find("ShipTo").text());
				$('#txtInvNo').val($(this).find("InvNo").text());
				$('#txtPolyWt').val($(this).find("ExWt").text());
				if($(this).find("MulYes").text()==1)
					document.getElementById('mul_yes').checked=true;
				if($(this).find("confirm").text()==1)
					{
						$('.td_confirm').css('display','none')	;
					}
				else
					{
						...
						$('.td_confirm').css('display','')	;
					}
					
				//load_ctn_value_arr(plno);
...
/*function load_ctn_value_arr(plno)
{
	var url				='shipmentpackinglistdb.php?request=load_ctn_value_arr&plno='+plno;
	var xml_http_obj	=$.ajax({url:url,async:false});
	...
	var xml_MaxPcs		=xml_http_obj.responseXML.getElementsByTagName('MaxPcs');
	
	for(var x=0; x<xml_Carton; x++)
		max_ctn[] = xml_Carton[0].childNodes[0].nodeValue;
...
function set_values_to_grid()
{
	...
	var xml_http_obj	=$.ajax({url:url,async:false});
		...
		var xml_PrePack			=xml_http_obj.responseXML.getElementsByTagName('prepack');
		var xml_Pack			=xml_http_obj.responseXML.getElementsByTagName('pack');
	...
	for(var r_loop=0; r_loop<xml_row_no.length;r_loop++)
	{
		...
		tr[row_index].cells[5].childNodes[0].value					=xml_PrePack[r_loop].childNodes[0].nodeValue;
		tr[row_index].cells[6].childNodes[0].value					=xml_Pack[r_loop].childNodes[0].nodeValue;
		tr[row_index].cells[7].childNodes[0].value					=xml_Color[r_loop].childNodes[0].nodeValue;
		tr[row_index].cells[8].childNodes[0].value					=xml_length[r_loop].childNodes[0].nodeValue;
...
function insert_row(no,val)
{
	...
	if(row_number>=0)
	{
		...
		if(val=='above')
		{
			$('#tblPL tbody tr')[lastRow].cells[5].childNodes[0].value = $('#tblPL tbody tr')[lastRow+1].cells[5].childNodes[0].value;
			$('#tblPL tbody tr')[lastRow].cells[3].childNodes[0].value = $('#tblPL tbody tr')[lastRow+1].cells[3].childNodes[0].value;
			$('#tblPL tbody tr')[lastRow].cells[6].childNodes[0].value = $('#tblPL tbody tr')[lastRow+1].cells[6].childNodes[0].value;
			$('#tblPL tbody tr')[lastRow].cells[7].childNodes[0].value = $('#tblPL tbody tr')[lastRow+1].cells[7].childNodes[0].value;
		}
		else
		{
			$('#tblPL tbody tr')[lastRow].cells[5].childNodes[0].value = $('#tblPL tbody tr')[lastRow-1].cells[5].childNodes[0].value;
...
function view_pl()
{
	...
	loadCombo('SELECT strPLNo, strPLNo as pldesc FROM shipmentplheader order by strPLNo desc','txtPLNo');	
...
function auto_cal_ctns()
{
	...
	{		
		...
		var ctnFrom=row[loop_row].cells[1].childNodes[0].value;
		var ctnTo  =row[loop_row].cells[2].childNodes[0].value;
		var noOfCtns=parseFloat(ctnTo)-parseFloat(ctnFrom)+1;
		var sizeQty=0;
		for(var loop_col=0;loop_col<pub_ratio_wt.length;loop_col++)
		{
			sizeQty+=parseFloat(row[loop_row].cells[loop_col+start_sizes].childNodes[0].value==""?0:row[loop_row].cells[loop_col+start_sizes].childNodes[0].value);
		}
		var totQty1=sizeQty*noOfCtns;
		row[loop_row].cells[start_auto_cell+2].childNodes[0].value=totQty1;
	}
}
...
function auto_cal_weight()
{
	...
	{		
		...
		for(var loop_col=0;loop_col<pub_ratio_wt.length;loop_col++)
		{
			...
			net_n_wt +=(pub_ratio_wt[loop_col]-parseFloat(exWt))*size_qty;
		...
		gross_wt+=(net_wt+parseFloat(pub_ctns_wt[ctn_wt]))
		//alert(pub_ctns_wt[ctn_wt]);
		var net_net_wt=(net_wt>0?(net_n_wt):0)
		//alert(net_net_wt);		
		row[loop_row].cells[start_auto_cell+9].childNodes[0].value=(net_net_wt*no_ctn).toFixed(3)
		row[loop_row].cells[start_auto_cell+8].childNodes[0].value=(net_wt*no_ctn).toFixed(3)
		row[loop_row].cells[start_auto_cell+7].childNodes[0].value=(gross_wt*no_ctn).toFixed(3)
		row[loop_row].cells[start_auto_cell+6].childNodes[0].value=net_net_wt.toFixed(3)
		row[loop_row].cells[start_auto_cell+5].childNodes[0].value=net_wt.toFixed(3)
		row[loop_row].cells[start_auto_cell+4].childNodes[0].value=gross_wt.toFixed(3)
	}
}


function gen_color_combo()
{
	
	var plno			=$('#txtPLNo').val();
	var style			=$('#cmbStyle').val();
	var url				='shipmentpackinglistdb.php?request=load_style_color&style='+style+'&plno='+plno;
	var xml_http_obj	=$.ajax({url:url,async:false});
	if(xml_http_obj.responseText!="")
	{
		color_combo			="<select class=\"txtbox keymove\" style=\"width:110px\">";	
		color_combo		   +=xml_http_obj.responseText;
		color_combo		   +="</select>";
	}
	else
	{
		color_combo		   ="<input type=\"text\" class=\"txtbox keymove\" style=\"width:110px;text-align:center;\"maxlength=\"30\"  />";
	}
...
{
	var style			=$('#cmbStyle').val();
	var url				='shipmentpackinglistdb.php?request=load_header&ID='+style;
	
	var xml_http_obj	=$.ajax({url:url,async:false});
	try{
	//alert(ID);	
		var xml_style			=xml_http_obj.responseXML.getElementsByTagName('strStyle_No')[0].childNodes[0].nodeValue;
		var xml_Material		=xml_http_obj.responseXML.getElementsByTagName('strMaterial')[0].childNodes[0].nodeValue;
		var xml_Fabric			=xml_http_obj.responseXML.getElementsByTagName('strFabric')[0].childNodes[0].nodeValue;
		var xml_Label    		=xml_http_obj.responseXML.getElementsByTagName('strLabel')[0].childNodes[0].nodeValue;
		var xml_Season			=xml_http_obj.responseXML.getElementsByTagName('strSeason')[0].childNodes[0].nodeValue;
		var xml_Division_Brand	=xml_http_obj.responseXML.getElementsByTagName('strDivision_Brand')[0].childNodes[0].nodeValue;
		var xml_strWash_Code	=xml_http_obj.responseXML.getElementsByTagName('strWash_Code')[0].childNodes[0].nodeValue;
		var xml_strItem_no		=xml_http_obj.responseXML.getElementsByTagName('strItem_no')[0].childNodes[0].nodeValue;
		var xml_strItem			=xml_http_obj.responseXML.getElementsByTagName('strItem')[0].childNodes[0].nodeValue;
		var xml_strSorting_Type	=xml_http_obj.responseXML.getElementsByTagName('strSorting_Type')[0].childNodes[0].nodeValue;
		
		document.getElementById('txtProductCode').value=xml_style;
		document.getElementById('txtMaterialNo').value=xml_Material;
		document.getElementById('txtFabric').value=xml_Fabric;
		document.getElementById('txtLable').value=xml_Label;
		document.getElementById('txtSeason').value=xml_Season;
		document.getElementById('txtDivision').value=xml_Division_Brand;
		document.getElementById('txtWashCode').value=xml_strWash_Code;
		document.getElementById('txtItemNo').value=xml_strItem_no;
		document.getElementById('txtItem').value=xml_strItem;
		document.getElementById('txtSortingType').value=xml_strSorting_Type;	
...
function gen_ctns_combo()
{
	...
	var url				='shipmentpackinglistdb.php?request=gen_ctns_combo';
	var xml_http_obj	=$.ajax({url:url,async:false});
	...
	ctn_combo			="<select class=\"txtbox keymove\" style=\"width:160px\">";	
...
function setTagNo(obj,evt)
{
	if (evt.keyCode == 13)
	{
		...
		{
			var val=obj.value;
			document.getElementById('tblPL').rows[rowNo].cells[4].childNodes[0].value=val;
			var ctnNumber=Number(document.getElementById('tblPL').rows[rowNo].cells[2].childNodes[0].value)-Number(document.getElementById('tblPL').rows[rowNo].cells[1].childNodes[0].value)+1;
			for(var i=rowNo+1; i<document.getElementById('tblPL').rows.length; i++)
			{
				//alert(ctnNumber);
				document.getElementById('tblPL').rows[i].cells[4].childNodes[0].value=Number(val)+ctnNumber;
				ctnNumber=Number(document.getElementById('tblPL').rows[i].cells[2].childNodes[0].value)-Number(document.getElementById('tblPL').rows[i].cells[1].childNodes[0].value)+1;
				val=document.getElementById('tblPL').rows[i].cells[4].childNodes[0].value;
			}
		}
	}
}
...
//				WHERE
//				commercial_invoice_detail.strPLno = '$plno'
//								";
//
//			$result_des = $db->RunQuery($sql_desc);
//			$row_des = mysql_fetch_array($result_des);
//			echo $row_des['strInvoiceNo'];
//			echo $row_des['strDescOfGoods'];
      ...
      <tr class="tbl-h-fnt">
		...
		/*echo $str_order="SELECT sum(OSD.dblPcs) AS dblPcs,intColumnNo,OSD.strSize,OSD.strColor FROM orderspecdetails OSD
					...
					WHERE SPH.strPLNo=$plno AND OSD.strColor='$color' AND intColumnNo =".$row_column['intColumnNo']." GROUP by intColumnNo";*/
							...
							WHERE SPSI.intColumnNo =".$row_column['intColumnNo']." AND SPSI.strPLNo='$plno' AND OSD.intOrderId=$intOrderId";
					
	if($color!="WHITE '1'")
	$str_order .= " and OSD.strColor ='$color' ";
					
	else
		$str_order .= " and OSD.strColor LIKE '%WHITE \'1\'%' ";
		$str_order.=" GROUP by intColumnNo";		

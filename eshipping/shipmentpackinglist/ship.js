...
var ctn_combo	="<select class=\"txtbox keymove\" style=\"width:110px\">";
ctn_combo		+="</select>";
gen_ctns_combo();
...
function cal_amounts(obj)
{
	...
	//alert(row.cells[start_auto_cell+1].childNodes[0].value);
	...
	{	
			...
			//alert(row.cells[loop].childNodes[0].value);
	}
...
function get_pl_header_data(plno)
{
...
function do_print()
{
	...
	if(document.getElementById('chkMY').checked==true)
		var myValue=1;
	else
		var myValue=0;
	if(plno=="")
	...
	url		=url_format+ "?plno="+plno+'&myValue='+myValue;
	window.open(url,'pl')
	var url				='shipmentpackinglistdb.php?request=save_pl_format&plno='+plno+'&pl_report='+url_format+'&myValue='+myValue;
	var xml_http_obj	=$.ajax({url:url,async:false});
}

function load_saved_pl_details(plno)
{
	//var plno			=$('#txtPLNo').val();
	...
	start_auto_cell	  =loop;
	//alert(start_auto_cell);
	load_saved_pl_data(plno);
}

function load_saved_pl_data(plno)
{
	//var plno			=$('#txtPLNo').val();
	pub_col=start_auto_cell+10;
	set_values_to_grid(plno)
}


function set_values_to_grid(plno)
{	
	set_value_subdetails(plno);
	hidePleaseWait();
}
 
function set_value_subdetails(plno)
{
	eventsetter();
	//hidePleaseWait();
	pl_fix_header()
...
function auto_cal_weight()
{
	...
	//alert(row[0].cells[loop_col+start_sizes].childNodes[0].value);
	for(var loop_row=0;loop_row<row.length;loop_row++)
	{		
		var net_wt	=0
		var gross_wt=0
		...
		//alert(row[loop_row].cells[3].childNodes[0].value);
...
function gen_ctns_combo()
{
	...
	ctn_combo			="<select class=\"txtbox keymove\" style=\"width:110px\">";	

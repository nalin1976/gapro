...
$sqlheaderfetch="	SELECT 	intIOUNo,
	(SELECT 	strName FROM customers WHERE strCustomerID=ih.strCompanyID)AS supliers,
	ih.strInvoiceNo, 
	(SELECT strName FROM buyers WHERE strBuyerID=ih.strBuyerID)AS customer,
	
	(SELECT strName FROM wharfclerks WHERE intWharfClerkID=ich.strWharfClerk)AS wharfclerk,
	intIOUPrint, 
	strReasonDupIOU, 
	strVessel AS vessel, 
	strType, 
	...
	intEDIIOU,
	strBL AS bl, 
	strPaymentTerms AS PaymentTerms, 
	strLCNumber AS LC	 
	FROM 
	invoiceheader ih LEFT JOIN tblexiouheader iou  ON iou.strInvoiceNo=ih.strInvoiceNo
	LEFT JOIN  exportcusdechead ich ON ich.strInvoiceNo=ih.strInvoiceNo
	WHERE 
	iou.intIOUNo='$iouno' ";
//die($sqlheaderfetch);
...
<DIV  style="z-index:25; position:absolute; left:8PX; top:8PX; width:72PX; height:15PX; " class="adornment12" >
<table width="67PX" border=0 cellpadding="0" cellspacing="0"><tr><td NOWRAP class="fontColor13" >Duplicate</td></tr></table>
</DIV>

<DIV  style="z-index:25; position:absolute; left:640PX; top:80PX; width:96PX; height:15PX; " class="adornment11" >
<table width="91PX" border=0 cellpadding="0" cellspacing="0"><tr><td NOWRAP class="fontColor14" >EXPORT</td></tr></table>
</DIV>
<?php
if($printStatus==0)
	$status	= "ORIGINAL";
  ...
  <table width="100%" border="0"  cellspacing="0" cellpadding="0">
    ...
    <tr>
      ...
      <td class="border-bottom-right"><div align="right" >&nbsp;</div></td>
		...
		$sql="SELECT (SELECT strDescription FROM expensestype WHERE tbl.intExpensesID=expensestype.intExpensesID)AS Expense , dblEstimate, dblActual, (dblActual-dblEstimate)AS short,dblInvoice FROM  tblexioudetail tbl WHERE intIOUNo='$iouno'";
		//die($sql);
...
<table width="83PX" border=0 cellpadding="0" cellspacing="0"><tr><td NOWRAP class="fontColor16" ><?php echo $content['container'];?></td></tr></table>
</DIV>

...
{
	$sql="SELECT strInvoiceNo FROM invoiceheader where strInvoiceNo in (select strInvoiceNo from cdn_header) and strInvoiceNo not in (select strPreInvoiceNo from commercial_invoice_header) order by intInvoiceId DESC";
	$result=$db->RunQuery($sql);

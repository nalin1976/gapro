...
<table width="794" border="0" align="center">
  ...
  <tr>
    <td colspan="3"><table width="100%" cellspacing="0">
      <tr>
        <td colspan="2" class="border-Left-Top-right-fntsize12"><strong> SHIPPER/ EXPORTER/ SELLER </strong></td>
        <td colspan="2" class="border-top-right-fntsize12"><strong> NO &amp; DATE OF INVOICE</strong></td>
        </tr>
      <tr>
        <td height="28" colspan="2" class="border-left-right-fntsize12" valign="bottom"><?php echo $dataholder['CustomerName'];?></td>
        <td colspan="2" class="border-bottom-right-fntsize12"><?php echo $dataholder['strInvoiceNo'];?> OF <?php echo $dateInvoice ;?></td>
        </tr>
      <tr>
      ...
      <tr>
        <td height="29" colspan="2" class="border-left-right-fntsize12" valign="bottom"><?php echo $dataholder['BuyerAName'];?></td>
        <td colspan="2" class="border-right-fntsize12" valign="bottom">
          <?php 
...
?>        </td>
  ...
  <tr>
    <td height="70" colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" id="tblInvoice"  ><thead>
  <tr>
    <td width="229" class="border-top-left-fntsize12"><strong>10. MARKS & NOS</strong></td>
    <td width="226"  class="border-top-left-fntsize12"><strong>11. DESCRIPTION</strong></td>
    <td width="115" class="border-top-left-fntsize12"><strong>12. QUANTITY</strong></td>
    <td width="117" class="border-top-left-fntsize12"><strong>13. UNIT PRICE</strong></td>
    <td width="101" class="border-Left-Top-right-fntsize12"><strong>14. AMOUNT</strong></td>
	</tr>	
	<tr>
  ...
  include "numbertotext.php";
		$sqlinvdtl="
SELECT 	
	bookingdetail.strInvoiceNo, (SELECT strMarksAndNos FROM bookingheader WHERE  bookingdetail.strInvoiceNo=bookingheader.strInvoiceNo)AS Marks,
	strStyleID, 
	intItemNo, 
	strBuyerPONo, 
	strDescOfGoods, 
	...
	FROM 
	bookingdetail  
	WHERE bookingdetail.strInvoiceNo='$invoiceNo'
	";  
...
' >" . $row['strDescOfGoods']. "</textarea></td>";
    ...
    echo "<td style='vertical-align:top;'class='normalfnt-rite_size12'>". $row['dblQuantity']." ".$row['strUnitID']."</td>";
    echo "<td style='vertical-align:top;' class='normalfnt-rite_size12'>".number_format($row['dblUnitPrice'],2)." ".$row['strPriceUnitID']."</td>";
    $totamt+=$row['dblAmount'];
    $totqty+=$row['dblQuantity'];
	$totcmp+=$row['dblQuantity']*$row['dblCMP'];
  ...
  <tr>
    <td colspan="2" class="normalfnt_size12">&nbsp;<?php if($totcmp>0) echo "ACC COST SUPPLIED ON N.F.E BASIS BY  BUYER";?></td>
    <td class="normalfnt-rite_size12" ><?php if($totcmp>0) echo number_format($totamt-$totcmp,2);?></td>
  </tr>
  <tr>
  	<td colspan="2" class="normalfnt_size12">&nbsp;<strong><?php if($totcmp==0) echo "BALANCE FOB " .$totuntprice *12 . "/ DOZEN US DOLLARS PAYBLE COLOMBO"; else echo "BALANCE CMP US DOLLARS PAYABLE COLOMBO";?></strong></td>	
    <td width="103" class="normalfnt-rite_size12" ><strong><?php if($totcmp==0)echo number_format($totamt,2);else echo number_format($totcmp,2); ?></strong></td>
  </tr>
  <tr><td colspan="2">&nbsp;</td>
  </tr>
  <tr ><td width="95" class="normalfnt_size12"><div align="left"></div></td>
    <td width="582" class="normalfnt_size12"><div align="left">COUNTRY OF ORIGIN : SRI LANKA <br/>
      ...
      TEXTILE QUOTA CATEGORY NO :<?php $sqlcate="select strCatNo from bookingdetail where strInvoiceNo='$invoiceNo' ";
	  $resultCat=$db->RunQuery($sqlcate);
	  while($row=mysql_fetch_array($resultCat)){echo $row["strCatNo"]."/";}
	  ?><br/>

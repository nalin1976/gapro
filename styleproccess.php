...
//echo "WAIT...<BR><BR>";
	...
	$booLockStatus = (bool)CheckLockStatus();
	//echo $booLockStatus;
	if($booLockStatus==0){
	...
	orders.intStyleId =  '$styleid_main'";
		...
		//SetLockStatus(0);
		echo "Process is running by a another user. ";
	...
	*/
	...
	while($row1=mysql_fetch_array($result1))
	{
		...
		$intstatus = $row1["intstatus"];
                ...
                /*
                 * Add raw material operation wastages to the specification table
                 * 
                 * Add On - 08/03/2017
                 * Add By - Nalin Jayakody
                 * =================================================================
                 */
                $booWashable = $row1["booWashable"];
                $dblPSP = $row1["dblPSP"];
                $dblDP = $row1["dblDP"];
                $dblEMB = $row1["dblEMB"];
                $dblPEOSP = $row1["dblPEOSP"];
                $dblDPE = $row1["dblDPE"];
                $dblGEW = $row1["dblGEW"];
                $dblGPD = $row1["dblGPD"];
                $dblSPE = $row1["dblSPE"];
                $dblPEODP = $row1["dblPEODP"];
                $dblGRW = $row1["dblGRW"];
                $intLgPanels =  $row1["intLgPanels"];
                $intEmbPanels =  $row1["intEmbPanels"];
                $intPrintPanels =  $row1["intPrnPanels"];
...
specificationdetails.strMatDetailID = '$intMatDetailID' "; 
		...
		`sngWastage`='$reaWastage', `dblReqQty`='$dblReqQty', `dblTotalQty`='$dblTotalQty', `dblTotalValue`='$dblTotalValue',
		`dblCostPC`='$dbltotalcostpc', `intOriginNo`='$intOriginNo' ,`intMillId` = '0' , `intMainFabricStatus` = '0' , `dblfreight` = '$dblFreight' , 
		`strPurchaseMode` = 'NONE' , `strPlacement` = '' , `intRatioType` = '0', `intStatus` = '$status' , `booWashable` = '$booWashable',
                `dblPSP` = '$dblPSP', `dblDP` = '$dblDP', `dblEMB` = '$dblEMB', `dblPEOSP` = '$dblPEOSP', `dblDPE` = '$dblDPE', `dblGEW` = '$dblGEW',
                `dblGPD` = '$dblGPD', `dblSPE` = '$dblSPE', `dblPEODP` = '$dblPEODP', `intEmbPanels` = '$intEmbPanels', `intPrnPanels` = '$intPrintPanels',
                `dblGRW` = '$dblGRW', `intLgPanels` = '$intLgPanels'   
		   WHERE (`intStyleId`='$styleID') AND (`strMatDetailID`='$intMatDetailID');";
                 // echo $sql;                  
		  $db->ExecuteQuery($sql);
				}
				else
				{
					...
					$sql2= "INSERT INTO specificationdetails (intStyleId,strMatDetailID,strUnit,dblUnitPrice,sngConPc,sngWastage,strPurchaseMode,strOrdType,
					strPlacement,intRatioType,dblReqQty,dblTotalQty,dblTotalValue,dblCostPC,dblfreight,intOriginNo,intMillId,intMainFabricStatus,intStatus,
                                        booWashable,dblPSP,dblDP,dblEMB,dblPEOSP,dblDPE,dblGEW,dblGPD,dblSPE,dblPEODP,intEmbPanels,intPrnPanels,dblGRW,
                                        intLgPanels) VALUES 
					('$styleID','$intMatDetailID','$strUnit',$dblUnitPrice,$reaConPc,$reaWastage,'NONE','','','0',
					$dblReqQty,$dblTotalQty,$dblTotalValue,$dbltotalcostpc,$dblFreight,$intOriginNo,0,0,1,
                                        $booWashable,$dblPSP,$dblDP,$dblEMB,$dblPEOSP,$dblDPE,$dblGEW,$dblGPD,$dblSPE,$dblPEODP,$intEmbPanels,$intPrintPanels,$dblGRW,$intLgPanels);";
	...
	#============================================================================================
	#
	# Description - To verify if style id exist before insert style to specification
	#============================================================================================
	
	$sql_spec = "SELECT * FROM specification WHERE intStyleId = ".$styleID;
		
	$res_spec = $db->RunQuery($sql_spec);
	...
	while($rowItems=mysql_fetch_array($resultItems))
	{
		...
		$intstatus = $rowItems["intstatus"];
                 ...
                 * =================================================================
                 */
                $booWashable = $rowItems["booWashable"];
                $dblPSP = $rowItems["dblPSP"];
                $dblDP = $rowItems["dblDP"];
                $dblEMB = $rowItems["dblEMB"];
                $dblPEOSP = $rowItems["dblPEOSP"];
                $dblDPE = $rowItems["dblDPE"];
                $dblGEW = $rowItems["dblGEW"];
                $dblGPD = $rowItems["dblGPD"];
                $dblSPE = $rowItems["dblSPE"];
                $dblPEODP = $rowItems["dblPEODP"];
                $dblGRW = $rowItems["dblGRW"];
                $intLgPanels =  $rowItems["intLgPanels"];
                $intEmbPanels =  $rowItems["intEmbPanels"];
                $intPrintPanels =  $rowItems["intPrnPanels"];
		...
		/*$sql2= "INSERT INTO specificationdetails (intStyleId,strMatDetailID,strUnit,dblUnitPrice,sngConPc,sngWastage,strPurchaseMode,strOrdType,
					strPlacement,intRatioType,dblReqQty,dblTotalQty,dblTotalValue,dblCostPC,dblfreight,intOriginNo,intMillId,intMainFabricStatus,intStatus,
                                        booWashable,opWastage,noPanels,embPanels,printPanels,printPanels,OperationId) VALUES 
					('$styleID','$intMatDetailID','$strUnit',$dblUnitPrice,$reaConPc,$reaWastage,'NONE','','','0',
					$dblReqQty,$dblTotalQty,$dblTotalValue,$dbltotalcostpc,$dblFreight,$intOriginNo,0,0,$status,
                                        $dblOpWastage,$intLgPanels,$intEmbPanels,$intPrintPanels,$intOperatioId);";*/
		$sql2= "INSERT INTO specificationdetails (intStyleId,strMatDetailID,strUnit,dblUnitPrice,sngConPc,sngWastage,strPurchaseMode,strOrdType,
					strPlacement,intRatioType,dblReqQty,dblTotalQty,dblTotalValue,dblCostPC,dblfreight,intOriginNo,intMillId,intMainFabricStatus,intStatus,
                                        booWashable,dblPSP,dblDP,dblEMB,dblPEOSP,dblDPE,dblGEW,dblGPD,dblSPE,dblPEODP,intEmbPanels,intPrnPanels,dblGRW,
                                        intLgPanels) VALUES 
					('$styleID','$intMatDetailID','$strUnit',$dblUnitPrice,$reaConPc,$reaWastage,'NONE','','','0',
					$dblReqQty,$dblTotalQty,$dblTotalValue,$dbltotalcostpc,$dblFreight,$intOriginNo,0,0,$status,$booWashable,
                                        $dblPSP,$dblDP,$dblEMB,$dblPEOSP,$dblDPE,$dblGEW,$dblGPD,$dblSPE,$dblPEODP,$intEmbPanels,$intPrintPanels,$dblGRW,$intLgPanels);";
		...
		{		
			...
			while($rowbpo=mysql_fetch_array($resulbpo))
			{
				...
				else
				{
					...
					$SQLPOQTY = "SELECT dblQty FROM deliveryschedule WHERE intStyleId = '$styleNo' AND
intBPO =  '$buyerPONo'";
				...
				{				
					...
					# =================================================================================
					# Modify On - 03/18/2016
					# Modify By - Nalin Jayakody
					# =================================================================================

					$qty = ($row["sngConPc"] + ($row["sngConPc"] * $row["sngWastage"] / 100)) * $poQty;
...
function SaveMaterialRatio($styleNo,$buyerPONo,$item,$color,$size,$qty,$balQty,$dblFreightBal,$charPos)
{
	...
	# Modify On - 03/18/2016
	# Modify By - Nalin Jayakody
	# Comment updating of material ratio in figures with default values while style process is executing and add validate if row has been exist.
	#========================================================================
	if(mysql_num_rows($result)>0){
		...
		//$buyerPONo = str_replace("Main Ratio","#Main Ratio#",$buyerPONo);
                ...
                # Add On - 07/22/2016
                # Add By - Nalin Jayakody
                # Add For - Insert 0 to intStatus field
                # ============================================
		$sql="insert into materialratio (intStyleId,strMatDetailID,strColor,strSize,strBuyerPONO,dblQty,dblBalQty,dblFreightBalQty,materialRatioID,dblRecutQty,intStatus) values ('$styleNo','$item','$color','$size','$buyerPONo',$qty,$balQty,$dblFreightBal,'$matRatioID',$recutQty,0);";
...
function SetLockStatus($prmStatus){
	
	global $db;
	
	$sql = "UPDATE lock_process SET locked = '$prmStatus'";
	//echo $sql;
	$db->ExecuteQuery($sql);

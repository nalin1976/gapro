...
class Orders{
   ...
   public function GetOrdersListByCode($prmStyleId){
        
       
       $sql = "SELECT orders.intStyleId,orders.strStyle,strDescription, companies.strComCode, buyers.strName,intQty,dtmAppDate,intSRNO, buyers.buyerCode FROM orders INNER JOIN companies ON orders.intCompanyID = companies.intCompanyID INNER JOIN buyers ON orders.intBuyerID = buyers.intBuyerID INNER JOIN specification ON orders.intStyleId = specification.intStyleId  WHERE orders.intStatus = 11 and orders.intStyleId = '$prmStyleId' ORDER by intSRNO DESC";

...
class MaterialCostInfo{
	...
	function CalculateRMCost($styleId, $deliQty){
		...
		$sql = " SELECT orderdetails.dblUnitPrice, orderdetails.reaConPc, orderdetails.reaWastage, orderdetails.dbltotalcostpc, ".
		       "        matitemlist.intMainCatID, orderdetails.intOriginNo, orders.intQty, orders.reaFinance, specification.intSRNO ".
               " FROM   orderdetails Inner Join matitemlist ON orderdetails.intMatDetailID = matitemlist.intItemSerial ".
               "        Inner Join orders ON orderdetails.intStyleId = orders.intStyleId Inner Join specification ON ". 
               "        orders.intStyleId = specification.intStyleId ".
			   " WHERE orderdetails.intStyleId = '$styleId' AND (orderdetails.intstatus = '1' OR orderdetails.intstatus IS NULL) ". 
		...
		while($rowmatinfo = mysql_fetch_array($rs_matinfor)){
			...
			//$mat_cost	= round(($rowmatinfo["dbltotalcostpc"] * $deliQty),2);

			//$dblReqQty		= ($rowmatinfo["reaConPc"] * $deliQty);
			$dblReqQty		= $rowmatinfo["reaConPc"] * $rowmatinfo["intQty"];
			$dblWastageQty	= (($dblReqQty / 100) * $rowmatinfo["reaWastage"]);
			//$dblTotReqQty	= $dblReqQty + $dblWastageQty;
			$dblTotReqQty	= round($dblReqQty + $dblWastageQty);
			$mat_cost       = $dblTotReqQty * $rowmatinfo["dblUnitPrice"];
			
			$costPC			= ($mat_cost / $rowmatinfo["intQty"]);
			
			$mat_cost 		= round(($deliQty * $costPC),2);
			...
			$rea_Finance = $rowmatinfo["reaFinance"];
			
			$tot_finance = round(($TransportCharge + $ClearingCost + $InterestCharge + $ExportExpenses),2); 
			
			$scIntCost += $InterestCharge;
			$scFinCost += floatval($TransportCharge + $ClearingCost + $ExportExpenses); 
			
			$tot_mat_cost +=  $mat_cost + $tot_finance;

			
			$sc = $rowmatinfo["intSRNO"];
		}

		//echo $sc . " -> " . $mainCatId . " - " . $tot_mat_cost . " - " . $deliQty. "<br />";

		/*$tot_finance = $rea_Finance * $deliQty;

		$tot_mat_cost += $tot_finance;*/
		...
		$tot_mat_cost = round($tot_mat_cost,2);

		//echo $sc ." -> ".$deliQty." -> ".$tot_mat_cost." -> ".$_dblESCValue." -> ".$_dblUpCharge." -> ".$scFinCost." -> ". $scIntCost. "<br />";
		return $tot_mat_cost;
	}
	...
	function GetStyleIdList($deliMonth, $deliYear, $deliStatus, $buyerId, $divisionId){
		...
		$sql = " SELECT orders.intStyleId, deliveryschedule.dblQty, buyers.intBuyerID, buyerdivisions.intDivisionId ".
			   ...
			   "       deliveryschedule.strShippingMode <> '7' AND deliveryschedule.intDeliveryStatus = '$deliStatus' and orders.intBuyerID = $buyerId and   "; //orders.intStyleId = 4192 and 

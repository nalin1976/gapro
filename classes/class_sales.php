...
class salesmonitoring{
	...
	public function GetConfirmUser($prmUserID){
        ...
        public function GetShippedQty($prmSCNo, $prmBPONo, $prmImpDbConnect){
            
            $dblShippedQty = 0;
            
            $sql = " SELECT pre_invoice_detail.strSC_No, pre_invoice_detail.strStyleID, cdn_detail.dblQuantity,
                            cdn_detail.strUnitID, shipmentforecast_detail.strPackType
                     FROM  cdn_detail INNER JOIN pre_invoice_detail ON cdn_detail.intPreItemNo = pre_invoice_detail.intItemNo
                     LEFT  JOIN shipmentforecast_detail ON cdn_detail.strBuyerPONo = shipmentforecast_detail.strPoNo
                     WHERE pre_invoice_detail.strSC_No = '$prmSCNo' AND cdn_detail.strBuyerPONo = '$prmBPONo'";
            
            $resShippedQty = $prmImpDbConnect->RunQuery($sql);
            
            while($rowShipped = mysql_fetch_array($resShippedQty)){
                $dblShippedQty = $rowShipped["dblQuantity"];
            }

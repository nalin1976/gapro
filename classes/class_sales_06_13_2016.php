...
class salesmonitoring{
	...
	public function getDeliveries($buyerid, $fromdate, $todate, $prmManuLocation){
		...
		$sql = " SELECT orders.strStyle, specification.intSRNO, freeze_schedule.dtmHandOverDate, freeze_schedule.intBPO, 
						freeze_schedule.dblQty, freeze_schedule.dblFOB, orders.intStyleId,
				 ...
				 FROM orders Inner Join specification ON orders.intStyleId = specification.intStyleId Inner Join freeze_schedule ON specification.intStyleId = freeze_schedule.intStyleId
		...
		if($buyerid != "-1"){
	        ...
	        $sql .=" order by  freeze_schedule.dtmHandOverDate ";
	...
	public function GetConfirmUser($prmUserID){
        ...
        public function GetFrozenValue($prmFromDate, $prmToDate){
            ...
            $sql = " SELECT sum(freeze_schedule.dblQty * freeze_schedule.dblFOB) as FrozenValue
                     FROM freeze_schedule
                     WHERE freeze_schedule.dtmHandOverDate BETWEEN  '$prmFromDate' AND '$prmToDate'";

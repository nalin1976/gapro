...
class MaterialCostInfo{
	...
	function CalculateRMCost($styleId, $deliQty){
		...
		while($rowmatinfo = mysql_fetch_array($rs_matinfor)){
			...
			$dblTotReqQty	= intval($dblReqQty + $dblWastageQty);
			//$mat_cost       = $dblTotReqQty * $rowmatinfo["dblUnitPrice"];
			$mat_cost       = floatval($dblTotReqQty * $rowmatinfo["dblUnitPrice"]);
			$costPC			= floatval($mat_cost / $rowmatinfo["intQty"]);
			
			$mat_cost 		= floatval($deliQty * $costPC);

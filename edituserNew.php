                ...
                <tr class="bcgl1">
                  <td><table width="100%" border="0" class="tableBorder">
                    <tr>
					  ...
					  <?php if(!$canEditAnyUserAccount){ ?> disabled="disabled"  <?php  } ?> class="txtbox" onchange="LoadUserDetails(this);">
        <?php
             if(!$canEditAnyUserAccount){   
			//$SQL="SELECT intUserID, UserName,Name FROM useraccounts WHERE intCompanyID = '".$_SESSION["FactoryID"]."' AND intUserID='".$userId."'";
			$SQL="SELECT intUserID, UserName,Name,intExPercentage FROM useraccounts WHERE intUserID='".$userId."' order by UserName";

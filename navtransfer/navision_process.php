...
while($row=mysql_fetch_array($rsOrderListToUpdate)){
	...
	while($row_OrderDetails = mysql_fetch_array($rs_OrderDetails)){
		...
		#======================================================================
		# Comment On - 02/12/2013
		# Description - To chnage the labour cost calculation
		#======================================================================
		...
		//echo "Raw material cost".$_dblRawMatCost."<br />";
		# Get total cost of the raw meterial excluding Services & Other Category
		$_dblFabAcceePackCost = $class_Navision->GetFabAccPackCost($_intStyleId);
		
		# Calculate export expenses 1%
		$_dblExportExpenses = (($_dblFabAcceePackCost/100)*2)/$_dblOrderQty;
		//echo $_dblExportExpenses."<br />";
		
		$_dblBudgetCost = $_dblRawMatCost / $_dblOrderQty;   
		//echo  "Bud Cost ".$_dblBudgetCost."<br />";
		//echo  "Finance Cost ".$_dblFinanceCost."<br />";
		//echo  "ESC Cost ".$_dblESC."<br />";
		$_dblBudgetCost += $_dblFinanceCost + $_dblESC ;

		//echo  "Bud Cost ".$_dblBudgetCost."<br />";
		
		#======================================================================
		# Comment On - 01/30/2013
		# Description - To remove corporate cost deduction from estimated cost
		#======================================================================
		#	$_dblEstimatedCost = $_dblBudgetCost + $_dblLabourCost + $_dblCorporateCost;		
		#======================================================================
		
		$_dblEstimatedCost = ($_dblBudgetCost + $_dblLabourCost) - $_dblExportExpenses;
		//echo  "Labour Cost ".$_dblLabourCost."<br />";
		//echo  "Estimated Cost ".$_dblEstimatedCost."<br />";
		...
		$_dblFOB = $_dblFOB / $_intNoStyleParts;
		...
		if($res_success == 1){
			...
			echo "Process Complete - ".$_intOrderNo." <br />";
		}else{
			echo $res_success ."<br />";

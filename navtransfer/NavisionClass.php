...
class classNavision{
	...
	#============================================================================================
	# Description - Get the raw materail cost of the order 
	#               	
	#============================================================================================
	function GetRawMaterialCost($prmStyleId){
		...
		$strSql = " SELECT SUM(((orders.intQty * orderdetails.reaConPc)+(((orders.intQty * orderdetails.reaConPc) * orderdetails.reaWastage) / 100)) * orderdetails.dblUnitPrice) AS TotalValue ".
                  ...
                  " WHERE  orders.intStyleId =  '$prmStyleId' AND orderdetails.intstatus = 1";// AND matitemlist.intMainCatID <>  '4' AND matitemlist.intMainCatID <>  '5'";
	...
	#               	
	#============================================================================================
	function GetFabAccPackCost($prmStyleId){
	...
	#               	
	#============================================================================================	
	function UpdateFinshGoods($prmFOB, $prmEstCost, $prmSMV, $prmCMPc, $prmLabor, $prmJobNo, $prmNoParts, $prmBuyerName){
		...
		$strSql = ' UPDATE [dbHela].[dbo].[HELA CLOTHING (PVT) LTD$Item Transfer] '.
				  ...
				  '     EstimatedLabourOHPiece = '.$prmLabor.', PCS_Pack = '. $prmNoParts.", Transfer = 'N', Buyer = '".$prmBuyerName."'".
	...
	#               	
	#============================================================================================	
	function InsertFinishGoods($prmJobNo, $prmStyleId, $prmDivision, $prmSeason, $prmFOB, $prmSMV, $prmEstCost, $prmCMPc, $prmLabor, $prmStyleDesc, $prmNoParts, $prmBuyerName){
		
		global $classMSSQLConnect;
		
		$iPosition = strpos($prmStyleDesc,"'");
		
		if($iPosition>0){
			$prmStyleDesc = str_replace($prmStyleDesc, "'","''");
		}
		...
		$strSql = ' INSERT INTO [dbHela].[dbo].[HELA CLOTHING (PVT) LTD$Item Transfer] ([Item No_], StyleNo, BuyerDivision, GarmentType, Season, '.
                  ...
                  '              Description, BaseUnitofMeasure, SalesUOM, PurchaseUOM, GenProdPostingGroup, Transfer, PCS_Pack, Buyer) '.
				  ' VALUES     ('."'".$prmJobNo."', '".$prmStyleId."','".$prmDivision."','".$prmDivision."','".$prmSeason."',".$prmFOB.
				  "           ,".$prmEstCost.",".$prmSMV.",".$prmCMPc.",".$prmLabor.", 'FINGDGMT', '".$prmStyleDesc."', 'PCS','PCS','PCS','GOODGARM','N',".$prmNoParts.
				  "           ,'".$prmBuyerName."' )";				
	...
	function InsertJob($prmJobNo, $prmStyleDescription, $prmOrderQty, $prmOrderDate, $prmStyle, $prmCost, $prmMerchandiser, $prmLastDeliDate, 
		...
		$iPosition = strpos($prmStyleDescription,"'");
		
		if($iPosition>0){
			$prmStyleDescription = str_replace($prmStyleDescription, "'","''");
		}
	...
	#============================================================================================
	# Description - Get number of style parts from a style
	#               	
	#============================================================================================
	function GetNoOfStyleParts($prmStyleId){
		
		global $db;		
		
		$strSql = " SELECT Count(stylepartdetails.intStyleId) AS NoParts  ".
		          " FROM   stylepartdetails ".
				  " WHERE  stylepartdetails.intStyleId = '$prmStyleId' ";
		$result = $db->RunQuery($strSql);

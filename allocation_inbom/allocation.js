...
var pub_checkLoop   = 0;
var pub_BAlloUrl    = "/gapro/allocation_inbom/";
...
function ValidationLeftOver()
{
	...
	{
		if(tbl.rows[loop].cells[9].childNodes[0].checked){
			check = false;
		}
	}
	if(check){
        ...
        for(loop=1;loop<tbl.rows.length;loop++)
	{
           //alert(loop);
            if(tbl.rows[loop].cells[9].childNodes[0].checked)
            {
                req_qty = parseFloat(tbl.rows[loop].cells[6].id);
                qty += parseFloat(tbl.rows[loop].cells[10].childNodes[0].value);
                stock_qty += parseFloat(tbl.rows[loop].cells[8].childNodes[0].nodeValue);
              
            } 
        }
        //alert("Stock Qty " + stock_qty);
        if(qty > req_qty){
            alert("Total allocated quantity cannot be exceeded required quantity ");
            return false;
        }
	...
	xmlHttpAllocation[1].onreadystatechange=GetNoRequest;
	//xmlHttpAllocation[1].open("GET",'allocation_inbom/allocationdb.php?RequestType=GetNo&mainStoreID='+mainStoreID,true);
        xmlHttpAllocation[1].open("GET",'allocation_inbom/allocationdb.php?RequestType=GetNo&mainStoreID='+manufactCom,true);
	xmlHttpAllocation[1].send(null);
}
...
function SaveDetails()
{
	...
	for(loop=1;loop<tbl.rows.length;loop++)
	{
		if(tbl.rows[loop].cells[9].childNodes[0].checked)
		{
			...
			var buyerPoNo 	= tbl.rows[loop].cells[4].id;
			...
			var qty 	= tbl.rows[loop].cells[10].childNodes[0].value;
			var grnNo 	= tbl.rows[loop].cells[11].childNodes[0].nodeValue;
			var grnYear	= tbl.rows[loop].cells[12].childNodes[0].nodeValue;
			var grnType	= tbl.rows[loop].cells[13].id;
                        var fromStoresId = tbl.rows[loop].cells[1].id;
                        var subLocation  = tbl.rows[loop].cells[14].id;
                        var locationId   = tbl.rows[loop].cells[15].id;
                        var binId        = tbl.rows[loop].cells[16].id;
			...
			url += '&no='+pub_No;
					...
					url += '&grnYear='+grnYear;
					url += '&grnType='+grnType;
                                        url += '&storesId='+fromStoresId;
                                        url += '&subId='+subLocation;
                                        url += '&locationId='+locationId;
                                        url += '&binID='+binId;
					url += '&toStores='+manufactCom;
...
function GetQty(obj)
{
	...
	if(obj.checked)
	{
		...
		var ratioQty = parseFloat(rw.cells[6].id);
		
		//get already allocated bulk qty
		var allocatedQty = parseFloat(getAlreadyLeftoverAlloQty(rw.cells[5].childNodes[0].nodeValue,rw.cells[6].childNodes[0].nodeValue,'tblLeftOver'));
		...
		if(canAllocateTotalRatioQty)
		{
			if(allocatedQty >= totRatioQty)
			{
                            ...
                            //obj.checked = false;
                            //return false;
                            rw.cells[10].childNodes[0].value = totRatioQty
			}
			else
			{	
				...
				if(baltoAllocate>stockQty)
					...
					rw.cells[10].childNodes[0].value = baltoAllocate;
			}
		}
		else
		{
			...
			{
                            //alert("Allocated Qty " + allocatedQty + " Ratio Qty " +ratioQty );
			...
			{	
				...
				if(baltoAllocate>stockQty)
					rw.cells[10].childNodes[0].value = stockQty;
                                    ...
                                    dblValidateQty = ValidateQtySelected();  
                                    
                                    if(dblValidateQty>baltoAllocate){
                                        alert("You are exceeding total required quantity");
                                        rw.cells[10].childNodes[0].value = "";
                                        obj.checked = false;
                                        return ;
                                    }
			}
...
function ValidateQtySelected(){
    //alert("E");
    var dblTotQty = 0;
    $("#tblLeftOver tr").each(function(){
      //alert($(this).find('td:eq(9) input[type="checkbox"]').is(':checked'));
      if($(this).find('td:eq(9) input[type="checkbox"]').is(':checked')){
          dblTotQty += parseFloat($(this).find('td:eq(10) input').val());
      }
      //
...
function GetLiabilityQty(obj)
{
	...
	if(obj.checked)
	{
		...
		//get already allocated bulk qty
		var allocatedQty = parseFloat(getAlreadyLeftoverAlloQty(rw.cells[5].childNodes[0].nodeValue,rw.cells[6].childNodes[0].nodeValue,'tblLiability'));
...
function viewLeftOver()
{
	...
	//var MainStoreID = document.getElementById('cboMainStore').value;
        //alert(toBuyerPoNo);
        var strItemColor = document.getElementById('hndColor').value;
        var strItemSize = document.getElementById('hndSize').value;
	...
	url=url+"?RequestType=viewLeftOver";
	url += '&matDetailId='+matDetailId;
	url += '&itemColor='+strItemColor;
	url += '&itemSize='+strItemSize;
        url += '&toStyleId='+toStyleId;
...
function closeAllocationWindow()
{

...
INNER JOIN storesbins ON storesbins.strLocID = S.strLocation AND storesbins.strMainID = S.strMainStoresID AND storesbins.strSubID = S.strSubStores AND storesbins.strBinID = S.strBin
                   where  S.intMatDetailId='$matDetailId' and strColor in ($strColor) and strSize in ($itemSize)  
                   group by strMainStoresID,intStyleId,strBuyerPoNo,strColor,strSize,intGrnNo,intGrnYear,strGRNType, substores.strSubStoresName, storeslocations.strLocName, storesbins.strBinName "; 

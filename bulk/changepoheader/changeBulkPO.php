...
<title>Confirmed Bulk PO - Correction</title>
...
$sql = "SELECT strSupplierID,intPayMode,strPayTerm,intShipmentModeId,intShipmentTermID,intInvoiceComp,intDeliverTo, 
DATE_FORMAT(dtDeliveryDate,'%d/%m/%Y') as DeliveryDate,DATE_FORMAT(dtmETD,'%d/%m/%Y') as ETDDate,
DATE_FORMAT(dtmETA,'%d/%m/%Y') as ETADate  FROM bulkpurchaseorderheader
WHERE intBulkPoNo = '$pono' AND intYear = '$poyear';";
$result = $db->RunQuery($sql);	
while($row = mysql_fetch_array($result))
{
	$strSupplierID 		= $row["strSupplierID"];
	$strPayTerm 		= $row["strPayTerm"];
	$strPayMode 		= $row["intPayMode"];
	$strShipmentMode 	= $row["intShipmentModeId"];
	$strShipmentTerm 	= $row["intShipmentTermID"];
	$intInvCompID 		= $row["intInvoiceComp"];
	$intDelToCompID 	= $row["intDeliverTo"];
...
if (!isset($_GET["pono"]))
{
	...
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),    'Supplier','$strSupplierID','$newSupplier','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($strPayTerm != $newPayTerm)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'Pay Term','$strPayTerm','$newPayTerm','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($strPayMode != $newPayMode)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'Pay Mode','$strPayMode','$newPayMode','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($strShipmentMode != $newShipmentMode)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'Shippment Mode','$strShipmentMode','$newShipmentMode','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($strShipmentTerm != $newShipmentTerm)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'Shippment Term','$strShipmentTerm','$newShipmentTerm','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($intInvCompID != $newInvoiceTo)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'Invoice To','$intInvCompID','$newInvoiceTo','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($intDelToCompID != $newDeleverTo)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'Deliver To','$intDelToCompID','$newDeleverTo','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($newDeleveryDate != $dtmDeliveryDate)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'Delivery Date','$dtmDeliveryDate','$newDeleveryDate','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($newETD != $etdDate)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'ETD Date','$etdDate','$newETD','$user');";
		$db->ExecuteQuery($sql);	
	}
	if ($newETA != $etaDate)
	{
		$sql = "INSERT INTO bulk_pochangelog 	(intPONO, 	intYear,	dtmchangedDate,	strsource,	strPrevious,	strNew,	intUserID) VALUES ('$pono',	'$poyear',NOW(),	'ETA Date','$etaDate','$newETA','$user');";
		$db->ExecuteQuery($sql);	
	}
	...
	$sql = "update bulkpurchaseorderheader 
			set strSupplierID = '$newSupplier' ,
			intPayMode = '$newPayMode' ,
			strPayTerm = '$newPayTerm',
			intShipmentModeId = $newShipmentMode,
			intShipmentTermID = $newShipmentTerm,
			intInvoiceComp = '$newInvoiceTo',
			intDeliverTo = '$newDeleverTo', 
			dtDeliveryDate='$dtDateofDelivery',
			dtmETA=$formatETA,dtmETD=$formatETD 
			where intBulkPoNo = '$pono' AND intYear = '$poyear';";
...
<table width="965" border="0" align="center">
  ...
  <tr>
    <td><form method="post" action="changeBulkPO.php"><table width="100%" border="0"  cellpadding="0" cellspacing="1" class="bcgl1">
      <tr>
        <td height="21">&nbsp;</td>
      </tr>
      <tr>
        <td height="57" align="center">
			...
			<table width="50%" >
			<tr>
			<td height="41" colspan="2"><span class="head1">Confirmed Bulk PO - Correction</span></td>			
			</tr>			
			<tr>
			...
			<td class="txtbox"><select name="cboSupFind" class="txtbox" id="cboSupFind" style="width:250px" >
	...
	while($row = mysql_fetch_array($result))
	{
		if ($row["intShipmentModeId"] == $strShipmentMode)
	...
	?>
			...
			<tr>
			<td></td>
			<td class="txtbox"><input  style="font-family: Verdana;font-size: 11px;color: #20407B;" type="submit" value="Update_PO" ></td>			
			</tr>
			<tr>
			<td></td>
			...
			<?php
			$sql="SELECT dtmchangedDate,strsource,strPrevious,strNew,useraccounts.Name FROM bulk_pochangelog 
INNER JOIN useraccounts ON bulk_pochangelog.intUserID = useraccounts.intUserID
...
<?php
	$sql="select count(intBulkGrnNo)as count from bulkgrnheader where intBulkPoNo='$pono' and intBulkPoYear='$poyear'";
	$result=$db->RunQuery($sql);

	...
	Class ExcelWriter
	{
		...
		*/
		function open($file)
		{
			...
			if(!empty($file))
			{
				$this->fp=@fopen($file,"w+");//permission 
			}
			else
			{
...
EOH;
		...
		*/
		...
		function writeLine($line_arr)
		{
			...
			fwrite($this->fp,"<td class=xl26 width=70>$col</td>");
			fwrite($this->fp,"</tr>");
		...
		*/
		function writeCol($value,$count)
		{
			...
			($count%2==1)?fwrite($this->fp,"<td class=x129 width=64 >$value</td>"):fwrite($this->fp,"<td class=x130 width=64 >$value</td>");
		}
		
		function writeAnywhere($top,$left)
		{
			if($top!="")
			{
				for($i=0;$i<$top;$i++)

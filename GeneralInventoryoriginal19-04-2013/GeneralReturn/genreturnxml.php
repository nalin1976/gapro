...
{		
	...
	$ResponseXML .= "<loadMrnDetailsToGrid>";
		...
		$SQL = "SELECT genmatitemlist.strItemDescription,
				...
				Inner Join costcenters CC ON CC.intCostCenterId=genissuesdetails.intCostCenterId
				WHERE
...
else if ($RequestType=="SaveIssueDetails")
{
	...
	$GLAllowId =$_GET["GLAllowId"]; 
	 ...
	 $SQL= " INSERT INTO genreturndetail (intReturnId,intRetYear,intGRNNo,intGRNYear,intIssueNo,intIssYear,intMatDetailID,dblQty,dblBalQty,strUnit,intCostCenterId,intGLAllowId) VALUES ('$issueNo','$issueYear','$grnNoArray[1]','$grnNoArray[0]','$issuenoArray[1]','$issuenoArray[0]','$itemdDetailID','$qty','$qty','$itmUnit','$costCenterId','$GLAllowId'); ";	
		$db->executeQuery($SQL);	
...
function getGenReturnDetailsData($GPNo,$GPyear)
{
	...
	$sql = "select 	GRD.intReturnId, 
			...
			inner join costcenters CC on CC.intCostCenterId=GRD.intCostCenterId

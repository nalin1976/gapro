...
if($booCheck)
	$result_check = InsertPOHeader($genPoNo,$poYear,$supplierId,$dtmDate,$dtmDeliveryDate,$currencyId,$userId,$invoiceCompId,$FactoryID,$deliverToId,$payTerm,$payMode,$instructions,$totalValue,$shipmentMode,$shipmentTerms);
else
	$result_check = UpdatePOHeader($genPoNo,$poYear,$supplierId,$dtmDate,$dtmDeliveryDate,$currencyId,$userId,$invoiceCompId,$FactoryID,$deliverToId,$payTerm,$payMode,$instructions,$totalValue,$shipmentMode,$shipmentTerms);
...
function InsertPOHeader($genPoNo,$poYear,$supplierId,$dtmDate,$dtmDeliveryDate,$currencyId,$userId,$invoiceCompId,$FactoryID,$deliverToId,$payTerm,$payMode,$instructions,$totalValue,$shipmentMode,$shipmentTerms)
{
global $db;
		$sql="insert into generalpurchaseorderheader (intGenPONo, intYear, intSupplierID, dtmDate, dtmDeliveryDate, strCurrency, intStatus,  intUserID, intInvoiceComp, intPrintStatus, intCompId, intDeliverTo, intPreviousUserId, strPayTerm, intPayMode, strInstructions, dblPOValue, dblPoBalance,intCreatedBy,dtmCreatedDate,intShipmentMode,intShipmentTerms) values ('$genPoNo', '$poYear', '$supplierId', now(), '$dtmDeliveryDate', '$currencyId', '0','$userId', '$invoiceCompId', '0', '$FactoryID', '$deliverToId', '0', '$payTerm', '$payMode', '$instructions','$totalValue', '$totalValue','$userId',now(),'$shipmentMode','$shipmentTerms');";
		return $db->RunQuery($sql);
}

function UpdatePOHeader($genPoNo,$poYear,$supplierId,$dtmDate,$dtmDeliveryDate,$currencyId,$userId,$invoiceCompId,$FactoryID,$deliverToId,$payTerm,$payMode,$instructions,$totalValue,$shipmentMode,$shipmentTerms)
{
global $db;
		$sql="update generalpurchaseorderheader set intSupplierID='$supplierId', dtmDate=now(), dtmDeliveryDate='$dtmDeliveryDate', strCurrency='$currencyId', intStatus=0, intUserID='$userId', intInvoiceComp='$invoiceCompId', intPrintStatus=0, intCompId='$FactoryID', intDeliverTo='$deliverToId', strPayTerm='$payTerm', intPayMode='$payMode', strInstructions='$instructions', dblPOValue='$totalValue', dblPoBalance='$totalValue', intShipmentMode='$shipmentMode', intShipmentTerms='$shipmentTerms'  where intGenPONo='$genPoNo' and intYear='$poYear';";
		return $db->RunQuery($sql);
}
?>
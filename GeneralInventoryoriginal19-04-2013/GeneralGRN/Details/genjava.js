    ...
    for (var loop=1;loop < tbl.rows.length ; loop ++ )
  	{
		...
		if (cbo.checked) {
			...
			var matDetailId = tbl.rows[loop].cells[4].lastChild.nodeValue;
			...
			for (var mainLoop =1 ;mainLoop < tblMain.rows.length; mainLoop++ )
			{
				...
				if(booCheck){
					...
					var url = 'genxml.php?id=addItemToGrn&strDescription='+ URLEncode(strDescription) +'&No='+ loop + '&poNo=' + poNo+ '&year=' + year+'&matDetailId='+matDetailId+'&costCenterId='+costCenterId+'&glAlloId='+glAlloId;
					var htmlobj=$.ajax({url:url,async:false});
...
{
	var value=sCell.value;
	sCell.parentNode.parentNode.cells[9].lastChild.nodeValue=RoundNumbers(parseFloat(value*sCell.parentNode.parentNode.cells[5].lastChild.nodeValue),4);
	if(parseFloat(sCell.parentNode.parentNode.cells[6].lastChild.nodeValue) < value)
	{
		var exQty = parseFloat(value) - parseFloat(sCell.parentNode.parentNode.cells[6].lastChild.nodeValue);
		sCell.parentNode.parentNode.cells[8].lastChild.nodeValue =RoundNumbers(exQty,4);
	}
	else
	{
		sCell.parentNode.parentNode.cells[8].lastChild.nodeValue = 0;
	}
}
...
function findGrnValue(objBox)
{
	...
	for(var loop=1;loop<=tblGrn.rows.length;loop++)
	{
		try
		{
		var value = parseFloat(tblGrn.rows[loop].cells[9].lastChild.nodeValue);
		dblTotalValue+=value;
		}
		catch(err)
		{}
...
function save_GrnDetail(strBulkGrn_Year,type)
{
		...
		var url  = "gendb.php?id=saveGrnDetails";
			...
			url += "&strGenGrnNo="+strGenGrnNo;
			url += "&intBulkGrnYear="+intBulkGrnYear;
			url += "&intMatDetailID="+row.cells[10].childNodes[0].nodeValue;
			url += "&dblQty="+row.cells[7].childNodes[0].value;
			url += "&dblCapacityQty="+row.cells[6].childNodes[0].nodeValue;
			url += "&dblExcessQty="+row.cells[8].lastChild.nodeValue;
			url += "&dblRate="+row.cells[5].lastChild.nodeValue;
			url += "&intYear="+row.cells[11].lastChild.nodeValue;
			url += "&intGenPONo="+row.cells[12].lastChild.nodeValue;
			url += "&strUnit="+row.cells[4].lastChild.nodeValue;
			url += "&strInvoiceNo="+document.getElementById("txtinvoiceno").value;
			url += "&strDesc="+row.cells[3].lastChild.nodeValue;
...
function saveStockTransaction()
{
	...
	for(var loop=1;loop<tblGrn.rows.length;loop++)
		{
			var row = tblGrn.rows[loop];
			arrMatDetID = row.cells[10].lastChild.nodeValue ;
			arrQty = row.cells[7].childNodes[0].value;
			arrUnit  = row.cells[4].lastChild.nodeValue;

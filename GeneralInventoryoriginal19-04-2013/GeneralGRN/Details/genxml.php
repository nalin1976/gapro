...
gpd.strUnit,gpd.dblUnitPrice,gpd.dblPending,gpd.intMatDetailID,gpd.intGenPONo,gpd.intYear,c.strDescription as costCenter,
gpd.intCostCenterId,(select gl.strAccID from glaccounts gl where gl.intGLAccID= gla.GLAccNo) as strAccID,
...
and gpd.intCostCenterId ='$costCenterId' and gpd.intGLAllowId='$glAlloId' ";
...
if($id=="loadgrndetail")
{
	...
	$intYear=$_GET["intYear"];
		...
		GSub.StrCatName AS Category, GD.intCostCenterId,GD.intGLAllowId,
		(select c.strDescription from costcenters c where c.intCostCenterId = GD.intCostCenterId) as costCenter,
		(select GL.strAccID from glaccounts GL where GL.intGLAccID = GLA.GLAccNo) as strAccID,
		(select C.strCode from costcenters C where C.intCostCenterId = GLA.FactoryCode) as costCenterCode
		...
		GPD.intMatDetailID = GD.intMatDetailID and GD.intCostCenterId = GPD.intCostCenterId
		...
		inner join glallowcation GLA on GLA.GLAccAllowNo = GPD.intGLAllowId and GPD.intGLAllowId = GD.intGLAllowId

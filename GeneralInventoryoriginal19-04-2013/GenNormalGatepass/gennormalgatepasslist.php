...
<body>
<form name="frmNGList" id="frmNGList" action="gennormalgatepasslist.php" method="post">
  <tr>
    <td><?php include "${backwardseperator}Header.php"; ?></td>
  </tr>
<table width="950" border="0" align="center" bgcolor="#FFFFFF" class="tableBorder">
  <tr>
    <td class="mainHeading">General Normal Gate Pass Listing</td>
  </tr>
  <tr>
...
$toArray 	= explode('/',$dateTo);
	$sql ="SELECT distinct concat(GSH.intYear,'/',GSH.intGatepassId) as no , GSH.dtmDate, GSH.strToStores AS strName
FROM  general_nomgatepass_header GSH inner join general_nomgatepass_detail NGD on NGD.intGatepassId=GSH.intGatepassId and NGD.intYear=GSH.intYear
inner join genmatitemlist GL on GL.intItemSerial=NGD.intMatDetailId  
WHERE GSH.intCompanyID='$companyId'";
	...
	if ($searchDescription!="")
			$sql .="AND GL.strItemDescription like '%$searchDescription%'";
	if($chkdate)
	{
		if($datefrom!="")
				$sql .="AND date(GSH.dtmDate) >= '" . $fromArray[2].'-'.$fromArray[1].'-'.$fromArray[0] . "' ";
		
		if($dateTo!="")
				$sql .="AND date(GSH.dtmDate) <= '" . $toArray[2].'-'.$toArray[1].'-'.$toArray[0] . "' ";
	}
...
<td class="normalfntMid"><a target="_blank" href="<?php echo "gennormalgatepass.php?No=".$row["no"] ;?>" ><?php echo $row["no"]?></a></td>

...
function loadPopupItems(costCenterId)
{	
	...
	url += '&chkAllItem='+chkAllItem+'&costCenterId='+costCenterId;
	...
	var itemDisI =htmlobj.responseXML.getElementsByTagName("Item")
		...
		craeteGrid(itemDis,matDetaiID,balanceQty,mrnRaised,issueQty,stockBalance,GRNNo,GRNYear,GLCode,GLAllowId); 
		}
	hidePleaseWait();		 
 ...
 function craeteGrid(itemDis,matDetaiID,balanceQty,mrnRaised,issueQty,stockBalance,GRNNo,GRNYear,GLCode,GLAllowId)
 {
	...
	var cellGRNYear=row.insertCell(7);
...
function saveGenMrnHeader()
{
	...
	var url = 'genMRNMiddle.php?RequestType=SaveHeader&genMRNno='+pub_MRNno+'&genMRNyear='+pub_MRNyear;
		url += '&departementCode='+departementCode+'&costCenterId='+costCenterId;
	htmlobj=$.ajax({url:url,async:false});
...
function createRequestItemPopUp()
{
	...
	var url = "genmrndetailpopup.php?costCenterId="+costCenterId;

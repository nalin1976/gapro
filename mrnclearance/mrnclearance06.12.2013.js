...
function validateAdjust(adj)
{
	...
	var mrnqty=parseFloat(adj.parentNode.parentNode.childNodes[6].innerHTML);
	var issueqty=parseFloat(adj.parentNode.parentNode.childNodes[7].innerHTML);
	var adjust=adj.value;
	
	if(adjust>mrnqty || adjust<issueqty){
	alert("Sorry!value should between "+issueqty+" and "+mrnqty);
	adj.parentNode.parentNode.childNodes[9].childNodes[0].value=mrnqty;
...
function saveGrid()
{
	...
	{
		var style=tableMRN.rows[loop].cells[1].id;
		var buyerpo=tableMRN.rows[loop].cells[2].id;	
		var matdetailtid=tableMRN.rows[loop].cells[3].id;	
		var color=tableMRN.rows[loop].childNodes[4].innerHTML;	
		var size=tableMRN.rows[loop].childNodes[5].innerHTML;
		var issue=tableMRN.rows[loop].childNodes[7].innerHTML;
		var mrnqty=tableMRN.rows[loop].childNodes[6].innerHTML;
		if(parseFloat(issue)!=parseFloat(mrnqty))
		 mrnqty=tableMRN.rows[loop].childNodes[9].childNodes[0].value;
		var grnno   = tableMRN.rows[loop].childNodes[10].innerHTML;
		var grnYear = tableMRN.rows[loop].childNodes[11].innerHTML;
		var grnType	= tableMRN.rows[loop].childNodes[12].id;

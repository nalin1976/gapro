...
function deleteBeforeSave(){
    ...
    var path = "userEvents-db-set.php?type=deleteBeforeSave";
	    path += "&cboUser="+cboUser;	
	htmlobj=$.ajax({url:path,async:false});
...
function events_visibility_Save(){
 ...
 var rowCount=0;
	 ...
	 if(tblmaingrid.rows[a].cells[0].lastChild.checked == true){	
		...
		var path = "userEvents-db-set.php?type=events_user_groups_Save";
			path += "&cboUser="+cboUser;
...
function loadExEventsForUsers(){
	var cboUser = document.getElementById("cboUser").value;
    var path = "userEvents-db-get.php?type=loadExEventsForUsers";
	    path += "&cboUser="+cboUser;	
		htmlobj=$.ajax({url:path,async:false});
...
function checkSavedDetails(){
	var arrArrayCheck = [];
	var cboUser = document.getElementById("cboUser").value;
    var path = "userEvents-db-get.php?type=check";
	    path += "&cboUser="+cboUser;
		htmlobj=$.ajax({url:path,async:false});

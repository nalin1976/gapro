...
/*		else if(isNumeric(document.getElementById('txtComCode').value)){
		...
		//Start - 03-12-2010 - Validate with company sequance no
		else if(document.getElementById('txtSequenceStart').value.trim()=="")
		{
			alert("Please enter the 'Sequence No Start'.");
			document.getElementById('txtSequenceStart').select();
			return;
		}
		else if(document.getElementById('txtSequenceEnd').value.trim()=="")
		{
			alert("Please enter the 'Sequence No End'.");
			document.getElementById('txtSequenceEnd').select();
			return;
		}
		var sequaneStart 	= document.getElementById('txtSequenceStart').value.trim();
		var sequaneEnd 		= document.getElementById('txtSequenceEnd').value.trim();
		if(parseFloat(sequaneStart)>=parseFloat(sequaneEnd))
		{
			alert("'Sequence No End' must greater than the 'Sequence No Start'.");
			document.getElementById('txtSequenceEnd').select();
			return;
		}
		//End - 03-12-2010 - Validate with compant sequance no
...
function butCommand1(strCommand)
{
	...
	if(document.getElementById("cbocompany").value=="")
		...
		if(strCommand=="Save"){		
			...
			url=url+"&strSVATNo="+URLEncode(document.getElementById("txtSVATNo").value.trim());	
		...
		url=url+"&sequenceStart="+document.getElementById("txtSequenceStart").value.trim();
		url=url+"&sequenceEnd="+document.getElementById("txtSequenceEnd").value.trim();
...
{ 
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
 	{		
		...
		if((text[0]=="Updated successfully") || (text[0]=="Saved successfully") || (text[0]=="Deleted successfully"))
		{
			ClearCompanyForm();
		}
	} 
...
{
	document.getElementById('txtSequenceStart').disabled = false;
	document.getElementById('txtSequenceEnd').disabled = false;
		...
		if(xmlHttp.readyState == 4) 
		{
			...
			{  	
			  ...
			  //  var XMLLoad = xmlHttp.responseXML.getElementsByTagName("CompanyID");
				...
				var XMLLoad = xmlHttp.responseXML.getElementsByTagName("SVATNo");
				document.getElementById('txtSVATNo').value = XMLLoad[0].childNodes[0].nodeValue;
				//defaultFactory
				...
				var XMLSequenceStart = xmlHttp.responseXML.getElementsByTagName("SequenceStart");
				document.getElementById('txtSequenceStart').value = XMLSequenceStart[0].childNodes[0].nodeValue;
				document.getElementById('txtSequenceStart').disabled = true;
				
				var XMLSequenceEnd = xmlHttp.responseXML.getElementsByTagName("SequenceEnd");
				document.getElementById('txtSequenceEnd').value = XMLSequenceEnd[0].childNodes[0].nodeValue;
				document.getElementById('txtSequenceEnd').disabled = true;
			}
		}
	}

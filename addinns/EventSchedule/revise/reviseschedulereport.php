...
<title>GaPro - Revise Schedule Report</title>
...
<?php 				
				...
				if ($styleID == "")
				{
					$sql = "SELECT DISTINCT eventscheduleheader.strStyleId FROM eventscheduleheader INNER JOIN orders ON eventscheduleheader.strStyleId = orders.intStyleId WHERE 
...
companies.strAddress2, companies.strStreet, companies.strCity, companies.strState, companies.intCountry, companies.strZipCode,companies.strRegNo, 
...
<tr>
  ...
  <tr>
    <td colspan="3"><p class="head2BLCK"><br/>Critical Events As At : <?php echo date('jS \of F Y'); ?> </p>
      <p class="head2BLCK"></p>
  ...
  <tr>
    <td colspan="3"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tablez" bgcolor="#FFFFFF">
		<tr>
        <td height="25">&nbsp;</td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
...
LEFT JOIN specification ON eventscheduleheader.strStyleId = specification.intStyleId  AND specification.intStyleId = orders.intStyleId 
...
WHERE eventscheduledetail.intScheduleId = '$scheduleID' AND (eventscheduledetail.dtCompleteDate is not null)";
  ...
  <tr>
    <td height="25" colspan="3" class="normalfnth2B">
        </span></td>
  </tr>
  <tr bgcolor="#d6e7f5" id="revise" >
    <td width="40%"  >&nbsp;</td>
	 <td  ><div align="center"><img src="../../images/revise.png" alt="approve"   class="mouseover" onclick="ReviseSchedule();"/></div></td>
	  <td width="40%"  >&nbsp;</td>
  </tr>
...
function ReviseSchedule()
{
	...
	xmlHttp.onreadystatechange = ReviseScheduleRequest;
	xmlHttp.index = styleId;
	xmlHttp.open("GET",'../ManageScheduleXML.php?RequestType=ReviseSchedule&StyleId=' + URLEncode(styleId) , true);
	xmlHttp.send(null);
}
	function ReviseScheduleRequest()
	{
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)
		{
			alert ("Style name : "+ xmlHttp.index +" revised.");

...
<title>General Price List</title>
...
{
	var url     = 'genpricelistdb.php?request=setSubCategory&mainCatId='+obj.value;
...
function updateUnitPrice(obj,unitPrice)
{
	...
	var url      = 'genpricelistdb.php?request=updateUnitPrice&unitPrice='+unitPrice+'&itemCode='+itemCode;
	var htmlobj  = $.ajax({url:url,async:false});
}
...
<body>
<form id="frmStylePriceList" name="frmStylePriceList" method="post" action="genpricelist.php">
...
<tr>
    ...
    <tr>
     <td class="head2">General Price List</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    </tr>
      ...
      <td><table width="100%" align="center" border="0" cellspacing="2" cellpadding="2">
        <tr>
          ...
          <td width="20%" class="normalfnt">
            <select name="cboMainCategory" id="cboMainCategory" class="txtbox" style="width:150px" onchange="setSubCategory(this);">
              ...
              <?php
		$sql = "select intID,strDescription from genmatmaincategory order by intID";
		...
		?>
          ...
          <td width="19%" class="normalfnt"><select name="cboSubCategory" id="cboSubCategory" class="txtbox" style="width:150px">
            <option value="" selected="selected" >Select One</option>
            <?php
		$sql = "select intSubCatNo,StrCatName from genmatsubcategory where intStatus=1 and intCatNo='$MainCategory'";
		$result = $db->RunQuery($sql);		
		while($row = mysql_fetch_array($result))
			{
    ...
    <tr>
	  ...
	  $sql = "select intItemSerial,strItemDescription,strUnit,dblLastPrice
			  from genmatitemlist where intStatus='$status' ";
	  ...
	  ?>
            <tr class="bcgcolor-tblrowWhite">
              ...
              <td class="normalfntMid" id="<?php echo $row['intItemSerial']; ?>"><input name="txtUnitPrice" id="txtUnitPrice" style="text-align:right;width:100px" class="txtbox keymove" type="text" value="<?php echo $row['dblLastPrice']; ?>" onblur="updateUnitPrice(this,this.value);" onkeypress="return CheckforValidDecimal(this.value,3,event);" /></td>

					...
					HTMLText += "</table>"+
							...
							"<tr>"+
							 "<td class=\"normalfnt\">Reorder Level</td>"+
							  "<td ><input name=\"txtReLevel\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\"  type=\"text\" id=\"txtReLevel\" maxlength=\"10\" style=\"width:90px;text-align:right\" tabindex=\"7\" /></td>"+
...
{   
	...
	for ( var loop = 1 ;loop < tbl.rows.length ; loop ++ )
	{
		...
		propIDlist+=PropID+",";
		propValue = (propValue==''?'N/A':propValue);
...
/*	if (mainCatID == 1)
	...
	var unitID 			= document.getElementById("cboUnits").value;
	var purchaseID 		= document.getElementById("cboPurchaseUnit").value;

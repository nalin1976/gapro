...
{
	var url					= 'frmItemWiseLiabilityxml.php?RequestType=load_OrderNo';
...
function ClearForm()
{
	...
	location = "frmItemWiseLiability.php";
}
...
{
	var url  = "frmItemWiseLiabilityxml.php?RequestType=URLSetStyleNo";
		url += "&OrderId="+orderId;
...
{
	var url  = "frmItemWiseLiabilityxml.php?RequestType=URLLoadSubStore";
		url += "&MSId="+document.getElementById('cboMainStore').value;
...
{
	var url  = "frmItemWiseLiabilityxml.php?RequestType=URLLoadOrderNo";
...
{
	var url  = "frmItemWiseLiabilityxml.php?RequestType=URLLoadSCNo";
		url += "&StyleNo="+URLEncode(styleNo);
	htmlobj=$.ajax({url:url,async:false});
...
function LoadPopItems()
{
	...
	var url  = "frmItemWiseLiabilityxml.php?RequestType=URLLoadPopItems";
...
function AddToMainPage()
{
	...
	for(loop=1;loop<tbl.rows.length;loop++)
	{
		if(tbl.rows[loop].cells[p_chkBox ].childNodes[0].checked)
		{
			...
			{
				var mainMatId 		= tblMain.rows[mainLoop].cells[m_iDec].id;
				var mainOrderId 	= tblMain.rows[mainLoop].cells[m_order].id;
				var mainColor 		= tblMain.rows[mainLoop].cells[m_co].childNodes[0].nodeValue;
				var mainSize 		= tblMain.rows[mainLoop].cells[m_size].childNodes[0].nodeValue;
				var mainBuyerPo		= tblMain.rows[mainLoop].cells[m_bupo].childNodes[0].nodeValue;
				var mainGRNNo		= tblMain.rows[mainLoop].cells[m_grnno].childNodes[0].nodeValue;
				var mainGRNTypeId	= tblMain.rows[mainLoop].cells[m_grntype].id;
			...
			if(booCheck)
				CreadeMainGrid(tblMain,orderId,orderNo,matId,itemDesc,buyerPoNo,units,color,size,parseFloat(stockBal),grnNo,grnTypeId,grnType,status,cls,binAllo,Qty);
		}
	}
	CloseIWLOItem('popupLayer1');
}

function CreadeMainGrid(tbl,orderId,orderNo,matId,itemDesc,buyerPoNo,units,color,size,stockBal,grnNo,grnTypeId,grnType,status,cls,binAllo,Qty)
{
		...
		cell.className 	= "normalfnt";
		cell.innerHTML 	= "<input type=\"text\" onkeypress=\"return CheckforValidDecimal(this.value, 4,event);\" style=\"width: 81px; text-align: right;\" class=\"txtbox\"  value=\""+Qty+"\" onkeyup=\"SetQtyToArray(this,"+mainArrayIndex+");\">";
...
{
	var url  = "frmItemWiseLiabilityxml.php?RequestType=URLGetNewSerialNo";
	htmlobj=$.ajax({url:url,async:false});
...
function SaveHeader()
{
	var url  = "frmItemWiseLiabilityxml.php?RequestType=URLSaveHeader";
	    url += "&SerialNo="+pub_serialNo;
		url += "&SerialYear="+pub_serialYear;
		url += "&mainstoreId="+document.getElementById("cboMainStore").value;
	htmlobj=$.ajax({url:url,async:false});
}
...
function SaveDetails()
{
	...
	var url = 'frmItemWiseLiabilityxml.php?RequestType=URLSaveDetails&SerialNo=' +pub_serialNo+ '&SerialYear=' +pub_serialYear+ '&StyleID=' +URLEncode(orderId)+ '&BuyerPoNo=' +URLEncode(buyerPoNo)+ '&ItemDetailId=' +matId+ '&Color=' +URLEncode(color)+ '&Size=' +URLEncode(size)+ '&Units=' +URLEncode(units)+ '&Qty=' +qty+ '&GRNNo='+grnNo+ '&GRNType='+grnTypeId;
	var htmlobj=$.ajax({url:url,async:false});
...
function LoadSubCat(styleID)
{
	...
	var url = "frmItemWiseLiabilityxml.php?RequestType=URLloadPopupSubCategory&mainCat="+mainCat+"&styleID="+styleID;
	var htmlobj=$.ajax({url:url,async:false});
...
function LoadPopUpNo()
{
	...
	var url = "frmItemWiseLiabilityxml.php?RequestType=URLloadPopupSerialNo";
	url += "&status="+state;
	url += "&year="+year;
	var htmlobj=$.ajax({url:url,async:false});
...
function loadPopUpLiabilityDetails()
{
	Materials 			= [];
	mainArrayIndex =0;
	var liabilityNo = document.getElementById('cboNo').value;
	var liabilityYear = document.getElementById('cboYear').value;
	var state	= document.getElementById('cboState').value;
	document.getElementById('txtSerialNo').value = liabilityYear+'/'+liabilityNo;
	
	var url_h = "frmItemWiseLiabilityxml.php?RequestType=URLgetSavedHeaderDetails&liabilityNo="+liabilityNo;
	url_h += "&liabilityYear="+liabilityYear+"&state="+state;
	var htmlobj_h =$.ajax({url:url_h,async:false});
	
	document.getElementById('cboMainStore').value = htmlobj_h.responseText;
	LoadSubStore();
	RestrictInterface(state);
	
	var url_d = "frmItemWiseLiabilityxml.php?RequestType=URLgetSavedLiabilityDetails&liabilityNo="+liabilityNo;
	url_d += "&liabilityYear="+liabilityYear+"&state="+state;
	var htmlobj_d =$.ajax({url:url_d,async:false});
	
	var XMLMatDetailId 	= htmlobj_d.responseXML.getElementsByTagName("intMatDetailId");
	var tbl 		= document.getElementById('tblPopItem');
	ClearTable('tblMain');
	var tblMain =document.getElementById('tblMain');
	var binAllo=1;
	...
	{
		var orderId 	= htmlobj_d.responseXML.getElementsByTagName("intStyleId")[loop].childNodes[0].nodeValue;
		var orderNo 	= htmlobj_d.responseXML.getElementsByTagName("strOrderNo")[loop].childNodes[0].nodeValue;
		var matId 	= htmlobj_d.responseXML.getElementsByTagName("intMatDetailId")[loop].childNodes[0].nodeValue;
		var itemDesc 	= htmlobj_d.responseXML.getElementsByTagName("strItemDescription")[loop].childNodes[0].nodeValue;
		var buyerPoNo 	= htmlobj_d.responseXML.getElementsByTagName("strBuyerPoNo")[loop].childNodes[0].nodeValue;
		var units 	= htmlobj_d.responseXML.getElementsByTagName("strUnit")[loop].childNodes[0].nodeValue;
		var size 	= htmlobj_d.responseXML.getElementsByTagName("strSize")[loop].childNodes[0].nodeValue;
		var stockBal 	= htmlobj_d.responseXML.getElementsByTagName("stockBal")[loop].childNodes[0].nodeValue;
		var grnNo 	= htmlobj_d.responseXML.getElementsByTagName("GrnNo")[loop].childNodes[0].nodeValue;
		var grnTypeId 	= htmlobj_d.responseXML.getElementsByTagName("grnTypeId")[loop].childNodes[0].nodeValue;
		var grnType 	= htmlobj_d.responseXML.getElementsByTagName("strGRNType")[loop].childNodes[0].nodeValue;
		var dblQty 	= htmlobj_d.responseXML.getElementsByTagName("dblQty")[loop].childNodes[0].nodeValue;
		var color =  htmlobj_d.responseXML.getElementsByTagName("strColor")[loop].childNodes[0].nodeValue;
		
		CreadeMainGrid(tblMain,orderId,orderNo,matId,itemDesc,buyerPoNo,units,color,size,parseFloat(stockBal),grnNo,grnTypeId,grnType,state,cls,binAllo,dblQty);
		
		var url = "frmItemWiseLiabilityxml.php?RequestType=URLgetSavedBinDetails&liabilityNo="+liabilityNo;
		url += "&liabilityYear="+liabilityYear+"&matId="+matId+"&buyerPoNo="+URLEncode(buyerPoNo);
		url += "&color="+URLEncode(color)+"&size="+URLEncode(size)+"&grnNo="+grnNo+"&grnTypeId="+grnTypeId;
		url += "&state="+state+"&orderId="+orderId;
		var htmlobj =$.ajax({url:url,async:false});
		closeFindReturn();
		var XMLMainStore 	= htmlobj.responseXML.getElementsByTagName("MainStoresID");
		var BinMaterials = [];
		var mainBinArrayIndex = 0;
			for ( var i = 0;i<XMLMainStore.length;i++){
				
				var Bindetails = [];
				Bindetails[0] =   parseFloat(htmlobj.responseXML.getElementsByTagName("Qty")[i].childNodes[0].nodeValue); // issueQty
				Bindetails[1] =   htmlobj.responseXML.getElementsByTagName("MainStoresID")[i].childNodes[0].nodeValue; // MainStores
				Bindetails[2] =    htmlobj.responseXML.getElementsByTagName("SubStores")[i].childNodes[0].nodeValue;// SubStores
				Bindetails[3] =  htmlobj.responseXML.getElementsByTagName("Location")[i].childNodes[0].nodeValue; // Location
				Bindetails[4] =  htmlobj.responseXML.getElementsByTagName("Bin")[i].childNodes[0].nodeValue; // BinId							
				BinMaterials[mainBinArrayIndex] = Bindetails;
				mainBinArrayIndex ++ ;						
				
			}
		var pub_index=	mainArrayIndex-1;
		Materials[pub_index][5] = BinMaterials;				
...
function ViewReport()
{
	var SerialNo = document.getElementById('txtSerialNo').value;
	if(SerialNo == '')
	{
		alert("Serial No not available");
		return false;
	}	
	var url = "rptItemLiability.php?No="+SerialNo;	
	window.open(url,'frmStyleStockAdjestment');
	
}

function cancelLiability()
{
	var SerialNo = document.getElementById('txtSerialNo').value; 
	var url = "frmItemWiseLiabilityxml.php?RequestType=URLCancelLiability&SerialNo="+SerialNo;
	var htmlobj =$.ajax({url:url,async:false});
	var response = htmlobj.responseText;
	document.getElementById("butCancel").style.display="none";
	if(response==1)
	{
		alert("Liability Details Canceled Successfully");
		return false;
	}
	else
		alert(response);
...
{
		var reportName = "rptItemWiseLiabilityList.php";

...
WHERE (((orders.intStyleId)='".$strStyleID."'));";
		...
		{			
			...
			$productCategory 		= $row['strCatName'];
                        $dblApprovedEPMLevel    = $row["dblEPMLevel"];
                        $reaPackSMV = $row["reaPackSMV"];
		}
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td class="bcgl1txt1"><?php echo number_format($reaSMV+$reaPackSMV,2); $mobileSMV = $reaSMV + $reaPackSMV; ?></td>
          ...
          <tr>
            ...
            <td bgcolor="#BED6E9" class="bcgl1txt1B">
                <table width="100%">
                    <tr><td width="50%">S.SMV</td>
                        <td width="50%">P.SMV</td>
                    </tr>
                </table>
            </td>
          ...
          <tr>
            ...
            <td class="bcgl1txt1" id="gpfobnet">
                <table width="100%">
                    <tr><td width="50%"><?php echo round($reaSMV,2); ?></td>
                        <td width="50%"><?php echo $reaPackSMV; ?></td>
                    </tr>
                </table>
            </td>
      ...
      <tr>
        ...
        <thead>
				...
				{					
					...
					{	
						...
						echo "<td class=\"normalfntRiteTAB\">".number_format($row_order["dblTotalValue"],2)."</td>";	
                                                ...
                                                * ========================================================================
                                                */
                                                $itemcode = $row_order["intItemSerial"];
          ...
          <tr>
           ...
           <?php
         	...
         	$requiredMinimumCM = ($CMVAL * $BuyingOfficeMargin / 100);
                //echo "CM VAL = '$CMVAL' & Labour Cost = '$costinglabourCost' & Min CM = '$requiredMinimumCM' & Buying Office Profit - '$buyingOfficeProfit'  ";
         	$bgColor = "";
          ...
          $coporatecost = $reaFOB * 0.05;
			...
			if (($intQty + ($intQty * $reaExPercentage / 100)) == $subcontractQty)
			{
					...
					if ($inhouseStyle)
					{
						...
						$labsuboverheadcost =  ($reaSMV + $reaPackSMV) / ($reaEfficiencyLevel / 100 )* $companycostperminute;
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						$labsuboverheadcost = $subcontractcost + ( ($reaSMV + $reaPackSMV)/ ($reaEfficiencyLevel / 100) * $companycostperminute) ;
					}
				}
			}
          ...
          <tr>
			...
			echo $mobileCorpCost = number_format(($reaSMV+$reaPackSMV) * 0.0234,4);
			?></td>
            <td class="normalfntRiteTAB"><?php
			...
			echo number_format((round($intQty) * (($reaSMV+$reaPackSMV) * 0.0234)),2);
			# --------------------------------------------------------------------------------------------------
			?></td>
          </tr>
          <?php
		...
		<?php
			$SQL = "select intStyleId,intPartId,intPartNo,strPartName,dblsmv,dblSmvRate,dblEffLevel, dblPackSMV from stylepartdetails where intStyleId = '$strStyleID'";
			$result = $db->RunQuery($SQL);
			$rowId = 0;
			while($row = mysql_fetch_array($result))
			{
		?>
		  ...
		  ?> </td>
            ...
            <td width="6%" bordercolor="#666666" class="normalfntTAB"><?php echo number_format($row["dblsmv"] + $row["dblPackSMV"],2);?> </td>
            ...
            <td width="6%" bordercolor="#666666" class="normalfntTAB"><?php $um =  ($reaSMVRate * ($row["dblsmv"] + $row["dblPackSMV"])) / (($row["dblsmv"] + $row["dblPackSMV"]) / ($row["dblEffLevel"] / 100));  
																		echo number_format($um,4);
                                                                                //$um = ($reaSMVRate * ($reaSMV + $reaPackSMV)) / (($reaSMV + $reaPackSMV) / ($reaEfficiencyLevel / 100));                                                                
																		?></td>

...
/*			var state=htmlobj2.responseText;
	...
	var path = "../Details/db.php?id=checkForSecondApproval";
		path += "&grnNo="+no;
		path += "&grnYear="+year;
	htmlobj3=$.ajax({url:path,async:false});
	
	if(htmlobj3.responseText == 1){
	 alert("Excess Qty exceeds the Material Ratio Qty. Therefore you have to get the second approval");

	 document.getElementById("bottomTD").style.visibility	=	"hidden";
	 location = "expoconfirmationreport.php?serialNo="+ no + "&serialYear=" + year;
	}else{
		var path = "../Details/db.php?id=ConfirmExGrn";
		path += "&grnNo="+no;
		path += "&grnYear="+year;
	htmlobj4=$.ajax({url:path,async:false});
	
		    var state=htmlobj4.responseText;
			//alert(state);
			if(parseInt(state)=="1")
			{				
				location = "exreportpurchase.php?serialNo="+ no + "&serialYear=" + year;
...
<table width="800" border="0" align="center" cellpadding="0" celspacing="0">
  <tr>
    <td colspan="4">
		<table width="100%">
		<tr>
		<td colspan="3"> <?php include "exreportpurchase.php" ?></td>		
		</tr>
		<?PHP if($poStatus==0){ ?>
			<tr bgcolor="#d6e7f5" >
				...
				<td id="bottomTD"><div align="right"><img src="../../images/approve.png" class="mouseover" onclick="deductQtyFromMatRatio();" alt="view" /></div></td>
			</tr>
			<?php } ?>
		</table>    
    </td>

...
if($id=="addItems")
{
		...
		$ResponseXML .= "<Item>";
		$SQL= "SELECT strItemDescription AS Description,specification.intStyleId,specification.intSRNO,purchaseorderdetails.strUnit AS strUnit,dblPending AS Qty,strColor , strSize,strBuyerPONO ,purchaseorderdetails.intMatDetailID,purchaseorderdetails.dblInvoicePrice AS dblUnitPrice,purchaseorderdetails.strRemarks AS Remarks,orders.strStyle,orders.intStatus,orders.strOrderNo,orders.strStyle,
		purchaseorderdetails.dblQty as POQty,matitemlist.intSubCatID
		 FROM purchaseorderdetails ,matitemlist,specification,orders  WHERE purchaseorderdetails.intMatDetailId = matitemlist.intitemserial AND purchaseorderdetails.intStyleId  = specification.intStyleId AND orders.intStyleId= purchaseorderdetails.intStyleId AND intPoNo= $strPoNo AND intPOType=0
		...
		$result = $db->RunQuery($SQL);
			...
			while($row = mysql_fetch_array($result))
			{
				...
				else
				{
				 ...
				 $ResponseXML .= "<strBuyerPONO><![CDATA[" . $bpoNo  . "]]></strBuyerPONO>\n";

...
/*	if(!ValidateItemAddingForm()) 
	...
	drawPopupArea(950,465,'frmItems');
	var HTMLText ="<table width=\"950\" height=\"412\" border=\"0\" align=\"center\" bgcolor=\"#FFFFFF\">"+
	" <tr>"+
	...
	"	  <tr>"+
	"		<td width=\"100%\" class=\"normalfnt\"><div id=\"divItem\" style=\"overflow:scroll; height:260px; width:940px;\">"+
			...
			try
			{
				...
				var tableText = "<table width=\"960\" cellpadding=\"1\" cellspacing=\"1\" id=\"tblItem\" bgcolor=\"#D9DFDD\">"+
								...
								"<td width=\"5%\" bgcolor=\"#498CC2\" class=\"normaltxtmidb2L\">Unit Price</td>"+
                                                                "<td width=\"5%\" bgcolor=\"#498CC2\" class=\"normaltxtmidb2L\">Delivery Date</td>"+
								"<td width=\"8%\" bgcolor=\"#498CC2\" class=\"normaltxtmidb2L\">Buyer</td>"+
								"</tr>";
				...
				tableText +=" <tr class="+strColor+">"+
					...
					else
					{
						...
						" <td class=\"normalfntRite\">"+ XMLdblUnitPrice[loop].childNodes[0].nodeValue +"</td>"+
                                                " <td class=\"normalfntRite\">"+XMLestimateDate[loop].childNodes[0].nodeValue+"</td>"+
  	...
  	//{
		...
		if (cbo.checked) {
			...
			var matSubCat = tbl.rows[loop].cells[10].id;
                        ...
                        // Add For - If delivery date empty not add line to the main grid
                        // Add On - 11/07/2016
                        // =====================================================
                        var dtDeliveryDate = "";
                        if(tbl.rows[loop].cells[12].lastChild !== null){
                            dtDeliveryDate = tbl.rows[loop].cells[12].lastChild.nodeValue;
                        }
                        
                        // =====================================================
                        if(dtDeliveryDate == "" && strBuyerPoNo != "#Main Ratio#"){
                            return;
                        }
			//alert(styleID);
...
function calculation1(sCell)
{
	...
	var Rate=parseFloat(sCell.parentNode.parentNode.cells[8].lastChild.nodeValue);
	var FixQty = parseFloat(sCell.parentNode.parentNode.cells[10].lastChild.nodeValue).toFixed(2);
	var Qty = parseFloat(sCell.parentNode.parentNode.cells[11].lastChild.value).toFixed(2);
	...
	sCell.parentNode.parentNode.cells[15].lastChild.nodeValue=Math.round((Qty*Rate)*1000)/1000;
		...
		if(FixQty<Qty)
		{
			...
			{
				sCell.parentNode.parentNode.cells[13].lastChild.nodeValue=parseFloat(Qty-FixQty).toFixed(2);
				sCell.parentNode.parentNode.cells[14].lastChild.nodeValue=0;
			}
		...
		else
		{
			...
			sCell.parentNode.parentNode.cells[14].lastChild.nodeValue=parseFloat(FixQty-Qty).toFixed(2);
		}
...
function fnSticker(){
	...
	path += "/Details/rptSticker.php?grnno="+document.getElementById("txtgrnno").value+'&grnYear='+grnYear+'&poNo='+document.getElementById('txtpono').value;

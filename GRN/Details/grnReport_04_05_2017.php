...
<table width="800" border="0" align="center" >
  ...
  echo $SQL_alldetails="select grnheader.intGrnNo,grnheader.intGRNYear,grnheader.strInvoiceNo,grnheader.strPayDisReason ,Date(grnheader.dtmRecievedDate) as dtmRecievedDate,grnheader.intStatus as grnStatus,grnheader.strCusdecNo,strEntryNo,
    ...
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tablez">
        ...
        <?php
         ...
         ?>
          ...
          <td width="18%" class="normalfntBtab">Remarks</td>
...
left join orders on orders.intStyleId = grndetails.intStyleId
 ...
 AND purchaseorderdetails.intPOType=0";*/
  ...
  if($grnStatus==0)
          { 
		  	$tableName='stocktransactions_temp';
		  }
		  	else
		  {
...
FROM
$tableName ST
INNER JOIN grnheader ON ST.intGrnNo = grnheader.intGrnNo AND ST.intGrnYear = grnheader.intGRNYear
INNER JOIN purchaseorderdetails ON grnheader.intPoNo = purchaseorderdetails.intPoNo AND grnheader.intYear = purchaseorderdetails.intYear AND ST.intStyleId = purchaseorderdetails.intStyleId AND ST.intMatDetailId = purchaseorderdetails.intMatDetailID AND ST.strColor = purchaseorderdetails.strColor AND ST.strSize = purchaseorderdetails.strSize AND ST.strBuyerPoNo = purchaseorderdetails.strBuyerPONO
...
INNER JOIN grndetails ON grnheader.intGrnNo = grndetails.intGrnNo AND grnheader.intGRNYear = grndetails.intGRNYear AND ST.intStyleId = grndetails.intStyleId AND ST.strBuyerPoNo = grndetails.strBuyerPONO AND ST.intMatDetailId = grndetails.intMatDetailID AND ST.strColor = grndetails.strColor AND ST.strSize = grndetails.strSize
INNER JOIN storeslocations ON ST.strLocation = storeslocations.strLocID AND ST.strMainStoresID = storeslocations.strMainID AND ST.strSubStores = storeslocations.strSubID
INNER JOIN matitemlist ON grndetails.intMatDetailID = matitemlist.intItemSerial
INNER JOIN storesbins ON ST.strBin = storesbins.strBinID AND ST.strMainStoresID = storesbins.strMainID AND ST.strLocation = storesbins.strLocID AND ST.strSubStores = storesbins.strSubID
WHERE
...
ST.strType = 'GRN' 
 AND purchaseorderdetails.intPOType=0 ";
		...
		{		
			...
			$binLocation = $row["strLocName"]. "<br/>" .$row["strBinName"];
...
//	//echo $sqlLoc;
	...
	?><?php */?>
     <td class="normalfntTAB" "<?php //echo  $locationCls;?>"><?php echo $binLocation; ?> </td>
			<!--</td>-->
          <?php
          }
          ?>
        ...
        <tr> 
          ...
          <td class="normalfntRiteTAB" style="vertical-align:middle;">&nbsp;</td>
          <td class="normalfntRiteTAB" style="vertical-align:middle;"><?php echo $sum;?></td>
			 <?php
        
         if($DisplayLocationInReport == "true")
         {         
         ?>
         <td width="9%" height="35" class="normalfntTAB" style="vertical-align:middle;"><b><?php echo  number_format($sumvalue,2);?></b></td>
         <?php
         }
         ?>
        </tr>

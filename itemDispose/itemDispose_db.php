...
echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
	...
	//Get Remarks field 
	$_Remarks = addslashes($_GET["setRemarks"]);
	
	
	//insert data to item disposal data
	insertItemDisposeData($disNo,$intYear,$_Remarks);
...
function insertItemDisposeData($disNo,$intYear,$prmRemarks)
{
	global $db;
	global $user;
		
	$sql = "insert into itemdispose	(intDocumentNo,intYear,intStatus,intPreparedBy,dtmPDate,strRemarks) values ('$disNo','$intYear','1','$user',now(),'$prmRemarks')";
	//echo $sql;
	$res = $db->RunQuery($sql);
}

...
$tbl="";
	...
	$sqlS="SELECT
		...
		$tbl.intDocumentYear =  '$year' AND 
		$tbl.strType = 'StyleDispose'"; 
		
		/* Add 'StyleDispose' as a filter 
		   On - 10/01/2014 Nalin Jayakody
		 */
...
<table width="1000" border="1" align='center' CELLPADDING=1 CELLSPACING=1 rules="rows" >
    <tr>
     ...
     <td class='bcgl1txt1NB' align="left" width="45"><?php echo GetDisposseDate($documentNo, $year);?></td>
	</tr>
    <tr>
    	<td class='bcgl1txt1NB' align="left" width="76">Remarks :</td>
        <td class="normalfnt" colspan="3"><?php echo GetDisposeRemark($documentNo, $year); ?></td>
    </tr>
</table>
<table width="1000" border="1" align='center' CELLPADDING=1 CELLSPACING=1 rules="rows" >
      <tr>
	  <td class='bcgl1txt1NB' align="left" width="69">Order No</td>
      <!--<td class='bcgl1txt1NB' align="left" width="46">SC No</td>-->
      <td class='bcgl1txt1NB' align="left" width="96">Item Code</td>
	  <td class='bcgl1txt1NB' align="left" width="72">GRN No</td>
	  <td class='bcgl1txt1NB' align="left" width="72">Buyer PO</td>
      <td class='bcgl1txt1NB' align="left" width="193">Material Description</td>
      <td class='bcgl1txt1NB' align="left" width="84">Color</td>
      <td class='bcgl1txt1NB' align="left" width="59">Size</td>
      
      <td class='bcgl1txt1NB' align="left" width="83">Main Stores</td>
      <!--<td class='bcgl1txt1NB' align="left" width="90">Sub Stores</td>-->
      <td class='bcgl1txt1NB' align="left" width="64">Location</td>
      <td class='bcgl1txt1NB' align="left" width="35">Bin</td>        
	  <td class='normalfntRite' align="right" width="64"><strong>Dispose Qty</strong>&nbsp;</td>
      ...
      <?php while($rowD=mysql_fetch_array($resD))
	  {
		...
		#=================================================================================
		# Adding On - 08/15/2014 
		# Add itemcode from material ratio to the report
		#=================================================================================
		
		$sqlItemCode = " SELECT materialRatioID FROM materialratio WHERE intStyleId = ".$rowD['intStyleId']." AND strMatDetailID = ".$rowD['intMatDetailId']." AND strColor = '".$rowD['strColor']."' AND strSize = '".$rowD['strSize']."' AND strBuyerPONO = '".$rowD['strBuyerPoNo']."'";
				
		$resItemCode = $db->RunQuery($sqlItemCode);
		
		while($rowItemCode = mysql_fetch_array($resItemCode)){
			$strItemCode = $rowItemCode['materialRatioID'];	
		}
	  ...
	  ?>
      <tr>
	  <td class='normalfnt' align="left" width="69"><?php echo $rowD['strOrderNo'];?></td>
      <!--<td class='normalfnt' align="left" width="46"><?php echo $rowD['intSRNO'];?></td>-->
      <td class='normalfnt' align="left" width="96"><?php echo $strItemCode;?></td>
	  <td class='normalfnt' align="left" width="72"><?php echo $rowD['intGrnYear']."/".$rowD['intGrnNo'];?></td>
	  <td class='normalfnt' align="left" width="72"><?php echo $rowD['strBuyerPoNo'];?></td>
      <td class='normalfnt' align="left" width="193"><?php echo $rowD['strItemDescription'];?></td>
	  <td class='normalfnt' align="left" width="84"><?php echo $rowD['strColor'];?></td>
      <td class='normalfnt' align="left" width="59"><?php echo $rowD['strSize'];?></td>      
      <td class='normalfnt' align="left" width="83"><?php echo $rowD['strName'];?></td>
      <!--<td class='normalfnt' align="left" width="90"><?php echo $rowD['strSubStoresName'];?></td>-->
      <td class='normalfnt' align="left" width="64"><?php echo $rowD['strLocName'];?></td>
      <td class='normalfnt' align="left" width="35"><?php echo $rowD['strBinName'];?></td>    
	  <td class='normalfntRite' align="left" width="64"><?php echo ($rowD['dblQty']*(-1));?>&nbsp;</td>
...
<table width="1000" border="0" align="center" class="">&nbsp;	
	...
	<tr>
	  <td height="65" colspan="10" align="center" id="confrimDiv">
...
function GetDisposseDate($prmdocumentNo, $prmyear){
	
	global $db;
	
	$_dtDisspose;
	
	$s="select dtmPDate from itemdispose where intDocumentNo='$prmdocumentNo' and intYear='$prmyear';";	
	$r=$db->RunQuery($s);
	
	if(mysql_num_rows($r)>0){
	
	$rw=mysql_fetch_array($r);
		$_dtDisspose = substr($rw['dtmPDate'],0,10);
	}	
	
	return $_dtDisspose;
}

function GetDisposeRemark($prmdocumentNo, $prmyear){
	
	global $db;
	
	$_sRemarks;
	
	$sql = "select strRemarks from itemdispose where intDocumentNo='$prmdocumentNo' and intYear='$prmyear';";
	$res =$db->RunQuery($sql);
	
	if(mysql_num_rows($res)>0){
	
	$rw=mysql_fetch_array($res);
		$_sRemarks = $rw['strRemarks'];
	}

...
else if (strcmp($RequestType,"plConBpo") == 0)
{
	 $styleNo=$_GET["styleNo"];
	 $bpoNo = $_GET["bpoNo"]; 
	 $prvBpoNo = $_GET["prvBpoNo"]; 

	
	$sqlUpdates="INSERT INTO bpo_planning_con (
	 ...
	 while($row = mysql_fetch_array($result))
  	 {
            ...
            /*$ResponseXML .= "<PONO><![CDATA[" . $row["strBuyerPoName"]  . "]]></PONO>\n"; 
            ...
            $ResponseXML .= "<BuyerPoName><![CDATA[" . $row["strBuyerPONO"]  . "]]></BuyerPoName>\n";*/
...
else if(strcmp($RequestType,"SaveMatRatio") == 0)
{
	...
	SaveMaterialRatio($styleNo,$buyerPONo,$item,$color,$size,$qty,$balQty,$userID,$dblFreightBal,$matcde,$recutQty,$sizeserial,$yyQty,$wastage,$moq, $isFabric);
}
...
else if (strcmp($RequestType,"GetStyleRatio") == 0)
{
	...
	else
	{
		 ...
		 //$sql_bp = " SELECT dblQty FROM deliveryschedule WHERE deliveryschedule.intBPO = '$buyerPONo' AND deliveryschedule.intStyleId = '$styleNo'";
                 $sql_bp = " SELECT deliveryschedule.dblQty
                             ...
                             WHERE style_buyerponos.strBuyerPoName = '$buyerPONo' AND deliveryschedule.intStyleId = '$styleNo' ";
		 //echo $sql_bp;
		 $result_bp=$db->RunQuery($sql_bp);
		 $row_bp=mysql_fetch_array($result_bp);
...
else if (strcmp($RequestType,"GetMatRatio") == 0)
{
	 ...
	 $userId       = $_SESSION["UserID"]; 
		 ...
		 $chkBpo=chkBpoInMatRatio($styleNo,$buyerPONo,$ItemID);
		 
		 if($chkBpo>0){
             
             $buyerPONo=$buyerPONo;
         }else{
             $buyerPONo=getPrvBpo($styleNo,$buyerPONo);
...
elseif ($RequestType=="GetItemWastage") {
    
    $styleId = $_GET['styleId'];
    $ItemID  = $_GET['itemcode'];
    
    $sql = "select dblPSP,dblDP,dblEMB,dblPEOSP,dblDPE,dblGEW,dblGPD,dblSPE,dblPEODP,intEmbPanels,intPrnPanels,dblGRW,intLgPanels from orderdetails where intStyleId = '$styleId' and intMatDetailID = '$ItemID' ";
    //$sql = "select dblPSP,dblDP,dblEMB,dblPEOSP,dblDPE,dblGEW,dblGPD,dblSPE,dblPEODP,intEmbPanels,intPrnPanels,dblGRW,intLgPanels from specificationdetails where intStyleId = '$styleId' and strMatDetailID = '$ItemID' ";
    $result = $db->RunQuery($sql);
    ...
    //echo $sql;
    $ResponseXML = "";
    $ResponseXML = "<ITEM_WASTAGES>\n";
    
    while($row = mysql_fetch_array($result)){
        
        $ResponseXML.="<PSP_WASTAGE><![CDATA[" .$row["dblPSP"]. "]]></PSP_WASTAGE>\n";
        $ResponseXML.="<DP_WASTAGE><![CDATA[" .$row["dblDP"]. "]]></DP_WASTAGE>\n";
        $ResponseXML.="<EMB_WASTAGE><![CDATA[" .$row["dblEMB"]. "]]></EMB_WASTAGE>\n";
        $ResponseXML.="<PEOSP_WASTAGE><![CDATA[" .$row["dblPEOSP"]. "]]></PEOSP_WASTAGE>\n";
        $ResponseXML.="<DPE_WASTAGE><![CDATA[" .$row["dblDPE"]. "]]></DPE_WASTAGE>\n";
        $ResponseXML.="<GEW_WASTAGE><![CDATA[" .$row["dblGEW"]. "]]></GEW_WASTAGE>\n";
        $ResponseXML.="<GPD_WASTAGE><![CDATA[" .$row["dblGPD"]. "]]></GPD_WASTAGE>\n";
        $ResponseXML.="<SPE_WASTAGE><![CDATA[" .$row["dblSPE"]. "]]></SPE_WASTAGE>\n";
        $ResponseXML.="<PEODP_WASTAGE><![CDATA[" .$row["dblPEODP"]. "]]></PEODP_WASTAGE>\n";
        $ResponseXML.="<GRW_WASTAGE><![CDATA[" .$row["dblGRW"]. "]]></GRW_WASTAGE>\n";
        $ResponseXML.="<NO_LG_PANELS><![CDATA[" .$row["intLgPanels"]. "]]></NO_LG_PANELS>\n";
        $ResponseXML.="<PRINT_PANELS><![CDATA[" .$row["intPrnPanels"]. "]]></PRINT_PANELS>\n";
...
function getBuyerPOListByStyle($styleID)
{
	...
	//$sql = " SELECT deliveryschedule.intBPO, deliveryschedule.dblQty, deliveryschedule.intCountry FROM deliveryschedule WHERE deliveryschedule.intStyleId = '" . $styleID . "';";
        ...
        $sql = " SELECT DISTINCT deliveryschedule.intBPO, deliveryschedule.dblQty, deliveryschedule.intCountry,
                  style_buyerponos.strBuyerPoName
                 FROM deliveryschedule INNER JOIN style_buyerponos ON deliveryschedule.intStyleId = style_buyerponos.intStyleId AND deliveryschedule.intBPO = style_buyerponos.strBuyerPONO
                 WHERE deliveryschedule.intStyleId = '" . $styleID . "';";
	return $db->RunQuery($sql);
}
...
function SaveMaterialRatio($styleNo,$buyerPONo,$item,$color,$size,$qty,$balQty,$userID,$dblFreightBal,$materialRatioID,$recutQty,$sizeserial,$yyQty,$wastage,$moq, $isFabric)
{
...
materialratio.strSize = '$size'"; 
				...
				if(mysql_num_rows($result2))
				{
					...
					//$sql="UPDATE `materialratio` SET `strColor`='$color',`strSize` = '$size', `dblQty` = '$qty' ,`dblBalQty` = '$balQty' , `dblFreightBalQty` = '$dblFreightBal', `serialNo`  = '$sizeserial' ,  `dblRecutQty`  = '$recutQty' ,`intStatus` = '1' WHERE (`intStyleId`='$styleNo') AND (`strMatDetailID`='$item') AND (`strBuyerPONO`='$buyerPONo')AND (`strColor`='$color') AND (`strSize`='$size');";
                                        ...
                                        if(($isFabric == 1) && ($buyerPONo != "#Main Ratio#") && ($yyQty == 0)){
                                            $sql="UPDATE `materialratio` SET `strColor`='$color',`strSize` = '$size', `dblQty` = '0' ,`dblBalQty` = '0' , `dblFreightBalQty` = '0', `serialNo`  = '$sizeserial' ,  `dblRecutQty`  = '$recutQty' ,intStatus = '1',dblYYConsumption= '$yyQty', dblWastage='$wastage', intMoq='$moq'  WHERE (`intStyleId`='$styleNo') AND (`strMatDetailID`='$item') AND (`strBuyerPONO`='$buyerPONo')AND (`strColor`='$color') AND (`strSize`='$size');";
                                        }else{
...
function chkBpoInMatRatio($styleNo,$buyerPONo,$ItemID){
 ...
 $sql = "SELECT
	...
	materialratio.dblYYConsumption,
	materialratio.dblWastage,
	materialratio.intMoq
FROM
	materialratio
WHERE
	intStyleId = '$styleNo'
AND strBuyerPONO = '$buyerPONo'
AND strMatDetailID = '$ItemID'
GROUP BY
materialratio.intStyleId,
materialratio.strBuyerPONO";
...
while($row = mysql_fetch_array($result_to)){
		...
		//$rowCount = $row["strBuyerPONO"];
		$num_rows = mysql_num_rows($result_to);
	}  
//echo "XXX".$num_rows."A";
	return $num_rows;  
...
function getPrvBpo($styleNo,$buyerPON){
...
deliveryschedule
WHERE
deliveryschedule.intStyleId = '$styleNo' AND
deliveryschedule.intBPO = '$buyerPON'";

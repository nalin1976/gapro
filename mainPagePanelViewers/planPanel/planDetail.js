...
function Confirm(Ids){
		...
		var tbl = document.getElementById('plantbl2');
		//alert("len = "+tbl.rows.length);
		var stlNo=tbl.rows[Number(Ids.id)+1].cells[1].id;
		var bpno=tbl.rows[Number(Ids.id)+1].cells[3].childNodes[0].nodeValue;
		//alert("bpno = "+bpno);
		var chk=document.getElementById(Ids.id).checked;

		if(chk==true){
			
			var path= pnl_planUrl+"planDetailXML.php?RequestType=Confirm&StyleID=" +stlNo +"&bpno=" +bpno;
...
function sendPlConfirm(){
var tbl = document.getElementById('plantbl2');

for(var n=0;n<tbl.rows.length-1; n++){
		var stlNo=tbl.rows[n+1].cells[1].id;
		var bpno=tbl.rows[n+1].cells[3].childNodes[0].nodeValue;
		//alert("bpno = "+bpno);
...
//==end======================chk date gap==========2014-03-27=============================================================
		...
		for(var n=0;n<tbl.rows.length-1; n++){
				if(chk==0){
					...
					DateTo	= document.getElementById("txtToDate" +n).value;
						//======================2014-06-30===========================
					dtmPCD_Date	= document.getElementById("dtmPCD_Date" +n).value;
					dtmOCD_Date	= document.getElementById("dtmOCD_Date" +n).value;
					dtmIH_Date	= document.getElementById("dtmIH_Date" +n).value;
						//==end====================2014-06-30===========================
					...
					//======================2014-06-30===========================	
					dtmPCD_Date	= dtmPCD_Date + "," +document.getElementById("dtmPCD_Date" +n).value;
					dtmOCD_Date	= dtmOCD_Date + "," +document.getElementById("dtmOCD_Date" +n).value;
					dtmIH_Date	= dtmIH_Date + "," +document.getElementById("dtmIH_Date" +n).value;
					//======================2014-06-30===========================
		...
		var path= pnl_planUrl+"planDetailXML.php?RequestType=Confirm&StyleID=" +stlNo +"&lineNo=" +lineNo +"&DateFrom=" +DateFrom +"&DateTo=" +DateTo +"&dtmPCD_Date=" +dtmPCD_Date +"&dtmOCD_Date=" +dtmOCD_Date +"&dtmIH_Date=" +dtmIH_Date +"&planConfirm=" + planConfirm;
		//alert(path);

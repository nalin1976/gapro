...
WHERE H.dtmDate = '$tagetDate' AND H.intFactory = $srchFactory AND H.strTeamNo = $lineNo AND
...
H.intStyleId = $intStyleId ";
	...
	if(empty($achQty)){
		$SQL = "SELECT COUNT(H.intLineOutputSerial)AS noOfNextRec FROM productionlineoutputheader AS H
				WHERE DATE(H.dtmTargetAchieve) = '$tagetDate' AND H.intFactory = $srchFactory AND DATE_FORMAT(H.dtmTargetAchieve,'%H.%i')> '$tmEndTime24h' ";
		$result = $db->RunQuery($SQL);
		while($row = mysql_fetch_array($result))
		{

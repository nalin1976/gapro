...
function	ItemSelMenu(e)
{
	...
	document.getElementById('item6_teamParameters').style.backgroundColor="#FFFFFF";
	...
	document.getElementById('item6_teamParameters').style.color="#000099";
...
<table width=150 cellspacing=1 cellpadding=0 bgcolor=lightgray>
  <tr><td>
    <table bgcolor="" width=150 cellspacing=0 cellpadding=0>
      ...
      <tr>
         <td id="item6_teamParameters"  width="118" height="16" onMouseOver="this.style.backgroundColor='#D5EDFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'" class="onstripmenu" onclick="showTeamParameter();" ><b>Change Team Parameters</b></td>
      </tr>
	 <!-- <tr>
...
<div id="footer">
    <table width="101%"  height="48" cellpadding="0" cellspacing="0" bgcolor="" class="tablezRED">
      ...
      <tr>
        <td height="24" colspan="8" align="center" valign="top"><img  src="../../images/orderbook.png" alt="order Book" name="butOrderBook" class="mouseover" id="butOrderBook" onclick="loadOrderBook();" /><img src="../../images/teams.png" alt="calender" name="butCalender" class="mouseover" id="butCalender" onclick="loadTeamsWindow();" /><img src="../../images/calender.png" alt="calender" name="butCalender" id="butCalender" class="mouseover" onclick="loadCalenderWindow();"/><img src="../../images/learningcurve.png" alt="calender" name="butCalender" width="171" height="24" class="mouseover" id="btnLearningCurve" onclick="loadLearningCurve();" /><img src="../../images/save.png" alt="save" name="butSave" width="84" height="24" class="mouseover" id="butSave" onclick="savePlanningDetails();" /><img src="../../images/report.png" name="butReport" width="108" height="24" class="mouseover" id="butReport" onclick="grnReport();"/><a href="../../main.php"><img src="../../images/close.png" width="97" height="24" border="0" class="mouseover" /></a></td>
      </tr>
  </table>
</div>	
                        ...
                        <table border="0" cellspacing="0" cellpadding="0" class="normalfnt_week">
                            <tbody>
                                ...
                                <?php
									...
									$h = 0;
									$sql_holiday = "SELECT * FROM plan_holiday_calender WHERE dtmDate>= '$cellDate'";
									...
									for($i=0; $i<$no_of_dates;$i++)
									{
										...
										$holiday_check = false;
										// check for holidayes
										for($ho=0; $ho<=count($holiday); $ho++)
										{
											if($holiday[$ho]== date("Y-m-d",strtotime($i.'days',$d)))
											{
												//echo date("Y-m-d",strtotime($i.'days',$d));
												
												?>
												
                                                <td><div id="calanderDiv" style="width:40px; height:25px;background-color:#C8B156;  margin-left:1px;" ><div id="<?php echo date("Y-m-d",strtotime($i.'days',$d));?>" style="padding-top:5px;"><?php echo date("m-d",strtotime($i.'days',$d));?></div></div></td>  
                                                <?php
										...
										if($holiday_check == false)
										{
											...
											{
									?>        
											<td height="25"><div id="calanderDiv" style="width:40px; height:25px;background-color:#ECE2A4;  margin-left:1px;" ><div id="<?php echo date("Y-m-d",strtotime($i.'days',$d));?>" style="padding-top:5px;" onclick="calanderClick(this.id);"><?php echo date("m-d",strtotime($i.'days',$d));?></div></div></td>   										
											<?php
											}
											else
											{
...
FROM
plan_teams
WHERE plan_teams.intSubTeamOf <'1'";
										...
										while($row=mysql_fetch_array($result))
										{
											...
											$sql_validate = "SELECT plan_teamsvaliddates.dtmValidFrom,plan_teamsvaliddates.dtmValidTo FROM plan_teamsvaliddates 
WHERE plan_teamsvaliddates.intTeamId='$teamID'";
											$result_validate = $db->RunQuery($sql_validate);
											$i=0;
											$validateFrom = array();
											$validateTo   = array();
											while($row3=mysql_fetch_array($result_validate))
											{
												$dateValidateFrom       = explode(" ",$row3['dtmValidFrom']);
												$validateFrom[$i] 		= $dateValidateFrom[0];
												
												$dateValidateTO 		= explode(" ",$row3['dtmValidTo']);
												$validateTo[$i] 		= $dateValidateTO[0];
												$i++;
											}
									...
									?>
                                			...
                                			for($i=0; $i<$no_of_dates; $i++)
											{
												$datecell = date("Y-m-d",strtotime($i.'days',$d));
												$checkValidity = false ;
												for($k=0; $k<count($validateFrom); $k++)
												{
													//check valid from date with calander date
													if($datecell >= $validateFrom[$k] && $datecell<=$validateTo[$k])
													{
														$checkValidity = true ;
													}
												...
												if($checkValidity== true)
												{
													...
													{
											?>
                         								<div title="active" id="<?php echo date("Y-m-d",strtotime($i.'days',$d)); ?>" style="width:40px; height:25px; background-color:#C8B156 ; margin-right:1px; ">&nbsp;</div>                                                  
                                                <?php
													}
													else
													{
												...
												?>
															...
															<div title="active" id="<?php echo date("Y-m-d",strtotime($i.'days',$d)); ?>" style="width:40px; height:25px; background-color:#ECE2A4; margin-right:1px; ">&nbsp;</div>
                                                        <?php
														}
														else
														{
                                                ...
                                                else
												{
													...
													{
												?>
                                                	<div title="inActive" id="<?php echo date("Y-m-d",strtotime($i.'days',$d)); ?>" style="width:40px; height:25px; background-color:#C8B156; margin-right:1px; ">&nbsp;</div>
...
WHERE plan_teams.intSubTeamOf='$teamID'";
										   ...
										   while($row2 = mysql_fetch_array($result_subTeam))
										   {
											  ...
											  $sql_subTeam_valiDate = "SELECT plan_teamsvaliddates.dtmValidFrom,plan_teamsvaliddates.dtmValidTo FROM plan_teamsvaliddates 
WHERE plan_teamsvaliddates.intTeamId='$subTeamId'";
											  $result_subTeam_valiDate = $db->RunQuery($sql_subTeam_valiDate);
											  $s = 0;
											  $subValidateFrom = array();
											  $subvalidDateTo  = array();
											  while($row4 = mysql_fetch_array($result_subTeam_valiDate))
											  {
												  $dateValidateFromSub       = explode(" ",$row4['dtmValidFrom']);
												  $subValidateFrom[$s] 		= $dateValidateFromSub[0];
												
													$dateValidateTOSub 		= explode(" ",$row4['dtmValidTo']);
													$subvalidDateTo[$s] 		= $dateValidateTOSub[0];
													$s++;
											  }
										   ?>
                                           <tr>
                                           ...
                                           <td height="25" style="background-color:#FFFFFF" class="normaltxtmidb2" colspan="<?php echo $no_of_dates; ?>"><div class="classTeam" style="float:left; margin-bottom:1px;" id="team<?php echo $subTeamId; ?>">
                                			...
                                			for($i=0; $i<$no_of_dates; $i++)
											{
												$datecell = date("Y-m-d",strtotime($i.'days',$d));
												$checkValidity = false ;
												for($k=0; $k<count($subValidateFrom); $k++)
												{
													//check valid from date with calander date
													if($datecell >= $subValidateFrom[$k] && $datecell<=$subvalidDateTo[$k])
													{
														$checkValidity = true ;
													}
...
plan_stripes.intTeamEfficency,
plan_stripes.intNoOfMachine,
plan_stripes.intChangeParamStatus
FROM
plan_stripes
...
?>	
		...
		arrProp['paramTeamEff'] = '<?php echo $row['intTeamEfficency']; ?>';
		arrProp['paramNoOfMachine']	= '<?php echo $row['intNoOfMachine']; ?>';
		arrProp['changeTeamParam']	= '<?php echo $row['intChangeParamStatus']; ?>';
...
Inner Join plan_teamsvaliddates ON plan_teams.intTeamNo = plan_teamsvaliddates.intTeamId";
	...
	?>
		...
		var holidayWorkingHours = new Array();
		var holidayCount = new Array();
		<?php
		
		// holiday calender 
		$sql_holidayC = "SELECT *,(SELECT COUNT(DISTINCT dtmdate) 
FROM plan_holiday_calender 
WHERE dtmdate>='$cellDate')  AS count FROM plan_holiday_calender
WHERE dtmDate>='$cellDate'";
		
		$result_holiday = $db->RunQuery($sql_holidayC);
		while($row5 = mysql_fetch_array($result_holiday))
		{
			$dblWorkingHours = $row5["dblworkingHours"];
			
		?>
		holidayDate[h] = '<?php echo $row5["dtmDate"]; ?>';
		holidayWorkingHours[h] =  '<?php echo $dblWorkingHours; ?>';
		holidayCount[h]   = '<?php echo $row5["count"]; ?>';
		h++;
		<?php
		}
		?>
	//alert(holidayCount[0])
	createAllStrips();
</script>
</form>
</body>

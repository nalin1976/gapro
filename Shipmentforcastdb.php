...
header('Content-type: text/xml');
	...
	$FactoryDate	 = $sailingDateArrry[2].'-'.$sailingDateArrry[1].'-'.$sailingDateArrry[0];
		...
		if($request == "getdata"){
			...
			$sql="";
			if($Buyer != "Select One"){
				$sql.="AND shipmentforecast_detail.strBuyer = '$Buyer'";
				}
			if($PoNo != "Select One"){
				$sql.="AND shipmentforecast_detail.strPoNo = '$PoNo'";
				}
			if($datefrom != ""){
				$datefromArrry= explode('/',$datefrom);
				$FromDate	 = $datefromArrry[2].'-'.$datefromArrry[1].'-'.$datefromArrry[0];
				$sql.="AND shipmentforecast_detail.strshipmentfrom = '$FromDate'";
				}
			if($dateto != ""){
				$datetoArrry= explode('/',$dateto);
				$ToDate	 = $datetoArrry[2].'-'.$datetoArrry[1].'-'.$datetoArrry[0];
				$sql.="AND shipmentforecast_detail.strshipmentto = '$ToDate'";
				}
			...
			shipmentforecast_detail.strUserID
			FROM
			shipmentforecast_detail
			WHERE shipmentforecast_detail.strBuyer != ''
			$sql
			ORDER BY
			shipmentforecast_detail.intID DESC";
			...
			while($row=mysql_fetch_array($result_getdata))
	{
						...
						$ResponseXML .= "<Buyer><![CDATA[" . $row["strBuyer"]  . "]]></Buyer>\n";
						...
						$FactoryDate =substr($row["strFactoryDate"],0,10);
						$FactoryDateArray=explode('-',$FactoryDate);
						$formatedFactoryDateArray=$FactoryDateArray[2]."/".$FactoryDateArray[1]."/".$FactoryDateArray[0];
						$ResponseXML .= "<FactoryDate><![CDATA[" . $formatedFactoryDateArray  . "]]></FactoryDate>\n";
						$getName="SELECT
						useraccounts.Name
						FROM `useraccounts`
						WHERE
						useraccounts.intUserID = '". $row['strUserID'] ."'";
						$result_getName=$db->RunQuery($getName);
						$rowresult_getName=mysql_fetch_array($result_getName);
						$ResponseXML .= "<UserName><![CDATA[" . $rowresult_getName["Name"]  . "]]></UserName>\n";
...
if($request == "editdata"){
	...
	if($request == "SendEmail"){
		...
		$From=$_GET['From'];
		$To=$_GET['To'];
		$FactoryManager=$_GET['FactoryManager'];
		
		$mailTo = $FactoryManager . ', ';
		$mailTo .= 'kolitha@helaclothing.com';
		$headers = "From:gapro@helaclothing.com\r\n";
		$headers .= "Content-type: text/html\r\n";		
		$subject = "Shipment Schedule" ;									 
		$body = '<html>
				 <body>
				 <table border="0">
				 <tr>
				 ...
				 <tr>
				 <td><b>Buyer '. $Buyer .'</b></td>
				 </tr>
				 <tr>
				 <td><b>Date from '. $From .' to '. $To .'</b></td>
				 </tr>
				 <tr>
				 <td><b></b></td>
				 </tr>
				 ...
				 $retval = mail($mailTo, $subject, $body,$headers);
				 if($retval)
				 {
					 echo 'sent';
				 }
	...
	$getdata="SELECT
				...
				FROM `useraccounts`
				WHERE
				useraccounts.intUserID = '$userID'";
			$result_getdata=$db->RunQuery($getdata);
			$row=mysql_fetch_array($result_getdata);
			echo $row['UserName'];
		}
	...
	if($request == "getEmail"){
		...
		$getdata="SELECT
				...
				WHERE strSC_No='$SCNO' 
				AND strBuyer='$Buyer' 
				AND strPoNo='$BuyerPONo' 
				AND intID='$ShipmentID'";
			$result_getdata=$db->RunQuery($getdata);
			$row=mysql_fetch_array($result_getdata);
			if($row){
				$sql_email="SELECT
							...
							WHERE
							useraccounts.intUserID = '". $row['strUserID'] ."'";
				$result_result=$db->RunQuery($sql_email);
				$row_result=mysql_fetch_array($result_result);
		...
		$ShipmentID=$_GET['ShipmentID'];
		$From=$_GET['From'];
		$FromDateArrry= explode('-',$From);
		$FromDate	 = $FromDateArrry[2].'-'.$FromDateArrry[1].'-'.$FromDateArrry[0];
		//$FromDate    = substr($FromDate, 0, -2);
		$To=$_GET['To'];
		$ToDateArrry= explode('-',$To);
		$ToDate	 = $ToDateArrry[2].'-'.$ToDateArrry[1].'-'.$ToDateArrry[0];
		//$ToDate  = substr($ToDate, 0, -2);

		$sql_update="UPDATE shipmentforecast_detail SET 
						strshipmentfrom='$FromDate', 
						strshipmentto='$ToDate' 
						WHERE strSC_No='$SCNO' 
						AND strBuyer='$Buyer' 
						AND strPoNo='$BuyerPONo' 
						AND intID='$ShipmentID'";

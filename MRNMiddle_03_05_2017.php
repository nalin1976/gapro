...
else if (strcmp($RequestType,"getStyleNOS") == 0)
{
	 ...
	 $sql = "SELECT DISTINCT orders.strStyle
FROM materialratio INNER JOIN orders ON materialratio.intStyleId = orders.intStyleId AND 
(orders.intStatus = '$arrstatus[0]' or orders.intStatus = '$arrstatus[1]' or orders.intStatus = '$arrstatus[2]' or orders.intStatus = '5') ";
...
FROM materialratio INNER JOIN orders ON materialratio.intStyleId = orders.intStyleId AND 
(orders.intStatus = '$arrstatus[0]' or orders.intStatus = '$arrstatus[1]' or orders.intStatus = '$arrstatus[2]' or orders.intStatus = '5') order by strOrderNo";
...
else if(strcmp($RequestType,"GetSCNO") == 0)
{
	...
	// ==============================================================
         ...
         $sql="SELECT DISTINCT S.intSRNO,MR.intStyleId FROM materialratio MR INNER JOIN orders O ON MR.intStyleId = O.intStyleId inner join specification S on S.intStyleId=O.intStyleId where ((O.intStatus = '$arrstatus[0]') or (O.intStatus = '$arrstatus[1]') or (O.intStatus = '$arrstatus[2]') or (O.intStatus = '5')) order by  S.intSRNO desc";

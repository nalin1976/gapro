...
$sql = "select MMC.strDescription,MSC.StrCatName,STB.intMatDetailId,MIL.strItemDescription,STB.strColor,STB.strSize,STB.strUnit,STB.intBulkGrnYear,STB.intBulkGrnNo,
...
where STB.strMainStoresID='$mainStore' ";
...
if($itemDesc!="")
	...
	$sql .= "group by STB.intMatDetailId,STB.strColor,STB.strSize,STB.strUnit,STB.intBulkGrnNo,STB.intBulkGrnYear
having stockBal>0
order by MMC.intID,MSC.StrCatName,MIL.strItemDescription,STB.strColor,STB.strSize,STB.strUnit,STB.intBulkGrnYear,STB.intBulkGrnNo;";
	...
	while($row=mysql_fetch_array($result))
	{
		...
		$ResponseXML .= "<GrnYear><![CDATA[".$row['intBulkGrnYear']."]]></GrnYear>\n";
		$ResponseXML .= "<GrnNo><![CDATA[".$row['intBulkGrnNo']."]]></GrnNo>\n";
	}
$ResponseXML .= "</XMLLoadPopItems>\n";
echo $ResponseXML;
}
...
$styleID = $_GET["styleID"];
	...
	$sql="select distinct MSC.intSubCatNo,MSC.StrCatName 
			...
			where MSC.intStatus=1 ";
	if($mainCat != '')
		$sql .= " and MSC.intCatNo = '$mainCat' ";

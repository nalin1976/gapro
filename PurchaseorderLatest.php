...
function getSupplierData(id)
{
	...
	xmlHttp.onreadystatechange=getSupplierDataRequest;
	var url = "POMiddle.php?RequestType=getSupplierData&supId="+id;
	xmlHttp.open("GET",url,true);
	xmlHttp.send(null);
		
}
function getSupplierData1()
{
	if(document.getElementById("cboSupplier").value != "" || document.getElementById("cboSupplier").value != "0")
	{
	var url = "POMiddle.php?RequestType=getSupplierData1&supId="+document.getElementById("cboSupplier").value;
	 htmlobj=$.ajax({url:url,async:false});
	 var XMLstrCurrency1 = htmlobj.responseXML.getElementsByTagName("strCurrency1");
	document.getElementById('cbocurrency').innerHTML = XMLstrCurrency1[0].childNodes[0].nodeValue; 	
	}
	if(document.getElementById("cboSupplier").value == "" || document.getElementById("cboSupplier").value == "0")
	{
		
		var url = "POMiddle.php?RequestType=getCurrancyData";
		htmlobj=$.ajax({url:url,async:false});
		var XMLstrCurrency1 = htmlobj.responseXML.getElementsByTagName("strCurrency11");
		document.getElementById('cbocurrency').innerHTML = XMLstrCurrency1[0].childNodes[0].nodeValue;
	}
}

function getSupplierDataRequest()
{
  ...
  <tr>
	...
	?>
          ...
          <tr>
            ...
            <td colspan="3"><select name="cbocurrency" class="txtbox" id="cbocurrency" style="width:65px" onchange="convertRates();onchangePocalculate();" tabindex="4">
	...
	?>
          ...
          <tr>
            ...
            <td colspan="4"><select onchange="getSupplierData(this.value); getSupplierData1()" name="cboSupplier" class="txtbox" id="cboSupplier" style="width:312px" tabindex="5">
			<option value="0" selected="selected">Select One</option>
                          <?php
if($displaySupplierCode=="true"){	
	$SQL = "SELECT concat(strSupplierCode,' -> ',strTitle)as strTitle,strSupplierID FROM suppliers s where intApproved='1' order by strTitle;";	
	$result = $db->RunQuery($SQL);		
	while($row = mysql_fetch_array($result))
	{
  ...
  <tr bgcolor="#FFD5AA">
    <td height="30"><table width="100%" border="0">
      <tr >
        ...
        <img src="images/save.png" id="butSave" alt="Save" width="84" height="24" class="mouseover" onclick="savePOHeader();" />

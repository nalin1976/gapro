...
function getStyleNo()
{
	...
	var url="preordermiddletire.php?RequestType=GetStyleNo1&scNo="+scNo;
	htmlobj=$.ajax({url:url,async:false});
	var XMLName = htmlobj.responseXML.getElementsByTagName("OrderNos");
	document.getElementById('cboStyles').innerHTML =  XMLName[0].childNodes[0].nodeValue;
}
...
<body>
<form id="frmcomplete" name="frmcomplete" method="post" action="approvedCostSheets.php">
    <tr>
...
$reportname = $xml->PreOrder->ReportName;
	...
	$SQL = "select orders.strStyle from orders where orders.intStatus in (11, 13) and orders.intUserID =" . $_SESSION["UserID"] . ";";	
	$result = $db->RunQuery($SQL);	
	while($row = mysql_fetch_array($result))
	{
	...
	$SQL = "select O.intStyleId,O.strOrderNo from orders O where O.intStatus in ( 11, 13) and O.intUserID =" . $_SESSION["UserID"] . " ";
	if($styleID!="")
	...
	?>
                  ...
                  <td width="111" ><select name="cboSR" class="txtbox" style="width:130px" id="cboSR" onchange="GetStyleName(this);resetCompanyBuyer(); getStyleNo();">
                    <option value="" selected="selected">Select One</option>
                    <?php
	
	$SQL = "select specification.intStyleId,specification.intSRNO from specification inner join orders on specification.intStyleId = orders.intStyleId AND orders.intStatus in ( 11, 13) and orders.intUserID =" . $_SESSION["UserID"] . " order by intSRNO desc;";
	...
	?>
        ...
        <tr>
          <td colspan="2"><div id="divData" style="width:930px; height:400px; overflow: scroll; border-width:3px; border-style:solid;border-color:#99CCFF;">
			...
			$sql = "SELECT orders.strStyle,orders.intStyleId,strDescription, orders.strOrderNo, companies.strComCode, buyers.strName,intQty,dtmAppDate,intSRNO, useraccounts.Name FROM orders INNER JOIN companies ON orders.intCompanyID = companies.intCompanyID INNER JOIN buyers ON orders.intBuyerID = buyers.intBuyerID INNER JOIN specification ON orders.intStyleId = specification.intStyleId INNER JOIN useraccounts ON orders.intApprovedBy = useraccounts.intUserID  WHERE orders.intStatus in ( 11, 13) and orders.intUserID =" . $_SESSION["UserID"] ;

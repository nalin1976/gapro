...
var pub_url = pub_fgoodGPPath+"/washing/returnToFactoryList/";
...
function loadStyle()
{
	...
	xmlHttpreq[0].open("GET",pub_url+'returnToFactoryList_xml.php?RequestType=loadStyle&factoryID='+ factoryID ,true);
...
function LoadGrid(style)
{
	...
	var url = pub_url+'returnToFactoryList_xml.php?RequestType=LoadGrid&factoryID='+ factoryID + '&styleNo='+ styleNo + '&dateFrom='+ dateFrom + '&dateTo='+ dateTo;
...
function selectStyle(obj){
	var path="returnToFactoryList_xml.php?RequestType=loadStyleNo&po="+obj.value.trim();
...
function selectPo(obj){
	var path="returnToFactoryList_xml.php?RequestType=loadPoNo&po="+obj.value.trim();
	htmlobj=$.ajax({url:path,async:false});
	var sNo=htmlobj.responseXML.getElementsByTagName("sNo");
	if(sNo.length>0)

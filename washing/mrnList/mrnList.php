...
<script src="mrnList.js" language="javascript" type="text/javascript"></script>
    ...
    <tr>
      ...
      <!--<tr><td>&nbsp;</td></tr>-->
          ...
          <td><table width="800" border="0" cellspacing="0" cellpadding="3" align="center" class="tableFooter">
            <tr>
              <td width="800"><table width="800" border="0" cellspacing="0" cellpadding="0" class="normalfnt" align="center">
                <tr>
                  <td colspan="4" class="mainHeading" height="26">Washing MRN Listing</td>
                  </tr>
                <tr>
                  <td height="20" colspan="4"><table width="650" border="0" cellspacing="0" cellpadding="2" class="" align="center">
                    ...
                    <tr class="normalfnt">
                      ...
                      <td width="210"><select name="cboOrderNo" id="cboOrderNo" style="width:122px;height:18px;" onChange="getStyle(this);">
                        <option></option>
                          <?php 
                      ...
                      <td width="163"><select name="cboStyleNo" id="cboStyleNo" style="width:122px;height:18px;" onChange="getStylewiseOrderNo(this);">
                        <option></option>
                        <?php 
					  	$sql = "SELECT DISTINCT
								o.strStyle
                    ...
                    <tr class="normalfnt">
                      <td>Store</td>
                      <td><select name="wasMrn_cboStore" id="wasMrn_cboStore" style="width:150px;height:18px;" tabindex="1">
		...
		?>
                      ...
                      <td>Department</td>
                      <td><select name="wasMrn_cboDepartment" id="wasMrn_cboDepartment" style="width:150px;height:18px;" tabindex="2">
		...
		?>
                    ...
                    <tr class="normalfnt">
                      ...
                      <td><select name="cbomMrnNofrom" id="cbomMrnNofrom" style="width:121px;height:18px;">
                        <option value=""></option>
                        <?php 
					  	$SQL = "select DISTINCT  concat(m.intMrnYear,'/',m.dblMrnNo) MRN from was_mrn m  where m.intCompanyId = '".$_SESSION['FactoryID']."' order by m.dblMrnNo ";
						$result = $db->RunQuery($SQL);
						while($row = mysql_fetch_array($result))
						{
							if($issueNofrm == $row["MRN"])
								echo "<option selected=\"selected\" value=\"" . $row["MRN"] ."\">" . $row["MRN"] . "</option>";
							else
								echo "<option value=\"" . $row["MRN"] ."\">" . $row["MRN"] . "</option>";
						}
					  ?>
                      </select></td>
                      <td>MRN # To</td>
                ...
                <tr>
                  ...
                  <div id="divwash" style="overflow:scroll; width:800px; height:300px;">
                    <table width="778" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td width="98" class="grid_header" height="20">MRN No</td>
                        ...
                        <td width="80" class="grid_header">View</td>
                        <td width="93" class="grid_header">Report</td>
                      </tr>
                      <?php 
					  
					  
					  	$sqlIssue = " select concat(m.intMrnYear,'/',m.dblMrnNo) MRN,m.strColor,m.dblQty ,m.dblBalQty,
										o.intStyleId,o.strStyle,o.strOrderNo,m.dtmMrnDate,
										ms.strName,
										d.strDepartment 
										from was_mrn m
										inner join orders o on o.intStyleId = m.intStyleId
										inner join mainstores ms on ms.strMainID=m.intStore
										inner join department d on d.intDepID=m.intDepartment
						...
						if($issueNofrm != '')
							$sqlIssue .= " and  concat(m.intMrnYear,'/',m.dblMrnNo) >= '$issueNofrm' ";
							
						if($issueNoto != '')
							$sqlIssue .= " and  concat(m.intMrnYear,'/',m.dblMrnNo) <= '$issueNoto' ";
						
						if($dfrom != '')
							$sqlIssue .= " and  date(m.dtmMrnDate) >= '$dfrom' ";
							
						if($dto != '')
							$sqlIssue .= " and  date(m.dtmMrnDate) <= '$dto' ";
                      ...
                      <tr>
                        <td height="20" class="<?php echo $cls ;?>"><?php echo $row['MRN'];?></td>
                        ...
                        <td class="<?php echo $cls ;?>" style="text-align:left;"><?php echo substr($row['dtmMrnDate'],0,10);?></td>
                        <td class="<?php echo $cls ;?>" style="text-align:right;"><?php echo $row['dblQty'];?></td>
                        <td class="<?php echo $cls ;?>"><img src="../../images/view2.png" height="18" onClick="loadToEdit('<?php echo $row["MRN"];?>')" ></td>
                        <td class="<?php echo $cls ;?>"><img src="../../images/report.png" height="18"  onClick="loadReport('<?php echo $row["MRN"];?>')" ></td>
                      </tr>
                      <?php 
					  $i++;
					  }

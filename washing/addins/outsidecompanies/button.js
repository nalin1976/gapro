...
//var pub_taxurl	= "/gapro/addinns/washing/addins/outsidecompanies/";
var xmlHttp;
...
{	
	...
	var x_find = checkInField('was_outside_companies','strComCode',x_name,'intCompanyID',x_id);
	if(x_find)
	{
	...
	var x_find = checkInField('was_outside_companies','strName',x_name,'intCompanyID',x_id);
	if(x_find)
	{
		alert('"'+x_name+ "\" is already exist.");	
		document.getElementById("txtName").focus();
...
function ClearCompanyForm()
{
	...
	document.getElementById("txtComCode").focus();
	var sql = "select intCompanyID,strName from was_outside_companies where intStatus <> 10 order by strName";
...
function getCompanyDetails()
	...
	if(document.getElementById('cbocompany').value.trim()=='')
	{
		...
		xmlHttp.open("GET", 'companymiddle.php?request=getdata&Company='+Company, true);
...
function checkDefaultInvoiceStatus(obj)
{
	...
	if(compId != '')
	{
		if(obj.value=='Yes')
		{
			...
			var url = "companymiddle.php?request=checkDefaultCompany&compId="+compId;
			xmlHttp.open("GET",url,true);
			xmlHttp.send(null);				
		}
	}
...
function showCountryPopUppp(){
	var url  = "../../../addinns/country/countries.php";
	inc('../../../addinns/country/Button.js');
	var W	= 10;
	var H	= 60;

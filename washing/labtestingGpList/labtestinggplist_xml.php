...
if($request=='loadPo'){
	$style=$_GET['style'];
	$sql="SELECT o.intStyleId FROM orders o INNER JOIN was_labtestinggp ON o.intStyleId = was_labtestinggp.intStyleNo WHERE o.strStyle='$style';";
	//echo $sql;
	$res=$db->RunQuery($sql);
	$row=mysql_fetch_array($res);
...
if($request=='loadStyle'){
	$po=$_GET['po'];
	$sql="SELECT o.strStyle FROM orders o INNER JOIN was_labtestinggp ON o.intStyleId = was_labtestinggp.intStyleNo WHERE o.intStyleId='$po';";
	$res=$db->RunQuery($sql);
	$row=mysql_fetch_array($res);

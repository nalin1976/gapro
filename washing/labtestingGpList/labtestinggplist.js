...
function loadMrnDets(mrn){
	if(mrn==''){
		...
		var path="labtestinggplist_xml.php?req=loadDets&mrn="+mrn;
...
function loadReport(gp){
	...
	if(gp=="")
		...
		window.open('../labtestingGatepass/rptLabTestingGP.php?req='+gp,'new');
}

function getStylewiseOrderNo(obj){
	var path="labtestinggplist_xml.php?req=loadPo&style="+URLEncode(obj.value.trim());
	htmlobj=$.ajax({url:path,async:false});
	document.getElementById('cboOrderNo').value=htmlobj.responseXML.getElementsByTagName('StyleId')[0].childNodes[0].nodeValue; 
}

function getStyle(obj){
	var path="labtestinggplist_xml.php?req=loadStyle&po="+URLEncode(obj.value.trim());
	htmlobj=$.ajax({url:path,async:false});
...
function clearForm(){
	...
	document.getElementById("cboOFactory").value='';

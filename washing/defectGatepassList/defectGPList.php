...
$txtToFactory 	= $_POST["txtToFactory"];

if(!isset($_POST["txtToFactory"]) && $_POST["txtToFactory"]=="")
{
	$dateFrom 	= date('Y-m-d');
	$dateTo  	= date('Y-m-d');
	$chkDate	= 'on';
...
<title>Defect Gate Pass Listing</title>
...
<body>
<form name="frmDefectGPList" method="post" action="defectGPList.php" id="frmDefectGPList">
  ...
  <td><table width="800" border="0" cellspacing="2" cellpadding="0" align="center">
    <tr>
      <td><table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableBorder">
        <tr>
          <td colspan="4" class="mainHeading" height="26">Defect Gate Pass Listing</td>
        </tr>
        <tr>
          <td class="normalfnt"><table width="100%" border="0" cellspacing="1" cellpadding="1" class="bcgl1">
            <tr>
              <td colspan="2" class="normalfnt">To Factory</td>
              <td width="44%" class="normalfnt"><input name="txtToFactory" id="txtToFactory" type="text" class="txtbox" style="width:325px" value="<?php echo ($txtToFactory!=''?$txtToFactory:"") ?>"></td>
              ...
              <td width="21%" class="normalfnt"><select name="cboOrderNo" id="cboOrderNo" style="width:150px;" onChange="loadStyle(this);">
                ...
                <?php 
					  	$sql = " SELECT DISTINCT
									...
									O.strStyle
									FROM
									was_defectgatepass AS wdg
									INNER JOIN orders O ON O.intStyleId = wdg.intStyleId
									WHERE
									wdg.intFromFactory='$companyID'
         ...
         <div id="divDefectTrans" style="overflow:scroll; width:800px;height:300px">
		  <table width="100%" border="0" cellpadding="0" cellspacing="1" id="tblDefectTrans" bgcolor="#CCCCFF">
            <tr class="mainHeading4">
                        <td width="139"  height="20"> Defect GatePass No</td>
                        <td width="124" >PO No</td>
                        <td width="125" >Style No</td>
                        <td width="138" >Color</td>
                        <td width="107" >Date</td>
                        <td width="107" >Qty</td>
                        <td width="102" >Report</td>
            </tr>
            <?php
			$sql = "select O.strOrderNo,O.strStyle,concat(wdg.intDefectGPYear,'/',wdg.intDefectGPNo) as defectGPNo,
					wdg.strColour,wdg.dblGatePassQty, date(wdg.dtmDate) as dtmDate,wdg.strToFactory,wdg.intStyleId
					from was_defectgatepass wdg
					inner join orders O on O.intStyleId=wdg.intStyleId
					where wdg.intDefectGPNo <>'' ";
			if($dateFrom!="")
				$sql.="AND date(wdg.dtmDate)>='$dateFrom' ";
			if($dateTo!="")
				$sql.="AND date(wdg.dtmDate)<='$dateTo' ";
			if($txtToFactory!="")
				$sql.="AND wdg.strToFactory like '%$txtToFactory%' ";
			if($cboOrderNo!="")
				$sql.="AND wdg.intStyleId='$cboOrderNo' ";
			$sql.="order by defectGPNo";
			$result=$db->RunQuery($sql);
			while($row=mysql_fetch_array($result))
			{
			?>
                <tr class="bcgcolor-tblrowWhite">
                  <td  height="20" class="normalfnt">&nbsp;<?php echo $row['defectGPNo']; ?>&nbsp;</td>
                  <td class="normalfnt" >&nbsp;<?php echo $row['strOrderNo']; ?>&nbsp;</td>
                  <td class="normalfnt" >&nbsp;<?php echo $row['strStyle']; ?>&nbsp;</td>
                  <td class="normalfnt" >&nbsp;<?php echo $row['strColour']; ?>&nbsp;</td>
                  <td class="normalfnt" >&nbsp;<?php echo $row['dtmDate']; ?>&nbsp;</td>
                  <td class="normalfntRite" >&nbsp;<?php echo $row['dblGatePassQty']; ?>&nbsp;</td>
                  <td class="normalfntMid" >&nbsp;<img src="../../images/report.png" height="18">&nbsp;</td>
                </tr>
           <?php
			}
		   ?>

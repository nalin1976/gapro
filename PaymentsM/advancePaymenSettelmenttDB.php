...
session_start();
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
		...
		if($type=="S"){ 
		 ...
		 if($type=="G"){
		     $result=getAvailableGenaralPO($SupID);
		 }
...
function getAvailableStylePO($SupID)
{
	...
	$strSQL="	SELECT DISTINCT
               ...
               (advancepaymentpos.dblpaidAmount + if(isnull(purchaseorderheader.dblPOSettledAmt),0,purchaseorderheader.dblPOSettledAmt))AS dblPoBalance
				...
				purchaseorderheader.intPONo,
				advancepaymentpos.intPaymentNo
				ORDER BY advancepaymentpos.intPaymentNo,purchaseorderdetails.intPoNo";
...
Inner Join bulkgrndetails ON bulkgrnheader.intBulkGrnNo = bulkgrndetails.intBulkGrnNo AND bulkgrnheader.intYear = bulkgrndetails.intYear";*/
 ...
 $strSQL = "SELECT DISTINCT
				...
				advancepaymentpos.intPaymentNo,
               (advancepaymentpos.dblpaidAmount + if(isnull(bulkpurchaseorderheader.dblPOSettledAmt),0,bulkpurchaseorderheader.dblPOSettledAmt))AS dblPoBalance
				FROM
				...
				Inner Join advancepaymentpos ON bulkpurchaseorderheader.intBulkPoNo = advancepaymentpos.intPOno
				...
				ORDER BY advancepaymentpos.intPaymentNo,bulkpurchaseorderdetails.intBulkPoNo";
				$result=$db->RunQuery($strSQL);
	            return $result;
}

function getAvailableGenaralPO($SupID){
 ...
 $strSQL = "SELECT DISTINCT
				...
				(generalpurchaseorderheader.dblPOValue ) AS dblPOValue,
				generalpurchaseorderdetails.intMatDetailID,
				advancepaymentpos.dblpaidAmount as paidAmount, 
				if(isnull(generalpurchaseorderheader.dblPOSettledAmt),0,generalpurchaseorderheader.dblPOSettledAmt) as settledAmt,
				advancepaymentpos.intPaymentNo,
               (advancepaymentpos.dblpaidAmount + if(isnull(generalpurchaseorderheader.dblPOSettledAmt),0,generalpurchaseorderheader.dblPOSettledAmt))AS dblPoBalance
				FROM
				generalpurchaseorderdetails
				Inner Join generalpurchaseorderheader ON (generalpurchaseorderdetails.intGenPONo = generalpurchaseorderheader.intGenPONo) AND (generalpurchaseorderdetails.intYear = generalpurchaseorderheader.intYear)
				Inner Join popaymentterms ON (generalpurchaseorderheader.strPayTerm = popaymentterms.strPayTermId)
				
				Inner Join advancepaymentpos ON generalpurchaseorderheader.intGenPONo = advancepaymentpos.intPOno
				Left Join grnheader ON generalpurchaseorderheader.intGenPONo = grnheader.intPoNo AND generalpurchaseorderheader.intYear = grnheader.intYear
				Left Join grndetails ON grnheader.intGrnNo = grndetails.intGrnNo AND grnheader.intGRNYear = grndetails.intGRNYear
				WHERE  generalpurchaseorderheader.intSupplierID = '$SupID' AND popaymentterms.intAdvance='1' AND intSurended != 1
				GROUP BY  
				generalpurchaseorderheader.intSupplierID,
				generalpurchaseorderheader.intYear,  
				generalpurchaseorderheader.intGenPONo,
				advancepaymentpos.intPaymentNo
				ORDER BY advancepaymentpos.intPaymentNo,generalpurchaseorderdetails.intGenPONo";
				$result=$db->RunQuery($strSQL);
	            return $result;
}
...
$sql = "insert into tblAdvancePaySettlementGlAllocation(intAdvPaySetGlAccNo,paymentNo,intPoNo,glAccID,intCompany,intSupplier,dblAmt)

...
/*	CreateXMLHttpForBank();
	...
	var xmlHttpBank = $.ajax({url:url,async:false});
			...
			var XMLstrCurrency = xmlHttpBank.responseXML.getElementsByTagName("strCurrency");
			var XMLrate = xmlHttpBank.responseXML.getElementsByTagName("rate");
			
			var bankID   = XMLbankID[0].childNodes[0].nodeValue;
			var bank     = XMLbank[0].childNodes[0].nodeValue;
			var Currency = XMLstrCurrency[0].childNodes[0].nodeValue;
			var rate     = XMLrate[0].childNodes[0].nodeValue;

			document.getElementById("txtBank").value=bankID;
			document.getElementById("cboCurrency").value = rate;
...
function getScheduals()
{
	...
	var xmlHttpSup = $.ajax({url:url,async:false});
	document.getElementById("txtAccPacID").value = "";
    ...
    var url='paymentVoucherDB.php?DBOprType=getScheduals&strPaymentType=' + type + '&supID=' + supID;
	var xmlHttpSchedule = $.ajax({url:url,async:false});
	document.getElementById("cboSchedual").innerHTML = xmlHttpSchedule.responseText;
...
function getDets(){
	...
	var XMLbalance    = htmlobj.responseXML.getElementsByTagName("balance");
	var XMLadvPaidAmt = 0;
	...
	var strIvnsTable="<table width=\"930\" cellpadding=\"0\" cellspacing=\"0\" id=\"tblInvoice\">"+
	                "<thead>"+
					"<tr>"+
					"<td width=\"5%\" height=\"20\" bgcolor=\"#FBF8B3\" class='normalfntMid'>#</td>"+
					"<td width=\"14%\" bgcolor=\"#FBF8B3\" class='normalfntMid'>Invoice No</td>"+
					"<td width=\"16%\" bgcolor=\"#FBF8B3\" class='normalfntMid'>Total Amount</td>"+
					"<td width=\"16%\" bgcolor=\"#FBF8B3\" class='normalfntMid'>Paid Amount</td>"+
					"<td width=\"17%\" bgcolor=\"#FBF8B3\" class='normalfntMid'>Balance</td>"+
					"<td width=\"18%\" bgcolor=\"#FBF8B3\" class='normalfntMid'>Pay Amount</td>"+
					"<td  bgcolor=\"#FBF8B3\" class='normalfntMid'>ADV</td>"+
					//"<td width=\"15%\" bgcolor=\"#FBF8B3\" class='normalfntMid'>Advance</td>"+
					/*"<td width=\"15%\" bgcolor=\"\" >Currency</td>"+*/
					"</tr>"+
					"</thead>";
	...
	if(XMLInvNo.length==0)
	{
		...
		alert("No Schedules for this Supplier.");
		document.getElementById("cboSupliers").value = '0';
		closeWaitingWindow();
		return;
	...
	for ( var loop = 0; loop < XMLInvNo.length; loop ++)
	 {
		...
		strIvnsTable+="<tr class=\""+cls+"\">"+
						...
						"<td class=\""+cls+"\"><div align=\"center\" onmouseover=\"highlight(this.parentNode)\" style=\"text-align:left;\">" + invNo + "</div></td>"+
						...
						"<td class=\""+cls+"\">"+
						"<img src=\"../images/butt_1.png\" width=\"15\" height=\"15\" onclick=\"viewGrnPopUp();\" /></td>"+
...
function paymentVoucherSave()
{
	...
	{
		alert("Please select  the Accpak ID");
			...
			if (XMLResult[0].childNodes[0].nodeValue == "True")
			{
				...
				for(var i=1;i<theRow.length;i++)
				{
					...
					if(cells[0].firstChild.checked==true)
					{
						...
						var amount=cells[5].lastChild.value;
						...
						xmlHttpPVDetails.open("GET", 'paymentVoucherDB.php?DBOprType=saveNewPaymentVoucherDetails&strPaymentType=' + strPaymentType + '&voucherNo=' + voucherNo + '&invoiceNo=' + invoiceNo + '&amount=' + amount + '&balance=' + balance + '&paidAmount='+paidAmount+'&supId='+supId, true);
	   ...
	   if(xmlHttpPV.status == 200) 
		{
			...
			var strPVoucher="<table id=\"tblPVData\" width=\"922\" border=\"0\" cellpadding=\"0\" cellspacing=\"1\" bgcolor=\"\">"+
						"<tr>"+
						"<td width=\"25%\" height=\"29\" bgcolor=\"\" class=\"grid_header\">Supplier</td>"+
						"<td width=\"16%\" height=\"29\" bgcolor=\"\" class=\"grid_header\">Voucher No</td>"+
						"<td width=\"11%\" bgcolor=\"\" class=\"grid_header\">Date</td>"+
						"<td width=\"10%\" bgcolor=\"\" class=\"grid_header\">Amount </td>"+
						"<td width=\"12%\" bgcolor=\"\" class=\"grid_header\">Cheque No</td>"+
...
function loadBatch(){
	...
	var url='paymentVoucherDB.php?DBOprType=loadBatch&cboCurrency=' + cboCurrency;
	var hppt = $.ajax({url:url,async:false});
}

//-------------------------------------------------------------------------------------------------------------------------------------------------

function viewGrnPopUp()
{	
    var cbosupplier = document.getElementById("cboSupliers").value;
	var path = "advPaymentPopUp.php?cbosupplier="+cbosupplier+"&Type="+document.getElementById('cboPaymentType').value.trim();
	htmlobj=$.ajax({url:path,async:false});	

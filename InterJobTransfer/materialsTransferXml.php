	...
	while ($row=mysql_fetch_array($result))
	{
		...
		if($row["strBuyerPONO"] != '#Main Ratio#'){
			//$BpoName = getBuyerPOName($styleId,$row["strBuyerPONO"]);
                    $BpoName = getStyleBuyerPOName($styleId,$row["strBuyerPONO"]);
                }    
...
WHERE IT.intStatus='$state' AND IT.intTransferYear='$year'
ORDER BY IT.intTransferId DESC";    
...
else if ($RequestType=="LoadStyleNamesTo")
{
...
else if ($RequestType=="GetToStyleReqQty"){
    
    $toStyleId  = $_GET["tostyle"];
    $toItemCode = $_GET["toitemcode"];
    $toColor    = $_GET["tocolor"];
    $toSize     = $_GET["tosize"];
    
    $sql = " SELECT Sum(materialratio.dblQty) AS ReqQty
             FROM materialratio
             WHERE materialratio.intStyleId =  '$toStyleId' AND materialratio.strMatDetailID =  '$toItemCode' AND 
                   materialratio.strColor =  '$toColor' AND materialratio.strSize =  '$toSize'";
    
    $result = $db->RunQuery($sql);
    
    $ResponseXML .="<ItemReqQty>\n";
    
    while($row = mysql_fetch_array($result)){
        $ResponseXML .= "<ReqQty><![CDATA[" . $row["ReqQty"] . "]]></ReqQty>\n";
    }
...
else if ($RequestType=="GetTransferIn"){
    ...
    $sql = " SELECT Sum(stocktransactions.dblQty) AS IJINQty
             FROM stocktransactions 
             WHERE stocktransactions.intStyleId =  '$toStyleId' AND stocktransactions.intMatDetailId =  '$toItemCode' AND
                   stocktransactions.strColor =  '$toColor' AND stocktransactions.strSize =  '$toSize' AND
    ...
    while($row = mysql_fetch_array($result)){
        $ResponseXML .= "<INQty><![CDATA[" . $row["IJINQty"] . "]]></INQty>\n";
    }
    ...
    $sql = " SELECT Sum(stocktransactions.dblQty) AS InHouseQty
             FROM stocktransactions 
             WHERE stocktransactions.intStyleId =  '$toStyleId' AND stocktransactions.intMatDetailId =  '$toItemCode' AND
                   stocktransactions.strColor =  '$toColor' AND stocktransactions.strSize =  '$toSize' AND
    ...
    while($row = mysql_fetch_array($result)){
        $ResponseXML .= "<GQty><![CDATA[" . $row["InHouseQty"] . "]]></GQty>\n";
    }
...
else if ($RequestType=="GetRatioBalQty"){
    ...
    $sql = " SELECT Sum(materialratio.dblQty) AS ReqQty
             FROM materialratio
             WHERE materialratio.intStyleId =  '$toStyleId' AND materialratio.strMatDetailID =  '$toItemCode' AND 
                   materialratio.strColor =  '$toColor' AND materialratio.strSize =  '$toSize' AND materialratio.strBuyerPONO = '$toBPO'" ;
    
    $result = $db->RunQuery($sql);
    
    $ResponseXML .="<BPOItemReqQty>\n";
    
    while($row = mysql_fetch_array($result)){
        $ResponseXML .= "<BPOReqQty><![CDATA[" . $row["ReqQty"] . "]]></BPOReqQty>\n";
    }
...
function getStyleBuyerPOName($styleID,$BuyerPOName)
{
	global $db;

...
function LoadDetailsToMainPage()
{
	...
	for(loop=1;loop<tblPopUp.rows.length;loop++)
	{
		...
		if (chkBx.checked)
		{
			...
			var stockBal 		=  Math.round(tblPopUp.rows[loop].cells[7].childNodes[0].nodeValue,2);
			...
			if (booCheck == false)
			{
                        ...
                        var url = "materialsTransferXml.php?RequestType=GetRatioBalQty&tostyle="+toStyle+"&toitemcode="+itemDetailID+"&tocolor="+color+"&tosize="+size+"&tobpo="+bpoNo;
                        var htmlObjx = $.ajax({url:url, async:false});
                    
                        var dblBPOBalQty = htmlObjx.responseXML.getElementsByTagName("BPOReqQty")[0].childNodes[0].nodeValue;
			...
			celltxt.innerHTML ="<input type=\"text\" name=\"txtIssueQty\" id=\"txtIssueQty\" class=\"txtbox\" size=\"8\" style=\"text-align:right\" onkeypress=\"return isNumberKey(event);\" value=\""+dblBPOBalQty+"\" onkeyup=\"ValidateQty(this);SetItemQuantity(this," + mainArrayIndex  + ");\" />";		
...
function SaveValidation()
{
        ...
        var toStyleId = $("#cboToScno").val();
        var dblTarnsferQty = 0;
	...
	if (tblMain.rows.length<1)
	{
		...
		tmpColors = tblMain.rows[1].cells[3].childNodes[0].nodeValue;
                tmpSizes = tblMain.rows[1].cells[4].childNodes[0].nodeValue;
                //alert("Colors " + tmpColors + " Sizes " + tmpSizes);
                for (loop = 1 ;loop < tblMain.rows.length; loop++)
		{
                    
                    
                    var itemId   = tblMain.rows[loop].cells[1].id;
                    var strItem  = tblMain.rows[loop].cells[1].childNodes[0].nodeValue;
                    var strColor = tblMain.rows[loop].cells[3].childNodes[0].nodeValue;
                    var strSize  = tblMain.rows[loop].cells[4].childNodes[0].nodeValue;
                    var issueQty = parseFloat(tblMain.rows[loop].cells[7].childNodes[0].value);
                    
                    //Get total requirement of the To-Style 
                    // ====================================
                    var url = "materialsTransferXml.php?RequestType=GetToStyleReqQty&tostyle="+toStyleId+"&toitemcode="+itemId+"&tocolor="+strColor+"&tosize="+strSize;
                    var htmlObj = $.ajax({url:url, async:false});
                    
                    var dblReqQty = htmlObj.responseXML.getElementsByTagName("ReqQty")[0].childNodes[0].nodeValue;                    
                    //================================================
                    
                    //Get transfer qty to the To-Style 
                    // ====================================
                    var url = "materialsTransferXml.php?RequestType=GetTransferIn&tostyle="+toStyleId+"&toitemcode="+itemId+"&tocolor="+strColor+"&tosize="+strSize;
                    var htmlObj = $.ajax({url:url, async:false});
                    
                    var dblJobTrInQty = (htmlObj.responseXML.getElementsByTagName("INQty")[0].childNodes[0].nodeValue);                    
                    //================================================
                    
                    //Get GRN to the To-Style 
                    // ====================================
                    var url = "materialsTransferXml.php?RequestType=GetGRN&tostyle="+toStyleId+"&toitemcode="+itemId+"&tocolor="+strColor+"&tosize="+strSize;
                    var htmlObj = $.ajax({url:url, async:false});
                    
                    var dblGRNQty = (htmlObj.responseXML.getElementsByTagName("GQty")[0].childNodes[0].nodeValue);                    
                    //================================================
			...
			return false;				
                    ...
                    if((tmpColors != strColor) || (tmpSizes != strSize)){
                    ...
                    if((dblGRNQty === null) || (dblGRNQty == "")){dblGRNQty = 0;}
                    if((dblJobTrInQty == null) || (dblJobTrInQty == "")){dblJobTrInQty = 0;}
                    
                    //dblTarnsferQty += issueQty;
                    dblTarnsferQty = issueQty;
                    var totQty = dblTarnsferQty + parseFloat(dblJobTrInQty) + parseFloat(dblGRNQty);
                    var totBalQty = dblReqQty - totQty;
                   /* alert(dblTarnsferQty);
                    alert("Colors " + tmpColors + " - " + strColor + " Sizes " + tmpSizes + " - " + strSize);*/
                    if(totQty > dblReqQty){
                        
                        alert ("Transfer quantity cannot be more than, required quantity in destination order. \n Transfer item " + strItem + " color " + strColor + ", size " + strSize + " total required " + dblReqQty + " and balance can be transfer " + totBalQty);
			return false;	
                    }
	...
	if (xmlHttp1.readyState==4)
	{
		...
		{	
			...
			if(pub_Status==0){
			 ...
			 strInnerHtml+="<td class=\"normalfntRite\">"+parseFloat(XMLStockBal[loop].childNodes[0].nodeValue).toFixed(2)+"</td>";
...
function Confirm()
{
	...
	var htmlobj=$.ajax({url:url,async:false});
		...
		if(document.getElementById("tblMain").rows.length+1 == count){
...
function clearMainGrid(){ 
	...
	for(var loop=1;loop<binCount2;loop++)
	{
...
$(document).ready(function(e){
    ...
    $(document).on('change', '.textvalue', function(){
        
        // Get the text box value 
        var valTransfer = parseFloat($(this).closest('tr').find('.textvalue').val());
        var valTr = parseFloat($(this).closest('td').prev('td').text());
        
        if(valTransfer > valTr){
            alert('Sorry, you cannot exceed the available quantity');
            $(this).closest('tr').find('.textvalue').val(valTr);
            return;
        }

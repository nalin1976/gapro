...
function CreatePopUpItemGrid(htmlobj)
{
	...
	for(loop=0;loop<XMLMatDetailID.length;loop++)
	{
		...
		cell.innerHTML = "<input name=\"checkbox\" type=\"checkbox\" id=\"chksel\" onclick=\"validate("+loop+") \">";
...
//========================================================================
// Created - Nalin Jayakody
// Date - 02/18/2015
// Description - Validate requested quantity more than the GRN balance quantity.
//               If more the reject     
//========================================================================
function validate(prmLineNo){
	
	var tbl = document.getElementById('IssueItem');
	
	var Req_Qty = tbl.rows[prmLineNo+1].cells[5].childNodes[0].nodeValue;
	var GrnQty 	= tbl.rows[prmLineNo+1].cells[6].childNodes[0].nodeValue;
	
	if(parseFloat(Req_Qty)>parseFloat(GrnQty)){
		alert('Requesting quantity exceed the GRN quantity');
		tbl.rows[prmLineNo+1].cells[0].childNodes[0].checked = false;
		return;
	}

...
$poDtFrom       = $class_common->GetFormatDateUS($_GET["pofrom"]);
$poDtTo         = $class_common->GetFormatDateUS($_GET["poto"]);
...
switch($selecCriteria){
    ...
    case 1:
        ...
        if($poDtFrom != ""){
            $sql .= " AND DATE_FORMAT(purchaseorderheader.dtmDate, '%Y-%m-%d') >= '$poDtFrom'";
        }
        
        if($poDtTo != ""){
            $sql .= " AND DATE_FORMAT(purchaseorderheader.dtmDate, '%Y-%m-%d') <= '$poDtTo'";
        }
        break;
    ...
    case 2:
        ...
        if($poDtFrom != ""){
            $sql .= " AND DATE_FORMAT(purchaseorderheader.dtmETD, '%Y-%m-%d') >= '$poDtFrom'";
        }
        
        if($poDtTo != ""){
            $sql .= " AND DATE_FORMAT(purchaseorderheader.dtmETD, '%Y-%m-%d') <= '$poDtTo'";
        }
        break;

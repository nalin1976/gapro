...
function GetShippedStatusFromD2D($prmSCNo, $prmBuyerPONo, $prmD2DConnt){
	...
	$sqlD2D = " SELECT Sum(d2d_Pack_Export_Details.transferOutQty) AS Shipped_Qty 
				...
				WHERE d2d_Pack_Export_Header.scNumber =  '$prmSCNo' AND d2d_Pack_Export_Header.bpo =  '$prmBuyerPONo' AND d2d_Pack_Export_Header.`status` =  'SEND' AND
                      d2d_Pack_Export_Header.destination <>  '20'";	
...
function GetShippedFromHLC($prmSCNo, $prmBuyerPONo, $prmD2DConnt){
	...
	$sqlD2D = " SELECT Sum(stock_aod_detail.qty) AS Shipped_Qty 

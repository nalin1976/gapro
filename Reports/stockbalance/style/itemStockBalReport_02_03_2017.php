  ...
  <tr>
    <td><table width="100%" cellpadding="0" cellspacing="0">
      <tr>
		...
		$SQL_alldetails = " SELECT companies.*, mainstores.strName FROM companies Inner Join mainstores ON companies.intCompanyID = mainstores.intCompanyId
WHERE
mainstores.strMainID =  $mainStores";
...
<p class="normalfntMid">
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#000000">
		...
		$sql = "(select mil.strItemDescription,st.intMatDetailId,st.strColor,st.strSize,st.strUnit, sum(st.dblQty) as stockQty,
...
inner join orders o on o.intStyleId = sl.intStyleId
 ...
 $sql .= " ) as liabilityQty  from stocktransactions st inner join matitemlist mil on mil.intItemSerial= st.intMatDetailId
inner join orders o on o.intStyleId = st.intStyleId
INNER JOIN materialratio ON st.intStyleId = materialratio.intStyleId
AND st.intMatDetailId = materialratio.strMatDetailID
AND st.strColor = materialratio.strColor
AND st.strSize = materialratio.strSize
AND st.strBuyerPoNo = materialratio.strBuyerPONO
AND o.intStyleId = materialratio.intStyleId
where st.strMainStoresID>0 ";
		...
		if($OrderType !='')
			$sql .= " and o.intOrderType ='$OrderType' ";
		$sql .= " group by st.intMatDetailId,st.strColor,st.strSize, materialratio.materialRatioID)
union 
(select mil.strItemDescription,st.intMatDetailId,st.strColor,st.strSize,st.strUnit,0,sum(st.tempQty) as tempQty,0,0,0 
...
inner join orders o on o.intStyleId = stt.intStyleId
 where stt.intMatDetailId = st.intMatDetailId and stt.strColor = st.strColor and stt.strSize = st.strSize ";
		...
		if($dfrom!='')
			$sql .=" and DATE(stt.dtmDate) >='$dfrom' ";
		if($dTo!='')
			$sql .=" and DATE(stt.dtmDate) <='$dTo' ";
		if($size!='')
	...
	union
 (select mil.strItemDescription,st.intMatDetailId,st.strColor,st.strSize,st.strUnit,0,0,sum(leftoverStock) as leftoverStock,0,0
...
inner join orders o on o.intStyleId = stt.intStyleId
 where stt.intMatDetailId = st.intMatDetailId and stt.strColor = st.strColor and stt.strSize = st.strSize ";
 	...
 	$sql .=" )  group by st.intMatDetailId,st.strColor,st.strSize ) ";
 	$sql .= " union
 (select mil.strItemDescription,st.intMatDetailId,st.strColor,st.strSize,st.strUnit,0,0,0,sum(bulkQty) as bulkQty,0
...
(select stt.intMatDetailId from stocktransactions stt inner join matitemlist mil on mil.intItemSerial= stt.intMatDetailId
 ...
 if($mainStores != '')
 	...
 	$sql .= " ) group by st.intMatDetailId,st.strColor,st.strSize ) 
	union
 (select mil.strItemDescription,st.intMatDetailId,st.strColor,st.strSize,st.strUnit,0,0,0,0,sum(liabilityQty) as liabilityQty
...
where st.intMatDetailId=$matId and bulkpurchaseorderheader.strBulkPOType='1' and st.strGRNType='B' ";
		...
		?>
          ...
          <td class="normalfnt"  nowrap="nowrap"><?php echo $row["strUnit"]; ?></td>
          <?php 
		  if($x == 'all' || $x == "running")
			{
		  ?>
        ...
        <tr bgcolor="#EBEBEB" class="normalfntRite">
          <td height="20" class="normalfnt"  nowrap="nowrap" colspan="4">&nbsp;&nbsp;<b>Grand Total</b></td>
          <?php 
		  if($x == 'all' || $x == "running")
			{
		  ?>

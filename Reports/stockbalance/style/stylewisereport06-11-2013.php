...
(SELECT intSRNO FROM specification WHERE specification.intStyleId = st.intStyleId ) AS ScNo, mil.strItemDescription, st.intMatDetailId, 
st.strColor, st.strSize, st.intStyleId, Sum(st.stockQty) AS stockQty, 
(SELECT sum(temp.tempQty) FROM tempstock_view temp 
INNER JOIN matitemlist mil ON mil.intItemSerial = temp.intMatDetailId 
INNER JOIN orders o ON o.intStyleId = temp.intStyleId  ) AS tempQty, 
(SELECT sum(sl.leftoverStock) FROM leftstock_view sl 
INNER JOIN matitemlist mil ON mil.intItemSerial = sl.intMatDetailId 
INNER JOIN orders o ON o.intStyleId = sl.intStyleId  ) AS leftoverQty, materialratio.materialRatioID 
FROM stock_view AS st INNER JOIN matitemlist AS mil ON mil.intItemSerial = st.intMatDetailId 
INNER JOIN orders AS o ON o.intStyleId = st.intStyleId 
INNER JOIN materialratio ON o.intStyleId = materialratio.intStyleId AND mil.intItemSerial = materialratio.strMatDetailID AND st.strColor = materialratio.strColor 
AND st.strSize = materialratio.strSize WHERE st.strMainStoresID > 0 AND mil.intMainCatID = '$mainId' AND intOrderType = '1' 
AND mil.intSubCatID = '$subId'
GROUP BY strOrderNo, strItemDescription ORDER BY materialRatioID ";

...
<p class="normalfntMid">
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#000000">
		...
		mil.strItemDescription,st.intMatDetailId,st.strColor,st.strSize,st.intStyleId,st.strUnit, sum(st.dblQty) as stockQty,
...
(select  o.strOrderNo,o.strStyle,
		(select intSRNO from specification where specification.intStyleId=st.intStyleId)As ScNo,
 mil.strItemDescription,st.intMatDetailId,st.strColor,st.strSize,st.intStyleId,st.strUnit,0,sum(st.tempQty) as tempQty,0
...
inner join orders o on o.intStyleId = stt.intStyleId 
 ...
 (select o.strOrderNo,o.strStyle,
		(select intSRNO from specification where specification.intStyleId=st.intStyleId)As ScNo,
  mil.strItemDescription,st.intMatDetailId,st.strColor,st.strSize,st.intStyleId,st.strUnit,0,0,sum(leftoverStock) as leftoverStock
...
inner join orders o on o.intStyleId = stt.intStyleId 
 where stt.intMatDetailId = st.intMatDetailId and stt.strColor = st.strColor and stt.strSize = st.strSize and stt.intStyleId = st.intStyleId ";
 	if($mainStores != '')
		...
		if($txtmatItem!='')
			$sql .=" and mil.strItemDescription like '%$txtmatItem%' ";
 	$sql .=" )  group by st.intStyleId, st.intMatDetailId, st.strSize, st.strColor) ";
 	 $sql .= " #HAVING 	sum(st.dblQty)>0
	 order by ScNo, strItemDescription, strSize,strColor ";
	//echo $sql;
$preStyleId = '';
			$result = $db->RunQuery($sql);
		while($row = mysql_fetch_array($result))
        ...
        <tr bgcolor="#EBEBEB">
         <td height="20" class="normalfnt" colspan="7" ><b>&nbsp;&nbsp;&nbsp;&nbsp;Grand Total</b></td>

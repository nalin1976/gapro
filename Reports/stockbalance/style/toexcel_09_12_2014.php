...
Sum(stocktransactions.dblQty) AS balance,
...
matmaincategory.strDescription	";
			...
			if($with0 == false){
			 $SQL3 = " HAVING Sum(stocktransactions.dblQty) > 0 ";
...
while($row = mysql_fetch_array($result))
{
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3,$i,$row['strColor']);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4,$i,$row['strSize']);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5,$i,$row['strName']);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7,$i,$row['strUnit']);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8,$i,$balance);
	$i++;
	}
}

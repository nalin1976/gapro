...
$remarks 	= $_GET["remarks"];	
	...
	$sql="select intTransferNo from commonstock_liabilityheader where intTransferNo='$no' and intTransferYear='$year'";
	$result=$db->RunQuery($sql);
	$rowCount=mysql_num_rows($result);
	if($rowCount>0)
	{
		$sql_update="update commonstock_liabilityheader set intStatus=1,intConfirmBy='$userId',dtmConfirmDate=now() where intTransferNo='$no' and intTransferYear='$year'";
		$db->executeQuery($sql_update);
	}
	else
	{
		$sql_insert="insert into commonstock_liabilityheader (intTransferNo,intTransferYear,intToStyleId,intStatus,strRemarks,intUserId,dtmDate,intCompanyId)values('$no','$year','intToStyleId','intStatus','$remarks','$userId',now(),'intCompanyId');";
...
$grnType	 = $_GET["grnType"];
	...
	$sql="select intTransferNo from commonstock_liabilitydetails where intTransferNo='$no' and intTransferYear='$year'";
	$result=$db->RunQuery($sql);
	$rowCount=mysql_num_rows($result);
	if($rowCount>0)
	{
		$sql_update="update commonstock_liabilitydetails set dblQty = $qty where intTransferNo = '$no' and intTransferYear = '$year' and intFromStyleId = '$styleId' and strBuyerPoNo = '$buyerPoNo' and intMatDetailId = '$matDetailId' and strColor = '$color' and strSize = '$size' and strUnit = '$units' and intGrnYear='$grnNoArray[0]' and intGrnNo='$grnNoArray[1]' and strGRNType='$grnType';";
		$db->executeQuery($sql_update);
	}
	else
	{
		$sql_insert="insert into commonstock_liabilitydetails (intTransferNo,intTransferYear,intFromStyleId,strBuyerPoNo,intMatDetailId,strColor,strSize,strUnit,dblQty,intGrnYear,intGrnNo,strGRNType) values('$no','$year','$styleId','$buyerPoNo','$matDetailId','$color','$size','$units','$qty','$grnNoArray[0]','$grnNoArray[1]','$grnType');";
		$db->executeQuery($sql_insert);
	}
}
...
InsertStock("stocktransactions_liability",$no,$year,$styleId,$buyerPoNo,$matDetailId,$color,$size,$units,$qty,$mainStoreId,$subStoreId,$locationId,$binId,$Year,"LBAlloOut",$grnNoArray[0],$grnNoArray[1],$grnType);
InsertStock("stocktransactions",$no,$year,$toStyleId,$toBuyerPoId,$matDetailId,$color,$size,$units,$stockQty,$mainStoreId,$subStoreId,$locationId,$binId,$Year,"LBAlloIn",$grnNoArray[0],$grnNoArray[1],$grnType);
...
echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
	...
	$SQLHeder="SELECT COUNT(intTransferNo) AS headerRecCount FROM commonstock_liabilityheader where intTransferNo=$no AND intTransferYear=$year";
	...
	$SQLDetail="SELECT COUNT(intTransferNo) AS DetailsRecCount FROM commonstock_liabilitydetails where intTransferNo=$no AND intTransferYear=$year";
	...
	$resultDetail=$db->RunQuery($SQLDetail);
		...
		$SQL="SELECT COUNT(intDocumentNo) AS binDetails FROM stocktransactions where intDocumentNo=$no AND intDocumentYear=$year and strType='LBAlloIn'";	
...
$ResponseXML  = "<XMLLoadPopNo>\n";
	...
	$sql="select distinct intTransferNo,concat(intTransferYear,'/',intTransferNo)as concatNo from commonstock_liabilityheader
...
$validate 	= true;
	...
	switch($status)
	{
		...
		case 1:
		{
			$sql="select strMainStoresID,strSubStores,strLocation,strBin,intStyleId,strBuyerPoNo,intMatDetailId,strColor,strSize,strUnit,dblQty,intGrnNo,intGrnYear,strGRNType from stocktransactions where intDocumentNo='$noArray[1]' and intDocumentYear='$noArray[0]' and strType='LBAlloIn'";
			$result=$db->RunQuery($sql);
			while($row=mysql_fetch_array($result))
			{
				$qty = $row["dblQty"];
				...
				if($qty>$validateQty)
				{
					...
					$ResponseXML .= "<Message><![CDATA["."Sorry!\nYou cannot cancel this allocation,Because not enough stock qty available in the Bin."."]]></Message>\n";
					$validate 	  = false;
					$ResponseXML .= "</XMLCancel>";
					echo $ResponseXML;
					return;	
			...
			if($validate)
			{		
				Cancel("stocktransactions_liability",$noArray[0],$noArray[1],"LBAlloOut");
				Cancel("stocktransactions",$noArray[0],$noArray[1],"LBAlloIn");
				UpdateStatus($noArray[0],$noArray[1],$reason);
...
global $db;
global $userId;
	$sql_update="update commonstock_liabilityheader 
...
{
global $db;
	$sql="select intStatus from commonstock_liabilityheader 	
	where
	intTransferNo = '$no' 
	and intTransferYear = '$year';";
	$result=$db->RunQuery($sql);

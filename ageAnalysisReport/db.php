...
elseif($RequestType=="LoadSubStore"){
    
    $mainStoreId = $_GET["mainId"];
    
    $ResponseXML = "<XMLLoadSubStores>\n";
    
    $sql = "SELECT strSubID, strSubStoresName FROM substores WHERE strMainID = $mainStoreId";
    //echo $sql;
    $result = $db->RunQuery($sql);
    
    $ResponseXML .= "<option value=\"". "" ."\">".""."</option>\n";
    
    while(list($subId, $subStore) = mysql_fetch_array($result)){
        
        $ResponseXML .= "<option value=\"". $subId ."\">".$subStore."</option>\n";

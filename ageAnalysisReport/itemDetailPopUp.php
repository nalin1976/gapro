...
<table width="850" border="0" cellspacing="0" cellpadding="2" align="center">
  ...
  <tr>
	...
	$grandTotalValue =0;
	//echo $sql;
	$result = getGRNDetails('stocktransactions',$MatDetailID,$color,$size,$mainStores,$StyleID,$OrderType,$Merchandiser,$bulkPOType);
  ...
  <?php 
	...
	$result_t = getGRNDetails('stocktransactions_temp',$MatDetailID,$color,$size,$mainStores,$StyleID,$OrderType,$Merchandiser,$bulkPOType);
  ...
  ?>
  <?php 
		$result_l = getGRNDetails('stocktransactions_leftover',$MatDetailID,$color,$size,$mainStores,$StyleID,$OrderType,$Merchandiser,$bulkPOType);
  ...
  <?php 
	...
	$result_B = getGRNDetails('stocktransactions_bulk',$MatDetailID,$color,$size,$mainStores,$StyleID,$OrderType,$Merchandiser,$bulkPOType);
	...
	$result_L = getGRNDetails('stocktransactions_liability',$MatDetailID,$color,$size,$mainStores,$StyleID,$OrderType,$Merchandiser,$bulkPOType);
	$rowCount_L = mysql_num_rows($result_L);
...
where gd.intGrnNo='$grnNo' and gd.intGRNYear='$grnYear'  and gd.intMatDetailID='$matDetailID' ";
...
inner join suppliers s on s.strSupplierID = poh.strSupplierID
inner join currencytypes c on c.intCurID= poh.strCurrency
where gd.intBulkGrnNo='$grnNo' and gd.intYear='$grnYear'  and gd.intMatDetailID='$matDetailID' ";
	}
	if($color!='')
		$sqlSup .= " and gd.strColor='$color'";
	if($size!='')
		$sqlSup .= " and d.strSize='$size'";
 $sqlSup;
	return $db->RunQuery($sqlSup);
}

function getGRNDetails($tbl,$MatDetailID,$color,$size,$mainStores,$StyleID,$OrderType,$Merchandiser,$bulkPOType)
{
	global $db;
	if($tbl != 'stocktransactions_bulk')
	{
		...
		$sql .= " where intMatDetailId='$MatDetailID' ";	
		if($mainStores !='')
...
and st.intBulkGrnNo = bgd.intBulkGrnNo and st.intBulkGrnYear = bgd.intYear
 where st.intMatDetailId ='$MatDetailID'  ";
		...
		if($bulkPOType != '')
			$sql .= " and bpo.strBulkPOType = $bulkPOType ";
		if($color!='')
			$sql .= " and st.strColor='$color'";
		if($size!='')
			$sql .= " and st.strSize='$size'"; 	

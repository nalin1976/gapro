      ...
      <?php
		...
		$totQty = 0;
			$sql = "select 
					...
					where st.intMatDetailId=$matId   and st.strGRNType='S' and st.strMainStoresID = $mainStores  ";
		if($fromVal == '120')
		...
		//echo $sql;	
			while($row_det = mysql_fetch_array($resultDetail))
			{
				if($row_det['strType']=='BAlloIn')
					...
					$type = 'GRN';
				else if($row_det['strType']=='SGatePass')
					$type = 'Gate Pass Out';	
				else if($row_det['strType']=='TI')
					$type = 'Gate Pass In';	
				else if($row_det['strType']=='SRTSUP')

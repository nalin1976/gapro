...
require_once 'classes/class_itemmaster.php';

$d2dConnectClass = new ClassConnectD2D();
$class_item_master = new ItemMaster();
...
$class_item_master->SetConnection($db);
...
else if(strcmp($RequestType,"GetItemDiscription")==0)
{
	...
	while($row=mysql_fetch_array($result))
	{
		...
		$ResponseXML.="<UnitPrice><![CDATA[" .($row["dblUnitPrice"]=="" ? 0:$row["dblUnitPrice"]). "]]></UnitPrice>\n";
                $ResponseXML.="<FabType><![CDATA[" .$row["Type_Name"]. "]]></FabType>\n";
                $ResponseXML.="<FabTypeCode><![CDATA[" .$row["Type_Code"]. "]]></FabTypeCode>\n";
...
else if(strcmp($RequestType,"SaveItems")==0)
{
	...
	$mainFabricStatus=$_GET["mainFabricStatus"];
        ...
        $dbl_wastepsp   = $_GET["wastepsp"];
        $dbl_wastedp   = $_GET["wastedp"];
        $dbl_wasteemb   = $_GET["wasteemb"];
        $dbl_wastedpe   = $_GET["wastedpe"];
        $dbl_wastegrw   = $_GET["wastegrw"];
        $dbl_wastegew   = $_GET["wastegew"];
        $dbl_wastegpd   = $_GET["wastegpd"];
        $dbl_wastespe   = $_GET["wastespe"];
        $dbl_wastepeosp   = $_GET["wastepeosp"];
        $dbl_wastepeodp   = $_GET["wastepeodp"];
        
        $intPanels      = $_GET["lgpanels"];
        $intEmbPanels   = $_GET["embpanels"];
        $intPrintPanels = $_GET["prnpanels"];
...
WHERE  intStyleId = '$intStyleId' AND intMatDetailID = '$intMatDetailID'";
		...
		else
		{
			 ...
			 $sqlInsert = "insert into orderdetails(strOrderNo,intStyleId,intMatDetailID,strUnit,dblUnitPrice,reaConPc,reaWastage,strCurrencyID,intOriginNo,dblReqQty,dblTotalQty,dblTotalValue,
                                                                dbltotalcostpc,dblFreight,intMillId,intMainFabricStatus,intstatus, booWashable,dblPSP,dblDP,dblEMB,dblPEOSP,dblDPE,dblGEW,dblGPD,dblSPE,
                                                                dblPEODP,intEmbPanels,intPrnPanels,dblGRW,intLgPanels) 
                                       values ('$strOrderNo','$strStyleID','$intMatDetailID','$strUnit','$dblUnitPrice','$reaConPc','$reaWastage','$strCurrencyID','$intOriginNo','$dblReqQty',
                                               '$dblTotalQty','$dblTotalValue','$dbltotalcostpc','$freight','0','0','1','$isWashable','$dbl_wastepsp','$dbl_wastedp','$dbl_wasteemb','$dbl_wastepeosp',
                                               '$dbl_wastedpe','$dbl_wastegew','$dbl_wastegpd','$dbl_wastespe','$dbl_wastepeodp','$intEmbPanels','$intPrintPanels','$dbl_wastegrw','$intPanels');";
...
else if(strcmp($RequestType,"UpdateItems")==0)
{
	...
	$mainFabric=$_GET["mainFabric"];
        ...
        $dbl_psp = $_GET["wastepsp"];
        $dbl_dp = $_GET["wastedp"];
        $dbl_emb = $_GET["wasteemb"];
        $dbl_dpe = $_GET["wastedpe"];
        $dbl_grw = $_GET["wastegrw"];
        $dbl_gew = $_GET["wastegew"];
        $dbl_gpd = $_GET["wastegpd"];
        $dbl_spe = $_GET["wastespe"];
        $dbl_peodp = $_GET["wastepeodp"];
        $dbl_peosp = $_GET["wastepeosp"];
        $int_embpanel = $_GET["embpanels"];
        $int_prnpanel = $_GET["prnpanels"];
        $int_lgpanels = $_GET["lgpanels"];
	...
	$result=UpdateOrderDetails($strOrderNo,$strStyleID,$intMatDetailID,$strUnit,$dblUnitPrice,$reaConPc,$reaWastage,$strCurrencyID,$intOriginNo,$dblReqQty,$dblTotalQty,$dblTotalValue,$dbltotalcostpc,$freight,$mill,
                                   $mainFabric,$IsToBeWashed,$dbl_psp,$dbl_dp,$dbl_emb,$dbl_dpe,$dbl_grw,$dbl_gew,$dbl_gpd,$dbl_spe,$dbl_peodp,$dbl_peosp,$int_embpanel,$int_prnpanel,$int_lgpanels);
	$ResponseXML.="<SaveOrderDeatils>";
	$ResponseXML.="<SaveState><![CDATA[True]]></SaveState>\n";
	$ResponseXML.="</SaveOrderDeatils>";
	echo $ResponseXML;
...
else if(strcmp($RequestType,"getOrderDetailData")==0)
{
	...
	while($row=mysql_fetch_array($result))
	{
		...
		$ResponseXML.="<PSP><![CDATA[" .$row["dblPSP"]. "]]></PSP>\n";
		$ResponseXML.="<DP><![CDATA[" .$row["dblDP"]. "]]></DP>\n";
		$ResponseXML.="<EMB><![CDATA[" .$row["dblEMB"]. "]]></EMB>\n";
		$ResponseXML.="<PEOSP><![CDATA[" .$row["dblPEOSP"]. "]]></PEOSP>\n";
		$ResponseXML.="<DPE><![CDATA[" .$row["dblDPE"]. "]]></DPE>\n";
		$ResponseXML.="<GEW><![CDATA[" .$row["dblGEW"]. "]]></GEW>\n";
		$ResponseXML.="<GPD><![CDATA[" .$row["dblGPD"]. "]]></GPD>\n";
		$ResponseXML.="<SPE><![CDATA[" .$row["dblSPE"]. "]]></SPE>\n";
		$ResponseXML.="<PEODP><![CDATA[" .$row["dblPEODP"]. "]]></PEODP>\n";
		$ResponseXML.="<GRW><![CDATA[" .$row["dblGRW"]. "]]></GRW>\n";
		$ResponseXML.="<LG_PANELS><![CDATA[" .$row["intLgPanels"]. "]]></LG_PANELS>\n";
		$ResponseXML.="<EMB_PANELS><![CDATA[" .$row["intEmbPanels"]. "]]></EMB_PANELS>\n";
		$ResponseXML.="<PRN_PANELS><![CDATA[" .$row["intPrnPanels"]. "]]></PRN_PANELS>\n";
...
else if(strcmp($RequestType,"UpdateRevice")==0)
{
	...
	if($reviseOrder)
	{
		...
		$db->ExecuteQuery($sql);
                ...
                /*
                 * 
                 * Comment On - 08/03/2017
                 * Comment By - Nalin Jayakody
                 * Comment For - Add new feilds to the query to avoid re-initialize 
                 * =================================================================
                 */
...
else if (strcmp($RequestType,"AddDeliveryBPO")==0)
{
	...
	//$sql = "Insert into deliveryschedule(intStyleId,dtDateofDelivery,dblQty,dbExQty,strShippingMode,isDeliveryBase,intSerialNO,strRemarks,intUserId,dtmDate,dtmHandOverDate,intBPO,intRefNo,estimatedDate,intCountry,intDeliveryStatus, dtmCutOffDate, intManufacturingLocation, intShortShipped, prvBPO, shortShipId)values('".$styleID."','".$bpoDelivery."',".$bpoQty.",'0',".$bpoShipMode.",'N','$bpoLeadTime','$bpoRemarks','$userID','$bpoDelivery','$bpoHandover','$buyerPO','0','$bpoEstimate','$bpoCountry','$bpoDeliveryStatus','$bpoCutOffDate','$bpoManuLocation','$iShipStatus','$previousBuyerPO', '$ishtShipReason');";
	$sql = "Insert into deliveryschedule(intStyleId,dtDateofDelivery,dblQty,dbExQty,strShippingMode,isDeliveryBase,intSerialNO,strRemarks,intUserId,dtmDate,dtmHandOverDate,intBPO,intRefNo,estimatedDate,intCountry,intDeliveryStatus, dtmCutOffDate, intManufacturingLocation, intShortShipped, prvBPO, shortShipId,dtmPCDDate,intPlanConfirm)values('".$styleID."','".$bpoDelivery."',".$bpoQty.",'0',".$bpoShipMode.",'N','$bpoLeadTime','$bpoRemarks','$userID','$bpoDelivery','$bpoHandover','$buyerPO','0','$bpoEstimate','$bpoCountry','$bpoDeliveryStatus','$bpoCutOffDate','$bpoManuLocation','$iShipStatus','$previousBuyerPO', '$ishtShipReason','$pcdDate','1');";
	//echo $sql;
	...
	echo $ResponseXML; 
	  ...
	  $sqlUp = "SELECT
				bpo_planning_con.strBuyerpoNo,
				bpo_planning_con.strPrvBpo,
				bpo_planning_con.intStatus,
				bpo_planning_con.intStyleId
				FROM
				bpo_planning_con
				WHERE
				bpo_planning_con.intStatus = '0' AND
				bpo_planning_con.intStyleId = '$styleID' AND
				bpo_planning_con.strBuyerpoNo = '$buyerPO' AND
				bpo_planning_con.strPrvBpo = '$previousBuyerPO'";
	$resultUp=$db->RunQuery($sqlUp);
	while($rowUp=mysql_fetch_array($resultUp))
	{
		$num_rows = mysql_num_rows($resultUp);
		
		$styleId = $rowUp["intStyleId"];
		$buyerPO = $rowUp["strBuyerpoNo"];
		$prvBuyerPO = $rowUp["strPrvBpo"];
		
			if($num_rows>0){
				
				 $sql = "UPDATE deliveryschedule
						SET intPlanConfirm= '0'
						WHERE
							(intStyleId = '$styleId ')
						AND (intBPO = '$buyerPO')
						AND (	prvBPO = '$prvBuyerPO')";
				 $db->executeQuery($sql);
...
elseif($RequestType=="GetFabricTypes"){
    
    $ResponseXML = "";
    $ResponseXML="<FABRIC_TYPES>\n";
   
    $rs_fabrictypes = $class_item_master->GetFabricType();
    
    while($rowFabricTypes = mysql_fetch_array($rs_fabrictypes)){
        $ResponseXML.= "<TYPE_CODE><![CDATA[" .$rowFabricTypes["Type_Code"]. "]]></TYPE_CODE>\n";
        $ResponseXML.= "<TYPE_NAME><![CDATA[" .$rowFabricTypes["Type_Name"]. "]]></TYPE_NAME>\n";
    }
...
elseif($RequestType=="GetFabOps"){
    
    $intOrderQty = $_GET["orderqty"];
    
    $ResponseXML = "";
    $ResponseXML="<FABRIC_OPERATIONS>\n";
   
    //$rs_fabric_ops = $class_item_master->GetFabricOperation();
    $rs_fabric_ops = $class_item_master->GetWastageWithFabricOperation($intOrderQty);
    
    while($rowFabricOps = mysql_fetch_array($rs_fabric_ops)){
        $ResponseXML.= "<OP_CODE><![CDATA[" .$rowFabricOps["Operation_Code"]. "]]></OP_CODE>\n";
        $ResponseXML.= "<OP_NAME><![CDATA[" .$rowFabricOps["Operation_Name"]. "]]></OP_NAME>\n";
        $ResponseXML.= "<OP_WASTAGE><![CDATA[" .$rowFabricOps["Wastage"]. "]]></OP_WASTAGE>\n";
...
elseif($RequestType == "GetOrderQtyCode"){
    
    $dblOrderQty = $_GET["ordeqty"];
    
    $rs_OrderQty = $class_item_master->GetOrderQtyCode($dblOrderQty);
    
    $ResponseXML = "";
    $ResponseXML="<ORDER_QTYS>\n";
    
    while($rowOrderQty = mysql_fetch_array($rs_OrderQty)){
        $ResponseXML.= "<QTY_CODE><![CDATA[" .$rowOrderQty["Order_Qty_ID"]. "]]></QTY_CODE>\n";
...
elseif($RequestType == "GetFabWastage"){
    
    $intOrderQtyId = $_GET["qtyid"];
    $intFabTypeId = $_GET["fabtype"];
    
    
    $dbl_FabWaste = $class_item_master->GetFabricTypeWastage($intOrderQtyId, $intFabTypeId);
    
    $ResponseXML = "";
    $ResponseXML="<FAB_WASTAGE>\n";
    $ResponseXML.= "<WASTE_FIG><![CDATA[" .$dbl_FabWaste. "]]></WASTE_FIG>\n";
...
elseif($RequestType == "GetFabOpWastage"){
    
    $intOrderQtyId = $_GET["qtyid"];
    $intFabOpCode = $_GET["fabOpCode"];
    
    $dbl_FabOpWaste = $class_item_master->GetFabricOpWastage($intOrderQtyId, $intFabOpCode);
    
    $ResponseXML = "";
    $ResponseXML ="<FAB_OP_WASTAGE>\n";
    $ResponseXML.= "<WASTE_FIG_OP><![CDATA[" .$dbl_FabOpWaste. "]]></WASTE_FIG_OP>\n";
...
function getOrderDetailsData($styleID)
{
	...
	global $db;
			$sql="SELECT strOrderNo,intStyleId,intMatDetailID,o.dblUnitPrice,reaConPc,reaWastage,intOriginNo,
                                     dblReqQty,dblTotalQty,dblTotalValue,dbltotalcostpc,dblFreight,m.strItemDescription,o.strUnit,
                                     intMillId,intMainFabricStatus,intMainCatID, dblPSP, dblDP,dblEMB,dblPEOSP,dblDPE,dblGEW,
                                     dblGPD,dblSPE,dblPEODP,intLgPanels,intEmbPanels,intPrnPanels,dblGRW
			FROM
			orderdetails AS o
...
function getItemDiscription($itemID)
{
	global $db;
        ...
        /*
         * Comment On - 07/20/2017
         * Comment BY - Nalin Jayakody
         * Comment For - add fabric type to selected fabric item
         * ===========================================================================
         */
	...
	//$sql="SELECT strUnit,sngWastage,dblUnitPrice, fabric_type.Type_Name, case fabric_type.Type_Code FROM matitemlist m LEFT JOIN fabric_type ON m.intFabricType = fabric_type.Type_Code where intItemSerial=".$itemID." and intStatus=1;";
	$sql=" SELECT strUnit,sngWastage,dblUnitPrice, fabric_type.Type_Name,
               CASE WHEN fabric_type.Type_Code IS NULL
                    THEN -1
                    ELSE fabric_type.Type_Code
               END AS Type_Code     
               FROM matitemlist m LEFT JOIN fabric_type ON m.intFabricType = fabric_type.Type_Code where intItemSerial=".$itemID." and intStatus=1;";
	return $db->RunQuery($sql);	
}
...
function UpdateOrderDetails($strOrderNo,$strStyleID,$intMatDetailID,$strUnit,$dblUnitPrice,$reaConPc,$reaWastage,$strCurrencyID,$intOriginNo,$dblReqQty,$dblTotalQty,$dblTotalValue,$dbltotalcostpc,$freight,$mill,$mainFabric,
                            $isToBeWash,$dbl_psp,$dbl_dp,$dbl_emb,$dbl_dpe,$dbl_grw,$dbl_gew,$dbl_gpd,$dbl_spe,$dbl_peodp,$dbl_peosp,$int_embpanel,$int_prnpanel,$int_lgpanels)
{
	global $db;
	...
	$sql="update orderdetails set strOrderNo = '$strOrderNo' , strUnit = '$strUnit' ,dblUnitPrice = '$dblUnitPrice' ,reaConPc = '$reaConPc' ,reaWastage = '$reaWastage' ,strCurrencyID = '$strCurrencyID' ,
                 	              intOriginNo = '$intOriginNo' ,dblReqQty = '$dblReqQty',dblTotalQty = '$dblTotalQty',dblTotalValue = '$dblTotalValue' ,dbltotalcostpc = '$dbltotalcostpc' ,dblFreight = '$freight',
                                      intMillId='0' , intMainFabricStatus='0', intstatus = '1', booWashable = '$isToBeWash',dblPSP='$dbl_psp',dblDP='$dbl_dp',dblEMB='$dbl_emb',dblPEOSP='$dbl_peosp',dblDPE='$dbl_dpe',
                                      dblGEW='$dbl_gew',dblGPD='$dbl_gpd',dblSPE='$dbl_spe',dblPEODP='$dbl_peodp',intEmbPanels='$int_embpanel',intPrnPanels='$int_prnpanel',dblGRW='$dbl_grw',intLgPanels='$int_lgpanels'
              where	intStyleId = '$strStyleID' and intMatDetailID = '$intMatDetailID' ;";
	//echo $sql;
...
function updateItemDetails($styleID,$qty,$exPercentage)
{
	...
	$sql= "select orderdetails.intMatDetailID,matitemlist.intMainCatID, orderdetails.dblUnitPrice, orderdetails.reaConPc,
                      orderdetails.reaWastage, orderdetails.dblFreight, orderdetails.dblPSP, orderdetails.dblDP, orderdetails.dblEMB, orderdetails.dblPEOSP, orderdetails.dblDPE,orderdetails.dblGEW,
                      orderdetails.dblGPD,orderdetails.dblSPE,orderdetails.dblPEODP,orderdetails.intLgPanels,orderdetails.intEmbPanels,orderdetails.intPrnPanels,orderdetails.dblGRW
               from orderdetails inner join matitemlist on orderdetails.intMatDetailID = matitemlist.intItemSerial 
               where orderdetails.intStyleId = '$styleID' AND (orderdetails.intstatus != '0' or orderdetails.intstatus IS NULL);";
	...
	while($row=mysql_fetch_array($result))
	{
		...
		$dblFreight = $row["dblFreight"];
                 /* */
                $dblPSPWaste = $row["dblPSP"];
                $dblDPWaste = $row["dblDP"];
                $dblEMBWaste = $row["dblEMB"];
                $dblPEOSPWaste = $row["dblPEOSP"];
                $dblDPEWaste = $row["dblDPE"];
                $dblGEWWaste = $row["dblGEW"];
                $dblGPDWaste = $row["dblGPD"];
                $dblGRWWaste = $row["dblGRW"];
                $dblSPEWaste = $row["dblSPE"];
                $dblPEODPWaste = $row["dblPEODP"];
                                
                $intLargePanels = $row["intLgPanels"];
                $intEmbPanels = $row["intEmbPanels"];
                $intPrintPanels = $row["intPrnPanels"];
		/* */
		...
		{	
			...
			$totalQty = $totalQty + ($reqQty*$reaWastage/100);
                        ...
                        /* ============================================== */
                        //$dblOperationWastageQty = ((($totalQty * $dblOpWastage)/100) * ($conPc*($intEmbPanels/$intLargePanels))) + ((($totalQty * $dblOpWastage)/100) * ($conPc*($intPrintPanels/$intLargePanels)));
                        
                        // Print Embroidery On Single Panel
                        $dblOperationWastageQty = ((($totalQty * $dblPEOSPWaste)/100) * ($conPc*($intEmbPanels/$intLargePanels))); 
                        
                        // Print Embroidery On different Panel
                        $dblOperationWastageQty +=  ((($totalQty * $dblPEODPWaste)/100) * ($conPc*($intEmbPanels/$intLargePanels))) + ((($totalQty * $dblPEODPWaste)/100) * ($conPc*($intPrintPanels/$intLargePanels)));
                        
                        // Print single Panel/ Disrcharge printing / Embroidery
                        $dblOperationWastageQty +=  ((($totalQty * $dblPSPWaste)/100) * $conPc) + ((($totalQty * $dblDPWaste)/100) * $conPc) + ((($totalQty * $dblEMBWaste)/100) * $conPc);
                        
                        // Discharge printing & embroidery / Garment enzyme washed / Garment pigment dye
                        $dblOperationWastageQty +=  ((($totalQty * $dblDPEWaste)/100) * $conPc) + ((($totalQty * $dblGEWWaste)/100) * $conPc) + ((($totalQty * $dblGPDWaste)/100) * $conPc);
                        
                        // Garment Regular Washed / Special effect
                        $dblOperationWastageQty +=  ((($totalQty * $dblGRWWaste)/100) * $conPc) + ((($totalQty * $dblSPEWaste)/100) * $conPc);
                        
                        
			$totalQty += $dblOperationWastageQty;
                        /* ============================================== */
			if($totalQty <1)
				$totalQty =1;
			/*else

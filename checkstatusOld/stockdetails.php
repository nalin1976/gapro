...
<?php
$sqlgrn="select 
		(select strName from mainstores MS where MS.strMainID=ST.strMainStoresID)AS MainStore,
		...
		strUnit,
		sum(dblQty)AS StckBal,
		intDocumentNo,
		intDocumentYear 
		from stocktransactions ST 
		where intStyleId='$styleID'
		AND strBuyerPoNo='$buyerPoNo'
		AND intMatDetailId=$matItemList
		AND strColor='$color'
		AND strSize='$size'
		AND strType='GRN'
		AND strMainStoresID='$mainStore'
...
sum(dblQty)AS StckBal,
(select UA.Name from useraccounts UA where UA.intUserID=ST.intUser)AS UserName,
...
<?php
$sqlcanceljob="select sum(dblQty) AS cancelQty
from stocktransactions ST
where intDocumentNo='".$row_job["intDocumentNo"]."' 
AND intDocumentYear='".$row_job["intDocumentYear"]."'
AND strType='CIJTOUT'
...
AND strSize='$size'
group by
intStyleId,strBuyerPoNo,intMatDetailId,strColor,strSize,strType,intDocumentNo,intDocumentYear";
                ...
                <tr>
                  <td><div id="divLocationTransfer" style="overflow:scroll; height:150px; width:846px;">
                    <table id="tblLocationTransfer" width="825" cellpadding="0" cellspacing="1" bgcolor="#CCCCFF">
                      ...
                      <?php
$sqlti="select TIH.intTransferInNo,TIH.intTINYear,sum(dblQty)AS TIQty,
TIH.intGatePassNo,TIH.intGPYear,
(select strItemDescription from matitemlist MIL where MIL.intItemSerial=TID.intMatDetailId)AS ItemDescription,
UA.Name AS UserName
from gategasstransferinheader TIH
inner join gategasstransferindetails TID
...
AND strSize='$size'
AND intStatus=1
AND UA.intCompanyID='$companyId'
Group By intTransferInNo,
intTINYear,
intMatDetailId";
...
$sqlgpqty="select sum(dblQty)AS GPQty from gatepassdetails GD
inner join gatepass GH
on GH.intGatePassNo=GD.intGatePassNo AND GH.intGPYear=GD.intGPYear
where intStyleId='$styleID'
AND strBuyerPONO='$buyerPoNo'
AND intMatDetailId=$matItemList
AND strColor='$color'
...
AND ST.intMatDetailId='$matItemList'

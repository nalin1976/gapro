...
(orders.intStatus = '$arrstatus[0]' or orders.intStatus = '$arrstatus[1]' or orders.intStatus = '$arrstatus[2]' or orders.intStatus = '20' or orders.intStatus = '25') order by orders.strStyle";
...
else if (strcmp($RequestType,"getStyleNOS") == 0)
{
	 ...
	 $sql = "SELECT DISTINCT orders.strStyle
FROM materialratio INNER JOIN orders ON materialratio.intStyleId = orders.intStyleId AND 
(orders.intStatus = '$arrstatus[0]' or orders.intStatus = '$arrstatus[1]' or orders.intStatus = '$arrstatus[2]' or orders.intStatus = '5' or orders.intStatus = '20' or orders.intStatus = '25') ";
...
FROM materialratio INNER JOIN orders ON materialratio.intStyleId = orders.intStyleId AND 
(orders.intStatus = '$arrstatus[0]' or orders.intStatus = '$arrstatus[1]' or orders.intStatus = '$arrstatus[2]' or orders.intStatus = '5' or orders.intStatus = '20' or orders.intStatus = '25') order by strOrderNo";
...
else if(strcmp($RequestType,"CheckPossibility") == 0)
{
	...
	//echo "stockQty = ".$stockQty.' getIssueQty =  '.$issueQty.' mrnQty =  '.$mrnQty;
	$balQty=round(($stockQty + $issueQty)-$mrnQty,2);
...
else if(strcmp($RequestType,"GetSCNO") == 0)
{
	...
	global $headerPub_AllowOrderStatus;
	 ...
	 //$sql="SELECT DISTINCT s.intSRNO FROM specification s INNER JOIN materialratio m ON s.intStyleId=m.intStyleId where s.intOrdComplete='0';";
        ...
        // ============================================================== 
        // Comment For - Add status 5 to the SC list
        // Comment By - Nalin Jayakody
        // Comment On - 07/28/2016
        // ==============================================================
	...
	// ==============================================================
         ...
         $sql="SELECT DISTINCT S.intSRNO,MR.intStyleId FROM materialratio MR INNER JOIN orders O ON MR.intStyleId = O.intStyleId inner join specification S on S.intStyleId=O.intStyleId where ((O.intStatus = '$arrstatus[0]') or (O.intStatus = '$arrstatus[1]') or (O.intStatus = '$arrstatus[2]') or (O.intStatus = '5') or (O.intStatus = '20') or (O.intStatus = '25')) order by  S.intSRNO desc";
...
if (strcmp($RequestType,"confirmMrn") == 0)
{
	...
	while($row = mysql_fetch_array($stockAv))
	{
		...
		if($mrnQty>$grnQty)
			$result = "MRN item ".$row["strItemDescription"]." \n "." Color ".$row["strColor"]."\n"." Size ".$row["strSize"]." not in stock order no - ".$orderNo." Mrn Qty = ".$mrnQty." Grn Qty = ".$grnQty;
	}

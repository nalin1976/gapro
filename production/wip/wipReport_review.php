...
$str_comment_date="SELECT dtmDateComment FROM wip_comment GROUP BY dtmDateComment ORDER BY dtmDateComment";
$result_commentdate=$db->RunQuery($str_comment_date);
$noOfRowsCmtDate=mysql_num_rows($result_commentdate);
while($row_commentDate=mysql_fetch_array($result_commentdate))
{
	$dataArrayCommentDate[$indx++]=$row_commentDate["dtmDateComment"];
}
...
$(document).ready(function() 
{
	...
	$('#tblMain2 tbody tr').click(function(){
		...
		$("#switchreveiew").click(function(){
			...
			location.href='wipReport_final.php';
		})
})
</script>
<script type="text/javascript">
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr>
    <td class=" main-border"><table width="100%" border="0" cellspacing="0" cellpadding="3">
      <tr>
        <td class="head2 " style="color:#045182;text-align:left;" > WIP Review<span style="color:#318cc5;font:12px Verdan;display:none"></span></td>
      </tr>
      <tr>
        <td><table  width="100%" cellspacing="1" cellpadding="5"  bgcolor="#D0D7E5" id="tblMain2" class="fixHeader">
          <thead>
            <tr bgcolor="#D9E2EE">
              ...
              <th nowrap="nowrap" class="normalfntMid cutting" id='group_cutting' style="background:#BCD75C" colspan="<?php echo $noOfRowsCmtDate;?>">Comment</th>
            ...
            <tr bgcolor="#9EB6CE">
              ...
              <?php foreach($dataArrayCommentDate as $dtarr){?>
              <th  class="normalfntMid cutting-sub cutting" nowrap="nowrap" title="(Cut Quatity - Dispatch Quantity)"><?php echo $dtarr?></th>
              <?php }?>
		...
		while($row=mysql_fetch_array($result))
		{
			...
			$cutRate = round(((($row["reaSMV"]*$row["dblCutting"])/100)*($row["reaSMVRate"]))+($row["FabricCostPerPCS"]),4);
			
			$sewingTotBal = ($row["intOutPutQty"]-$row["intWashReady"]);
			$sewingRate = round(((($row["reaSMV"]*$row["dblSewing"])/100)*($row["reaSMVRate"]))+($row["FabricCostPerPCS"]+$row["AccessoriesCostPerPCS"]),4);
        ...
        ?>
            <tr bgcolor="#FFFFFF" class="trclass">
              ...
              <?php foreach($dataArrayCommentDate as $dtarr2){?>
              <td  nowrap="nowrap"  class="normalfntRite cutting-fnt cutting"><?php echo getcommentByDate($row["intStyleId"],$dtarr2)?></td>
              <?php }?>
...
<div style=" padding: 2px; display: none; position: absolute; background-color: #F8F8F8; top: 667px; left: 330px;" id="grid_menu" >
			<table width="150" id='menu-hide' class="normalfnt menu-hide-class" cellpadding="3" style="border:1px solid #9EB6CE">
                ...
                <tr>
                    <td id="switchreveiew" class="normalfnt" height="20" >Switch Detail View</td>
                </tr>
            </table>
...
<?php 
	...
	function getcommentByDate($style,$date)
	{
		global $db;
		$str="SELECT stComment FROM wip_comment WHERE dtmDateComment='$date' AND intStyleId='$style'";
		$result=$db->RunQuery($str);
		while($row=mysql_fetch_array($result))
		{
			$comment.="<p>".$row["stComment"]."</p>";
		}
		return $comment;
	}
?>

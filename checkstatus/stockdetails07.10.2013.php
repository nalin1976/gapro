...
<?php
echo $sqlcanceljob="select sum(ST.dblQty) AS cancelQty
from 
stocktransactions ST,materialratio M 
where intDocumentNo='".$row_job["intDocumentNo"]."' 
AND intDocumentYear='".$row_job["intDocumentYear"]."'
...
AND TID.strColor='$colo' AND TID.strSize='$size' AND intStatus=1 AND UA.intCompanyID='$companyId' Group By intTransferInNo, intTINYear, intMatDetailId ";

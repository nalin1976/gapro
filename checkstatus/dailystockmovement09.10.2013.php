...
/*echo $sql="SELECT
S.strType,
S.dtmDate AS dtmDate,
S.dblQty AS Qty,
Concat(intDocumentYear,'/',intDocumentNo) AS refNo,
materialratio.materialRatioID
FROM
stocktransactions AS S
Left Join stocktype AS ST ON S.strType = ST.strType
Inner Join materialratio ON S.intMatDetailId = materialratio.strMatDetailID
WHERE S.intStyleId='$styleID' AND S.strBuyerPoNo='$buyerPoNo' AND materialRatioID='$matDetailID' 
AND S.strColor='$color' AND S.strSize='$size' AND strMainStoresID='$mainStores'
group by strType,dtmDate,intDocumentNo,intDocumentYear
...
Left Join stocktype AS ST ON S.strType = ST.strType
Inner Join materialratio ON S.intMatDetailId = materialratio.strMatDetailID
Inner Join storesbins ON  storesbins.strBinID = S.strBin
WHERE S.intStyleId='$styleID' AND S.strBuyerPoNo='$buyerPoNo' AND materialRatioID='$matDetailID' 
AND S.strColor='$color' AND S.strSize='$size' AND strMainStoresID='$mainStores'
Order By dtmDate,intType
;";
...
<tr class="bcgcolor-tblrowWhite">
	...
	<?php if(round($inQty,2)== 0){
			echo 0; 
		   }
		   else{
		   	if($row['strType']=='BINTRIN'){
		   		echo "(" . $row['strBinName']. ")".round($inQty,2);
				}
			else{
				echo  round($inQty,2);
			}
	...
	<td class="normalfntRite">
	<?php if(round($outQtyArray[1],2)==0){
				echo 0; 
				}
		   else{
		   	if($row['strType']=='BINTROUT'){
	 			echo "(" . $row['strBinName']. ")".round($outQtyArray[1],2);
				}
			else{
				echo round($outQtyArray[1],2);
			}
	 		}?>
    
	<!--<td class="normalfntRite"><?php echo round($inQty,2);?></td>-->
	<!--<td class="normalfntRite"><?php echo round($outQtyArray[1],2)?></td>-->
	<td class="normalfntRite"><?php echo $A1 = round(($A1 + $qty),2);?></td>
</tr>
<?PHP
...
?>
            ...
            <div align="center"><?php echo round($A1,2);?></div>
            </label></td>

...
function ShowExtra(obj)
{
	...
	var styleID		= document.getElementById('cboOrderNo').value;
	var buyerPoNo	= rw.cells[0].id;			
	var matItemList	= rw.cells[27].childNodes[0].nodeValue;
	var color		= rw.cells[4].childNodes[0].nodeValue;
	var size		= rw.cells[5].childNodes[0].nodeValue;
	var mainStoreID		= document.getElementById('cbocompany').value;
...
{
	var stytleNo = document.getElementById('cbostyleno').value;
	var url="checkstatusmiddle.php";
					url=url+"?RequestType=getMainCategories";
					url += '&stytleNo='+URLEncode(stytleNo);
					var htmlobj=$.ajax({url:url,async:false});
					//alert(htmlobj.responseText);
					var matID = htmlobj.responseXML.getElementsByTagName("matCat")[0].childNodes[0].nodeValue;
		document.getElementById('cboMainMatCategory').innerHTML =  matID;
...
function loadItemDescription()
{
	var stytleNo = document.getElementById('cbostyleno').value;
	var url="checkstatusmiddle.php";
					url=url+"?RequestType=getItemDescription";
					url += '&stytleNo='+URLEncode(stytleNo);
					var htmlobj=$.ajax({url:url,async:false});
					//alert(htmlobj.responseText);
					var ItemDescription = htmlobj.responseXML.getElementsByTagName("ItemDescription")[0].childNodes[0].nodeValue;
		document.getElementById('cboDescription').innerHTML =  ItemDescription;
}

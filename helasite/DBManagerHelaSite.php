...
public function getFormName()
{
	...
	public function RunQuery2($SQL)
	{
		...
		createQueryStringHela($SQL,$this->formName);
		...
		$db_formname = getformnameHela(trim($db_tablename));
...
function createQueryStringHela($SQL,$formName)
{
	...
	if($intPos>0)
	{
		...
		saveQueriesHela($strTable,1,$SQL,$formName);
	}
	else
	{
		...
		if($intPos>0)
		{
				...
				saveQueriesHela($strTable,2,$SQL,$formName);
		}
		else
		{
				...
				if($intPos>0)
				{
					...
					saveQueriesHela($strTable,3,$SQL,$formName);
				}
		}
	}
}

function failQueryHela($SQL)
{
	$query = strtoupper("#".$SQL);
	$query2 = "#".$SQL;
	$intPos = stripos($query,"INSERT INTO");
	...
	{
		$strTable = trim(substr($query2,$intPos+11,strpos($query,"(")-($intPos+11)));
		saveFailQueriesHela($strTable,1,$SQL);
	}
	else
	{
		$intPos = stripos($query,"UPDATE");
		...
		{
				$strTable = trim(substr($query2,$intPos+6,strpos($query,"SET")-($intPos+6)));
				saveFailQueriesHela($strTable,2,$SQL);
		}
		else
		{
				$intPos = stripos($query,"DELETE FROM");
				...
				{
					$strTable = trim(substr($query2,$intPos+11,strpos($query,"WHERE")-($intPos+11)));
					saveFailQueriesHela($strTable,3,$SQL);
				}
		}
	}
}

function saveFailQueriesHela($tableName,$operation,$query)
{
	global $_SESSION;
	global $_SERVER;
...
function saveQueriesHela($tableName,$operation,$query,$formName)
{
	global $_SESSION;
	global $_SERVER;

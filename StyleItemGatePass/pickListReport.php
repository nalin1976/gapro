...
<title>GaPro | Pick List - Report</title>
...
<?php
 $sql_header="SELECT distinct ".
			...
			"INNER JOIN gatepass ON GD.intGatePassNo = gatepass.intGatePassNo AND GD.intGPYear = gatepass.intGPYear ".
			"WHERE ".
...
<?php
	if($status==0){
		...
		$SQL_details = "SELECT ".
					...
					" ($tbl.dblQty*-1) as StockQty ".
					" FROM ".
					" gatepassdetails AS GPD ".
					" INNER JOIN matitemlist ON matitemlist.intItemSerial = GPD.intMatDetailId ".
					" INNER JOIN materialratio ON GPD.intStyleId = materialratio.intStyleId AND GPD.intMatDetailId = materialratio.strMatDetailID AND ".
					" GPD.strColor = materialratio.strColor AND GPD.strSize = materialratio.strSize AND GPD.strBuyerPONO = materialratio.strBuyerPONO ".
					" INNER JOIN $tbl ON GPD.intStyleId = $tbl.intStyleId AND GPD.strBuyerPONO = $tbl.strBuyerPoNo AND ".
					" GPD.intMatDetailId = $tbl.intMatDetailId AND GPD.strColor = $tbl.strColor ".
					" AND GPD.strSize = $tbl.strSize AND GPD.intGatePassNo = $tbl.intDocumentNo AND GPD.intGPYear = $tbl.intDocumentYear ".
					" INNER JOIN storesbins ON $tbl.strBin = storesbins.strBinID AND $tbl.strLocation = storesbins.strLocID AND ".
					" $tbl.strSubStores = storesbins.strSubID AND $tbl.strMainStoresID = storesbins.strMainID ".
					" INNER JOIN storeslocations ON storesbins.strLocID = storeslocations.strLocID AND storesbins.strMainID = storeslocations.strMainID AND storesbins.strSubID = storeslocations.strSubID ".
					" WHERE ".
					"GPD.intStyleId =  '".$row_header["intStyleId"]."' AND ".
					"GPD.strBuyerPONO =  '".$row_header["strBuyerPONO"]."' AND ".
					"GPD.intGatePassNo ='$GatepassNo' AND ".
					"GPD.intGPYear ='$Year' GROUP BY $tbl.lngTransactionNo;";

   $result_details = $db->RunQuery($SQL_details);
   while($row_details = mysql_fetch_array($result_details))  
   {	
   		$binName =  $row_details["strBinName"];
		$locationName =  $row_details["strLocName"];
		$unit =  $row_details["unit"];
$TotQty += $row_details["StockQty"];
?>
      <tr>
       <td class="normalfntTAB1"><?php echo $row_details["materialRatioID"];?></td>
        ...
        <td class="normalfntRiteTAB1"><?php echo $row_details["StockQty"];?></td>
		<td class="normalfntTAB1"><?php  echo $locationName."->".$binName;?></td>

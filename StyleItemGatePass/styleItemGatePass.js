...
function DeleteRow(obj)
{
	...
	var grnNo 		= rw.cells[13].childNodes[0].nodeValue;
	var grnYear 	= rw.cells[14].childNodes[0].nodeValue;
	var grnType 	= rw.cells[15].id;
	
	createXMLHttpRequest();
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=DeleteRow&GatePassNO=' +GatePassNO+ '&styleId=' +URLEncode(styleId)+ '&buyerPoNo=' +URLEncode(buyerPoNo)+ '&color=' +URLEncode(color)+ '&size=' +URLEncode(size)+ '&matDetailId=' +matDetailId+'&grnNo='+grnNo+'&grnYear='+grnYear+ '&GrnType=' +grnType ,true);
	xmlHttp.send(null);
}
...
function LoadInternalDestination()
{
	...
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=LoadInternalDestination' ,true);
	xmlHttp.send(null);
}
...
function LoadExternalDestination()
{
	...
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=LoadExternalDestinationRequest' ,true);
	xmlHttp.send(null);
}
...
{	
	...
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=LoadBuyetPoNo&styleId=' + URLEncode(styleId) ,true);
	xmlHttp.send(null);
}
...
{	
	...
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=LoadDetails&StyleId=' + URLEncode(StyleId) + '&BuyerPONO=' + URLEncode(BuyerPONO) + '&mainStoreId=' +mainStoreId ,true);
	xmlHttp.send(null);	
}
		...
		{	
			...
			var booCheck =false;
				for(var mainLoop=1;mainLoop<tblMain.rows.length;mainLoop++)
				{
					...
					var mainGRNno 		= tblMain.rows[mainLoop].cells[13].lastChild.nodeValue;
					var mainGRNYear 	= tblMain.rows[mainLoop].cells[14].lastChild.nodeValue;
					var mainGRNTypeId 	= tblMain.rows[mainLoop].cells[15].id;
					var mainGRNType 	= tblMain.rows[mainLoop].cells[15].lastChild.nodeValue;
			...
			if (booCheck == false)
			{
				...
				if(xml_FabricRollInspectionAllow=="true" && mainCategory=="FABRIC"){
				var cellRoll 			= row.insertCell(12);
				cellRoll.id				= mainArrayIndex;
				cellRoll.innerHTML 		= "<img src=\"../images/add.png\" alt=\"edit\" width=\"15\" height=\"15\"  onclick=\"OPenFabricRollInspection(this," + mainArrayIndex  + ");\" />";				
				}
				else{
				var cellRoll 			= row.insertCell(12);
				cellRoll.id				= mainArrayIndex;
				...
				var cellGRNNo 			= row.insertCell(13);
...
//BEGIN - Adding grn type S = Style , B = Bulk
				var cellGRNYear 		= row.insertCell(15);
				cellGRNYear.className 	= "normalfntRite";		
...
{
		var rw = obj.parentNode.parentNode;
		Materials[index][4] = rw.cells[9].childNodes[0].value;		
}
...
{	
	...
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=LoadBinDetails&styleId=' +URLEncode(styleId)+ '&buyerPoNo=' +URLEncode(buyerPoNo)+ '&matDetailId=' +matDetailId+ '&color=' +URLEncode(color)+ '&size=' +URLEncode(size) + '&mainStore='+mainStore+ '&GrnNo='+grnNo+ '&GrnYear='+grnYear+ '&GrnType=' +grnType , true);
	xmlHttp.send(null);
}
	...
	if (xmlHttp.readyState==4)
	{
		...
		{	
			...
			try{
				...
				var url ="styleItemGatePassXml.php?RequestType=GetAllocatedBinQty";
...
function LoadGatePassNo()
{
	...
	if (GPNO=="")
	{		
		var url = 'styleItemGatePassXml.php?RequestType=LoadGatePassNo';
...
function SaveGatePassDetails()
{
	...
	var url = 'styleItemGatePassXml.php?RequestType=SaveHeaderDetails&gatePassNo=' +gatePassNo+ '&gatePassYear=' +gatePassYear+ '&Destination=' +Destination+ '&remarks=' +URLEncode(remarks) + '&state=' + state + '&attention=' +URLEncode(attention)+ '&category=' +category+ '&noOfPackages=' +noOfPackages;
	var htmlobj=$.ajax({url:url,async:false});

	//start 2010-12-28 delete stock temp data before save
	var url1 = 'styleItemGatePassXml.php?RequestType=deleteStockTemp&gatePassNo=' +gatePassNo+ '&gatePassYear=' +gatePassYear;
	...
	for (loop = 0 ; loop < Materials.length ; loop ++)
	{
		...
		if 	(details!=null)		
		{
				...
				validateCount++;
//alert(Qty)
				var urlDet = 'styleItemGatePassXml.php?RequestType=SaveDetails&gatePassNo=' +gatePassNo+ '&gatePassYear=' +gatePassYear+ '&StyleId=' +URLEncode(StyleId)+ '&BuyerPoNo=' +URLEncode(buyerPoNo)+ '&matDetailId=' +matDetailId+ '&color=' +URLEncode(color)+ '&size=' +URLEncode(size)+ '&Qty=' +Qty+ '&units=' +units+ '&validateCount=' +validateCount+ '&rtn=' +rtn +'&grnNo='+grnNo+ '&grnYear='+grnYear+'&state=' +state+'&pub_commonBin='+pub_commonBin+ '&GrnType='+grnType;
...
//Start-fabric roll Save part
			try{
					for(var rollLoop=0;rollLoop < FabricRollArray.length;rollLoop++)
					{
						...
						var url = 'styleItemGatePassXml.php?RequestType=SaveFabricRollDetails&gatePassNo='+gatePassNo+ '&gatePassYear='+gatePassYear+  '&rollSerialNo='+rollSerialNo+ '&rollNo='+rollNo+ '&styleId='+URLEncode(StyleId)+ '&BuyerPoNo='+URLEncode(buyerPoNo)+ '&itemdDetailID='+matDetailId+ '&color='+URLEncode(color)+ '&size='+URLEncode(size)+ '&batchNo='+batchNo+ '&rollIssueQty='+rollIssueQty+ '&state=' +state;
						var htmlobj=$.ajax({url:url,async:false});
					}
				}
			catch(err){
	...
	//ResponseValidate();
	var url2 = 'styleItemGatePassXml.php?RequestType=ResponseValidateGP&gatePassNo=' +gatePassNo+ '&gatePassYear=' +gatePassYear+ '&validateCount=' +validateCount+ '&validateBinCount=' +validateBinCount+ '&state=' +state;
...
function ResponseValidate()
{
	...
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=ResponseValidate&gatePassNo=' +gatePassNo+ '&gatePassYear=' +gatePassYear+ '&validateCount=' +validateCount+ '&validateBinCount=' +validateBinCount+ '&state=' +state ,true);
	xmlHttp.send(null);
}
	function ResponseValidateRequest()
	{
		if(xmlHttp.readyState == 4) 
		{
			if(xmlHttp.status == 200)
			{				
				var gatePassHeader= xmlHttp.responseXML.getElementsByTagName("recCountGatePassHeader")[0].childNodes[0].nodeValue;
				var gatePassDetails= xmlHttp.responseXML.getElementsByTagName("recCountGatePassDetails")[0].childNodes[0].nodeValue;
				if(state==1){
					var binDetails= xmlHttp.responseXML.getElementsByTagName("recCountBinDetails")[0].childNodes[0].nodeValue;		
...
function RestrictInterface(Status){
	if (Status==1){
		...
		document.getElementById("cmdConfirm").style.visibility="hidden";
		...
		//document.getElementById("cmdCancel").style.visibility="hidden";
		document.getElementById("cmdAddNew").style.visibility="visible";
	}
}
...
function loadGatePassDetails(intGPNO,intGPYear,intStatus)
{
	...
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=LoadHeaderDetails&intGPNO=' +intGPNO+ '&intGPYear=' +intGPYear ,true);
	xmlHttp.send(null);
	
	createXMLHttpRequest1();
	xmlHttp1.onreadystatechange=LoadGatePassDetailsRequest;
	xmlHttp1.open("GET",'styleItemGatePassXml.php?RequestType=LoadGatePassDetails&intGPNO=' +intGPNO+ '&intGPYear=' +intGPYear +'&gatePassStatus='+gatePassStatus,true);
	xmlHttp1.send(null);
}
	...
	xmlHttp.open("GET", 'styleItemGatePassXml.php?RequestType=LoadStores&category='+ obj , true);
	xmlHttp.send(null);  
...
function LoadGatePassDetailsRequest()
{
	...
	if(xml_FabricRollInspectionAllow=="true" && XMLMainCategory[loop].childNodes[0].nodeValue=="FABRIC"){
		strInnerHtml+="<td><img src=\"../images/add.png\" alt=\"edit\" width=\"15\" height=\"15\"  onclick=\"OPenFabricRollInspection(this," + mainArrayIndex  + ");\"/></td>";
	}
	else{
		strInnerHtml+="<td>&nbsp;</td>";
	}
...
function LoadPendingConfirmBinDetailsRequest()
{
	if (xmlHttp2[this.index].readyState==4)
	{
		...
		{	
			...
			//	document.getElementById('cboMainStore').value =1;
		}
	}
}
...
function Confirm()
{
	...
	{
		GatePassComfirm();	
		/*if(confirm('Are you sure you want to confirm this GatePassNo - '+GPNO+'?'))
		{	
			GatePassComfirm();	
		}	*/
	}
...
function GatePassComfirm()
{
	...
	xmlHttp1.open("GET",'styleItemGatePassXml.php?RequestType=GatePassComfirm&GPNO=' +GPNO ,true);
	xmlHttp1.send(null);
}
...
{	
	...
	xmlHttp.open("GET",'styleItemGatePassXml.php?RequestType=GatePassCancel&GPNO=' +GPNO ,true);
	xmlHttp.send(null);
}
...
/*		var GPNO =document.getElementById("cboGatePassNo").value;
	...
	path += "/gatepassReport.php?GatePassNo="+gatePassNo+"&GatePassYear="+gatePassYear;
	
	document.location.href = path;*/
	
/*   var url= "styleItemGatePassXml.php";
					url=url+"?RequestType=loadReportName";

   var htmlobj=$.ajax({url:url,async:false});
   var XMLReportName		= htmlobj.responseXML.getElementsByTagName("ReportName");
   var ReportName           = XMLReportName[0].childNodes[0].nodeValue;
	
	newwindow=window.open(''+ReportName+'?GatePassNo=' +gatePassNo+ '&GatePassYear=' +gatePassYear ,'name');
	if (window.focus) {newwindow.focus()}*/
	
	var cboGatePassNo		= document.getElementById('cboGatePassNo').value;
	if(cboGatePassNo==""){alert("Sorry!\nNo saved details appear to view.");return;}
	if(cboGatePassNo!="0"){			
		newwindow=window.open('gatepassnote.php?cboGatePassNo='+cboGatePassNo,'name');
			if (window.focus) {newwindow.focus()}
	}
}
...
function autoBin()
{
	...
	for(var loop=1;loop<rwCount;loop++)
	{
		if(tbl.rows[loop].cells[0].id==0)	
		{
			...
			var grnNo = (tbl.rows[loop].cells[13].innerHTML)
			var grnYear = tbl.rows[loop].cells[14].innerHTML;
			...
			var url = 'styleItemGatePassXml.php?RequestType=getItemStockAvQty&mainstore='+mainstore+'&styleId='+styleId+'&buyerPo='+buyerPo+'&matId='+matId+'&color='+URLEncode(color)+'&size='+URLEncode(size)+'&grnNo='+grnNo+'&grnType='+grnType+'&issueQty='+issueQty+'&grnYear='+grnYear;

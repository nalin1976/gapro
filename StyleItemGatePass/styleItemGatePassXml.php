...
if($RequestType=='confirmGatePass')
{
	...
	if($stockResponse == '')
	{
		...
		//$SQL="SELECT COUNT(intDocumentNo) AS binDetails FROM stocktransactions where intDocumentNo=$GatepassNo AND intDocumentYear=$GPyear AND strType='SGatePass'";	
		 $SQL="SELECT
Count(stocktransactions.intDocumentNo) AS binDetails,
mainstores.intCompanyId
FROM
stocktransactions
INNER JOIN mainstores ON stocktransactions.strMainStoresID = mainstores.strMainID
WHERE
stocktransactions.intDocumentNo = '$GatepassNo' AND
stocktransactions.intDocumentYear = '$GPyear' AND
stocktransactions.strType = 'SGatePass' AND
mainstores.intCompanyId = '$companyId'";
		$result=$db->RunQuery($SQL);
		$row = mysql_fetch_array($result);
		$recCountBinDetails=$row["binDetails"];			
				if($recCountBinDetails==$validateBinCount)

...
{	
					 ...
					 " <tr>".
					  ...
					  "<tr>".
						"<td height=\"19\" bgcolor=\"#C9E4F5\"><div align=\"center\" class=\"style5\">".
						  "<div align=\"center\">GaPro - 2009. California Link (PVT) Ltd (c) All Rights Reserved.</div>".
						"</div></td>".
					  "</tr>".
					"</table>".
					"</body>".
	...
	public function sendMailThroughNet($body,$sender,$reciever,$subject)
	{
		$haddle = fopen("http://www.softknowedge.com/email.php?message=$body&senderName=$sender&reciever=$reciever&subject=$subject",'rb');
	}
	
	public function SendMail($fieldName,$body,$sender,$reciever,$subject)
	{	
		global $db;
		global $UserID;
		if($fieldName!='')
		{
			$permissionId = $this->GetPermissionId($fieldName);
			...
			{
				$emailAccount = $this->GetEmailAccount($values);
				if($emailAccount!=""){
					$email_list[$counter++] = trim($emailAccount);
				}
			}
				$reciever 	= implode(",",$email_list);	
		}
			
		$sender		= $this->GetSenderName($UserID);
		//$this->sendMailThroughNet(urlencode($body),$sender,$reciever,urlencode($subject));
	}
	...
	public function GetEmailAccount($userId)
	{
		global $db;
	...
	public function GetPermissionId($fieldName)
	{
		global $db;
		$sql="select intSerialId from emailconfig where strFildName='$fieldName'";
		$result=$db->RunQuery($sql);
		while($row=mysql_fetch_array($result))
		{
			$permissionId = $row["intSerialId"];
		}
		return $permissionId;
	}
}

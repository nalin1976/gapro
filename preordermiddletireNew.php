...
else if(strcmp($RequestType,"SaveItems")==0)
{
	...
	$result=saveOrderDetails($strOrderNo,$strStyleID,$intMatDetailID,$strUnit,$dblUnitPrice,$reaConPc,$reaWastage,$strCurrencyID,$intOriginNo,$dblReqQty,$dblTotalQty,$dblTotalValue,$dbltotalcostpc,$freight,$mill,$mainFabricStatus);
...
else if(strcmp($RequestType,"SaveSchedule")==0)
{
	...
	$result=saveDeliveryDetails($strStyleID,$dtDateofDelivery,$dblQty,$exQty,$modeID,$dbExQty,$isbase,$leadID,$remarks,$dtestimated,$userID,$dtmDate,$handoverDate,$bpo,$refNo,$dtestimated);
...
else if(strcmp($RequestType,"ChangeSchedule")==0)
{
	...
	$result=ChangeAndSaveDeliveryDetails($strStyleID,$dtDateofDelivery,$dblQty,$exQty,$modeID,$dbExQty,$isbase,$leadID,$remarks,$oldDate,$dtEstimated,$userID,$dtmDate,$bpo,$refNo,$handOverdate);
...
else if (strcmp($RequestType,"updateCellChanges")==0)
{
	...
	$sql = "update orderdetails set dblUnitPrice = $unitprice, reaConPc = $conpc ,strUnit='$units' where intStyleId = '$styleID' and intMatDetailID = '$itemCode'";
	$db->executeQuery($sql);
...
else if (strcmp($RequestType,"AddDeliveryBPO")==0)
{
	...
	if($buyerPoId!=""){
		$sql="update style_buyerponos set strBuyerPoName='$buyerPO' where intStyleId='$styleID' and strBuyerPONO='$buyerPoId'";
		$db->executeQuery($sql);
	}
	else
	{	
	...
	$sql="select strBuyerPONO from  style_buyerponos where intStyleId='$styleID' and strBuyerPoName='$buyerPO'";
	$result=$db->RunQuery($sql);
	while($row=mysql_fetch_array($result))
	{
	$bp = $row["strBuyerPONO"];
	...
	//---------------------
	$sql = "INSERT INTO deliveryschedule 
	(intStyleId, 
	dtDateofDelivery, 
	strShippingMode,
	intSerialNO,
...
{
	global $db;
	$sql="SELECT COUNT(intStyleId)AS ItemCount FROM orderdetails where intStyleId='".$styleID."';";
	$result=$db->RunQuery($sql);
	while($row=mysql_fetch_array($result))
	{
		if($row["ItemCount"]>0)
...
{
global $db;
/*$sql="SELECT intStyleId, DATE_FORMAT(dtDateofDelivery, '%d/%m/%Y') as dtDateofDelivery,dblQty,dbExQty,strRemarks,strShippingMode,isDeliveryBase,deliveryschedule.intSerialNO,reaLeadTime, DATE_FORMAT(estimatedDate, '%d/%m/%Y') as estimatedDate FROM deliveryschedule LEFT JOIN eventtemplateheader ON deliveryschedule.intSerialNO = eventtemplateheader.intSerialNO where intStyleId='".$styleID."'";	*/
...
strRemarks,strShippingMode,isDeliveryBase,d.intSerialNO,o.intApprovalNo,DATE_FORMAT(dtmHandOverDate, '%d/%m/%Y') as dtmHandOverDate,d.intBPO,d.intRefNo,DATE_FORMAT(estimatedDate, '%d/%m/%Y') as estimatedDate
...
",intFirstApprovedBy,dtmFirstAppDate,strScheduleMethod,dblFacProfit,strOrderColorCode,intCompanyOrderNo,dtmOrderDate,intOrderType,wipOrderNo,intManufactureCompanyID,strBuyerOrderNo)values('".$orderNo."','".$styleID."',".$companyID.",'".$description."',".$buyerID.",".$qty.",".$Coordinator.",".$status.",'".$customerRefNo."',".$smv.",NOW(),".$smvRate.",".$fob.",".$finance.",'".$userID."','".$approvedBy."','".$appRemarks."',null,".$exPercentage.",".$finPercntage.",0,'".$revisedReason."',null,'".$revisedBy."',".$confirmedPrice.",'".$conPriceCurrency."',".$commission.",".$efficiencyLevel.",".$costPerMinute.",null,'".$sentForApprovalsTo."','".$deliverTo."',".$freightCharges.",".$ECSCharge.",".$labourCost.",".$buyingOfficeId.",".$divisionId.",".$seasonId.",'".$RPTMark."',".$subContractQty.",".$subContractSMV.",".$subContractRate.",".$subTransportCost.",".$subCM.",".$lineNos.",0,".$uPCharges.",'".$UPChargeDescription."',".$fabFinance.",".$trimFinance.",0,0,0,null,'$ScheduleMethod','$facProfit','$color','$companyOrderNo',now(),'$OrderType','$orderNo',$manufacCompany,'$buyerOrderNo');";
return $db->ExecuteQuery($sql);
}
//-------------------------------------------------------------------------------------------------------------------------------------
...
{
	global $db;
	$sql="insert into orderdetails(strOrderNo,intStyleId,intMatDetailID,strUnit,dblUnitPrice,reaConPc,reaWastage,strCurrencyID,intOriginNo,dblReqQty,dblTotalQty,dblTotalValue,dbltotalcostpc,dblFreight,intMillId,intMainFabricStatus) values ('".$strOrderNo."','".$strStyleID."',".$intMatDetailID.",'".$strUnit."',".$dblUnitPrice.",".$reaConPc.",".$reaWastage.",'".$strCurrencyID."',".$intOriginNo.",".$dblReqQty.",".$dblTotalQty.",".$dblTotalValue.",".$dbltotalcostpc.",'".$freight."','".$mill."','".$mainFabricStatus."');";
...
function saveDeliveryDetails($strStyleID,$dtDateofDelivery,$dblQty,$exQty,$modeID,$dbExQty,$isbase,$leadID,$remarks,$estimateddate,$userID,$dtmDate,$handoverDate,$bpo,$refNo,$dtestimated)
{
	global $db;
	$sql="Insert into deliveryschedule(intStyleId,dtDateofDelivery,dblQty,dbExQty,strShippingMode,isDeliveryBase,intSerialNO,strRemarks,intUserId,dtmDate,dtmHandOverDate,intBPO,intRefNo,estimatedDate)values('".$strStyleID."','".$dtDateofDelivery."',".$dblQty.",".$dbExQty.",".$modeID.",'$isbase',$leadID,'$remarks','$userID','$dtmDate','$handoverDate','$bpo','$refNo','$dtestimated');";
...
{
	global $db;
	$sql="DELETE FROM orderdetails WHERE intMatDetailID='$matID' AND intStyleId= '$styleID' ";	
	$db->executeQuery($sql);
...
function ChangeAndSaveDeliveryDetails($strStyleID,$dtDateofDelivery,$dblQty,$exQty,$modeID,$dbExQty,$isbase,$leadID,$remarks,$oldDate,$dtestimated,$userID,$dtmDate,$bpo,$refNo,$handOverdate)
{
	global $db;
...
$db->executeQuery($sql);
	...
	$sql="update deliveryschedule 	set 	dtDateofDelivery = '$dtDateofDelivery' ,  	dblQty = '$dblQty' ,  	strRemarks = '$remarks' ,  	strShippingMode = '$modeID' ,  	dbExQty = '$dbExQty' ,  	isDeliveryBase = '$isbase' , estimateddate = '$dtestimated', intSerialNO= '$leadID' , intUserId='$userID' , dtmDate='$dtmDate' , intBPO='$bpo', intRefNo='$refNo', dtmHandOverDate='$handOverdate'		where	intStyleId = '$strStyleID' and dtDateofDelivery = '$oldDate' ;";


	return	$db->executeQuery($sql);	
}

function removeScheduleBPOAllocation($styleID,$delDate)
{
	global $db;
	$sql="delete from bpodelschedule where intStyleId = '$styleID' AND dtDateofDelivery = '$delDate';";
...
select * from deliveryschedule where intStyleId = '$strStyleID' AND dtDateofDelivery = '$dtDateofDelivery';";
...
function updateItemDetails($styleID,$qty,$exPercentage)
{
	...
	global $db;
	$sql= "select orderdetails.intMatDetailID,matitemlist.intMainCatID, orderdetails.dblUnitPrice, orderdetails.reaConPc, orderdetails.reaWastage, orderdetails.dblFreight from orderdetails inner join matitemlist on orderdetails.intMatDetailID = matitemlist.intItemSerial where orderdetails.intStyleId = '$styleID'";
